...
$\displaystyle R \subset \RR^n$ is a set of
the form $\displaystyle R = \prod_{i=1}^n I_i$ where the $I_i$ are
bounded intervals. The {\bfseries volume}\index{Volume} of a rectangle
$\displaystyle R = \prod_{i=1}^n I_i$
is the value $\displaystyle V(R) = \prod_{i=1}^n |I_i|$ where $|I_i|$
...
and \ref{JordanMS}) can be extended to $\displaystyle \RR^n$.

\begin{defn} \label{measure}
A set $\displaystyle S\in \RR^n$ has
{\bfseries zero content}\index{Zero Content} if
for every $\epsilon>0$ there exists a finite collection of closed rectangles
$\displaystyle \{ R_i \}_{0\leq i \leq M}$ such that
$\displaystyle S \subset \bigcup_{i=0}^M R_i$ and
$\displaystyle \sum_{i=0}^M V(R_i) < \epsilon$.

A set $\displaystyle S\in \RR^n$ has
{\bfseries measure zero}\index{Measure Zero} if
for every $\epsilon>0$ there exists a countable collection of closed
rectangles $\displaystyle \{ R_i \}_{i\geq 0}$ such that
$\displaystyle S \subset \bigcup_{i=0}^\infty R_i$ and
$\displaystyle \sum_{i=0}^\infty V(R_i) < \epsilon$.
\end{defn}

\begin{defn}
A set $\displaystyle S \subset \RR^n$ is
...
$\displaystyle S \subset \RR^n$ has measure zero according to the
definition using closed rectangles.  Given $\epsilon > 0$, there exists a
countable collection
$\displaystyle \{R_i\}_{i\geq 0}$ of closed rectangles such that
$\displaystyle S \subset \bigcup_{i=0}^\infty R_i$ and
$\displaystyle \sum_{i=0}^\infty V(R_i) < \epsilon/2$.
For each $i$, choose an open rectangle $V_i$ such that $R_i \subset V_i$
and $\displaystyle V(V_i) \leq V(R_i) + \epsilon/2^{i+2}$.  Then
$\displaystyle S \subset \bigcup_{i=0}^\infty R_i \subset
...
Conversely, suppose that $\displaystyle S \subset \RR^n$ has measure
zero according to the definition using open rectangles.  Given
$\epsilon > 0$, there exists a countable collection
$\displaystyle \{V_i\}_{i\geq 0}$ of open rectangles such that
$\displaystyle S \subset \bigcup_{i=0}^\infty V_i$ and
$\displaystyle \sum_{i=0}^\infty V(V_i) < \epsilon$.
For each $i$, let $R_i = \overline{V_i}$.  Then $V(R_i) = V(V_i)$ for all
$i$.  Hence $\displaystyle S \subset \bigcup_{i=0}^\infty V_i \subset
\bigcup_{i=0}^\infty R_i$ and
$\displaystyle \sum_{i=0}^\infty V(R_i) = \sum_{i=0}^\infty V(V_i) < \epsilon$.
Since $\epsilon$ is arbitrary, $S$ has measure zero according to the
definition using closed rectangles.
\end{rmk}

Given a bounded set $\displaystyle V\subset \RR^n$, we define the diameter of
$V$ as the number
\[
...
compact sets $\displaystyle \{ K_i \}_{i\geq 0}$ such that
$\displaystyle S \subset \bigcup_{i=0}^\infty K_i$ and
$\displaystyle \sum_{i=0}^\infty \diam K_i < \epsilon$,
then the set $\displaystyle S\in \RR^n$ has measure zero.
\end{prop}
...
compact sets $\displaystyle \{ K_i \}_{i\geq 0}$ such that
$\displaystyle S \subset \bigcup_{i=0}^\infty K_i$ and
$\displaystyle \sum_{i=0}^\infty \diam K_i < \epsilon^{1/n}$.

Each $K_i$ is a subset of a rectangle $R_i$ with edges of length
$d_i = \diam K_i$.  Thus $\displaystyle V(R_i) = d_i^n$.  Hence
$\displaystyle S \subset \bigcup_{i=0}^\infty K_i
...
collection $\displaystyle \{V_i\}_{i\geq 0}$ of open rectangles
such that
$\displaystyle K \subset Z \subset \bigcup_{i=0}^\infty V_i$
and $\displaystyle \sum_{i=0}^\infty V(V_i) < \epsilon$.
Since $K$ is compact, there exists a finite subcover
$\displaystyle \{V_{i_j}\}_{0\leq j \leq N}$ such that
$\displaystyle K \subset \bigcup_{j=0}^N V_{i_j}$.  We obviously
have that
$\displaystyle \sum_{j=0}^N V(V_{i_j}) \leq \sum_{i=0}^\infty V(V_i) <
...
$\displaystyle R = \prod_{i=1}^n I_i \subset \RR^n$, where each $I_i$
...
$\displaystyle R_{k_1,k_2,\ldots,k_n} = \prod_{i=1}^n [p_{k_i-1,i},p_{k_i,i}]$,
...
\begin{defn}
Let $\displaystyle R \subset \RR^n$ be a rectangle as defined above,
$f:R\to \RR$ be a bounded function, and $P$ be a partition of $[a,b]$.
Moreover, let $\displaystyle m_{k_1,k_2,\ldots,k_n}
= \inf\{ f(\VEC{x}) : \VEC{x} \in  R_{k_1,k_2,\ldots,k_n}\}$
and $\displaystyle M_{k_1,k_2,\ldots,k_n}
...
\begin{defn}
Let $R \subset \RR^n$ be a rectangle as defined above and 
...
\begin{defn}
Let $\displaystyle R \subset \RR^n$ be a rectangle as defined above and
$f:R\to \RR$ be a bounded function.  If $\U(f) = \LL(f)$, we say that
...
Suppose that $R$ is a rectangle in $\displaystyle \RR^n$ and
...
Suppose that $\displaystyle R \subset \RR^n$ is a closed set.  The set
$\displaystyle
...
induced on $R$ from $\displaystyle \RR^n$.
...
$\delta_y < \delta - \|\VEC{x} - \VEC{y}\|$.  We have that
...
as $\delta_y$ decreases, we have that
$o(\VEC{y},f) < \epsilon$.  Thus $\VEC{y} \in R \setminus Z_\epsilon$.
\end{proof}

\begin{lemma}\label{zeroLemma}
Let $R$ be a closed rectangle in $\displaystyle \RR^n$ and $f:R\to \RR$
...
such that $\displaystyle R_{k_1,k_2,\ldots,k_n} \subset R_{\VEC{x}_i}$.
...
Let $R$ be a rectangle in $\displaystyle \RR^n$ and $f:R\to \RR$ be
...
Given $\epsilon >0$, we have that $Z_{\epsilon/(2V(R))} \subset Z$.
...
finite collection $\displaystyle \{R_i\}_{0 \leq i \leq N}$ of closed
rectangles in $R$ such that
$\displaystyle Z_{\epsilon/(2V(R))} \subset \bigcup_{i=0}^N R_i$ and
\begin{equation} \label{zeroEqu4}
\displaystyle \sum_{i=0}^N V(R_i) < \frac{\epsilon}{4M} \ .
\end{equation}
...
define as before
$\displaystyle M_{k_1,k_2,\ldots,k_n}
= \sup \{ f(\VEC{x}) : \VEC{x} \in R_{k_1,k_2,\ldots,k_n} \}$ and
$\displaystyle m_{k_1,k_2,\ldots,k_n}
= \inf \{ f(\VEC{x}) : \VEC{x} \in R_{k_1,k_2,\ldots,k_n} \}$.
We have that $M_{k_1,k_2,\ldots,k_n} - m_{k_1,k_2,\ldots,k_n} < 2M$
...
as expected,
$\displaystyle \tilde{M}_{j_1,j_2,\ldots,j_n}
...
Proposition~\ref{propRef} in $\displaystyle \RR^n$ is used to justify the last 
statement.
...
Theorem~\ref{intCauchyCR} in $\displaystyle \RR^n$ that $f$ is integrable.

\stage{$\Leftarrow$} We have that
\[
...
$\displaystyle \U_P(f) - \LL_P(f) < \epsilon/(2m)$.  Moreover, let $S_1$
be the collection of all subrectangles $R_{k_1,k_2,\ldots,k_n}$ of the
partition $P$ such that
$\displaystyle R_{k_1,k_2,\ldots,k_n}^\circ \cap Z_{1/n} \neq \emptyset$.
...
$\displaystyle  M_{k_1,k_2,\ldots,k_n} - m_{k_1,k_2,\ldots,k_n} \geq 1/m$
for each $R_{k_1,k_2,\ldots,k_n} \in S_1$ \footnote{The condition
$\displaystyle R_{k_1,k_2,\ldots,k_n} \cap Z_{1/m} \neq \emptyset$ is
not sufficient to ensure that
$\displaystyle  M_{k_1,k_2,\ldots,k_n} - m_{k_1,k_2,\ldots,k_n}
\geq 1/m$.  We may have that
$\displaystyle \VEC{x} \in R_{k_1,k_2,\ldots,k_n} \cap Z_{1/m}
...
such that $\displaystyle B \subset \bigcup_{T\in S_2} T$ and
...
Given $\displaystyle W \subset \RR^n$,
let $\displaystyle \Chi_W:\RR^n \to \RR$ be
the function defined by
\[
\Chi(\VEC{x}) = \begin{cases}
1 & \quad \text{if} \ \VEC{x} \in W \\
0 & \displaystyle \quad \text{if} \ \VEC{x} \in \RR^n\setminus W
\end{cases}
\]
...
\begin{defn}
Let $\displaystyle W \subset \RR^n$ be a Jordan measurable set.  Given
a bounded function $f:W\to \RR$, we define the
{\bfseries Riemann integral of $f$ on $W$}\index{Riemann
Integral of a Function}, denoted $\displaystyle \int_W f$ or
$\displaystyle \int_W f(\VEC{x})\dx{\VEC{x}}$, as the integral
$\displaystyle \int_R \Chi_W f$ if this integral exists
...
$\displaystyle \Chi_Z h: R\to \RR$ is discontinuous has zero content
...
$\displaystyle Z \subset \bigcup_{i=0}^N R_i$ and
$\displaystyle \sum_{i=0}^N V(R_i) < \epsilon$, then
$\displaystyle \overline{Z} \subset \bigcup_{i=0}^N R_i$ and we still
have that $\displaystyle \sum_{i=0}^N V(R_i) < \epsilon$.}.
...
open rectangles $\displaystyle \{ R_i\}_{0 \leq i \leq N}$ such that
$\displaystyle Z \subset \bigcup_{i=0}^N R_i$ and
$\displaystyle \sum_{i=0}^N V(R_i) < \frac{\epsilon}{M-m}$.

Choose a partition $P$ of $R$ such that the vertices of all the
rectangles $R_i$ are included in $P$.  We then have that
$\displaystyle M_{k_1,k_2,\ldots,k_n} = \sup \{ \Chi_Z(\VEC{x}) h(\VEC{x}) :
 \VEC{x} \in R_{k_1,k_2,\ldots,k_n} \} =0$ and
$\displaystyle m_{k_1,k_2,\ldots,k_n} = \inf \{ \Chi_Z(\VEC{x}) h(\VEC{x}) :
 \VEC{x} \in R_{k_1,k_2,\ldots,k_n} \} = 0$ for all
subrectangles $\displaystyle R_{k_1,k_2,\ldots,k_n}$ of the partition
$P$ such that $\displaystyle R_{k_1,k_2,\ldots,k_n} \cap R_i = \emptyset$
for all $i$.  Hence
\begin{align*}
...
$\displaystyle R_{k_1,k_2,\ldots,k_n} \cap R_i \neq \emptyset$ implies
that $R_{k_1,k_2,\ldots,k_n}^\circ \subset R_i$ because that $R_i$ are
open.  Hence
\[
...
$\displaystyle \RR^n$ that $\Chi_Z h$ is integrable on $R$.

\stage{ii} To prove that $\displaystyle \int_Z h = 0$, we consider
$\displaystyle h = h^+ - h^-$ where
\[
h^+(\VEC{x}) = \begin{cases}
...
\end{cases}
\]
It follows from (i) that $\displaystyle h^+$ and $\displaystyle h^-$
...
= \int_Z h^+ - \int_Z h^- \ .
\]
It suffices to prove that $\displaystyle \int_Z h^+$ and
$\displaystyle \int_Z h^-$ are null.

To prove that $\displaystyle \int_Z h^+$ is null, we prove that
$\LL_P(\Chi_Z h^+) = 0$ for all partitions $P$ of $R$.
Let $P$ be a partition of $R$ and let
$\displaystyle \delta = \min_{\substack{0<k_i\leq N_i\\1 \leq i \leq n}}
...
open rectangles $\displaystyle \{ R_i\}_{0 \leq i \leq N}$ such that
$\displaystyle Z \subset \bigcup_{i=0}^N R_i$ and
$\displaystyle \sum_{i=0}^N V(R_i) < \delta$.
We have that
\[
...
for every subrectangle $R_{k_1,k_2,\ldots,k_n}$ of the partition $P$
because
$\displaystyle R_{k_1,k_2,\ldots,k_n} \setminus Z \supset
...
\]
Thus $\LL_P(\Chi_Z h^+ ) = 0$.  The proof that
$\LL_P(\Chi_Z h^-) = 0$ for all partitions $P$ of $R$ is similar.
\end{proof}
...
$\displaystyle \Chi_Z h: R\to \RR$ is discontinuous has measure zero 
...
$\displaystyle \int_Z h = 0$ does not change.
...
Let $R$ be a rectangle in $\displaystyle \RR^n$.  Suppose that
$f,g:R \to \RR$ are two Riemann integrable functions such that
the set $Z = \{ \VEC{x} \in R : f(\VEC{x}) \neq g(\VEC{x}) \}$ has
zero content.  Then $\displaystyle \int_R f = \int_R g$.
\end{cor}

\begin{proof}
Let $h=f-g$, we have that $h = \Chi_Z h$.  From the previous
...
\section{Bounded Curves in $\displaystyle \mathbf{\RR^n}$}

A {\bfseries curve}\index{Curve} in $\displaystyle \RR^n$ is the image
of a continuous function $\displaystyle \sigma:[a,b]\to \RR^n$.
...
A bounded curve in $\displaystyle \RR^n$ should have zero content
...
$\displaystyle \left[\frac{j_1-1}{2^i}, \frac{j_1}{2^i}\right]\times
\left[\frac{j_2-1}{2^i}, \frac{j_2}{2^i}\right]$ for
$\displaystyle 1\leq j_1, j_2 \leq 2^i$ and $i \geq 1$.
By choosing a proper parametric representation $\sigma_i$ on $[0,1]$
for each $i$, one can show that
$\displaystyle \sup_{0\leq x\leq 1} \|\sigma_i(x) - \sigma_{i+1}(x)\| \leq
\frac{1}{2^{i+1}}$ for $i \geq 1$ as it is illustrated in green in the
figure above for $i=1$.
\end{egg}

However, if we require that the curve
$\displaystyle \sigma:[a,b]\to \RR^n$ be continuously differentiable,
then the image of the curve $\sigma$ must have zero content.

\begin{prop}\label{curveC1zero}
Suppose that $\displaystyle \sigma:[a,b]\to \RR^n$ belongs to
$\displaystyle C^1([a,b])$.  Then $\sigma([a,b])$ has zero content.
\end{prop}

\begin{proof}
We prove the proposition for $\displaystyle \sigma:[a,b]\to \RR^2$
only.  The proof is (almost) identical for
$\displaystyle \sigma:[a,b]\to \RR^n$.

Let $\displaystyle C = \sup \{ \|\sigma'(t)\| : t \in [a,b] \}$.
...
$\displaystyle 2C^2(b-a)^2/K < \epsilon$.  Let
...
$\displaystyle A(R_i) \leq \left( \sqrt{2} C(b-a)/K\right)^2
= 2C^2(b-a)^2/K^2$ such that
$\sigma([t_{i-1},t_i]) \subset R_i$.  Thus
$\displaystyle \sigma([a,b]) \subset \bigcup_{i=1}^K R_i$ with
\[
...
Let $\displaystyle P_j=\{ p_{i,j} : 0 \leq i \leq N_j\}$ be a partition
...
\end{equation}
for all $\displaystyle x_1^{[i]} \in [p_{i-1,1},p_{i,1}]$
with $0 < i \leq N_1$.  If $\displaystyle
...
\]
Since $\epsilon$ is arbitrary, it follows from the version of
Theorem~\ref{intCauchyCR} in $\displaystyle \RR^n$ that
...
$R$, we have that $\displaystyle \LL(f) = \U(f) = \int_R f$.  Hence
\[
\int_{I_1} h(x_1) \dx{x_1} = \LL(h) = \U(h) = \LL(f) = \U(f) = \int_R f \ .
\]

\stage{iv} Similarly to (iii), we have that
...
$\displaystyle h(x_1) = H(x_1) = \int_{I_2} f(x_1,x_2) \dx{x_2}$
...
$\displaystyle g(x_2) = G(x_2) = \int_{I_1} f(x_1,x_2) \dx{x_1}$ and
...
f(x_1,x_2) =
\begin{cases}
\displaystyle 1-\frac{1}{q} &
\qquad \displaystyle \text{if} \ x_2 \in [0,1]\cap \QQ 
...
[0,1]\times [0,1]$ and $\displaystyle \int_S f = 1$.  Given
...
We have that $M_{i,j} - m_{i,j} \leq 1$ for all $i$ and $j$.
...
$\displaystyle \frac{1}{q_0} < \frac{\epsilon}{2}$ and
\[
\sum\nolimits' (p_{i,1}-p_{i-1,1})(p_{j,2}-p_{j-1,2})
...
$\displaystyle \int_S f = \U(f) = 1$.

\stage{ii}
However, $\displaystyle \int_0^1 f_{x_1}(x_2) \dx{x_2} = 1$ for
...
\displaystyle 1-\frac{1}{q} & \quad \text{if} \ x_2 \in \QQ \cap [0,1]
\end{cases}
\]
...
\]
for $\displaystyle 0 <j,k \leq 2^i$.  For each $i>0$, we have that
$\displaystyle S = \bigcup_{0<j,k\leq 2^i} I_{i,j,k}$.
...
$i=s$, the set $S$ is the union of $\displaystyle 2^{2s}$ squares.  Let
$Q_s$ be the set containing one point from each square such that
each of these points is the unique point of
$\displaystyle \bigcup_{i=1}^{s} Q_i$ on the horizontal and vertical
lines on which it belongs.  We have by construction that
$\displaystyle Q_s \cap \left( \bigcup_{i=1}^{s-1} Q_i \right) = \emptyset$.
We have an inductive method to define all the sets $Q_s$.

The set $Q$ is the set $\displaystyle Q = \bigcup_{s=1}^{\infty} Q_s$.
...
$\displaystyle Q^\circ = \emptyset$ because there is at most one point
...
$\displaystyle \int_I \chi_Q(x_1,x_2) \dx{x_2} = 0$ for all $x_1\in I$, we get
that $\displaystyle \int_I \left( \int_I \chi_Q(x_1,x_2) \dx{x_2} \right)
\dx{x_1}= 0$.  Similarly, we get that
$\displaystyle \int_I \left( \int_I \chi_Q(x_1,x_2) \dx{x_1} \right)
\dx{x_2}= 0$.
\end{enumerate}
\end{rmkList}
...
Let $\displaystyle \{U_\alpha\}_{\alpha \in A}$ be an open cover of
$\displaystyle W \subset \RR^n$; namely,
$U_\alpha$ is an open set for every $\alpha$ and
$\displaystyle W \subset \bigcup_{\alpha \in A} U_\alpha$.  There exist
an open set $V \supset W$ and a collection
$\displaystyle \{\phi_j\}_{j \in \NNp}$ of
functions in $\displaystyle C^\infty(V)$ such that:
\begin{enumerate}
\item $0 \leq \phi_j(\VEC{x}) \leq 1$ for all
$\displaystyle \VEC{x} \in V$ and all $\displaystyle j \in \NNp$.
\item For each $\VEC{x}\in W$, there exists an open neighbourhood
$W_{\VEC{x}} \subset V$ of $\VEC{x}$ such that
$\displaystyle \phi_j\big|_{W_\VEC{x}} = 0$ for all but a finite
number of $\phi_j$.
\item For each $\VEC{x} \in W$, we have that
$\displaystyle \sum_{j \in \NNp} \phi_j(\VEC{x}) = 1$.  This sum
...
\begin{proof}
\stage{i} If $W$ is compact, there exists a finite subcover 
$\displaystyle \{U_{\alpha_i}: 1 \leq i \leq N\}$ of $W$.
...
\]
for $\alpha_i \in A$.
Since $\displaystyle \{B_{r_{\VEC{x},1}}(\VEC{x}) \}_{\VEC{x} \in W}$
is an open cover of the compact set $W$, there exists a finite
subcover
$\displaystyle \{B_{r_{\VEC{x}_k,1}}(\VEC{x}_k) : 1 \leq k \leq M \}$
...
We have that $\displaystyle \psi_k \in C^\infty(\RR^n)$,
$0 \leq \psi_k(\VEC{x}) \leq 1$ for all $\displaystyle \VEC{x} \in \RR^n$,
...
Let $\phi_1 = \psi_1$ and
$\displaystyle \phi_{j+1} = \psi_{j+1}\prod_{k=1}^j(1-\psi_k)$
for $1 <j < M$.

\begin{enumerate}
\item We have that
$0 \leq \phi_j(\VEC{x}) \leq 1$ for all
$\displaystyle \VEC{x} \in \RR^n$ and $1 \leq j \leq M$ because
$\phi_j$ is the product of functions whose image is in $[0,1]$.
\item For each $\VEC{x}\in W$, it is obvious that there exists an
open neighbourhood $\displaystyle W_{\VEC{x}} \subset \RR^n$ of
$\VEC{x}$ such that
$\displaystyle \phi_j\big|_{W_\VEC{x}} = 0$ for all but a finite
number of $\phi_j$ because there is only a finite number of $\phi_j$.
\item To prove that $\displaystyle \sum_{j=1}^M\phi_j(\VEC{x}) = 1$
...
Using the hypothesis of induction, we get that
\begin{align*}
...
Therefore, $\displaystyle \sum_{j=1}^M \phi_j(\VEC{x}) = 1$
for all $\VEC{x} \in W$.
...
We also have that $\supp \phi_j \subset U_{\alpha_j}$.
\end{enumerate}
All the conclusions of the theorem are satisfied by the collection
$\displaystyle \{\phi_j\}_{1 \leq j \leq M}$.

\stage{ii} Suppose that $\displaystyle W = \bigcup_{j=0}^\infty W_j$
where each $W_j$ is compact and $W_j \subset W_{j+1}^\circ$ for all $j$.
We also assume that $W_0 = \emptyset$ to simplify the discussion.
Let $V_1 = W_1$ and
$\displaystyle V_j = W_j \setminus W_{j-1}^\circ$ for $j \geq 2$.

Let $\displaystyle U_{\alpha,1} = U_\alpha \cap W_2^\circ$ for all $\alpha \in A$.
Since $V_1$ is a compact set and
$\displaystyle \{U_{\alpha,1}\}_{\alpha \in A}$ is an open cover of $V_1$,
we may use (i) to find $\displaystyle \{\psi_{k,1}: 1 \leq k \leq N_1\}$
such that $0 \leq \psi_{k,1}(\VEC{x}) \leq 1$ for all
$\displaystyle \VEC{x} \in \RR^n$ and $1\leq k \leq N_1$,
$\displaystyle \sum_{k=1}^{N_1} \psi_{k,1}(\VEC{x}) = 1$ for all
$\VEC{x} \in V_1$, and $\supp \psi_{k,1}$ is a compact set with
$\supp \psi_{k,1} \subset U_{\alpha_{k,1}}$ for some $\alpha_{k,1} \in A$.

For $j \geq 2$, let $U_{\alpha,j} = U_\alpha \cap
\left(W_{j+1}^\circ\setminus W_{j-2}\right)$ for all $\alpha \in A$.
Since $V_j$ is a compact set and
$\displaystyle \{U_{\alpha,j}\}_{\alpha\in A}$ is an open cover of $V_j$,
we may use (i) to find $\displaystyle \{\psi_{k,j}: 1 \leq k \leq N_j\}$
such that
$0 \leq \psi_{k,j}(\VEC{x}) \leq 1$ for all
$\displaystyle \VEC{x} \in \RR^n$ and $1\leq k \leq N_j$,
$\displaystyle \sum_{k=1}^{N_j} \psi_{k,j}(\VEC{x}) = 1$ for all
$\VEC{x} \in V_j$, and
...
Let $\displaystyle \psi(\VEC{x})
= \sum_{j=1}^\infty \sum_{k=1}^{N_j} \psi_{k,j}(\VEC{x})$ for
$\displaystyle \VEC{x} \in \RR^n$.  We first note that the sum is
...
because $\displaystyle \VEC{x} \in W = \bigcup_{j=1}^\infty V_j$.  Hence
\[
\psi(\VEC{x})
...
$\displaystyle V = \bigcup_{\VEC{x}\in W} V_{\VEC{x}}$.}.
Let
\[
\rho_{k,j}(\VEC{x}) = \frac{\psi_{k,j}(\VEC{x})}{\psi(\VEC{x})}
\]
for $\VEC{x} \in V$, $j \geq 1$ and $1 \leq k \leq N_j$.
We have that $\displaystyle \rho_{k,j} \in C^\infty(V)$ because
$\psi_{k,j}$ and $\psi$ are in $\displaystyle C^\infty(V)$, and
$\psi(\VEC{x}) \neq 0$ for all $\displaystyle \VEC{x} \in V$.

\begin{enumerate}
\item We have by construction that
$0 \leq \rho_{k,j}(\VEC{x}) \leq 1$ for all
$\displaystyle \VEC{x} \in V$, $j \geq 1$ and $1 \leq k \leq N_j$.
...
$\VEC{x} \in W$, we have that $\VEC{x} \in V_j$ for some $j$.
Since $\rho_{k,q}(\VEC{y}) = 0$ for all
$\VEC{y} \in W_{j+1}^\circ \setminus W_{j-2}$ if $q<j-2$ and $q>j+1$,
it suffices to choose an open neighbourhood
$W_{\VEC{x}} \subset W_{j+1}^\circ \setminus W_{j-2}$ of $\VEC{x}$.
\item We also have by construction that
$\displaystyle \sum_{j=1}^\infty \sum_{k=1}^{N_j}
...
\end{enumerate}
The collection $\displaystyle \{\phi_j\}_{j \in \NNp}$ that satisfies
all the conclusions of the theorem is defined as it follows.
$\phi_j = \rho_{j,1} $ for $1 \leq j \leq N_1$ and
$\phi_j = \rho_{k,q} $ for
$\displaystyle j = \sum_{i=1}^{q-1} N_i + k$ and
$1 \leq k \leq N_q$ with $q >1$.

\stage{iii} If $W$ is an open set, then
$\displaystyle W = \bigcup_{j=j}^\infty W_j$ where
$\displaystyle W_j = \{\VEC{x} : \dist{\VEC{x}}{\RR^n \setminus W}
\geq 1/j\} \cap \{\VEC{x} : \|\VEC{x}\| \leq j\}$.  The sets $W_j$ are
compact and $\displaystyle W_j \subset W_{j+1}^\circ$ for all $j\geq
1$.  We may therefore used (ii).  If $W$ is not open, we use (ii) with
the open set $\displaystyle \bigcup_{\alpha \in A} U_\alpha\supset W$
instead of $W$.
\end{proof}
...
$\displaystyle W \subset \RR^n$, then the
...
\begin{defn}
The collection $\displaystyle \{\phi_j\}_{j \in \NNp}$ of the
previous theorem is called a
{\bfseries partition of unity}\index{Partition of Unity}
subordinated to the open cover $\displaystyle \{U_\alpha\}_{\alpha\in A}$
of $W$.
\end{defn}

\begin{prop} \label{cov4}
Suppose that $\displaystyle \{\phi_j\}_{i \in \NNp}$ is a partition of
unity subordinated to the open cover $\displaystyle \{U_\alpha\}_{\alpha\in A}$
...
\begin{defn}
Let $W$ be a Jordan measurable subset of $\displaystyle \RR^n$.  In
this and the next sections, we denote the Riemann integral of a Riemann
integrable function $h:W\to \RR$ by $\displaystyle \rint_W h $.
\end{defn}
...
bounded rectangle $\displaystyle R \subset \RR^n$ such that
...
$\displaystyle \int_W \psi \, f  = \int_R \psi \, f$.
\end{proof}
...
subsets of $W$ and continuous on $W$ except possibly on a set of
measure zero.
Suppose that $\displaystyle \{\phi_j\}_{j \in \NNp}$ and
$\displaystyle \{\tilde{\phi}_k\}_{k \in \NNp}$
are two partitions of unity subordinated respectively to the admissible
open covers $\displaystyle \{U_{\alpha}\}_{\alpha\in A}$
and
$\displaystyle \{\tilde{U}_{\tilde{\alpha}}\}_{\tilde{\alpha}\in \tilde{A}}$
of the open set $W$.
Then $\displaystyle \sum_{j \in \NNp} \rint_W \phi_j |f|$
converges if and only if
$\displaystyle \sum_{k \in \NNp} \rint_W \tilde{\phi}_k |f|$
...
Let $K_j = \supp \phi_j |f|$ for $\displaystyle j \in \NNp$.
...
$\displaystyle \tilde{\phi}_k\big|_{K_j} \neq 0$.
It follows that the sum
$\displaystyle \sum_{k \in \NNp} \tilde{\phi}_k \phi_j |f|$ is a
finite sum for each $j$.  A similar argument proves that 
$\displaystyle \sum_{j \in \NNp} \phi_j\tilde{\phi}_k|f|$
...
$\displaystyle \sum_{k \in \NNp} \tilde{\phi}_k(\VEC{x}) = 1$ for all
...
comes from $\displaystyle \sum_{j\in \NNp} \phi_j(\VEC{x}) = 1$ for all
$\VEC{x} \in W$.
\end{proof}
...
Suppose that $\displaystyle \{\phi_j\}_{j \in \NNp}$
is a partition of unity subordinated to an admissible open cover
$\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ of the open set $W$.
...
on $W$ if
$\displaystyle \sum_{j \in \NNp} \rint_W \phi_j |f|$ converges.
\end{defn}
...
\begin{prop}
Suppose that $\displaystyle \{\phi_j\}_{j \in \NNp}$ and
$\displaystyle \{\tilde{\phi}_j\}_{j \in \NNp}$ are two partitions of
unity subordinated respectively to the admissible open cover 
$\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ and
$\displaystyle \{\tilde{U}_{\tilde{\alpha}}\}_{\tilde{\alpha}\in \tilde{A}}$
of the open set $W$.
Let $f:W\to \RR$ be a function which is bounded on bounded open
subsets of $W$ and continuous on $W$ except possibly on a set of
...
$\displaystyle \sum_{j \in \NNp} \rint_W \phi_j f 
= \sum_{j \in \NNp} \rint_W \tilde{\phi}_j f$.
\end{prop}

\begin{proof}
Since $f$ is integrable,
$\displaystyle \sum_{j \in \NNp} \rint_W \phi_j |f|$ and
$\displaystyle \sum_{k \in \NNp} \rint_W \tilde{\phi}_k |f|$
converge (and are equal).  Thus
$\displaystyle \sum_{j \in \NNp} \rint_W \phi_j f$ and
$\displaystyle \sum_{k \in \NNp} \rint_W \tilde{\phi}_k f$
converge absolutely because
$\displaystyle \left| \rint_W \phi_j f\right|
\leq \rint_W \phi_j |f|$ and for all $\displaystyle j \in \NNp$ and
$\displaystyle \left| \rint_W \tilde{\phi}_k f\right|
\leq \rint_W \tilde{\phi}_k |f|$ for all $\displaystyle k \in \NNp$.

As in the proof of Proposition~\ref{cov3}, we have that
$\displaystyle \sum_{k \in \NNp} \tilde{\phi}_k \phi_j f$ is a finite
sum for each $j$ and
$\displaystyle \sum_{j \in \NNp} \phi_j \tilde{\phi}_k f$
is a finite sum for each $k$.  It also follows from
(\ref{cov5}) in the proof of Proposition~\ref{cov3} that
$\displaystyle \sum_{j \in \NNp} \sum_{k \in \NNp}
...
\end{align*}
The first equality comes from
$\displaystyle \sum_{k \in \NNp} \tilde{\phi}_k(\VEC{x}) = 1$
for all $\VEC{x}$.
...
$\displaystyle \sum_{j \in \NNp} \phi_j(\VEC{x}) = 1$ for all
$\VEC{x}$.
\end{proof}
...
$\displaystyle C^\infty$ functions $\phi_j$ provided by the partition
...
Let $W$ be an open and Jordan measurable subset of $\displaystyle \RR^n$ and
$f:W\to \RR$ be a Riemann integrable function on $W$.  Then $f$ is
integrable on $W$ in the new sense.
\end{theorem}

\begin{proof}
Suppose that $\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ is an
admissible open cover of $W$ and that
$\displaystyle \{\phi_j\}_{j \in \NNp}$ is a partition of
unity subordinate to $\displaystyle \{U_{\alpha}\}_{\alpha\in A}$.
...
$\displaystyle Q \subset \NNp$, we have that
\[
...
$\displaystyle \sum_{j\in Q} \phi_j(\VEC{x})
\leq \sum_{j \in \NNp} \phi_j(\VEC{x}) = 1$
for all $\VEC{x} \in W$ and the third inequality comes from
$\displaystyle \rint_W \Chi_W = V(W)$ and $\displaystyle W \subset K$.  Since
$\displaystyle \sum_{j \in \NNp} \rint_W \phi_j |f|$ is a
...
\begin{theorem}
Suppose that $\displaystyle W \subset \RR^n$ is an open and Jordan
...
$\displaystyle \rint_{W\setminus K} 1 < \epsilon/M$.  More precisely,
to find $K$, we use a partition $P$ of a rectangle $R$ containing $W$
such that $\displaystyle \LL(\Chi_W) - \LL_P(\Chi_W) < \epsilon/M$.  Note that
...
Let $\displaystyle K = \bigcup_{R_{k_1,k_2,\ldots,k_n} \subset W}
...
\stage{ii} Suppose that $\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ is an
admissible open cover of $W$ and that
$\displaystyle \{\phi_j\}_{j \in \NNp}$ is a partition of
unity subordinate to $\displaystyle \{U_{\alpha}\}_{\alpha\in A}$.

According to Proposition~\ref{cov4}, there are only finitely many
$\phi_j$ such that $\displaystyle \phi_j\big|_K \neq 0$.
Let $\displaystyle Q\subset \NNp$ be the set of
indices $\displaystyle j \in \NNp$ such that
$\phi_j\big|_K \neq 0$.  Then, for any finite set
$\displaystyle G \subset \NNp$ such that $Q\subset G$, we have that
\begin{align*}
...
$\displaystyle \sum_{j\in Q} \phi_j(\VEC{x}) = 1$
for all $\VEC{x}\in K$ because the other $\phi_j$ are null on
$K$.  In particular, this last inequality implies that
$\displaystyle 1 - \sum_{j\in Q} \phi_j(\VEC{x})$ may be
non zero only if $\VEC{x} \in W\setminus K$.  Since
$\displaystyle G\subset \NNp$ with $Q \subset G$ is arbitrary, we get that
\[
...
Let $\displaystyle W\subset \RR^n$ be an open set, and
$\displaystyle g:W\to \RR^n$ be a
...
Let $\displaystyle W\subset \RR^n$ be an open set, and
$\displaystyle g:W\to \RR^n$ be a one-to-one and continuously
...
Let $\displaystyle W\subset \RR^n$ be an open set and
$\displaystyle f:W\to \RR^n$ be continuously differentiable function.
If $A \subset W$ has measure zero, then $f(A)$ has measure zero.
\end{lemma}

\begin{proof}
We can write $W$ as the countable union of compact sets
$Q_j$ for $\displaystyle j \in \NNp$ such that
$\displaystyle Q_j \subset Q_{j+1}^\circ \subset Q_{j+1} \subset W$
for all $j$.  For instance, we may write
$\displaystyle W = \bigcup_{j \in \NNp} Q_j$ where $Q_j$ is the compact set
$\displaystyle Q_j = \{ \VEC{x} \in W :
...
We consider an arbitrary but fixed $\displaystyle j \in \NNp$.
Let $M$ be the maximum of $\|\displaystyle \diff f\|$ on $Q_{j+1}$.
...
exists a collection of rectangles $\displaystyle \{ R_i \}_{i\geq 0}$ such
that $\displaystyle Z \subset \bigcup_{i=0}^\infty R_i$ and
$\displaystyle \sum_{i=0}^\infty V(R_i) < \frac{\epsilon}{n^{n/2}M^n}$.
By subdividing the rectangles $R_i$ if necessary we may assume that
the $R_i$ are squares with edges of length
$\displaystyle d_i < \frac{1}{j(j+1)\sqrt{n}}$.
Hence $\displaystyle \diam R_i < \frac{1}{j(j+1)}$.
Thus $R_i \subset Q_{j+1}^\circ$ for all $i$.
\pdfbox{mult_integrals/measurefC1}
Using the Mean Value Theorem for functions from
$\displaystyle \RR^n$ to $\displaystyle \RR^n$ (Theorem 9.19 in \cite{R}),
we have that
...
\]
for all $i$.  Thus, each $\displaystyle f(R_i)$ is a subset of a
square $\displaystyle S_i \subset \RR^n$ with edges of length
$\sqrt{n}\, M d_i$.  The volume of $S_i$ is
$\displaystyle V(S_i) = n^{n/2} M^n d_i^n$.  We have that
\[
...
\]
because $\displaystyle V(R_i) = d_i^n$.
Since $\epsilon$ is arbitrary, this proves that
$\displaystyle f(Z)$ has measure zero.
\end{proof}
...
We have that $\displaystyle g:W\to \RR^n$ is a homeomorphism of $W$
...
$\displaystyle \RR^n$ for all open sets $V \subset W$.  Note that open
subsets of $W$ with the induced topology from $\displaystyle \RR^n$
are open subsets of $\RR^n$ because $W$ is open.} thanks to the
Inverse Function Theorem.
Thus $\displaystyle g^{-1}(K) \subset W$ is a compact set because
continuous functions map compact sets onto compacts sets.

Since $(f \circ g)(\VEC{x}) = 0$ for
$\displaystyle \VEC{x} \in W \setminus g^{-1}(K)$,
the support of $(f \circ g)\, |\det \diff g|: W \to \RR$ is a subset
of the compact set $\displaystyle g^{-1}(K)$.  Therefore the support
...
$\displaystyle g^{-1}(K)$ reaches it maximum a point in
$\displaystyle g^{-1}(K)$.  Therefore
...
set of measure zero where $f$ is discontinuous, we show that
$\displaystyle g^{-1}(Z) \subset g^{-1}(K) \subset W$ is the set of
...
also get that
$\diff g^{-1}(\VEC{y})$ exists for all $\VEC{y} \in g(W)$.  In particular,
$\displaystyle \diff g^{-1}(\VEC{y})
= (\diff g)^{-1}(g^{-1}(\VEC{y}))$ for all $\VEC{y} \in g(W)$ according
to the Inverse Function Theorem.  Thus $g:W \to g(W)$ is a diffeomorphism.

It follows from Lemma~\ref{lemC1Meas0} that $\displaystyle g^{-1}(Z)$
has measure zero.
...
$\displaystyle h^+ = (h + |h|)/2$ and $\displaystyle h^- = (|h|-h)/2$.
We have that $\displaystyle h = h^+ - h^-$
with $\displaystyle h^+(\VEC{x}) \geq 0$ and
$\displaystyle h^-(\VEC{x}) \geq 0$ for all
...
$\displaystyle h^+$ and $\displaystyle h^-$ are Riemann integrable 
...
$\displaystyle f: \RR^n \to [0,\infty[$ by
setting $f(\VEC{x})=0$ for $\displaystyle \VEC{x} \in \RR^n \setminus g(W)$.
...
$\displaystyle 0 \leq \sum_{R_{k_1,k_2,\ldots,k_n} \subset g(W)}
...
because $\displaystyle \supp f \subset
\bigcup_{R_{k_1,k_2,\ldots,k_n}\cap \supp f \neq \emptyset}
R_{k_1,k_2,\ldots,k_n}$ and
$\displaystyle f \leq \sum_{R_{k_1,k_2,\ldots,k_n} \cap \supp f \neq \emptyset}
M_{k_1,k_2,\ldots,k_n} \Chi_{R_{k_1,k_2,\ldots,k_n}}$.
...
Let $\displaystyle W\subset \RR^n$ be an open set.
Proposition~\ref{cov8} is true if $\displaystyle g:W\to \RR^n$ is an
...
$\displaystyle g^{-1}(Q)$ because $|\det g|$ is constant.

Since $\displaystyle g:\RR^n \to \RR^n$ is a linear map such that
$\det g \neq 0$, we may express $g$ as $g = G_1 G_2 \ldots G_s$ where
the $G_i$ are linear maps $\displaystyle G:\RR^n \to \RR^n$ of the
following forms.
\begin{enumerate}
...
differentiable functions $\displaystyle g:W\to \RR^n$ such that
...
Since $(f \circ T) |\det T| : T^{-1}(h(W)) \to \RR$ is a Riemann
integrable function with compact support
and $\displaystyle g = T^{-1}\circ h: W \to \RR^n$ is a
one-to-one and continuously differentiable function such that
\[
\det (\diff g(\VEC{x}))
...
open intervals, we may use the linearity of the integral to reduce the
...
$\displaystyle g^{-1}(\supp f) \subset W$ is a compact set because $\supp f$ 
is compact and $\displaystyle g^{-1}:g(W) \to \RR$ is continuous as we
have shown in the proof of Lemma~\ref{cov27}, and
since $W$ is connected, there exists a closed interval $[\alpha,\beta]
\subset W$ such that $\displaystyle g^{-1}(\supp f) \subset [\alpha,\beta]$.  
...
$\displaystyle \tilde{g} = T^{-1} \circ g$.  The map
$\displaystyle \tilde{g}:W \to \RR^k$ is a
...
\diff Q_1(\VEC{x}) =
\begin{pmatrix}
\displaystyle \pdydx{\tilde{g}_1}{x_1}(\VEC{x}) &
\displaystyle \pdydx{\tilde{g}_1}{x_2}(\VEC{x}) & \ldots &
\displaystyle \pdydx{\tilde{g}_1}{x_k}(\VEC{x}) \\
\vdots & \vdots & \ddots & \vdots \\
\displaystyle \pdydx{\tilde{g}_{k-1}}{x_1}(\VEC{x}) &
\displaystyle \pdydx{\tilde{g}_{k-1}}{x_2}(\VEC{x}) & \ldots &
\displaystyle \pdydx{\tilde{g}_{k-1}}{x_k}(\VEC{x}) \\[1em]
0 & 0 & \ldots & 1
\end{pmatrix} \ .
\end{equation}
...
$\displaystyle \diff Q_1:W\to \RR^k$
...
that $\displaystyle Q_1:U_1\to \RR^k$ is a diffeomorphism of the open
...
0 & \ldots & 1 & 0 \\
\displaystyle \pdfdx{(\tilde{g}_k \circ Q_1^{-1})}{x_1}(\VEC{x}) &
\ldots &
\displaystyle \pdfdx{(\tilde{g}_k \circ Q_1^{-1})}{x_{k-1}}(\VEC{x}) &
\displaystyle \pdfdx{(\tilde{g}_k \circ Q_1^{-1})}{x_k}(\VEC{x})
\end{pmatrix} .
\end{equation}
Using the Inverse Function Theorem for $Q_1$, we get that
\[
...
$\displaystyle \diff Q_2:Q_1(U_1)\to \RR^k$ is continuous, we get that
...
that $\displaystyle Q_2:U_2\to \RR^k$ is a diffeomorphism of the open
...
$\displaystyle R_1 \subset \RR^{k-1}$ is a closed
...
$\displaystyle Q_1:U_1 \to \RR^k$ is
one-to-one.  Moreover
\[
\det \diff_{\VEC{y}} Q_{1,x_k}(\VEC{y})
= \det \diff Q_1(\VEC{y},x_k) \neq 0
\]
for all $\displaystyle (\VEC{y},x_k)\in U_1$ follows
...
Hence, by our hypothesis of induction, we have that
\[
\rint_{R_1} \dx{\VEC{y}}
...
$\displaystyle Q_1:U_1 \to \RR^k$ is a one-to-one and continuously
...
$Q_1^{-1}(R)$ is Jordan measurable because
$\displaystyle \partial Q_1^{-1}(R) = Q_1^{-1}(\partial R)$ is of
measure zero since $\partial R$ is of measure zero.  We also have that
$Q_1^{-1}(R)$ is bounded because $R$ is a compact set and thus
$Q_1^{-1}(R)$ is a compact set in $\displaystyle \RR^k$.
The Fubini's Theorem is applied to the function
$\displaystyle \Chi_{Q_1^{-1}(R)} |\det \diff Q_1|$ on a rectangle
containing $\displaystyle {Q_1^{-1}(R)}$.
...
\]
Let
$\displaystyle  Q_{2,\VEC{y}}(x_k) = \tilde{g}_k(Q_1^{-1}(\VEC{y},x_k))$
...
$\displaystyle Q_2:U_2 \to \RR^k$ is one-to-one.  Moreover
\[
...
$\displaystyle \det \diff Q_2 (\VEC{y},x_k) =
...
needs some clarifications.  Since
$\displaystyle Q_2:U_2 \to \RR^k$ is a one-to-one and continuously
differentiable function with $\det \diff Q_2 (\VEC{x}) \neq 0$ for all
$\VEC{x} \in U_2$, we have that $Q_2: U_2 \to Q(U_2)$ is a diffeomorphism.
It follows as in (iii.a) that $Q_2^{-1}(R)$ is a bounded and Jordan
measurable set.  The Fubini's Theorem is applied to the function
$\displaystyle \Chi_{Q_2^{-1}(R)} |\det \diff Q_2|$ on a rectangle
containing $\displaystyle {Q_2^{-1}(R)}$.
...
Recall that $\tilde{g} = Q_2\circ Q_1 = T^{-1} \circ g$ on $U_1$.
...
$\displaystyle \{ U_{\VEC{a}_i} \}_{1 \leq i \leq N}$ of open subsets of $W$
such that $\displaystyle \supp f \subset \bigcup_{i=1}^N g(U_{\VEC{a}_i})$
and (\ref{cov22}) is true for all Riemann integrable functions
with compact support in $g(U_{\VEC{a}_i})$.
Do not forget that $\displaystyle g:W\to \RR^n$ is an open mapping and so
$g(U_{\VEC{a}_i})$ is an open set for all $i$.

Let $\displaystyle \{\phi_i\}_{1 \leq i \leq N}$ be a partition of unity
subordinate to the open cover
$\displaystyle \{ g(U_{\VEC{a}_i}) \}_{1 \leq i \leq N}$ of $\supp f$
as described in Remark~\ref{rmkFPUcase}.  In particular,
$\supp \phi_i \subset g(U_{\VEC{a}_i})$ for all $i$.
We have
$\displaystyle f = \sum_{i=1}^N \phi_i f$ on $g(W)$ and
\begin{align*}
...
cover $\displaystyle \{\tilde{U}_{\alpha}\}_{\alpha\in A}$ of $g(W)$ and a
partition of unity $\displaystyle \{\phi_j\}_{j \in \NNp}$
subordinate to $\displaystyle \{\tilde{U}_{\alpha}\}_{\alpha\in A}$ such that
\[
  \int_{g(W)} f = \sum_{j \in \NNp} \rint_{g(W)} \phi_j f \ .
\]
In particular, the sum converges absolutely.
For each $\displaystyle j \in \NNp$, we have that
...
$\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ where
$\displaystyle U_\alpha = g^{-1}(\tilde{U}_{\alpha})$ for all $\alpha$
is an admissible open cover of $W$ and
$\displaystyle \{\phi_j\circ g\}_{j \in \NNp}$ is
a partition of unity subordinate to
$\displaystyle \{U_{\alpha}\}_{\alpha\in A}$ of class $\displaystyle C^1$
(see Remark~\ref{cov38}).  We have that
$\displaystyle \sum_{j \in \NNp} (\phi_j\circ g)(\VEC{x}) = 1$
for all $\VEC{x} \in W$ and
$\displaystyle \supp (\phi_j \circ g)
...
\]
for $h=|f|$ and $h=f$, and all $\displaystyle j \in \NNp$.  Thus
\begin{align*}
...
\begin{defn}
Let $U$ be an open subset of $\displaystyle \RR^n$ and
$\displaystyle f:U \to \RR^n$ be a continuously differentiable
function.

A point $\displaystyle \VEC{x} \in U$ is 
...
A point $\displaystyle \VEC{y} \in \RR^n$ is
a {\bfseries regular value}\index{Regular value} of $f$ if
none of the points $\displaystyle \VEC{x} \in f^{-1}(\{\VEC{y}\})$
is a critical point.
\end{defn}
...
Let $U$ be an open subset of $\displaystyle \RR^n$ and
$\displaystyle f:U \to \RR^n$ be a continuously differentiable
function.  If $\displaystyle \VEC{y} \in \RR^n$ is
a regular value of $f$, then $\displaystyle f^{-1}(\{\VEC{y}\})$ is
the union of isolated points.
\end{prop}

\begin{proof}
Suppose that $\VEC{x} \in \displaystyle f^{-1}(\{\VEC{y}\})$.  Then
...
$\displaystyle U_{\VEC{x}} \cap f^{-1}(\{\VEC{y}\}) = \{\VEC{x}\}$.
\end{proof}
...
we have that $\displaystyle \pdydx{f_i}{x_j}$ is uniformly continuous
on $R$ for $1\leq i, j \leq n$.   Given $\epsilon > 0$, there exists
$\delta > 0$ such that
$\displaystyle \left| \pdydx{f_i}{x_j}(\VEC{y}) - \pdydx{f_i}{x_j}(\VEC{x})
\right| < \frac{\epsilon}{n}$ for $\|\VEC{y} - \VEC{x}\| < \delta$
and $1\leq i, j \leq n$.
Choose $N$ large enough such that $\displaystyle n (L/N)^2 < \delta^2$
and partition the rectangle $R$ into $\displaystyle N^n$ subrectangles
...
\end{equation}
for all $\VEC{y} \in R_{k_1,k_2,\ldots,k_n}$.  We get that
...
$C$ of radius
$\displaystyle (M n^{3/2} L)/N$ and height $2 (\epsilon L \sqrt{n})/N$
...
radius $(M n^{3/2} L)/N$ in the subspace $S$ of dimension $n-1$.  The
volume of a sphere of radius $r$ in $\displaystyle \RR^m$ is
$\displaystyle (2 \pi^{m/2} r^m)/(m \Gamma(m/2))$ \footnote{
Integrating $\displaystyle e^{-\pi \|\VEC{x}\|_2^2}$ over
$\displaystyle \RR^m$ using spherical coordinates (i.e.\ coordinates of the
form $\VEC{x} = r \VEC{y}$ with $\|\VEC{y}\|=1$), the reader can verify that
the volume of $B_1(\VEC{0})$ is $\displaystyle 2 \pi^{m/2}/(m\Gamma(m/2))$.
In particular, the volume of $B_1(\VEC{0})$ in
$\displaystyle \RR^2$ and $\displaystyle \RR^3$ are respectively $\pi$
and $4\pi/3$.}

Hence $f(A \cap R)$ is contained in the union of $\displaystyle N^n$
cylinders whose total volume is at most
$\displaystyle \epsilon B (L/N)^n N^n = \epsilon B L^n$.  Since $\epsilon$ is
...
$\displaystyle U = \bigcup_{j>0} K_j$.
For each $\VEC{x} \in K_j$, choose a closed rectangle
$R_{\VEC{x}} \subset U$ such that $\VEC{x} \in R_{\VEC{x}}$.
Since $\displaystyle \{ R_{\VEC{x}}^\circ \}_{\VEC{x}\in K_j}$ is an open
cover of the compact set $K_j$, there exists a finite subcover
$\displaystyle \{ R_{j,i}^\circ \}_{1 \leq i \leq I_j}$ of $K_j$.
The union of the $\displaystyle \{ R_{j,i} \}_{1 \leq i \leq I_j}$ for
$j>0$ is a countable collection of closed rectangles such that
$\displaystyle U = \bigcup_{1 \leq i \leq I_j,\, j>0} R_{j,i}$.
\end{rmk}
...
$[a,b]$ such that $\displaystyle \U_P(f) - \LL_P(f) < \epsilon$
...
$f$ is a subset 
of $\displaystyle E = \bigcup_{i=1}^N E_i$ and the area of $E$ is
\[
...
$\displaystyle \left| A(S) - \int_a^b f(x)\dx{x} \right| < \epsilon$.
Since $\epsilon$ is arbitrary, we get that
$\displaystyle A(S) = \int_a^b f(x)\dx{x}$.
\end{sol}

\begin{question}
Let $S$ be a bounded subset of $\displaystyle \RR^2$.   \label{innerarea}
Prove that $S$ and $\displaystyle S^\circ$ have the same inner area;
namely, show that $\LL(\Chi_S) = \LL(\Chi_{S^\circ})$.
Note that we do not require $S$ to be Jordan measurable.  If it was, the
question would be easy to answer because
$\displaystyle S \setminus S^\circ \subset \partial S$ would be a
...
Given $\displaystyle V \subset \RR^2$, let
\[
...
We have that $m_{i,j}(S^\circ) \leq m_{i,j}(S)$ for $0 < i \leq N$ and
$0 < j \leq M$.
Hence
\begin{align*}
\LL_{\{P,Q\}}(\Chi_{S^\circ})
...
$\displaystyle \delta < \frac{\epsilon}{\LL(\Chi_S)}$.
...
$\displaystyle \breve{R}_{i,j} \subset R_{i,j}^\circ \subset S^\circ$ by
shrinking the rectangle $R_{i,j}$ by a factor of
$\sqrt{1-\delta}$ in both directions.
We have that $A(\breve{R}_{i,j}) = (1-\delta) A(R_{i,j})$ and
...
$\breve{R}_{i,j}$, we get that
\[
  \LL(\Chi_{S^\circ}) \geq \left( 1 - \frac{\epsilon}{ \LL(\Chi_S)}\right)
\LL_{\{P.Q\}}(\Chi_S) \ .
\]
Since this relation is true for all partitions $\{P,Q\}$ of $R$, we get that
\[
  \LL(\Chi_{S^\circ}) \geq \left( 1 - \frac{\epsilon}{ \LL(\Chi_S)}\right)
...
\begin{question}
Let $S$ be a bounded subset of $\displaystyle \RR^2$.    \label{outerarea}
...
\begin{question}
Let $S$ be a bounded subset of $\displaystyle \RR^2$.  Prove that outer
...
\begin{sol}
We have that $\displaystyle \partial S = \overline{S} \setminus S^\circ$.
...
\end{cases}
\]
for all $\displaystyle V \subset \RR^2$.
...
partitions $\{P,Q\}$ of $R$, we get that
$\displaystyle \U(\Chi_S) \leq \LL(\Chi_{S^\circ}) + \U(\Chi_{\partial S})$.

\stage{ii} We now prove that
$\displaystyle \U(\Chi_S) \geq \LL(\Chi_S) + \U(\Chi_{\partial S})$.
Again, according to the result of Question~\ref{innerarea}, it is
enough to prove that
$\displaystyle \U(\Chi_S) \geq \LL(\Chi_{S^\circ}) + \U(\Chi_{\partial S})$.

Let $R$ be rectangle containing $\overline{S}$.  Given $\epsilon >0$,
choose a partition $\{\tilde{P},\tilde{Q}\}$ of $R$ such that
$\displaystyle \LL_{\{\tilde{P},\tilde{Q}\}}(\Chi_{S^\circ})
...
$\{\tilde{P},\tilde{Q}\}$, we get that
$\displaystyle \U(\Chi_S) \geq \LL(\Chi_{S^\circ}) - \epsilon +
\U(\Chi_{\partial S})$.
...
$\displaystyle S\subset \RR^n$ be a compact, convex and Jordan
...
$\displaystyle \int_S f(\VEC{x}) g(\VEC{x}) \dx{\VEC{x}} =
...
Let
$\displaystyle m = \inf\{ f(\VEC{x}) : \VEC{x} \in S \}$ and
$\displaystyle M = \sup\{ f(\VEC{x}) : \VEC{x} \in S \}$.
...
Since $m \leq f(\VEC{x}) \leq M$ for all $\VEC{x} \in S$, we get that
\[
...
$\displaystyle \int_S g(\VEC{x}) \dx{\VEC{x}} = 0$.  We therefore
assume that $\displaystyle \int_S g(\VEC{x}) \dx{\VEC{x}} >0$.
Thus
\[
...
$\displaystyle \ell = \{\VEC{x}_1 + t(\VEC{x}_2-\VEC{x}_1) : 0 \leq t \leq 1\}$
...
\end{cases}
\]
Prove that $\displaystyle \int_0^1 \int_0^1 f(x,y) \dx{y}\dx{x} =1$ but
$\displaystyle \int_0^1 \int_0^1 f(x,y) \dx{x}\dx{y}$ does not exist.
\end{question}

\begin{question}
Consider the function
\[
f(x,y) = \begin{cases} \displaystyle \frac{1}{q} &
...
\subQ{d} Prove that
$\displaystyle \int_0^1 \int_0^1 f(x,y) \dx{x}\dx{y}$ and
$\displaystyle \int_0^1 \int_0^1 f(x,y) \dx{y}\dx{x}$ exist but are
not equal.
\end{question}
...
$\displaystyle f(z/2,z) = 1/z^2 \to \infty$ as $z \to 0$
and $\displaystyle f(z,z/2) = -1/z^2 \to -\infty$ as $z \to 0$.

\subQ{b} For $y \in ]0,1[$ fixed, we have that
\[
f_y(x) = f(x,y) =
\begin{cases}
y^{-2} & \quad \text{if}\ 0 < x < y < 1 \\
...
\end{cases}
\]
$f_y$ is integrable because it is a bounded function and the set of
points where $f_y$ is discontinuous is $\{0, y,1\}$ which is a set of
zero content.  We have that
\begin{align*}
...
Hence $f_y$ is integrable and $\displaystyle \int_0^1 f_y(x) \dx{x} = 0$.

\subQ{c} For $x \in ]0,1[$ fixed, we have that
\[
f_x(y) = f(x,y) =
\begin{cases}
-x^{-2} & \quad \text{if}\ 0 < y < x < 1 \\
...
\end{cases}
\]
$f_x$ is integrable because it is a bounded function and the set of
points where $f_x$ is discontinuous is the set $\{0, x,1\}$ of zero
...
Hence $f_x$ is integrable and $\displaystyle \int_0^1 f_x(y) \dx{y} = 0$.

\subQ{d}
Let $\displaystyle g(x) = \int_0^1 f_x(y) \dx{y}$ for $0 \leq x \leq 1$
and $\displaystyle h(y) = \int_0^1 f_y(x) \dx{x}$ for $0 \leq y \leq 1$.
We have that
\[
g(x) = \begin{cases}
-1 & \quad \text{if} \ 0 < x <1 \\
...
g_2(x) \right\}$ (Figure~\ref{VOLUME3}), it follows from Fubini's
...
\begin{question}
Consider the region $D$ enclosed by the curve $\displaystyle y=x^3$,
and the lines $x=1$ and $y=0$.  Give two integrals to compute the mass
of this thin region if the density is given by
$\displaystyle \rho(x,y) = \sqrt{x^4+1}$.  Compute the mass of this
...
$\displaystyle m = \iint_D \rho = \int_0^1 \int_0^{x^3} \rho(x,y) \dx{y}\dx{x} =
...
\begin{question}
Evaluate $\displaystyle I = \iint_S (x^2 - \sqrt{y})$, where $S$
is the region enclosed by the parabola $\displaystyle x=y^2$ and the
line $x=2y$.
\end{question}

\begin{sol}
The ordinates ($y$ values) of the points of intersection of
$\displaystyle x=y^2$ and $x=2y$ are given by
\[
...
We have that
$\displaystyle I = \int_0^4 \int_{x/2}^{\sqrt{x}} (x^2 - \sqrt{y})\dx{y}\dx{x}
...
\begin{question}
Express the following double integrals $\displaystyle \iint_S f$
in terms of iterated integrals in two different ways.

\subQ{a} $S$ is the region enclosed by $\displaystyle y=x^{1/3}$ and
$\displaystyle y=x^2$ for $y\geq 0$.\\
...
$\displaystyle y=x^2$ with $\displaystyle y=x^{1/3}$ are given by
\[
...
\subQ{a} $S$ is the region enclosed by $\displaystyle y=x^3$ and
$y=4x$ for $y\leq 0$.\\
\subQ{b} $S$ is the triangle with vertices at $(0,0)$, $(2,2)$ and
$(3,1)$.\\
\subQ{c} $S$ is the region enclosed by the parabolas $\displaystyle y=x^2$ and
$\displaystyle y=6 - 4x -x^2$.
\end{question}

\begin{sol}
\subQ{a} The abscissas (x values) of the points of intersection of
$\displaystyle y=x^3$ with $y=4x$ are given by
\[
...
parabolas $\displaystyle y=x^2$ and
$\displaystyle y=6 - 4x -x^2$ are given by
\[
...
\begin{question}
Evaluate the integral $\displaystyle \iint_S f$ in each of the
following cases.

\subQ{a} $\displaystyle f(x,y) = ye^{2x}$ and $S$ is the region
enclosed by $y=x$, $y=1$, $y=3$ and the $y$ axis.\\
\subQ{b} $\displaystyle f(x,y) = \cos(y^3+1)$ and $S$ is the region enclosed by
$y=\sqrt{x}$, $y=1$ and the $y$ axis.\\
\subQ{c} $\displaystyle f(x,y) = y e^{xy}$ and $S$ is the region enclosed by
$y=1/x$, $x=2$ and $y=1$.
\end{question}
...
$\displaystyle \int_0^1 \int_x^{1-x} f(y) \dx{y} \dx{x}$
...
$\displaystyle \int_0^1 \int_x^{1-x} f(y) \dx{y}\dx{x}$ because
\begin{align*}
...
&= \int_0^{1/2} \int_{x}^{1-x} f(y)\dx{y}\dx{x}
  - \int_{1/2}^1 \int_{1-x}^x f(y) \dx{y}\dx{x} \ .
\end{align*}
To express $\displaystyle \iint_S f$ as the sum of two
...
$\displaystyle \int_1^3 \int_1^{\sqrt{x^2-1}} f(x,y)\dx{y}\dx{x}$.
...
\begin{question}
Reverse the order of integration of the integral
$\displaystyle \int_{-6}^8 \int_{x^{1/3}}^{(x+6)/7} xy \dx{y}\dx{x}$.
\end{question}

\begin{question}
Reverse the order of integration in the integral
$\displaystyle \int_0^a \int_0^{\sqrt{2ay-y^2}} f(x,y) \dx{x}\dx{y}$.
\end{question}

\begin{sol}
From $\displaystyle x=\sqrt{2ay-y^2}$, we get that
$\displaystyle x^2=2ay-y^2$.  By completing the square with respect to
the variable $y$, we get that $\displaystyle x^2 - a^2 = -(y - a)^2$.
Thus, the boundary of the domain of integration $S$ is an arc of the
circle $\displaystyle x^2+ (y - a)^2 = a^2$.  The following figure
shows the domain of integration.
\pdfbox{mult_integrals/midterm_1}
Solving $\displaystyle x^2+ (y - a)^2 = a^2$ for $y$, we get that
$\displaystyle y = a - \sqrt{a^2 - x^2}$ on the lower part of the circle.
Hence
\[
...
\begin{question}
Compute the integral of $\displaystyle f(x,y)=x^2+y^2$ over the region $D$
enclosed by the parabola $\displaystyle x=y^2-2y$ and the line $y=x$.
\end{question}
...
\begin{question}
Evaluate $\displaystyle I = \iint_S (x + 3y^3)$ where $S$ is
the upper half of the disk $\displaystyle x^2 + y^2  \leq 4$.
\end{question}
...
\end{align*}
where the famous trigonometric identity $\sin^2(\theta) = 1 - \cos^2(\theta)$
...
We need to compute $\displaystyle A = \iint_S 1$.  Since $S$ is
...
\begin{question}
Evaluate $\displaystyle \iint_S e^{-(x^2+xy+y^2)}$ where
$\displaystyle S = \{ (x,y) : x^2 + xy + y^2 \leq 1 \}$.
\end{question}
...
\]
Let $\displaystyle A = \begin{pmatrix} 1 & 1/2 \\ 1/2 & 1 \end{pmatrix}$.
...
If $\lambda_1 = \lambda_2$, we can always find two linearly
...
Hence, with
$R = \begin{pmatrix} \VEC{v}_1 & \VEC{v}_2\end{pmatrix}$, we have that
$\displaystyle R^\top A R = \begin{pmatrix} \lambda_1 & 0 \\ 0 &
...
$\displaystyle R = \begin{pmatrix} 1/\sqrt{2} & -1/\sqrt{2} \\
1/\sqrt{2} & 1/\sqrt{2} \end{pmatrix}$, we get that
$\displaystyle R^\top A R = \begin{pmatrix} 3/2 & 0 \\ 0 & 1/2 \end{pmatrix}$.
Note that $\displaystyle R = \begin{pmatrix}
...
$\displaystyle \left| \det \diff g \right| = \left| \det
...
\end{align*}
Hence $g$ maps
$\displaystyle D = \left\{ (u,v) : \frac{3u^2}{2} + \frac{v^2}{2} \leq 1
\right\}$ onto $S$.  We get that
\[
...
$\displaystyle \begin{pmatrix} x \\ y \end{pmatrix} = g(u,v)
= \begin{pmatrix} 2(u+v)/5 \\ (u+6v)/5 \end{pmatrix}$.  We have that
$\displaystyle |\det \diff g(u,v)| = \left|\det
...
are found as it follows.  Let $\displaystyle m = \iint_D \rho$ be the
mass of the thin plate.  Then
$\displaystyle \overline{x} = \frac{1}{m} \iint_D x \rho$ and 
$\displaystyle \overline{x} = \frac{1}{m} \iint_D y \rho$.
...
\begin{question}
Compute the integral $\displaystyle \iint_R (2x+y)^2(3y-x)^{-2}$
...
= \left|\det
\begin{pmatrix} \displaystyle \frac{1}{2}u^{-1/2}v^{-1/2} &
\displaystyle -\frac{1}{2} u^{1/2} v^{-3/2} \\[0.7em]
\displaystyle \frac{1}{2} u^{-1/2}v^{1/2} &
\displaystyle \frac{1}{2} u^{1/2} v^{-1/2}
...
$\displaystyle x^2 -y^2 = 1$ and $\displaystyle x^2-y^2= 9$ for $x$,
$y > 0$.  Compute the mass of the thin plate represented by this
region if the density is given by $\displaystyle \rho(x,y) = (x^2+y^2)^3$.
\end{question}
...
We want to compute the mass $\displaystyle m = \iint_S (x^2+y^2)^3$.
...
$\displaystyle (x^2+y^2)^3 = f(g(x,y))\, \left|\det \diff g(x,y)\right|$
and $g:R \to S$ is our change of variables.  Let $u = xy$ and
$\displaystyle v=x^2 - y^2$.  If we try to express $x$ and $y$ in
terms of $u$ and $v$, we get ugly formulae.  Therefore, we proceed
slightly differently than usual.  Let
$\displaystyle \begin{pmatrix} u \\ v \end{pmatrix} = g(x,y)
= \begin{pmatrix} xy \\ x^2 - y^2 \end{pmatrix}$
with $S = \{ (u,v) : 1 \leq u \leq 4 \text{ and } 1 \leq v \leq 9 \}$.
We have that
$\displaystyle \left| \det \diff g(x,y) \right)
=\left| \frac{\partial(u,v)}{\partial(x,y)} \right|
= \left| \det \begin{pmatrix} y & x \\ 2x & -2y \end{pmatrix} \right|
= 2(x^2 + y^2)$ and
$\displaystyle f(u,v) = \frac{v^2 + 4 u^2}{2} = \frac{(x^2+y^2)^22}{2}$.
Hence, the mass is
\begin{align*}
m &= \iint_R (x^2+y^2)^3
...
\begin{question}
Evaluate the integral $\displaystyle \iint_S \frac{1}{x+y}$
...
\stage{Second method}
If we do not want to introduce an improper integral, we have to
...
we use $\displaystyle y = \left(\frac{v}{1-v}\right)x$ for $0 < 0 < 1$.
This substitution yields $y=0$ when $v=0$ and approaches $x=0$ when
$\displaystyle v \to 1^-$.  If we solve for $v$, we get
$\displaystyle v = \frac{y}{x+y}$.  Thus, the change of variables that
...
points), we have that $\displaystyle y = \left(\frac{v}{1-v}\right)x$
yields all the lines through the origin that cover $S$ except for the
segments
$\{(x,0) : 1 \leq x \leq 4\}$ and $\{(0,y) : 1 \leq y \leq 4\}$.

We have that $\displaystyle \left|\frac{\partial(x,y)}{\partial(u,v)} \right|
...
the $x,y$ plane bounded by the curve $\displaystyle x^2+y^2 = x$, and
whose height is given by $z=4-y$.
\end{question}

\begin{sol}
One can rewrite $\displaystyle x^2+y^2 = x$ as
...
given by the $\displaystyle \frac{x^2}{16}+\frac{y^2}{16}+z^2 \leq 1$ with a
...
$\displaystyle (2x+y +z)^2 + (2x-y)^2  + 4z^2 \leq 4$.
\end{question}

\begin{sol}
Let $u = 2x+y+z$, $v=2x-y$ and $w = 2z$.  We get that
$\displaystyle x = \frac{u}{4} + \frac{v}{4} - \frac{w}{8}$,
$\displaystyle y = \frac{u}{2} - \frac{v}{2} - \frac{w}{4}$ and
$\displaystyle z = \frac{w}{2}$.  This is the change of variables that
we use.  We have that
...
easier to use the formula $\displaystyle \frac{4r^3\pi}{3}$ to
...
reader.  We get that $\displaystyle V = \frac{4\pi}{3}$.
\end{sol}

\begin{question}
If $S$ is the solid enclosed by the paraboloid
$\displaystyle z=x^2+y^2$ and the plane $z=1$, write the iterated
...
\subQ{a}
$\displaystyle \int_0^1 \int_0^{1-y^2}\int_0^y f(x,y,z) \dx{z}\dx{x}\dx{y}$
in the orders $\dx{z}\dx{y}\dx{x}$ and $\dx{y}\dx{z}\dx{x}$.\\
\subQ{b}
$\displaystyle \int_0^2 \int_0^{1-x/2} \int_0^{x/2} f(x,y,z)\dx{z}\dx{y}\dx{x}$
in the orders $\dx{y}\dx{x}\dx{z}$ and $\dx{x}\dx{y}\dx{z}$.\\
\subQ{c}
$\displaystyle \int_0^1 \int_0^z \int_0^y f(x,y,z)\dx{x}\dx{y}\dx{z}$
in the orders $\dx{z}\dx{y}\dx{x}$ and $\dx{y}\dx{x}\dx{z}$.\\
\subQ{d}
$\displaystyle \int_0^1 \int_0^y \int_0^{x/\sqrt{3}} f(x,y,z)\dx{z}\dx{x}\dx{y}$
in the orders $\dx{z}\dx{y}\dx{x}$ and $\dx{y}\dx{z}\dx{x}$.\\
\subQ{e}
$\displaystyle \int_1^2 \int_1^z \int_{1/y}^2 f(x,y,z)\dx{x}\dx{y}\dx{z}$
in the orders $\dx{z}\dx{y}\dx{x}$ and $\dx{y}\dx{z}\dx{x}$.\\
\subQ{f}
$\displaystyle
...
\subQ{g}
$\displaystyle \int_0^2 \int_1^{2-x/2} \int_x^2 f(x,y,z)\dx{z}\dx{y}\dx{x}$
in the orders $\dx{y}\dx{x}\dx{z}$ and $\dx{x}\dx{y}\dx{z}$.
\end{question}
...
$\displaystyle x=1-y^2$.  We have that
\[
\iiint_S f =
...
\begin{align*}
\displaystyle \int_0^2 \int_0^{1-x/2} \int_0^{x/2} f(x,y,z)\dx{z}\dx{y}\dx{x}
&= \displaystyle \int_0^1 \int_{2z}^2 \int_0^{1-x/2} f(x,y,z)\dx{y}\dx{x}\dx{z}
\\
&= \displaystyle \int_0^1 \int_0^{1-z} \int_{2z}^{2-2y}
...
$z=x$ and $y=2 - x/2$ is the line given by $z = 4 -2y$.  Hence, we get that
\begin{align*}
& \int_0^2 \int_1^{2-x/2} \int_x^2 f(x,y,z)\dx{z}\dx{y}\dx{x}
...
$\displaystyle \rho(a,y,z) = x^2y$, compute the mass of the solid.
\end{question}

\begin{sol}
The abscissas of the points of intersection of the curve
$\displaystyle z=x^2-3$ and the line $z=x-1$ are given by
\[
...
The two surfaces in $\displaystyle \RR^3$ given by
$\displaystyle z=x^2-3$ and $z=x-1$
...
by the cylinder $\displaystyle x^2+z^2=9$ and the planes $y = -x/2 +1$,
$x=0$, $y=0$ and $z=0$.
\end{question}
...
- \frac{9}{2} \ .
\end{align*}
We have used the substitution $\displaystyle u = 9-x^2$ to compute the
integral $\displaystyle \int_0^2 \frac{x}{2}\sqrt{9-x^2} \dx{x}$, and the
trigonometric substitution $x=3\sin(\theta)$ to compute the integral
$\displaystyle \int_0^2 \sqrt{9-x^2} \dx{x}$.
\end{sol}

\begin{question}
Let $R$ be the solid obtained from the intersection of the cylinders
$\displaystyle x^2+y^2 = 4$ and $\displaystyle x^2+z^2=4$.  Compute the
volume of $R$ and its mass if the density is given by
$\displaystyle \rho(x,y,z) = x^2$.
\end{question}
...
and $(0,1,1)$, compute $\displaystyle \iiint_D xz$.
\end{question}
...
$(0,0,0)$, compute\\ $\displaystyle \iiint_D (xy+z)$.
\end{question}

\begin{question}
If $D$ is the pyramid with vertices at $(0,0,0)$, $(2,0,0)$, $(1,1,0)$ and
$(0,0,3)$, compute $\displaystyle \iiint_D xy$.
\end{question}
...
$\displaystyle z = 3\left(1 -\frac{x}{2} - \frac{y}{2}\right)$.
Hence
\begin{align*}
...
$\displaystyle \sqrt{x^2+y^2} \leq z \leq 1$ and $x \geq 0$.  We
...
$r = \sqrt{x^2+y^2} \leq z \leq 1$.  We have that
$\displaystyle
...
ball $\displaystyle x^2 + y^2 + z^2 \leq 4$ for $x$, $y$, $z \geq 0$.
...
by the sphere $\displaystyle x^2 + y^2 + z^2 = 4$ and the cylinder
...
$y = r\cos(\theta)$ and
$z=r\sin(\theta)$ for $\displaystyle -\sqrt{4-r^2} \leq x \leq \sqrt{4-r^2}$,
$0 \leq r \leq 1$ and $0 \leq \theta \leq 2\pi$.  We have that
$\displaystyle \left|\frac{\partial(x,y,z)}{\partial(x,r,\theta)}\right| = r$.
Hence
\begin{align*}
...
the sphere $\displaystyle x^2+y^2+z^2=25$ and the plane $z=4$.
...
\begin{sol}
The intersection of the sphere $\displaystyle x^2 + y^2 + z^2 = 25$
with the plane $z=4$ is the circle $\displaystyle x^2+y^2=9$.
...
$y=r\sin(\theta)$ and $z=z$ for $\displaystyle 4 \leq z \leq \sqrt{25 -r^2}$,
$0 \leq r \leq 3$ and $0 \leq \theta \leq 2\pi$.  We have that
$\displaystyle \left|\frac{\partial(x,y,z)}{\partial(r,\theta,z)}\right| = r$.
The volume is
$\displaystyle V = \int_0^{2\pi} \int_0^3  \int_{4}^{\sqrt{25-r^2}}
r \dx{z}\dx{r}\dx{\theta}$.
...
$\displaystyle V = \int_0^{2\pi} \int_0^{\arccos(4/5)}
...
cylinder $\displaystyle (x-1)^2+y^2 =1$, the cone
$\displaystyle x^2 + y^2 - (z-2)^2 = 0$ and the $x,y$ plane.
\end{question}
...
$0 \leq z \leq 2 - \sqrt{x^2+y^2} = 2 - \sqrt{r^2 + 2r \cos(\theta) +  1}$,
...
solid $S$ enclosed by the sphere $\displaystyle x^2+y^2+z^2=12$ and the cone
$\displaystyle 3z^2 = x^2 + y^2$ for $z\geq 0$.  Answer the same question in
...
$\displaystyle \left| \det \diff g \right| = r^2\sin(\phi)$, the
...
\begin{question}
Set up an integral in cylindrical coordinates to compute the volume of the
solid $S$ enclosed by the sphere $\displaystyle x^2+y^2+z^2=5$ and the
paraboloid $\displaystyle 4z = x^2 + y^2$.   Answer the same question
...
On the intersection of the sphere $\displaystyle x^2+y^2+z^2=5$ and
the paraboloid $\displaystyle 4z = x^2 + y^2$ where $z \geq 0$, we
have that
\[
...
\]
Thus, the intersection of the two surfaces is the circle
$\displaystyle x^2+y^2= 4$ with $z=1$.

In cylindrical coordinates, we have that
$x =r\cos(\theta)$, $y=r\sin(\theta)$ and $z=z$
for $0 \leq \theta \leq 2\pi$, $0 \leq r \leq 2$ and
$\displaystyle r^2/4 \leq z \leq \sqrt{5-r^2}$.  Since
$\displaystyle \left| \det \diff g \right| = r$, the volume of the
solid is
\[
V =
...
$\displaystyle 0 \leq r \leq 4 \cos(\phi)/\sin^2(\phi)$.
...
\end{align*}
Since $\displaystyle \left| \det \diff g \right| = r^2\sin(\phi)$,
...
$\displaystyle \int_1^\infty \frac{\sin(x)}{x} \dx{x}$.  We have seen
in Question~\ref{questsinoxCD} that this improper integral converges
but $\displaystyle \int_1^\infty \frac{|\sin(x)|}{x} \dx{x}$
diverges.  Thus $\displaystyle \frac{\sin(x)}{x}$ is a converging
...
diverge.  If they converge, compute their value.
\begin{center}
\begin{tabular}{*{1}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\displaystyle \iiint_{\RR^3} \frac{1}{1+x^2+y^2+z^2}$  &
\subQ{b} & $\displaystyle \iint_{x,y>0} \frac{1}{(1+x^2+y^2)^3}$ \\[0.8em]
\subQ{c} & $\displaystyle \iiint_{x^2+y^2+z^2<1}
\frac{z^2}{(x^2+y^2+z^2)^{3/2}}$ &
\subQ{d} & $\displaystyle \iint_{x>0} x e^{-x^2-y^2}$ \\[0.8em]
\subQ{e} & $\displaystyle \iint_{x^2+y^2<1} \frac{x^2}{(x^2+y^2)^2}$ &  &
\end{tabular}
\end{center}
\end{question}
...
$q\to \infty$.  We have that $\displaystyle S(q) = B_q(\VEC{0}) \subset \RR^3$
...
We have that $\displaystyle S(q) = B_q(\VEC{0}) \cap \{ (x,y): x,y \geq 0\} 
...
$q\to 0$.  We have that $\displaystyle S(q) = B_1(\VEC{0}) \setminus
B_q(\VEC{0}) \subset \RR^3$ in our formal definition of improper
integrals.  Since
$\displaystyle \left| \frac{\partial(x,y,z)}{\partial(r,\theta,\phi)} \right|
= r^2 \sin(\phi)$, we get that
\begin{align*}
...
We have that $\displaystyle S(q) = B_q(\VEC{0}) \cap \{(x,y): x \geq 0\}
\subset \RR^2$ in our formal definition of improper integrals.
Since
$\displaystyle \left| \frac{\partial(x,y)}{\partial(r,\theta)} \right| = r$,
we get that
...
\end{align*}
because $\displaystyle \lim_{q\to\infty} q e^{-q^2}= 0$ as can be
...
We have that $\displaystyle S(q) = B_1(\VEC{0}) \setminus
