...
partition of $[a,b]$, we get that $\LL(f) \leq \U(f)$.

\begin{defn}
Consider a bounded function $f:[a,b]\to \RR$.
If $\U(f) = \LL(f)$, we say that
...
$\U_R(f) - \U(f) < \epsilon/2$.  Since $f$ is integrable, we have that
$\U(f) = \LL(f)$.  Therefore $\U_R(f) - \LL_Q(f) < \epsilon$.  If we
use the partition $P = Q \cup R$, a refinement of both $Q$ and $R$, we
get $\U_P(f) - \LL_P(f) \leq \U_R(f) - \LL_Q(f) < \epsilon$ because
...
of $[a,b]$.  Since $f$ is decreasing, we have that
...
= \U(g) \ .
\]
Since $f:[a,b]\to \RR$ and $g:[a,b]\to \RR$ are integrable, we get that
\[
...
\begin{defn}
If $\displaystyle S \subset \RR$ is a Jordan measurable set and
...
From the previous lemma and (\ref{DarbouxEq2}), we have that
\[
...
$u_i = \phi(w_i)$ for each $i$.  We have that
...
\end{equation}
If $\displaystyle C = \sup \{|f(x)| : a \leq x \leq b \}$, we get from
(\ref{intCompZ}) that
\begin{align}
...
\left| \LL(f) - \LL ((f\circ \phi)\phi') \right|
 &  \leq \left| \LL(f) - \LL_{P}(f) \right|
  ...
  \leq \epsilon (C+2) \ .
\end{align*}
Since $\epsilon$ is arbitrary, we finally get that
...
and if they are integrable, then we have that
\[
\int_a^b f(x) \dx{x} = \LL(f) = \U(f)
...
$\tilde{\phi}(x) = -\phi(x)$ for $x \in [\alpha,\beta]$.  We have that
...
that $f_{k+1}(x) \leq f_k(x)$ for all $x \in D$ and all $k\geq 0$
...
$\displaystyle \{f_k\}_{k=0}^\infty$ is a sequence of continuous
real valued functions on $D$ such that
that $f_{k+1}(x) \leq f_k(x)$ for all $x \in D$ and
...
Given $x \in D$, we have that $\displaystyle x \in
...
\]
for all $k \geq K$.
Since $x \in D$ is arbitrary, we have that
...
\end{equation}
for all $k$.  Hence, if we let $k$ converges to $\infty$, we get that
...
continuous functions.  Moreover, for $0 \leq j < k$, we have that
\begin{align*}
...
for all $x \in [a,b]$ and $0 \leq i < k$, we have that
...
Let $g_k = |f_k - f|$ for $k \geq 1$.  We have that
...
Since $g_k \leq h_k$ for all $k \geq 0$, we have that
...
$P=\{p_i : 0 \leq i \leq N\}$ of $[a,b]$, we have that
...
\]
for all $x$ and $y$ in $[p_{i-1}, p_i]$, we get that
...
and $f(x)\geq 0$ for all $x\in [a,b]$.   If there exists $x_0 \in
[a,b]$ where $f$ is continuous at $x_0$ and $f(x_0)>0$, then
...
\]
for $1 \leq i \leq N$, we get that
\begin{align*}
\LL_P(f) &= \sum_{i=1}^N m_i(p_i - p_{i-1})
\leq F(b) - F(a) = \sum_{i=1}^N f(y_i)(p_i - p_{i-1}) \\
& \leq \sum_{i=1}^N M_i(p_i - p_{i-1}) = \U_P(f) \ .
\end{align*}
We also have that $\displaystyle \LL_P(f) \leq \int_a^b f(x) \dx{x}
...
\]
Since this is true for all $\epsilon >0$, we get that
...
\]
where $\tilde{m}_i = \inf\{ h^2(x) : p_{i-1}\leq x \leq p_i \}$ and
$\tilde{M}_i = \sup\{ h^2(x) : p_{i-1}\leq x \leq p_i \}$.  Hence
\begin{align*}
...
= \frac{f^2(c)\delta}{4} \ .
\]
This is a contradiction that $f(c)>0$.  If $f(c)<0$, the previous reasoning
applied to $-f(x)$ shows that $-f(c)>0$ is not possible.  Thus $f(c)=0$.

The cases $c=a$ or $c=b$ are handled in a similar fashion.

If $f$ is continuous on $[a.b]$, the previous discussion can be applied to
every $x\in[a,b]$ to show that $f(x)=0$.
\end{sol}
...
differentiable function because $g$ is such a function.  Moreover, $h(b)=0$.  We may
apply part (i) to conclude that there exist $c\in[a,b]$ such that
\begin{align*}
...
have that $p(x)\leq P(x)$ for all $x$ by hypothesis.  If $x_0>a$, we
...
= P(x_0) e^{c(x-x_0)} = 0 \ .
\]
We have that $p(x) = 0$ for all $x >x_0$.  This contradict our hypothesis
...
$\delta >0$ be the length of $I$. We have that
\[
...
\]
Since $\displaystyle \delta^n \to 1$ as $n\to \infty$, we get that
\[
...
because it is continuous on $[0,1]$.  We have that
\[
...
continuous and $f(x) = 0$ for $x < c$, we get that
...
therefore have that $f(x)>0$ for all $x \in ]c,1]$.
...
(\ref{probl1}), we get $2 f(x)f'(x) = f(x)$.  Thus $f'(x) = 1/2$ for
...
If we substitute $f(x) = x/2 +C$ in (\ref{probl1}) and integrate, we
get that
\[
...
$y>0$ arbitrary but fixed.  Using the chain rule, we get that
\[
...
\]
for $x >0$.  Since $G(1)=L(y)$ and $G(x) = L(xy)$, we get that
...
$f$ is continuous on $[0,1]$, we get that $f(x) = 0$ for $x\in[0,1]$.
\end{sol}
...
$[0,1]$, we get that $\displaystyle \int_0^1 f = \LL(f) = 0$.
\end{sol}
...
\end{equation}
for $k\geq 0$ is by induction on $k$.   For $k=0$, we have that
...
\]
where $\xi = \xi(x)$ is between $x$ and $y$.  If $y=a$, then we get that
\[
  f(a) = f(x) + f'(x) (a -x) + \frac{f''(\xi)}{2} (a-x)^2
  \leq f(x) + f'(x) (a -x)
\]
because $f''(\xi) \leq 0$ for $\xi \in [a,b]$.  If we integrate
$f(a) \leq f(x) + f'(x) (a -x)$ from $a$ to $b$ on both sides, we get that
...
$\displaystyle \int_a^b f(x) \dx{x}$, then we get that
\[
...
The only issue is at the origin.  We first not that $f_x$ is
...
We also have that $\displaystyle \dydx{f_x}{y}(0) = 0$ because
...
assume that the reader know.  They will be useful for the questions below.
\begin{enumerate}
...
\begin{sol}
\subQ{a}
We have that $\displaystyle x^{1/3} (x^2+2x)^{1/2} = x^{5/6} (x+2)^{1/2} \geq
...
\subQ{h} Since $\displaystyle \tan(u) > u$ for $0< u <1$, we get that
...
exists.  We also have that $G(q) = F(q)$ for all positive integer $q$.  Thus
$\displaystyle \lim_{\substack{q\to \infty\\ q \in \NNp}} G(q) =
\lim_{\substack{q\to \infty\\ q\in \NNp}} F(q) = 1$.
Since $\displaystyle \lim_{q\to \infty} G(q)$ exists, we have that
...
$f(x) \geq 0$ for all $x\geq 1$, we have that
\[
  M = \int_1^\infty f(x) \dx{x} \geq \int_1^N f(x) \dx{x}
\]
for all $N>0$.  Since $f$ is monotonically decreasing, we have that
\[
...
It follows that $\left\{ S_N \right\}_{N=1}^\infty$ converges; namely,
$\displaystyle \sum_{n=1}^\infty f(n)$ converges.
...
$N\geq q$.  Since $f(x) \geq 0$ for all $x \geq 1$, we have that
\[
F(q) = \int_1^q f(x) \dx{x} \leq \int_1^N f(x) \dx{x} \ .
\]
Moreover, since $f$ is monotonically decreasing, we have that
\begin{align*}
