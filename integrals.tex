...
a subset $\DS P = \{ p_i :0 \leq i \leq N \}$ of
...
$\DS P = \{ p_i :0 \leq i \leq N\}$ of $[a,b]$.  Let
$\DS m_i = \inf\{ f(x) : x\in [p_{i-1},p_i] \}$
and $\DS M_i = \sup\{ f(x) : x\in [p_{i-1},p_i] \}$
...
\begin{proof}
Suppose that $\DS P = \{ p_i :0 \leq i \leq N\}$ and that
$Q$ contains one more point than $P$.  Let $q \in Q\setminus P$ be this point.

As usual, let $\DS m_i = \inf\{f(x) : x\in [p_{i-1},p_i] \}$ for
$1 \leq i \leq N$.  There exists $j$ such that $p_{j-1} < q < p_j$.  If
$\DS \tilde{m}_j = \inf\{ f(x) : x\in [p_{j-1},q] \}$ and
$\DS \breve{m}_j = \inf\{f(x) : x\in [q,p_j] \}$,
...
partitions of $[a,b]$, then $\DS \LL_P(f) \leq \U_Q(f)$.
\end{cor}

\begin{proof}
Let $R=P\cup Q$.  $R$ is a refinement of both $P$ and $Q$.  Hence
$\DS \LL_P(f) \leq \LL_R(f) \leq \U_R(f) \leq \U_Q(f)$
...
Since $\DS \LL_P(f) \leq \LL(f) \leq \U(f) \leq \U_p(f)$ for all
...
$\DS P_N = \{ p_i = a + i(b-a)/N : 0 \leq i \leq N \}$
of $[a,b]$.  Since $f$ is decreasing, we have
$\DS m_i = \inf \{f(x) : x \in [p_{i-1},p_i]\} = f(p_i)$
and
$\DS M_i = \sup \{f(x) : x \in [p_{i-1},p_i]\} = f(p_{i-1})$
for all $i$.  Hence
\begin{align*}
\U_{P_N}(f) - \LL_{P_N}(f) 
...
$\DS \int_a^b f(x) \dx{x} \leq \int_a^b g(x) \dx{x}$
\end{prop}
...
Given $\epsilon>0$, let
$\DS \tilde{\epsilon} = \frac{\epsilon}{b-a+2K}$.
...
$\DS \U_P(f) - \LL_P(f) < \delta^2$.
Let
$\DS m_i = \inf \{ f(x) : p_{i-1} \leq x \leq p_i \}$,
$\DS M_i = \sup \{ f(x) : p_{i-1} \leq x \leq p_i \}$,
$\DS \tilde{m}_i = \inf \{ h(x) : p_{i-1} \leq x \leq p_i \}$
and $\DS \tilde{M}_i = \sup \{ h(x) : p_{i-1} \leq x \leq p_i \}$.
...
integrable and
$\DS \left| \int_a^b f(x)\dx{x} \right| \leq \int_a^b |f(x)|\dx{x}$.
\end{prop}
...
intervals $\DS \{ I_i \}_{i=1}^M$ such that
$\DS S \subset \bigcup_{i=1}^M I_i$ and
$\DS \sum_{i=1}^M |I_i| < \epsilon$.
...
intervals $\DS \{ I_i \}_{i=1}^\infty$ such that
$\DS S \subset \bigcup_{i=1}^\infty I_i$ and
$\DS \sum_{i=1}^\infty |I_i| < \epsilon $.
\end{defn}
...
Given $S \subset \RR$, let $\DS \Chi_S:\RR \to \RR$ be the
function defined by
\[
\Chi(x) = \begin{cases}
...
$\DS \int_S f$ or $\DS \int_S f(x)\dx{x}$, as the
integral $\DS \int_a^b \Chi_S f$ for $[a,b] \supset S$
...
$\DS P = \{ p_i : 0 \leq i \leq N\}$ of the interval $[a,b]$
and a set of points $\DS P^\ast = \{ p_i^\ast:1 \leq i \leq N\}$
such that $\DS p_{i-1} \leq p_i^\ast \leq p_i$ for all $i$, the
...
\]
if the partition $\DS P = \{ p_i : 0 \leq i \leq N\}$
of $[a,b]$ is such that $|p_i-p_{i-1}|<\delta$ for all
$i$ and all sets $\DS P^\ast$ as defined above.
\end{theorem}

This theorem is the reason why, in elementary calculus courses, the
Riemann integral $\DS \int_a^b f(x) \dx{x}$
of a continuous function $f:[a,b]\to \RR$ is defined as the limit of Riemann
sums $\DS R(f,P_j,P^\ast_j)$ where the partition
$\DS P_j = \{ p_{j,i} :0 \leq i \leq N_j\}$ of the interval
$[a,b]$ is such that
$\DS \max_{1\leq i \leq N_j} (p_{j,i}-p_{j,i-1}) \to 0$
as $j\to \infty$.  The points in
$\DS P^\ast_j = \{ p_{j,i}^\ast : 1 \leq i \leq N_j\}$ are often
...
Suppose that 
$\DS P = \{ p_i : 0 \leq i \leq N_P\}$ and
$\DS Q = \{ q_i : 0 \leq i \leq N_Q\}$ are two partitions of
...
\begin{proof}
Let $\DS P\cup Q = \{ z_j : 0 \leq j \leq N\}$,
$\DS M_i = \sup \{ f(x) : p_{i-1} \leq x \leq p_i \}$
for $1 \leq i \leq N_P$ and
$\DS \tilde{M}_j = \sup \{ f(x) : z_{j-1} \leq x \leq z_j \}$
for $1 \leq j \leq N$.
...
$\DS \LL_P(f) \leq R(f,P,P^\ast) \leq \U_P(f)$
for all partitions $P$ of $[a,b]$ and sets $\DS P^\ast$ as
...
Suppose that $\DS P = \{ p_i : 0 \leq i \leq N_P\}$ is a
...
exists a partition
$\DS \tilde{P} = \{\tilde{p}_i: 0 \leq i \leq N\}$ of
...
\end{equation}
where
$\DS \tilde{M}_i = \sup \{\phi'(y) : \tilde{p}_{i-1} \leq y
\leq \tilde{p}_i \}$ and
$\DS \tilde{m}_i = \inf \{\phi'(y) : \tilde{p}_{i-1} \leq y
\leq \tilde{p}_i \}$ for all $i$.

Let $p_i = \phi(\tilde{p}_i)$ for $0 \leq i \leq N$.  The set
$\DS P = \{p_i : 0 \leq i \leq N\} = \phi(\tilde{P})$ is a
...
\end{equation}
If $\DS C = \sup \{|f(x)| : a \leq x \leq b \}$, then we get from
(\ref{intCompZ}) that
\begin{align}
...
$[\alpha,\beta]$ such that
$\DS \left| \U_{Q_1}(f) - \U(f)\right|<\epsilon$ and
$\DS \left| \U_{Q_2}((f\circ \phi)\phi') -
...
$\DS \U(f) = \U ((f\circ \phi)\phi')$ and
$\DS \LL(f) = \LL ((f\circ \phi)\phi')$.

Thus, $f$ is integrable (i.e.\ $\DS \U(f)=\LL(f)$) if and only if
$(f\circ \phi)\phi'$ is integrable (i.e.\
$\DS \U ((f\circ \phi)\phi')= \LL ((f\circ \phi)\phi')$),
...
\]
We leave it to the reader to verify that $\DS 
\int_{\tilde{\phi}(\alpha)}^{\tilde{\phi}(\beta)} \tilde{f}
...
Suppose that $\DS \{ f_k\}_{k=0}^\infty$ is a sequence of
integrable real valued functions defined on the interval $[a,b]$.  Moreover,
suppose that $\DS \{ f_k\}_{k=0}^\infty$ converges uniformly on
...
\begin{proof}
Choose $\epsilon >0$.  Since $\DS \{ f_k\}_{k=0}^\infty$ converges
uniformly on $[a,b]$ to the function $f:[a,b]\to \RR$, there exists $K>0$
($K$ depends on $\epsilon$) such that
$\DS \left| f_k(x) - f(x) \right| < \frac{\epsilon}{4(b-a)}$
...
\end{equation}
Let
$\DS g_m(x) = f_m(x) - \frac{\epsilon}{4(b-a)}$
and $\DS h_m(x) = f_m(x) + \frac{\epsilon}{4(b-a)}$
...
\]
It follows that
$\DS \left| \U_{P_m}(f) - \LL_{P_m}(f) \right| < \epsilon$.
...
continuous function,
$\DS \{f_k\}_{k=0}^\infty$ is a sequence of continuous
...
$\DS \lim_{k\to \infty}f_k(x) = f(x)$ for
...
all $k \geq 0$, and $\DS \lim_{k\to \infty}f_k(x) = 0$
for all $x \in D$.
...
Let $\DS K = \max_{1\leq j \leq J} K_{x_j}$.
Given $x \in D$, we have $\DS x \in
]x_j - \delta_{K_{x_j},x_j} ,x_j + \delta_{K_{x_j},x_j}[$
for some $0 \leq j \leq J$.  Hence
$\DS \big|f_{K_{x_j}}(x) - f_{K_{x_j}}(x_j)\big| < \epsilon/2$.
Moreover $\DS 0 \leq f_{K_{x_j}}(x_j) < \epsilon/2$.
Hence
\[
...
$\DS \{g_k\}_{k=1}^\infty$ where $g_k = f_k - f$ for all
...
$\DS \lim_{k\to \infty}f_k(x) = 0$ for all $x \in [a,b]$.
Then $\DS \lim_{k\to \infty}\LL(f_k) = 0$.
\end{lemma}
...
$\DS \LL(f_k) - \int_a^b g_k < \frac{\epsilon}{2^{k+1}}$.

Let $\DS h_k = \min_{0 \leq j \leq k} g_j$
...
$\DS 0 \leq \lim_{k\to \infty} \LL(f_k) \leq \epsilon$.
Since $\epsilon$ is arbitrary, this will prove that
$\DS \lim_{k\to \infty} \LL(f_k) = 0$.

Let $\DS H_{i,k} = \max_{ i\leq j \leq k} g_j$ for
...
Since
$\DS H_{i,k}(x) \leq \max_{i\leq j \leq k} f_j(x) \leq f_i(x)$
...
$\DS \lim_{k\to \infty} g_k(x) = 0$ for all $x \in[a,b]$.

Let $h_k:[a,b] \to \RR$ be the function defined by
$\DS h_k(x) = \sup_{i\geq k} g_i(x)$ for all $x \in [a,b]$
...
and
$\DS \lim_{k\to \infty} h_k(x) = \limsup_{k\to \infty} g_k(x)
= \lim_{k\to \infty} g_k(x) = 0$ for all $x \in [a,b]$ because
$\DS \lim_{k\to \infty}f_k(x) = f(x)$ for all $x\in[a,b]$.
It follows from Lemma~\ref{lemmaBCT2} that
$\DS \lim_{k\to \infty} \LL(h_k) = 0$.

Since $g_k \leq h_k$ for all $k \geq 0$, we have
$0 \leq \DS \int_a^b g_k = \LL(g_k) \leq \LL(h_k) \to 0$
as $k \to \infty$.
...
Thus $\DS \LL_P(f) = \sum_{i=1}^N m_i ( p_i - p_{i-1}) = 0$
and $\DS \U_P(f) = \sum_{i=1}^N M_i ( p_i - p_{i-1}) = b - a > 0$.
...
$\DS \int_a^b cf(x)\dx{x} = c \int_a^b f(x)\dx{x}$.
\end{question}
...
that $\DS \U_P(f) - \LL_p(f) < \epsilon/|c|$.
Let
$\DS m_i = \inf\{ f(x) : p_{i-1} \leq x \leq p_i\}$,
$\DS M_i = \sup\{ f(x) : p_{i-1} \leq x \leq p_i\}$,
$\DS \tilde{m}_i = \inf\{ cf(x) : p_{i-1} \leq x \leq p_i\}$
and $\DS \tilde{M}_i = \sup\{ cf(x) : p_{i-1} \leq x \leq p_i\}$.
...
\LL_P(cf) = c\, \U_P(f) \leq c \int_a^b f(x) \dx{x} \leq
  c \LL_P(f) = \U_P(cf)
\]
if $c<0$.   So $\DS \int_a^b cf(x)\dx{x}$ and
$\DS c \int_a^b f(x)\dx{x}$ are in the interval
...
that $\DS \int_a^b f(x)\dx{x} = \int_{a-c}^{b-c} f(x+c)\dx{x}$.
\end{question}
...
\]
Thus, $\DS \int_a^b f(x) \dx{x}$ and
$\DS  \int_{a-c}^{b-c} f(x+c) \dx{x}$ are both in the interval
...
$\DS \frac{1}{c} \int_a^b f(x)\dx{x} = \int_{a/c}^{b/c} f(c x)\dx{x}$.
\end{question}
...
$\DS \int_a^b f(x)\dx{x} = \int_{-b}^{-a} f(-x)\dx{x}$.
\end{question}
...
$[a,b]$ such that $\DS \U_P(f) - \LL_P(f) < \epsilon/K$.
...
\begin{question}
Let $\DS \{x_n\}_{=0}^\infty$ be a convergent sequence in $\RR$.
Show that the set $\DS \{x_n : n \geq 0\}$ has content
zero.
\end{question}

\begin{sol}
Let $\DS x = \lim_{n\to \infty} x_n$.  Given $\epsilon>0$,
choose $N$ such that
$\DS |x_n - x| < \frac{\epsilon}{4}$ for $n> N$.

Let $\DS R_n =
[x_n - \frac{\epsilon}{5^{n+1}},x_n + \frac{\epsilon}{5^{n+1}}]$ for
$0 \leq n \leq N$.  Moreover, let
$\DS R_{N+1} = \left[x - \frac{\epsilon}{4},
x + \frac{\epsilon}{4} \right]$.

We have $\DS \{x_n: n \geq 0\} \subset \bigcup_{n=0}^{N+1} R_n$
and
\begin{align*}
...
$\DS \{x_n : n \geq 0\}$ has content zero.
\end{sol}
...
that $\DS \int_a^b f(x) \dx{x} > 0$ if $x_0 = a$.
\end{sol}
...
& \leq \sum_{i=1}^N M_i(p_i - p_{i-1}) = \U_P(f) \ .
\end{align*}
We also have $\DS \LL_P(f) \leq \int_a^b f(x) \dx{x}
\leq \U_P(f)$.  Thus, both $\DS \int_a^b f(x) \dx{x}$ and
...
$[a,b]$ implies that $\DS h^2$ is integrable on $[a,b]$.
\end{question}

\begin{sol}
\stage{i} We first prove that $\DS h^2$ is integrable on $[a,b]$ if
...
Given $\epsilon>0$, choose a partition $P=\{p_i : 0 \leq i \leq N\}$ of
$[a,b]$ such that $\DS \U_P(h) - \LL_P(h) < \epsilon/(2C)$.
...
\]
where
$\DS \tilde{m}_i = \inf\{ h^2(x) : p_{i-1}\leq x \leq p_i \}$ and
$\DS\tilde{M}_i = \sup\{ h^2(x) : p_{i-1}\leq x \leq p_i \}$.  Hence
\begin{align*}
...
$\DS f g = \frac{1}{2}\left( (f+g)^2 - f^2 - g^2 \right)$.
Since $\DS (f+g)^2$, $\DS f^2$ and $\DS
...
$\U(f) = 1$ and $\LL(f) = -1$.  However, $|f|$ and $\DS f^2$
are integrable on the interval $[0,1]$ because $|f(x)|=1$ and
$\DS f^2(x) = 1$ for all $x \in [0,1]$.
\end{sol}

\begin{question}
Suppose that $f:[a,b]\to \RR$ is an integrable function such that
$\DS \int_0^1 f(x)g(x)\dx{x} =0$ for all continuous functions
...
$\DS F(x) = \int_a^x f(t) \dx{t}$.  Since $f$ is continuous
...
that $\DS \int_a^b f(x)h(x) \dx{x} = h(a) \int_a^\xi f(x)\dx{x}$.\\
...
$\DS p(x) \leq c \int_a^x p(t)\dx{t}$ for all $x\in[a,b]$.  
...
Let $\DS P(x) = c \int_a^x p(t) \dx{t}$ for $x\in[a,b]$.  We
...
$\DS M=\sup\{f(x) : x \in [a,b]\}$.  Prove that
$\DS M =
...
\]
for all $n>0$.  Since $\DS (b-a)^{1/n} \to 1$ as $n\to \infty$,
we get
\[
...
$f$ is continuous, $\DS J = f^{-1}(]M-\epsilon, \infty[)$ is
...
\]
Since $\DS \delta^n \to 1$ as $n\to \infty$, we get
\[
...
\begin{question}
Consider the functions $\DS f_n(x) = \frac{nx}{1+nx}$ for
$n \geq 1$.  Let
\[
f(x) =
\begin{cases}
...
$\DS \int_0^1 f_n(x)\dx{x} \to \int_0^1 f(x) \dx{x}$
as $n\to \infty$.
\end{question}

\begin{question}
Prove that
$\DS \lim_{n\to \infty} \int_a^\pi \frac{\sin(nx)}{nx} \dx{x} =0$
...
$\DS \Big|\frac{\sin(n x)}{nx}\Big| \leq \frac{1}{n a}$ 
...
\[
f_n(x) = \begin{cases}
\DS \frac{\sin(nx)}{nx} & \qquad 0 < x \leq \pi \\
1 & \qquad x = 0
\end{cases}
\]
for $\DS n\in \NNp$.  Recall that
$\DS -1 < \frac{\sin(y)}{y} <1$ for $y>0$ and
$\DS \lim_{y\to 0} \frac{\sin(y)}{y} = 1$.  Therefore, the functions
$f_n:[0,\pi]\to \RR$ are continuous and $\DS  |f_n(x)| \leq 1$ for
all $x\in [0,\pi]$ and $\DS n \in \NNp$.  Moreover,
$\DS |f_n(x)| \leq \frac{1}{nx} \to  0$ as $ n \to \infty$ for all
$x\in ]0,\pi]$ and $\DS \lim_{n\to \infty}f_n(x) = 1$ for $x = 0$.
Hence, the sequence of functions $\DS \{f_n\}_{n=1}^\infty$ converge
...
\begin{question}
Compute $\DS \lim_{n \to \infty} \int_0^1 f_n(x) \dx{x}$ for
the following sequences $\DS \{f_n\}_{n=1}^\infty$ of functions.\\
\subQ{a}
$\DS f_n(x)= \left( 1 + \frac{x}{n} \right)^n$ for $x\in [0,1]$.\\
\subQ{b}
$\DS f_n(x)= \sum_{k=0}^{n} (-x)^k$ for $x \in [0,1]$.
...
$\DS \{f_n\}_{n=0}^\infty$ on $[0,1]$ that converges pointwise to a
...
\subI{Second example}
Let $\DS f_n(x) = \frac{nx}{1+nx^2}$ for $0\leq x \leq 1$.
...
\]
for $x >0$ and $\DS \lim_{n\to \infty} f_n(x) =0$ for $x=0$ because
...
$\DS f(c)=\lim_{x\to c^-}f(x) = 0$.  Therefore
...
$x \in ]c,1]$.  Since
$\DS \int_0^x f(t) \dx{t} = (f(x))^2 \geq 0$, we must
...
$\DS F(x) = \int_0^x f(t) \dx{t}$ is differentiable.
Since (i) implies that
$\DS f(x) = \left( \int_0^x f(t) \dx{t} \right)^{1/2}
...
\]
If we simplify this equation, then we get $\DS C^2=0$.
...
Let $\DS L(x) = \int_1^x \frac{1}{t}\dx{t}$.  Prove that
...
for $x>0$.  Hence $L$ is of class $\DS C^1$ on $]0,\infty[$.

Let $\DS G(x) = L(xy) = \int_1^{xy}\frac{1}{t} \dx{t}$ for
...
$\DS L(xy) = L(x) + L(y)$ for $x,y >0$.
\end{sol}

\begin{question}
Describe the continuous functions $f:[0,1]\to \RR$ that satisfy
$\DS \int_0^x f(t) \dx{t} = \int_x^1 f(t) \dx{t}$ for all
$x\in[0,1]$.
\end{question}

\begin{sol}
Since $f$ is a continuous function,
$\DS F(x) = \int_0^x f(t) \dx{t}$ and
$\DS G(x) = \int_x^1 f(t) \dx{t} = - \int_1^x f(t) \dx{t}$
...
$\DS \int_0^1 f(x)\dx{x} = 0$.
Given $\epsilon>0$, choose $\DS q_0 \in \NNp$ large enough such that
...
$\DS \sum_{q=1}^{q_0} q = \frac{q_0(q_0+1)}{2}$.  However,
...
$\DS \sum_{i=0}^N (r_i-r_{i-1}) = 1$; the length of the interval
$[0,1]$.
...
$[0,1]$, we get $\DS \int_0^1 f = \LL(f) = 0$.
\end{sol}
...
\[
F'(x) = \begin{cases}
\DS 2x \sin\left(\frac{1}{x^2}\right) -
\frac{2}{x} \cos\left(\frac{1}{x^2}\right) & \quad \text{if} \ x > 0 \\
0 & \quad \text{if} \ x \leq 0
\end{cases}
\]
Note that $F'$ is not bounded in any neighbourhood of the origin.  For
instance, with $\DS x_n = \frac{1}{\sqrt{2 n \pi}}$, we have
$\DS x_n \to 0^+$ and $F'(x_n) = -2 \sqrt{2n\pi} \to -\infty$ as
...
$\DS f_{k+1}(x) = \int_0^x f_k(t) \dx{t}$ for
$x\in[0,1]$ and $k \in \NN$.  Prove that
$\DS |f_k(x)| \leq M x^k/k!$ for all
$k\geq 0$ and all $x \in [0,1]$ where
$\DS M = \sup\{|f(x)|: x\in [0,1]\}$.  Moreover, prove that
...
$\DS \int_a^b f(x) \dx{x}$, then we get
\[
...
f(x,y) =
\begin{cases}
\DS \frac{x^3}{y^2}\, e^{-x^2/y} & \quad \text{if}\ y > 0 \\
  0 & \quad \text{if}\ y \leq 0
\end{cases}
\]
and $f_x(y) = f_y(x) = f(x,y)$ for all $x$ and $y$.\\
\subQ{a} Prove that $\DS f_y \in C^1(\RR)$ for all $y$ fixed
and $\DS f_x \in C^1(\RR)$ for all $x$ fixed.\\
...
\begin{sol}
\subQ{a} We have that $\DS f_y \in C^1(\RR)$ for all $y$
fixed because $f_y(x) = 0$ for all $x\in \RR$ when $y\leq 0$ and
$\DS f_y(x) = x^3e^{-x^2/y}/y^2$ for all $x$ when $y>0$.

We have that $\DS f_x$ is continuously differentiable
on $\RR \setminus \{0\}$ for all $x$ fixed because $f_x(y) = 0$ for
all $y <0$ and $\DS f_x(y) = x^3e^{-x^2/y}/y^2$ for all $y>0$.
The only issue is at the origin.  We first note that $f_x$ is
continuous at the origin because
$\DS \lim_{y \to 0^-} f_x(y) = 0$ and
$\DS \lim_{y \to 0^+} f_x(y)
= \lim_{y \to 0^+} \frac{x^3}{y^2}\,e^{-x^2/y} = 0$ \footnote{One may
use l'Hospital Rule to compute this limit.}.
We also have $\DS \dydx{f_x}{y}(0) = 0$ because
$\DS \lim_{y\to 0^-} \frac{f_x(y)-f_x(0)}{y} = 0$
and
$\DS \lim_{y\to 0^+} \frac{f_x(y)-f_x(0)}{y}
= \lim_{y\to 0^+} \frac{x^3}{y^3} e^{-x^2/y} = 0$ \footnote{Again, one may
use l'Hospital Rule to compute this limit.}.
Thus
\[
\dydx{f_x}{y}(y) = \begin{cases}
\DS \left(\frac{-2x^3}{y^3} + \frac{x^5}{y^4} \right) e^{-x^2/y} &
\quad \text{if } y > 0 \\
0 & \quad \text{if } y \leq 0
\end{cases}
\]
Again, it is easy to show that
$\DS \lim_{y\to 0} \dydx{f_x}{y}(y) = \dydx{f_x}{y}(0)$.  Thus
$\DS f_x \in C^1(\RR)$.

\subQ{b} We have
$\DS f(\sqrt{y},y) = \frac{1}{\sqrt{y}\, e} \to \infty$ as
$\DS y \to 0^+$.
...
\end{cases}
\]
is a function of class $\DS C^1$ on $\RR$
and $\DS \dydx{g_x}{y} = f_x$ for all $x$ fixed.  Hence, we
...
\]
Thus
$\DS \pdfdx{\int_0^1 f(x,y) \dx{y}}{x} = e^{-x^2} -2 x^2 e^{-x^2}$
and
...
\DS \left(\frac{3x^2}{y^2} - \frac{2x^4}{y^3}\right)e^{-x^2/y} &
\quad \text{if } y > 0 \\
0 & \quad \text{if } y \leq  0
\end{cases}
\]
Let $\DS h_x(y) = \pdydx{f}{x}(x,y)$ for all $y \in \RR$ and
$x$ fixed.  Again, as we have done in (a), we can prove that
\[
k_x(y) =
\begin{cases}
\DS \left( 1- \frac{2x^2}{y}\right) e^{-x^2/y}
& \quad \text{if}\ y > 0 \\
0 & \quad \text{if}\ y \leq 0
\end{cases}
\]
is a function of class $\DS C^1$ on $\RR$
and $\DS \dydx{k_x}{y} = h_x$ for all $x$ fixed.  Hence, we
have from the Fundamental Theorem of Calculus that
\[
...
However, since $\DS \pdydx{f}{x}(0,y) = 0$ for all $y$, we get
$\DS \int_0^1 \pdydx{f}{x}(0,y) \dx{y} = 0$.

The fact that (\ref{supplTEq1}) is true does not come from the theory
because $\DS \pdydx{f}{x}$ are not bounded in any
neighbourhood of the origin.  Thus (2) cannot be satisfied.
\end{sol}

\begin{question}
Let $\DS F(x) = \int_{g(x)}^{h(x)}f(x,y)\dx{y}$     \label{derintf}
...
\subQ{a} & $\DS F(x) = \int_x^{x^2} \frac{e^{xy}}{y} \dx{y}$ &
\subQ{b} & $\DS G(x) = \int_{2x}^{3x} \cos(x^2y) \dx{y}$
\end{tabular}
\end{center}
\end{question}
...
\end{align*}
where we have use the substitution $\DS u = x^2y$ for the
...
\]
for $\DS n \in \NNp$ and $\VEC{x}\in \RR$.  Prove that
$\DS \dydx{f_n}{x}(x) = f_{n-1}(x)$ and use this result to
prove that $f_n$ is the $\DS n^{th}$ order primitive of
$f$; namely, $\DS \dydxn{f_n}{x}{n} = f$.
\end{question}
...
\begin{enumerate}
\item $\DS \int_1^\infty \frac{1}{x^p} \dx{x}$ converges for $p>1$
and diverges for $p\leq 1$.
\item $\DS \int_0^1 \frac{1}{x^p} \dx{x}$ converges for
...
$\DS \int_a^b f(x) \dx{x}$ converges if
$\DS \int_a^b g(x) \dx{x}$ converges.  Moreover,
$\DS \int_a^b g(x) \dx{x}$ diverges if
$\DS \int_a^b f(x) \dx{x}$ diverges.  These results are known
as the {\bfseries comparison test}\index{Comparison Test}.
\item Suppose that $f:]a,b[\rightarrow \RR$ is a continuous function.
If $\DS \int_a^b |f(x)| \dx{x}$ converges, then
$\DS \int_a^b f(x) \dx{x}$ converges.
\end{enumerate}
...
\subQ{a} & $\DS \int_0^1 \frac{1}{x^{1/3} (x^2+2x)^{1/2}}\dx{x}$  &
\subQ{b} & $\DS \int_0^1 \frac{x^2}{\sqrt{1-x^4}} \dx{x}$ &
\subQ{c} & $\DS \int_0^\infty \sqrt{x} e^{-x} \dx{x}$ \\[0.8em]
\subQ{d} & $\DS \int_1^\infty \frac{1}{x\sqrt{x+3}} \dx{x}$ &
\subQ{e} & $\DS \int_1^\infty \frac{x^2 -2 x -3}{x(x^2+1)} \dx{x}$ & 
\subQ{f} & $\DS \int_0^\infty x^2 e^{-x^2}\dx{x}$ \\[0.8em]
\subQ{g} & $\DS \int_3^\infty \frac{\sin(4x)}{x^2 -x -2}\dx{x}$ &
\subQ{h} & $\DS \int_1^\infty \tan\left(\frac{1}{x}\right) \dx{x}$ & 
\subQ{i} & $\DS \int_0^\infty \frac{e^x}{1+e^{2x}} \dx{x}$ \\[0.8em]
\subQ{j} & $\DS \int_0^\infty \frac{1-\cos(x)}{x^2} \dx{x}$ &
\subQ{k} & $\DS \int_0^1 \frac{1}{\sqrt{x^3+x^2}} \dx{x}$ &
\subQ{l} & $\DS \int_1^\infty \frac{1}{\sqrt{x^3+x^2}} \dx{x}$
\\[0.8em]
\subQ{m} & $\DS \int_0^\infty e^x \sin(e^x) \dx{x}$ &
\subQ{n} & $\DS \int_0^\infty x \sin(e^x) \dx{x}$ &
\subQ{o} & $\DS \int_0^1 \frac{\sin(1/x)}{x} \dx{x}$ \\[0.8em]
\subQ{p} & $\DS \int_0^1 x\ln(x) \dx{x}$ & & & &
\end{tabular}
\end{center}
\end{question}
...
\begin{sol}
\subQ{a}
We have $\DS x^{1/3} (x^2+2x)^{1/2} = x^{5/6} (x+2)^{1/2} \geq
x^{5/6} \sqrt{2}$ for $0\leq x \leq 1$.  Thus
$\DS \frac{1}{x^{1/3} (x^2+2x)^{1/2}} \leq \frac{1}{x^{5/6} \sqrt{2}}$
for $0 < x \leq 1$.
Since $\DS \int_0^1 x^{-p} \dx{x}$ converges for $p<1$, we
have by the comparison test that
$\DS \int_0^1 \frac{1}{x^{1/3} (x^2+2x)^{1/2}}\dx{x}$ converges.
...
\]
With the substitution $\DS u=1-x^2$ for $0\leq x < 1$, we
get
\[
\lim_{q\to 1^-} \int_0^q \frac{x^2}{\sqrt{1-x^4}} \dx{x}
...
for $0 < u \leq 1$
and $\DS \int_0^1 u^{-1/2} \dx{u}$ converges, we
get from the comparison test that
$\DS \int_0^1 \frac{\sqrt{1-u}}{\sqrt{u(2-u)}} \dx{u}$
converges.  Hence $\DS \int_0^1 \frac{x^2}{\sqrt{1-x^4}} \dx{x}$
converges.

\subQ{c} Since $\DS \sqrt{x}e^{-x}$ is continuous on $[0,1]$, the
integral $\DS \int_0^1 \sqrt{x} e^{-x} \dx{x}$ exists.  We
only have to consider $\DS \int_1^\infty \sqrt{x} e^{-x} \dx{x}$.

We first prove that $\DS h(x) = \sqrt{x} e^{-x/2}$ is
...
$\DS h(x) \leq h(1) = e^{-1/2}$ for $x>1$.  Since
$\DS 0 \leq \sqrt{x} e^{-x} = h(x) e^{-x/2} \leq e^{-1/2}
e^{-x/2}$ for $x \geq 1$ and
$\DS \int_1^\infty e^{-x/2} \dx{x}$ converges, we get from
the comparison test that
$\DS \int_1^\infty \sqrt{x} e^{-x} \dx{x}$ converges.  Thus
\[
...
\subQ{d}
Since $\DS \frac{1}{x\sqrt{x+3}} \leq \frac{1}{x^{3/2}}$
and $\DS \int_1^\infty x^{-3/2}\dx{x}$ converges because
$\DS \int_1^\infty x^{-p}\dx{x}$ converges for $p>1$, we get
from the comparison test that
$\DS \int_1^\infty \frac{1}{x\sqrt{x+3}} \dx{x}$
converges.

\subQ{e} Since
$\DS \lim_{x\to \infty} \frac{x^2 -2x-3}{x^2+1} = 1$, there
exists $C > 0$ such that
$\DS \frac{x^2 -2x-3}{x^2+1} > \frac{1}{2}$
for $x \geq C$.  Hence
$\DS \frac{x^2 -2x-3}{x(x^2+1)} > \frac{1}{2x}$ for $x \geq C$.
Since $\DS \int_C^\infty \frac{1}{2x} \dx{u}$ diverges, we
get from the comparison test that
$\DS \int_C^\infty \frac{x^2 -2x-3}{x(x^2+1)} \dx{x}$ diverges.
It follows from
\[
...
\]
that $\DS \int_1^\infty \frac{x^2 -2x-3}{x(x^2+1)} \dx{x}$ diverges.
Note that $\DS \int_1^C \frac{x^2 -2 x -3}{x(x^2+1)} \dx{x}$ is a real
number because $\DS \frac{x^2 -2 x -3}{x(x^2+1)}$ is
continuous for $1 \leq x \leq C$.

\subQ{f} Since $\DS x^2 e^{-x^2} \leq x^3 e^{-x^2}$ for $x\geq 1$ and
$\DS \int_1^\infty x^3 e^{-x^2} \dx{x}$ converges, we get
from the comparison test that 
$\DS \int_1^\infty x^2 e^{-x^2} \dx{x}$ converges.
Note that
\begin{align*}
...
\]
for $x\geq 3$.  Moreover, $\DS x -2 \geq \frac{x}{3}
\Rightarrow \frac{1}{x-2} \leq \frac{3}{x}$ and
$\DS x+1 > x \Rightarrow \frac{1}{x+1} < \frac{1}{x}$
for $x\geq 3$.  Thus
$\DS \left| \frac{\sin(4x)}{x^2 -x -2}\right| <\frac{3}{x^2}$
for $x\geq 3$.
Since $\DS \int_3^\infty \frac{1}{x^2} \dx{x}$ converges
because $\DS \int_3^\infty x^{-p}\dx{x}$ converges for $p>1$,
we get from the comparison theorem that
$\DS \int_3^\infty \frac{\sin(4x)}{x^2 -x -2}\dx{x}$
converges absolutely, and so converges.

\subQ{h} Since $\DS \tan(u) > u$ for $0< u <1$, we get
$\DS \tan\left(\frac{1}{x}\right) > \frac{1}{x}$ for
$1\leq x<\infty$.  Since $\DS \int_1^\infty \frac{1}{x} \dx{x}$
diverges, we get from the comparison test that
$\DS \int_1^\infty \tan\left(\frac{1}{x}\right) \dx{x}$
diverges.
\end{sol}
...
\subQ{b} Prove that $\DS \int_0^\infty f(x) \dx{x}$ converges and is
equal to $1$.\\
\subQ{c} Construct an unbounded function $f:\RR\to \RR$ such that
$\DS \int_0^\infty f(x) \dx{x}$ converges.
\end{question}
...
$\DS \lim_{q\to \infty} F(q) = \sup_{q\geq 0} F(q)$
exists.

Given $\epsilon >0$, choose $M$ large enough such that
$\DS \left| \sum_{n=1}^m \frac{1}{2^n} - 1 \right| < \epsilon$
for $m \geq M$.  This is possible because the geometric series 
$\DS \sum_{n=1}^\infty \frac{1}{2^n}$ converges to $1$.
Hence, for $\DS q> M+ 1/2^M$, we have
\[
1 \geq F(q) \geq F(M + 1/2^M) = \sum_{n=1}^M \frac{1}{2^n} > 1-\epsilon \ .
\]
Since $\epsilon$ is arbitrary, we get
$\DS \lim_{q\to \infty} F(q) = \sup_{q\geq 0} F(q) = 1$.
...
monotonically increasing and bounded, we have that
$\DS \lim_{q\to \infty} G(q) = \sup_{q\geq 0} G(q)$
exists.  We also have $G(q) = F(q)$ for all positive integer $q$.  Thus
$\DS \lim_{\substack{q\to \infty\\ q \in \NNp}} G(q) =
\lim_{\substack{q\to \infty\\ q\in \NNp}} F(q) = 1$.
Since $\DS \lim_{q\to \infty} G(q)$ exists, we have
$\DS \lim_{q\to \infty} G(q) = 1$.
\end{sol}

\begin{question}
Prove that                     \label{questsinoxCD}
$\DS \int_1^\infty \frac{|\sin(x)|}{x} \dx{x}$
diverges but
$\DS \int_1^\infty \frac{\sin(x)}{x} \dx{x}$ converges
\end{question}

\begin{sol}
We first prove that $\DS \int_1^\infty \frac{|\sin(x)|}{x} \dx{x}$
diverges.  We have
\begin{align*}
...
\]
for $\DS q \in \NNp$, and
$\DS \int_\pi^\infty \frac{1}{x} \dx{x}$ diverges, we get
that $\DS \int_\pi^\infty \frac{|\sin(x)|}{x} \dx{x}$
diverges and therefore
$\DS \int_1^\infty \frac{|\sin(x)|}{x} \dx{x}$
diverges.

To prove that $\DS \int_1^\infty \frac{\sin(x)}{x} \dx{x}$
...
\]
Since $\DS \frac{|\cos(x)|}{x^2} \leq \frac{1}{x^2}$ and
$\DS \int_1^\infty \frac{1}{x^2}\dx{x}$ converges, we get that
\[
...
Suppose that
$\DS F(x) = \int_0^x f(y) \dx{y}$ is bounded on $[0,\infty[$,
$g'(x) \leq 0$ for $x\geq 0$, and
$\DS \lim_{x\to \infty} g(x) = 0$.
Prove that $\DS \int_0^\infty f(x) g(x) \dx{x}$ converges.
\end{question}
...
$\DS \int_0^\infty F(x) g'(x) \dx{x}$ converges absolutely
and therefore converges.  It follows from (\ref{impprodA}) and 
(\ref{impprodB}) that $\DS \int_0^\infty f(x) g(x) \dx{x}$
converges.
\end{sol}

\begin{question}
Suppose that $f:[0,1]\to \RR$ is Riemann integrable, prove that
$\DS \lim_{q\to 0^+} \int_q^1 f(x) \dx{x} = \int_0^1 f(x) \dx{x}$.
...
$\DS \lim_{q\to a^+} \int_q^b f(x) \dx{x} = \int_a^b f(x) \dx{x}$
but
$\DS \lim_{q\to a^+} \int_q^b |f(x)| \dx{x}$ does not exist.
...
$\DS \int_1^\infty f(x)\dx{x}$ converges if and only if the
series $\DS \sum_{k=1}^\infty f(k)$ converges.  This is
called the {\bfseries integral test}\index{Integral Test}.
\end{question}

\begin{sol}
\stage{$\Rightarrow$}
Suppose that $\DS \int_1^\infty f(x)\dx{x}$ converges.  Let
$\DS M = \int_1^\infty f(x)\dx{x} < \infty$.  Since
...
\]
for all $N>0$.  Thus $\DS S_N= \sum_{n=1}^N f(n)$ is an
non-decreasing and bounded sequence; more precisely, bounded by $M + f(1)$.
It follows that $\DS \left\{ S_N \right\}_{N=1}^\infty$
converges; namely, $\DS \sum_{n=1}^\infty f(n)$ converges.

\stage{$\Leftarrow$}
Suppose that $\DS \sum_{n=1}^\infty f(n)$ converges.
Let $\DS M = \sum_{n=1}^\infty f(n) < \infty$.
We have that $\DS F(q) = \int_1^q f(x) \dx{x}$ for $q \geq 1$
is a non-decreasing, non-negative function because $f(x)\geq 0$ for all
$x \geq 1$.   Given $q \geq 1$, choose $\DS N \in \NNp$ such that
...
have that $\DS \lim_{q\to \infty} F(q)$ exists; namely,
$\DS \int_1^\infty f(x)\dx{x}$ converges.
\end{sol}

\begin{question}
Suppose that $f:[a,\infty[\to \RR$ is integrable on all bounded
intervals of the form $[a,q]$.  Prove that
$\DS \int_a^\infty f(x)\dx{x}$
converges if and only if for every $\epsilon>0$ there exists $K>a$
such that
$\DS \left| \int_c^d f(x)\dx{x} \right| < \epsilon$ for
$K\leq c \leq d$.
\end{question}

\begin{sol}
Let $\DS F(q) = \int_a^q f(x) \dx{x}$ for $q \geq a$.
The question can be reformulated as follows.
Show that $\DS \lim_{q\to \infty} F(q)$ exists if and only
...
\begin{question}
Suppose that $\DS \{ f_k\}_{k=0}^\infty$ is a sequence of functions
...
$\DS \int_0^\infty g(x)\dx{x}$ converges.  Prove that
\[
