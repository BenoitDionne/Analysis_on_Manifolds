...
convex independent subset of $\DS \RR^n$.   The set
$\DS \left\{ \sum_{j=0}^k a_j\VEC{x}_j : a_j \geq 0 \ \text{and} \
...
$\DS \VEC{x} = \sum_{j=0}^k a_j\VEC{x}_j$ with $a_j \geq 0$ and
$\DS \sum_{j=0}^k a_k = 1$ are called
...
\begin{defn}
Let $s = \{\VEC{x}_0,\VEC{x}_1,\VEC{x}_2,\ldots, \VEC{x}_k\}$ be a
convex independent subset of $\DS \RR^n$.
The set $\DS \left\{ \sum_{j=0}^k a_j\VEC{x}_j : a_j > 0
...
$3$-simplex
$\DS [\VEC{x}_0,\VEC{x}_1,\VEC{x}_2,\VEC{x}_3] \subset \RR^3$.
...
$\DS \RR^n$ is a finite set of open simplices such that:
\begin{enumerate}
...
$\DS \left\{ (\VEC{x}_0,\VEC{x}_4,\VEC{x}_5),
...
of $K$, denoted $\DS K^r$, is the set
$\DS K^r = \left\{ (s) : (s) \in K \ \text{and}\ \dim\, (s)
\leq r \right\}$.
\end{defn}
...
$0 \leq r \leq k$, denoted $\DS s^r$, is the set of all open
faces of $[s]$ corresponding to open $m$-simplex with $m \leq r$.
Moreover, $\DS [s^r]$ will denote the union
of all the open simplices in $\DS s^r$.

\begin{defn}
Given $\DS \VEC{x} \in \RR^n$ and
$\DS A \subset \RR^n$, the pair $(\VEC{x},A)$ is in
...
$\DS \VEC{x} \ast A = \bigcup_{\VEC{a} \in A} [\VEC{x},\VEC{a}]$.
...
expressed as $\DS [s] = \VEC{x} \ast [s^{k-1}]$ for any
$\VEC{x} \in (s)$.
...
$\DS \VEC{b}_{(s)} = (k+1)^{-1} \sum_{j=0}^k \VEC{x}_j$.
\end{defn}
...
\begin{theorem}
Suppose that $K$ is a simplicial complex in $\DS \RR^n$.  Let
$\DS K^{[1]}$ be the simplicial complex defined by
\begin{align*}
...
\end{align*}
Then $\DS K^{[1]}$ is a subdivision of $K$
...
$\DS s^{[1]}$ where $s$ is the simplicial complex
...
It follows from the definition of $\DS K^{[1]}$ that
$\DS \dim K^{[1]} = \dim K$.

\begin{defn} \label{defnBarySubd}
Let $K$ be a simplicial complex in $\DS \RR^n$.
The simplicial complex $\DS K^{[1]}$ is called the
{\bfseries first barycentric subdivision}\index{Barycentric
Subdivision!First Barycentric Subdivision} of $K$.  Inductively, we
may define the {\bfseries $\DS \mathbf{m^{th}}$ barycentric
subdivision}\index{Barycentric Subdivision!$m^{th}$
Barycentric Subdivision} of $K$ as the simplicial complex
$\DS K^{[m]} = (K^{[m-1]})^{[1]}$ for $m>1$.
\end{defn}

Recall that the diameter of a set $\DS S \subset \RR^n$ is defined
by $\DS \diam S = \inf_{\VEC{x},\VEC{y} \in S}
\|\VEC{x}- \VEC{y}\|$ where $\|\cdot\|$ is a norm on
$\DS \RR^n$, usually the Euclidean norm. 
...
of $K$ is defined as $\DS \mesh K = \max_{(s) \in K} \diam\, [s]$.
\end{defn}

\begin{prop} \label{propMeshMBS}
Suppose that $K$ is a simplicial complex of dimension $k$.  Then
$\DS \mesh K^{[1]} \leq (k/(k+1)) \mesh K$.
\end{prop}

\begin{proof}
By definitions of $\DS K^{[1]}$ and the mesh of a simplicial
...
$\DS \mesh^{[1]} K = \|\VEC{b}_{(s_p)} - \VEC{b}_{(s_q)}\|$ for some
...
$\DS \mesh K^{[m]} \leq \big( (k/(k+1)\big)^m \mesh K \to 0$
as $m \to \infty$.
...
and $\DS \VEC{x} = \sum_{j=0}^k a_j \VEC{x}_j$ with $a_j >0$
and $\DS \sum_{j=0}^k a_j = 1$, then 
$\DS \phi(\VEC{x}) = \sum_{j=0}^k a_j \phi(\VEC{x}_j)$.
\end{enumerate}
\end{defn}

The set $\DS \bigg\{ \phi(\VEC{x}) :
\DS \VEC{x} = \sum_{j=0}^k a_j \VEC{x}_j \ \text{with}\ a_j >0
...
$\DS \RR^n$.  Moreover, $\VEC{x}$ is the only vertex of $K$ in
$\St(\VEC{x})$ and
$\DS \{\St(\VEC{x}) : \VEC{x} \ \text{is a vertex of} \ K\}$
...
$\DS \VEC{x} = \sum_{j=0}^m a_j \VEC{x}_j$ for some
$a_j> 0$ such that $\DS \sum_{j=0}^m a_j = 1$.  Hence, we
get from the definition of simplicial map that
$\DS \phi(\VEC{x}) = \sum_{j=0}^m a_j \phi(\VEC{x}_j)$ with
$a_j> 0$ such that $\DS \sum_{j=0}^m a_j = 1$.  Thus
...
$\DS \phi\big|_{[s]}(\VEC{x}) = \sum_{j=0}^m a_j \phi(\VEC{x}_j)$ for
$\DS \VEC{x} = \sum_{j=0}^k a_j \VEC{x}_j$ with $a_j >0$
and $\DS \sum_{j=0}^k a_j = 1$.  In particular,
...
\begin{proof}
Since $\DS \{\St(\VEC{y}) : \VEC{y} \ \text{is a vertex of} \ L\}$
...
is a subset $\DS f^{-1}(\St(\VEC{y}))$ for some vertex
...
$\DS \St(\VEC{x}) \subset B_\epsilon(\VEC{x}) \subset 
...
(\ref{scapproEq1}) that
$\DS f(\St(\VEC{x})) \subset \St(\phi(\VEC{x}))$.
...
$\DS c = \sum_{\substack{(s) \in K\\\dim(s)=k}} a_{(s)}
...
$\DS \partial_k \bigg( \sum_{\substack{(s) \in K\\ \dim(s) = k}}
a_{(s)}\os{s}{}{}{}{} \bigg)
...
The {\bfseries (simplicial) $\DS \mathbf{k^{th}}$ homology
...
$\DS C(f)\Big( \sum_{\substack{(s) \in K\\ \dim(s) =k}}
a_{(s)}\os{s}{}{}{}{} \Big)
...
are of the form $\DS c = \sum_{(\VEC{x}_j) \in K} a_{(\VEC{x}_j)}
...
\cong \ZZ \ .
\]
We also have that $\DS B_1(K^1;\ZZ) = \{ 0 \}$ because
$\DS C_2(K^1;\ZZ) = \{ 0 \}$.  Therefore
$\DS H_1(K^1;\ZZ) = Z_1(K^1;\ZZ) / B_1(K^1;\ZZ) \cong \ZZ$.
...
The {\bfseries $\DS \mathbf{j^{th}}$ Betti
number}\index{$\DS j^{th}$ Betti Number}
...
$\DS \Chi(K) = \sum_{j=0}^{\dim(K)} (-1)^j \beta_j$.
\end{defn}
...
\end{cases}
\]
Also, let $\DS \partial_k^\sharp = \partial_k$ for $k \neq 0$ and
\[
...
$\DS \partial_0^\sharp \circ \partial_1^\sharp= 0$.
We can then proceed as we did before to define
$\DS Z_k^\sharp(X;R)$, $\DS B_k^\sharp(X;R)$
and $\DS H_k^\sharp(X;R)$.

\begin{defn}
Let $K$ be a simplicial complex and $R$ be an integral domain.
The quotient $\DS H_k^\sharp(K;R) =
Z_k^\sharp(X;R) / B_k^\sharp(X;R)$ is called the
{\bfseries reduced (simplicial) $\DS \mathbf{k^{th}}$ homology
...
$\DS C^k(K;\RR)$ denote the dual space of $C_k(K;\RR)$.
Namely, $\DS C^k(K;\RR)$ is the vector space of all linear
functionals on $C_k(K;\RR)$.  The elements of $\DS C^k(K;\RR)$
...
$\DS \dfC_{k-1}: C^{k-1}(K;\RR) \to C^k(K;\RR)$ defined
by
\begin{equation} \label{simplCoOpEq1}
\big(\dfC_{k-1}(\phi)\big)(c) = \phi\big(\partial_k(c)\big)
\end{equation}
for all $c \in C_k(K;\RR)$ and $\DS \phi \in C^{k-1}(K;\RR)$.
The map $\DS \dfC_k$ is called
the {\bfseries coboundary operator}\index{Coboundary Operator}.
\end{defn}

It is clear that $\DS \dfC_k$ is well and uniquely
defined by (\ref{simplCoOpEq1}).  It is easy to prove using
$\partial_{k+1} \circ \partial_{k+2} = 0$ that
$\DS \dfC_{k+1} \circ \dfC_k = 0$.

\begin{defn}
Let $K$ be a simplicial complex,
$\DS Z^k(K;\RR) = \{ \phi \in C^k(K;\RR) :
\dfC_k \phi = 0 \}$
and $\DS B^k(K;\RR) = \{ \dfC_{k-1} \phi : \phi \in
C^{k-1}(K;\RR) \}$.
The {\bfseries $\DS \mathbf{k^{th}}$ cohomology
module}\index{Cohomology Module!$k^{th}$ Cohomology Module}
of $K$ is the
quotient $\DS H^k(K;\RR) = Z^k(K;\RR) / B^k(K;\RR)$.
The elements of $\DS Z^k(K;\RR)$ are called
{\bfseries cocycles}\index{Cocycle} and those of
$\DS B^k(K;\RR)$ are called
{\bfseries coboundaries}\index{Coboundary}.
The equivalence class in $\DS H^k(K;R)$ associated to
$\DS \phi \in Z^k(K;R)$ is denoted $[\phi]_K$.
\end{defn}

\begin{theorem} \label{thmHupeHdk}
Let $K$ be a simplicial complex.  Then $\DS H^k(K;\RR)$ is
isomorphic to $\DS (H_k(K;\RR))^\ast$.
\end{theorem}

\begin{proof}
The map $\DS h:H^k(K;\RR) \to (H_k(K;\RR))^\ast$ defined by
$h([\phi]_K) = \phi$ for $\DS \phi \in Z^k(K;\RR)$ is well
defined because
\begin{align*}
...
Then $\DS \phi_1 - \phi_2 = \psi \in B^k(K;\RR)$.  Hence
...
We have that $h$ is onto.  Given $\DS \phi \in (H_k(K;\RR))^\ast$,
then $\DS [\phi]_K \in H^k(K;\RR)$ because
\begin{align*}
...
Hence, the map $h$ is an isomorphism between $\DS H^k(K;\RR)$ and
$\DS (H_k(K;\RR))^\ast$.
\end{proof}

It is possible to describe explicitly the action of the coboundary
operator $\DS \dfC_k$.  Suppose that $K$ is a
simplicial complex.   Given an open $k$-simplex $(s)$ in $K$, we defined
$\DS \phi_{\osscript{s}{}{}{}{}} \in C^k(K;\RR)$ by
...
\phi_{\osscript{s_m}{}{}{}{}} \big\}$ is a basis of $\DS C^k(K;\RR)$.
...
$\DS \overline{\dfC}_{k-1} : \big(C_{k-1}(K,L;R)\big)^\ast
...
$\DS \phi \in \big(C_{k-1}(K,L;R)\big)^\ast$.
\end{defn}

It is clear that $\DS \overline{\dfC}_k$ is well
and uniquely defined by (\ref{relsimplCoOpEq1}).

\begin{defn}
Let $L$ be a simplicial subcomplex of a simplicial complex $K$, and $R$
be an integral domain.
The {\bfseries relative $\DS \mathbf{k^{th}}$ cohomology
module}\index{Cohomology Module!Relative $k^{th}$ Cohomology Module} of
$K \mod L$ is the
quotient space $\DS H^k(K,L;R) = \KE(\overline{\dfC}_k)/
...
$\DS \phi_1 \in C^{k_1}(K;R)$ with
$\DS \phi_2 \in C^{k_2}(K;R)$ is the linear functional
$\DS \phi_1 \cup \phi_2 \in C^{k_1+k_2}(K;R)$ defined first by
...
after extended linearly to $\DS C^{k_1+k_2}(K;R)$.
\end{defn}
...
$\DS \delta \in C^0(K;R)$ defined by
$\delta(\os{\VEC{x}_i}{}{}{}{}) = 1$ for all vertices $\VEC{x}_i$ of $K$.
We define the cup product of
$\DS [\phi_1]_K \in H^{k_1}(K;R)$ and
$\DS [\phi_2]_K \in H^{k_2}(K;R)$ as
$\DS [\phi_1]_K\cup[\phi_2]_K = [\phi_1 \cup \phi_2]_K$.
...
\begin{enumerate}
\item $S$ is a manifold of class $\DS C^\infty$,
...
and a map $h_{[s]}:U_{[s]} \to S$ of class $\DS C^\infty$
...
and the circle $\DS S^1$ inscribed in $[K]$.  The maps $h$
...
{\bfseries $\DS \mathbf{j^{th}}$ barycentric
...
\begin{rmk}
The $\DS j^{th}$ barycentric coordinates given in
...
$\DS \VEC{x} = \sum_{j=2}^4 a_j \VEC{x}_j$
with $\DS \sum_{j=2}^4 a_j = 1$.
...
respectively where
$\DS \tilde{\VEC{x}} = \sum_{j=3}^4 \tilde{a}_j \VEC{x}_j$
with $\DS \sum_{j=3}^4 \tilde{a}_j = 1$.
...
respectively where
$\DS \breve{\VEC{x}} = \sum_{j=2}^3 \breve{a}_j \VEC{x}_j$
with $\DS \sum_{j=2}^3 \breve{a}_j = 1$.
...
$\DS \VEC{x} = \sum_{j=0}^q b_j(\VEC{x}) \VEC{x}_j$ with
$b_j(\VEC{x}) \geq 0$ and $\DS \sum_{j=0}^q b_j(\VEC{x}) = 1$.
More precisely, since
$\VEC{x} \in (\VEC{x}_{j_0},\VEC{x}_{j_1}, \ldots, \VEC{x}_{j_p}) \in K$
for some $j_i \in \{0,1,2,\ldots,q\}$ and $0 \leq i \leq p$, we get
that $\DS \VEC{x} = \sum_{i=0}^p b_{j_i}(\VEC{x}) \VEC{x}_{j_i}$
with $b_{j_i}(\VEC{x}) > 0$ and
$\DS \sum_{i=0}^p b_{j_i}(\VEC{x}) = 1$.  Moreover,
...
Hence $\DS \VEC{x} = \sum_{i=0}^q b_{j_i}(\VEC{x}) \VEC{x}_{j_1}$
with $b_{j_i}(\VEC{x}) > 0$ for $0 \leq i \leq q$
and $\DS \sum_{i=0}^q b_{j_i}(\VEC{x}) = 1$.  
\item It follows from (3) that
$\DS [K]\setminus \St\big((s)\big) =
...
like to find a relation between the $\DS k^{th}$ cohomology
module of $S$ defined in Definition~\ref{defnqdeRhamCM} and the
$\DS k^{th}$ cohomology group $\DS H^k(K;\RR)$ of
$K$.  For that, we will define a map
$\DS F_k : C^\infty\big(S,\Omega^k(S)\big) \to C^k(K,\RR)$
...
\end{align*}
and $\DS W_{[s]} = \phi_{[s]}^{-1}(U_{[s]})$.
Hence, the local representation of $\DS h_{[s]}^\ast(\omega)$
is given by $\DS \phi_{[s]}^\ast(h_{[s]}^\ast(\omega)) =
g_{[s]} \df{y_1}\wedge \df{y_2} \wedge \ldots \wedge \df{y_q}$ where
$g_{[s]}:W_{[s]} \to \RR$ is of class $\DS C^\infty$ and
\[
...
\begin{prop}
We have that $\DS \dfC_k \circ F_k = F_{k+1} \circ \df{}$.
\end{prop}
...
$\DS \tilde{F}_k:H^k(S) \to H^k(K;\RR)$ be the map defined by
$\DS \tilde{F}_k([\omega])([c]) = [F_k(\omega)(c)]_K$ for the
...
$\DS G_k: C^k(K;\RR) \to C^\infty\big(S,\Omega^k(S)\big)$
for $0 \leq k \leq \dim(S)$ such that:
\begin{enumerate}
\item $\DS \df{}\circ G_k = G_{k+1}\circ \dfC_k$ for
$0 \leq k < \dim(S)$.
\item $F_k \circ G_k = \Id$.
\item If $\DS \phi \in C^0(K,\RR)$ satisfies
$\phi(\os{\VEC{x}}{}{}{}{}) = 1$
...
$\DS \sum_{i=0}^m b_{j_i}(\VEC{x}) = 1$.  Therefore, we must
...
$\DS V_j \subset \RR^n$ such that
$\DS  A_j \subset V_j \subset \overline{V}_j
\subset \RR^n \setminus B_j$.
Using partition of unity for instance, we can select a function
$\DS f_j \in C^\infty(\RR^n)$ such that
$0 \leq f_j(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in \RR^n$,
...
Let $\DS g = \sum_{j=0}^pf_j$ on $\DS \RR^n$.  We
...
$\DS g_j\in C^\infty(W)$ and
$\DS \sum_{j=0}^pg_j = 1$ on $[K]$.  Moreover,
...
$\DS F_k(\omega) = \dfC_k(\phi)$ for some
$\DS \phi \in C^{k-1}(K;R)$,
...
in an open neighbourhood of $B_{(s)}$ in $\DS \RR^n$
...
$\DS \int_{\partial [s]} \omega =0$.
\item Assuming $q,k>0$, if $\omega$ is a closed differential $k$-form
defined in an open neighbourhood of $[s]$ in $\DS \RR^n$ where
...
require that $\DS \int_{\partial [s]} \rho = \int_{[s]} \omega$.
\end{enumerate}
...
neighbourhood of $B_{(s)}$ such that $\DS \pdydx{\omega}{x_i} = 0$ in
...
have that
$\DS 0 = \int_{\partial [s]} \omega = \omega(\VEC{x}_{j_1}) - 
...
$\DS H^k(U) = H^k(\RR^n) = 0$ for $k>0$ because $U$ is
diffeomorphic to $\DS \RR^n$.}.  Since $\mu_1$ may not be
...
$\DS f \in C^\infty(\RR^n)$ such that $f=1$ in an open
...
$\DS [K^j]$ of $K$,
$\df{\rho_j} = \omega$ in this open neighbourhood of $\DS [K^j]$,
$\rho_j = \rho_{j-1}$ in an open neighbourhood of $\DS [K^{j-1}]$
...
$\DS [K^0]$ by $\rho_0\big|_{B_j} = \tilde{\rho}_0 + a_j$ for all $j$
satisfies all the required conditions.  In particular,
$\DS F_0(\rho_0)\big((\VEC{x}_j)\big) = \int_{(\VEC{x}_j)}
...
of $\DS [K^j]$ as $\rho_j = \tilde{\rho}_j$.

If $k-1 = j$, then we use a trick similar to the one that we have used in
the case $j=0$ to ensure that $F_j(\rho_j) = \phi$ on $\DS C^j(K;R)$.
Let $\DS \alpha_j = \phi - F_{k-1}(\tilde{\rho}_{k-1}) \in C^j(K;R)$.
We define $\rho_j$ in a neighbourhood $U$ of $\DS [K^j]$ as
...
$\DS [K^{q-1}]$ for all open $q$-simplices $(t)$ in $K$.
Since all elements of $\DS C^q(K;R)$ are linear combinations
of elements of the form $\phi_{\osscript{t}{}{}{}{}}$ for $(t)$ an
open $q$-simplex in $K$, we get that $G_q(\phi) = 0$ in a
neighbourhood of $\DS [K^{q-1}]$ for all
$\DS \phi \in C^q(K;R)$.  Thus, for $q=k-1 = j$, we
get that
\[
...
\]
in a neighbourhood of $\DS [K^{j}]$, and
\[
\rho_j = \tilde{\rho}_j + G_{k-1}(\alpha_j)
= \tilde{\rho}_j = \rho_{j-1}
\]
in a neighbourhood of $\DS [K^{j-1}]$.  Moreover,
\[
F_j(\rho_j)(\os{s}{}{}{}{})
...
$\DS G_k(Z^k(K;\RR)) \subset Z^k(S)$ where
$\DS Z^k(S)$ is the set of closed differential $k$-forms, and
$\DS G_k(B^k(K;\RR)) \subset B^k(S)$ where
$\DS B^k(S)$ is the set of exact differential $k$-forms.
Thus $G_k$ defines a map, that we denote $\tilde{G}_k$, from
$\DS H^k(K;\RR)$ to $\DS H^k(S)$.  Hence, it
follows from (2) that $\tilde{G}_k$ is a right inverse for
$\tilde{F}_k$ and so $\tilde{F}_k$ is onto.

It follows from Claim 2 that, if $\DS \omega \in Z^k(S)$ and
$\DS F_k(\omega) \in B^k(K;\RR)$, then
$\DS \omega \in B^k(S)$.  Since $F_k$ is a linear map, this
