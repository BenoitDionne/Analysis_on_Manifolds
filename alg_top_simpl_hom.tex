...
$\{K_m\}_{m\in \NNp}$ of $K$, we find that there exists $m > 0$ and
...
\stage{d} As before, we have that
\[
Z_1(K;\ZZ)
= \left\{ a \left( \os{\VEC{x}_0}{}{}{}{\VEC{x}_1}
+ \os{\VEC{x}_1}{}{}{}{\VEC{x}_2}
...
{\bfseries reduced (simplicial) $\mathbf{k^{th}}$ homology
...
$\overline{\dfC}_{k-1} : \big(C_{k-1}(K,L;R)\big)^\ast
...
$S$ and $\os{s}{}{}{}{} \in C_k(K,\RR)$, we have that
\begin{align*}
...
$q=k+1$, we must also require that
...
$H^k(U) = H^k(\RR^n) = 0$ for $k>0$ because $U$ is diffeomorphic to
$\displaystyle \RR^n$.}.  Since $\mu_1$ may not be equal to $\rho$ in a
neighbourhood of $B_{(s)}$, we consider
...
If $q>1$, we can show that (2) can be used with $\rho$ replaced by
...
If $q=1$, we also have that there is a differential $k-1$-form $\tau$
...
If $k-1 > 0$, we may take $\rho_0 = \tilde{\rho}_0$.
If $k-1 = 0$, we have to make sure that
...
of $[K^j]$ by $\tilde{\rho}_j\big|_{U_{(s)}} = \tilde{\rho}_{(s)}$ for
...
If $k-1 \neq j$, we may define $\rho_j$ in a neighbourhood
of $[K^j]$ as $\rho_j = \tilde{\rho}_j$.

If $k-1 = j$, we use a trick similar to the one that we have used in
