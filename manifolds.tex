...
Intuitively, if $\DS V \subset \RR^3$ is a small enough open
neighbourhood of a point $\VEC{x}$ on a nice surface $S$ in
$\DS \RR^3$, then the set $V \cap S$ looks like a piece of
$\DS \RR^2$.  How can we give a meaningful sense to this
...
The interior $\DS H_k^\circ$ is open in the induced topology because
$\DS H_k^\circ = \left\{ \VEC{x}\in \RR^k : x_k>0\right\} \cap H_k$.
...
that may not be defined on an open subset of $\DS \RR^n$.
...
$\VEC{x}$ in $\DS \RR^k$ and a continuously differentiable function
$\DS F:V_{\VEC{x}} \to \RR^m$ such that $\DS F = f$ on
$V_{\VEC{x}}\cap U$.  The function $F$ is an
{\bfseries extension}\index{Extension of a Function} of $f$ to an open
neighbourhood of $\VEC{x}$ in $\DS \RR^k$.  We define
$\diff f(\VEC{x})$ as $\diff F(\VEC{x})$.
We say that $\DS f:U \to \RR^m$ is 
...
If $U$ is an open subset of $H_k$ and $\DS f:U \to \RR^m$ is
continuously differentiable at $\VEC{x} \in U$, then the definition of
$\diff f(\VEC{x})$ is independent of the extension chosen.
\end{prop}

\begin{proof}
For $i=1,2$, suppose that $\DS V_{\VEC{x}}^{[i]}$ is an open
neighbourhood of $\VEC{x}$ in $\DS \RR^k$ and that
$\DS F^{[i]}:V_{\VEC{x}}^{[i]} \to \RR^m$ is a continuously
differentiable function such that $\DS F^{[i]} = f$ on
$\DS V_{\VEC{x}}^{[i]}\cap H_k$.  Let
$\DS V = V_{\VEC{x}}^{[1]} \cap V_{\VEC{x}}^{[2]} \cap U \cap H_k^\circ$.

If $\DS \VEC{x}\in H_k^\circ$, then $V$ is an open neighbourhood of
$\VEC{x}$ in $\DS \RR^k$.
We have that $\DS F^{[1]} = F^{[2]} = f$ in $V$.  Since the
derivative at a point is a local property, we get
$\DS \diff F^{[1]}(\VEC{x}) = \diff F^{[2]}(\VEC{x}) = \diff
f(\VEC{x})$, where $\diff f$ denotes the standard derivative of $f$.

Suppose that $\VEC{x} \in U \cap \partial H_k$.  Choose
$\DS \{ \VEC{x}_i \}_{i=1}^\infty \subset V$ such that
$\DS \{ \VEC{x}_i \}_{i=1}^\infty$ converges to
$\VEC{x}$.  Since $\DS F^{[1]} = F^{[2]} = f$ in $V$, we
have as in the previous paragraph that
$\DS \diff F^{[1]}(\VEC{x}_i) = \diff F^{[2]}(\VEC{x}_i)$
for all $i$.  Thus $\DS \diff F^{[1]}(\VEC{x}) = \lim_{i\to
  \infty} \diff F^{[1]}(\VEC{x}_i) =
\lim_{i\to \infty} \diff F^{[2]}(\VEC{x}_i) = \diff F^{[2]}(\VEC{x})$
by continuity of $\DS \diff F^{[1]}$ and
$\DS \diff F^{[2]}$ on $V$.
\end{proof}
...
$\DS \diff (g\circ f)(\VEC{x}) = \diff g(f(\VEC{x})) \diff f(\VEC{x})$
for all $\VEC{x} \in U$.
\end{prop}

\begin{proof}
Given $\VEC{x} \in U$, suppose that $U_1$ is an open neighbourhood of
$\VEC{x}$ in $\DS \RR^k$ and that
$\DS F:U_1 \to \RR^k$ is a continuously differentiable
function such that $\DS F = f$ on $U_1 \cap U$.  Similarly,
suppose that $V_1$ is an open neighbourhood of $f(\VEC{x})$ in
$\DS \RR^k$ and that $\DS G:V_1 \to \RR^k$ is a
continuously differentiable function such that $\DS G = g$ on
$V_1 \cap V$.

Since $F$ is continuous, $\DS U_2 = F^{-1}(V_1) \cap U_1$ is an open
neighbourhood of $\VEC{x}$ in $\DS \RR^k$.  Hence
$\DS G\circ F: U_2 \to \RR^k$ is a
continuously differentiable function such that $G\circ F = g\circ f$ on
$U_2 \cap U$.

We have that $\DS \diff (G\circ F)(\VEC{x}) = \diff G( F(\VEC{x}))
\diff F(\VEC{x})$.  Since
...
we get that
$\DS \diff (g\circ f)(\VEC{x}) = \diff g(f(\VEC{x})) \diff
f(\VEC{x})$.
\end{proof}
...
$h$ is a homeomorphism, and $h$ and $\DS h^{-1}$ are continuously
differentiable.
\end{defn}
...
Let $S$ be a subset of $\DS \RR^n$.  We assume that the
topology on $S$ is the induced topology from $\DS \RR^n$.  A
...
\begin{rmk}
For $k=0$, we define $H_k$ and $\DS \RR^k$ as $\{ 0 \} \subset \RR$.
...
An {\bfseries atlas}\index{Atlas} of class $\DS C^j$,
denoted $\A(k,j)$, for a set $\DS S \subset \RR^n$ is a
collection of local charts
$\DS \left\{ (W_i, U_i,\phi_i)\right\}_{i\in I}$, where
$I$ is an index set, such that the following conditions are satisfied.
\begin{enumerate}
\item $\DS S = \bigcup_{i\in I} U_i$.
...
\end{equation}
is a $\DS C^j$-diffeomorphism (Figure~\ref{MANIFOLD2}).
\end{enumerate}
\end{defn}

For $j>0$, the condition that
$\DS \phi_{i_2}^{-1} \circ \phi_{i_1}: M_1 \rightarrow M_2$
be a $\DS C^j$-diffeomorphism for all indices $i_1,i_2 \in I$
implies that $\diff \phi_i(\VEC{w})$ is of rank $k$ for all
$\VEC{w} \in W_i$ and
...
charts}{The map $\DS \phi_2^{-1}\circ\phi_1$ obtained
from two local charts provides a diffeomorphism from
$\DS M_1 =\phi_{i_1}^{-1}(U_{i_1} \cap U_{i_2})$ to
$\DS M_2 = \phi_{i_2}^{-1}(U_{i_1}\cap U_{i_2})$.}{MANIFOLD2}

In the previous definition, we could have considered only homeomorphisms
$\DS \phi_{i_2}^{-1}\circ \phi_{i_1}$ instead of
...
class $\DS C^j$ is a subset $S$ of $\DS \RR^n$ with a
...
To determine if a set $\DS S \subset \RR^n$ is a manifold,
...
$\DS W \subset H_k^\circ$.  The set of all interior points
...
such that $\DS \phi^{-1}(\VEC{u}) \in \partial H_k$.
...
If $\partial S = \emptyset$, then we say that we have a
{\bfseries manifold without boundary}\index{Manifold!Manifold Without Boundary}.
If $\partial S \neq \emptyset$, then we say that we have a
...
$\DS H_k^\circ$ in the definition of a manifold by
$\DS \RR^k$ without any consequence.  This is often very
...
= (-\sqrt{4-w^2}\,,w-2) \right) \ .
\end{align*}
We have that $\DS ]-2,2[ \subset \RR$.

The open upper-hemisphere
$\DS S_o= \left\{ \VEC{x} \in \RR^3 : \| \VEC{x} \| = 2 \ \text{and} \
x_3> 0 \right\}$
...
\]
We have that $\DS \{(w_1,w_2):w_1^2+w_2^2<4\} \subset \RR^2$.

Both $S$ and $S_o$ are manifolds of class $\DS C^\infty$
without boundary.

The closed upper-hemisphere
$\DS S_c= \left\{ \VEC{x} \in \RR^3 : \| \VEC{x} \| = 2 \
\text{and} \ x_3 \geq 0 \right\}$
is a $2$-dimension manifold with boundary.
Let $\DS B = \{(w_1,w_2):w_1^2+(w_2-2)^2<4\} \subset H_2^\circ$
and $\DS B_o = \{(w_1,w_2):w_1^2+w_2^2<4, w_2\geq 0\} \subset H_2$.
...
\end{align*}
where $\DS B = \{(w_1,w_2):w_1^2+(w_2-2)^2<4\} \subset H_2^\circ$
and $\DS B_o = \{(w_1,w_2):w_1^2+w_2^2<4, w_2\geq 0\} \subset H_2$.
...
$\DS \tilde{\VEC{w}} \in W \cap H_k^\circ$.  Then
$\DS \diff \phi(\tilde{\VEC{w}}) (\RR^k) \subset \partial H_k$.
\end{prop}

\begin{proof}
Let $\DS \pi_k:\RR^k \to \RR$ be the canonical projection on
the $\DS k^{th}$ coordinate.  Choose
$\DS \VEC{x} \in \RR^k$.  Since $\phi$ is differentiable at
$\DS \tilde{\VEC{w}} \in W \cap H_k^\circ$, we
have that
$\DS \phi(\tilde{\VEC{w}} + t\VEC{x}) = \phi(\tilde{\VEC{w}}) +
\diff \phi(\tilde{\VEC{w}}) (t\VEC{x}) + g(t\VEC{x})$
for $|t|$ small enough where $\DS g:\RR^k \to \RR^k$ a
function such that
$\DS \lim_{\VEC{y}\to \VEC{0}} \frac{\|g(\VEC{y})\|}{\|\VEC{y}\|}=0$.

Since $\phi(\tilde{\VEC{w}}) \in \partial H_0$ and
$\pi_k(\phi(\VEC{w})) \geq 0$
for all $\DS \VEC{w}\in \RR^k$ by hypothesis, we have
...
\]
If we let $t$ converges to $0$, then we get
$\DS 0 \leq
...
+ \frac{1}{t} \left(\pi_k \circ g\right)(t\VEC{x}) \ .
\]
Again, if we let $t$ converges to $0$, then we get
$\DS 0 \geq
\left(\pi_k \circ \diff \phi(\tilde{\VEC{w}})\right) (\VEC{x})$.

Hence $\left(\pi_k \circ \diff \phi(\tilde{\VEC{w}})\right) (\VEC{x}) = 0$.
Since $\DS \VEC{x}\in \RR^k$ is arbitrary, we get the conclusion
of the proposition.
\end{proof}
...
$\DS f(W_1 \cap H_k^\circ) \subset W_2 \cap H_k^\circ$ and
$f(W_1 \cap \partial H_k) \subset W_2 \cap \partial H_k$.  Moreover,
$\DS f\big|_{W_1 \cap H_k^\circ}: W_1 \cap H_k^\circ 
\to W_2 \cap H_k^\circ$ and 
$\DS f\big|_{W_1 \cap \partial H_k}:W_1 \cap \partial H_k
\to W_2 \cap \partial H_k$ are diffeomorphisms.
\end{prop}

\begin{proof}
\subQ{i} We consider the case with $\DS W_1 \subset H_k^\circ$.   We
prove that $\DS W_2 \subset H_k^\circ$.   Hence
$\DS f\big|_{W_1 \cap H_k^\circ}:W_1 \cap H_k^\circ \to
W_2 \cap H_k^\circ$ will be a diffeomorphism because
$\DS W_1 \cap H_k^\circ = W_1$ and
$\DS W_2 \cap H_k^\circ = W_2$ are open subsets 
of $\DS \RR^k$, and $f:W_1 \to W_2$ is then a diffeomorphism
...
$\diff f(\tilde{\VEC{w}})$ maps $\DS \RR^k$ onto
$\DS \RR^k$.  This result contradicts the
conclusion of Proposition~\ref{propPHsbH} which says that
$\DS \diff f(\tilde{\VEC{w}}) (\RR^k) \subset \partial H_k$.  Thus
...
$\DS f^{-1}$ instead of $f$ would then yield
$W_1 \cap \partial H_k = \emptyset$.

Given $\DS \VEC{w} \in W_1 \cap H_k^\circ$, there exists an
open neighbourhood $\DS V_1 \subset W_1 \cap H_k^\circ$ of
$\VEC{w}$.  If we apply (i) with $W_1$ replaced by $V_1$, then we get that
$\DS f(V_1) \subset H_k^\circ$.  Thus $f(V_1)$ is an open
neighbourhood of $f(\VEC{w})$ in $\DS W_2 \cap H_k^\circ$.
Since $\VEC{w}$ is arbitrary, this shows that
$\DS f(W_1\cap H_k^\circ) \subset W_2 \cap H_k^\circ$.
The same reasoning applied to $\DS f^{-1}$ shows that
$\DS f^{-1}(W_2 \cap H_k^\circ) \subset W_1 \cap H_k^\circ$.
Thus $\DS f(W_1 \cap H_k^\circ) = W_2 \cap H_k^\circ$ and
$\DS f\big|_{W_1\cap H_k^\circ}:W_1 \cap H_k^\circ\to
...
of $\DS \partial H_k \cong \RR^{k-1}$.  So
$\DS f\big|_{W_1 \cap \partial H_k}:W_1 \cap \partial H_k
...
$\DS \phi_i^{-1} \circ \phi_j$, where $(W_i, U_i, \phi_i)$ and
$(W_j, U_j, \phi_j)$ are two local charts of $S$ such that 
$U_i \cap U_j \neq \emptyset$.

If $\DS \{ (W_i,U_i,\phi_i) \}_{i\in I}$ is a maximal atlas
for $S$, then
\[
...
\]
Recall that
$\DS W_i \cap \partial H_k$ is an open subset of
$\DS \{ \VEC{x} \in \RR^k : x_k = 0 \} \cong \RR^{k-1}$ for
...
$\DS \left\{ (W_i, U_i,\phi_i)\right\}_{i\in I}$ for $S$ such
that $\DS U_i \cap M = \phi_i(W \cap H_k^{[k_1,k_2,\ldots,k_q]})$ 
for all $i \in I$, where
$\DS H_k^{[k_1,k_2,\ldots,k_q]}
= \{ \VEC{w} \in H_k : w_{k_i} =0 \ \text{for} \ 1 \leq i \leq q \}$.
\end{defn}

If we set $\DS \tilde{W}_i = W \cap H_k^{[k_1,k_2,\ldots,k_q]}$ and
$\tilde{U}_i = U_i \cap M$ for all $i \in I$, then
$\DS \left\{ (\tilde{W}_i, \tilde{U}_i,\phi_i)\right\}_{i\in I}$
is an atlas for $M$.  Hence $M$ is itself a $q$-dimensional manifold.

\subsection{Manifolds in $\DS \mathbf{\RR^n}$}
\label{manifRRn}
...
Let $S$ be a subset of $\DS \RR^n$ and $\VEC{u} \in S$.  The
following two statements are equivalent.
\begin{enumerate}
\item There exist an open neighbourhood
$\DS V_1 \subset \RR^n$ of $\VEC{u}$, an open set
$\DS V_2 \subset \RR^n$, and a diffeomorphism $h:V_1\to V_2$
such that
$h(V_1 \cap S) = \{ \VEC{v} \in V_2 : v_{k+1} = v_{k+2} = \ldots = v_n = 0\}$
(Figure~\ref{MANIFOLD4}).
\item There exist an open subset $\DS W \subset \RR^k$, an
open neighbourhood $\DS V \subset \RR^n$ of $\VEC{u}$ and a
function $\phi:W \to V \cap S$ such that $\phi$ is a homeomorphism,
$\phi$ is of class $\DS C^1$ and $\diff \phi(\VEC{w})$ is of
rank $k$ for all $\VEC{w} \in W$.
\end{enumerate}
Moreover, (1) implies that $\DS \phi^{-1}:V\cap S\to W$ in
...
$\DS \phi(\VEC{w}) =  h^{-1}(\VEC{w},\VEC{0})$ for
$\VEC{w} \in W$.
We have that $\phi(W) = U$,
$\DS \phi = h^{-1}\big|_W$ and $\phi^{-1} = h\big|_U$ are
continuous, and $\DS \phi \in C^1(W)$ because
$\DS h^{-1} \in C^1(V_2)$ with $W \subset V_2$.
Moreover,  $\DS \tilde{h}:V_1 \to \RR^k$ defined by
$\DS \tilde{h}(\VEC{x}) =
\begin{pmatrix}
h_1(\VEC{x}) & h_2(\VEC{x}) & \ldots & h_k(\VEC{x})
\end{pmatrix}^\top$ for all $\VEC{x} \in V_1$ is a continuously
differentiable extension of $\DS \phi^{-1}$.

It follows from $\tilde{h} \circ \phi = \Id$ on $W$ that
$\DS \diff \tilde{h} (\phi(\VEC{w})) \, \diff \phi(\VEC{w})
= \diff (\tilde{h} \circ \phi) (\VEC{w}) = \Id_k$
for all $\VEC{w} \in W$ where $\Id_k$ is the identity matrix on
$\DS \RR^k$.  Thus, the rank of $\diff \phi(\VEC{w})$ is $k$.
...
changing the order of the coordinates in $\DS \RR^n$ if
necessary, we may assume that the matrix
\[
M = \begin{pmatrix}
\DS \pdydx{\phi_1}{w_1}(\VEC{w}) &
\DS \pdydx{\phi_1}{w_2}(\VEC{w}) & \ldots &
\DS \pdydx{\phi_1}{w_k}(\VEC{w}) \\[1em] 
\DS \pdydx{\phi_2}{w_1}(\VEC{w}) &
\DS \pdydx{\phi_2}{w_2}(\VEC{w}) & \ldots &
\DS \pdydx{\phi_2}{w_k}(\VEC{w}) \\
\vdots & \vdots & \ddots & \vdots \\
\DS \pdydx{\phi_k}{w_1}(\VEC{w}) &
\DS \pdydx{\phi_k}{w_2}(\VEC{w}) & \ldots &
\DS \pdydx{\phi_k}{w_k}(\VEC{w})
\end{pmatrix}
\]
is invertible.  Let
$\DS g: W \times \RR^{n-k} \to \RR^n$ be the function
defined by
$\DS g\left( \begin{pmatrix} \VEC{z} \\ \VEC{y} \end{pmatrix} \right)
= \phi(\VEC{z}) + \begin{pmatrix} \VEC{0} \\ \VEC{y} \end{pmatrix}$
for $\VEC{z} \in W$ and $\DS \VEC{y} \in \RR^{n-k}$.
Since
$\DS \det \left( \diff g\left(
...
an open neighbourhood $\DS \tilde{V}_2 \subset W \times \RR^{n-k}$ of
$\DS \begin{pmatrix} \VEC{w} & \VEC{0}\end{pmatrix}^\top$
and an open neighbourhood
$\tilde{V}_1 \subset V$ of $\DS  g\left(
...
$\DS \RR^k \subset \RR^n$ is the usual topology on
$\DS \RR^k$.
Since $\phi:W \to V\cap S$ is a homeomorphism, we
have that $\phi(D)$ is an open subset of $V\cap S$.  Hence, there
exists an open set $\DS U \subset \RR^n$ such that
$U\cap S = \phi(D)$.

Let $V_1 = \tilde{V}_1 \cap U$, $\DS V_2 = g^{-1}(V_1)
\subset \tilde{V}_2$, and set
$\DS h = g^{-1}\big|_{V_1}$.  Since
\[
...
\item There exist an open neighbourhood
$\DS V_1 \subset \RR^n$ of $\VEC{u}$, an open set
$\DS V_2 \subset \RR^n$, and a diffeomorphism $h:V_1\to V_2$ such that
...
$\DS V \subset \RR^n$ of $\VEC{u}$ and a function
$\phi:W \to V \cap S$ such that $\phi$ is a homeomorphism, $\phi$ is
of class $\DS C^1$ and $\diff \phi(\VEC{w})$ is of rank $k$
for all $\VEC{w} \in W$.
\end{enumerate}
Moreover, (1) implies that $\DS \phi^{-1}:V\cap S\to W$ in
(2) can be extended to a continuously differentiable function in the
...
If $\DS V \subset \RR^n$ is open and
$\DS g:V \to \RR^{n-k}$ is a function of class
$\DS C^1$ such that $\diff g(\VEC{x})$ is of rank $n-k$ for all
$\DS \VEC{x} \in g^{-1}(\{\VEC{0}\})$, then
$\DS S = g^{-1}(\{\VEC{0}\}) \subset V$
...
$\DS W_{\VEC{v}} \subset \RR^n$, an open neighbourhood
$U_{\VEC{v}} \subset V$ of $\VEC{v}$, and a diffeomorphism
$h:W_{\VEC{v}} \to U_{\VEC{v}}$ such that
$\DS g(h(\VEC{x})) = (x_{n-k+1} \ x_{n-k+2} \ \ldots \  x_n)^\top$ for
$\VEC{x} \in W_{\VEC{v}}$.

Permuting the coordinates in $V$ if necessary, we may assume that
\[
P = \begin{pmatrix}
\DS \pdydx{g_1}{x_{k+1}}(\VEC{v}) &
\DS \pdydx{g_1}{x_{k+2}}(\VEC{v}) & \ldots
& \DS \pdydx{g_1}{x_n}(\VEC{v}) \\
\DS \pdydx{g_2}{x_{k+1}}(\VEC{v}) &
\DS \pdydx{g_2}{x_{k+2}}(\VEC{v}) & \ldots
& \DS \pdydx{g_2}{x_n}(\VEC{v}) \\
\vdots & \vdots & \ddots & \vdots \\
\DS \pdydx{g_{n-k}}{x_{k+1}}(\VEC{v}) &
\DS \pdydx{g_{n-k}}{x_{k+2}}(\VEC{v}) &
\ldots & \DS \pdydx{g_{n-k}}{x_n}(\VEC{v})
\end{pmatrix}
\]
has rank $n-k$ and thus is invertible.  Consider the function
$\DS f:W_{\VEC{v}} \to \RR^n$ defined by
$\DS
f(\VEC{x}) = \begin{pmatrix} x_1 & x_2 & \ldots & x_k & g(h(\VEC{x}))
\end{pmatrix}^\top$ for $\VEC{x} \in W_{\VEC{v}}$.
If $\DS \VEC{u} = h^{-1}(\VEC{v})$, then we have
\[
\diff f (\VEC{u}) =
\begin{pmatrix}
\Id_{k\times k} & 0 \\
...
$\DS \pdydx{g_{i}}{x_{j}}(\VEC{v})$ with $1\leq i \leq n-k$
...
The function $f$ maps $\DS h^{-1}(S) \cap W_{\VEC{v}}$ onto
$\left\{ \VEC{x} \in V_{\VEC{v}} : x_{n-k+1} = x_{n-k+2} = \ldots = x_n = 0
\right\}$.  It follows from Theorem~\ref{manifSPIVAK} that there exists a
local chart $(W, U, \phi)$ of $\DS h^{-1}(S) \cap W_{\VEC{v}}$ about
$\DS \VEC{w} = h^{-1}(\VEC{v})$.  Thus $(W, h(U), h\circ \phi)$
...
Let $\DS S\subset \RR^n$ be a $k$-dimensional manifold of
class $\DS C^1$ and $\VEC{u} \in S$.  Suppose that
$(W,U,\phi)$ is a local chart such that $\VEC{u} \in U$ and
$\DS \VEC{w} = \phi^{-1}(\VEC{u})$.  The
...
Recall that $\DS \phi_\ast: \TS_{\VEC{w}} W \to \TS_{\VEC{u}} \RR^n$
is defined by $\DS \phi_\ast (\VEC{w}, \VEC{v})
= \left( \phi(\VEC{w}) , \diff \phi (\VEC{w}) \VEC{v} \right)$
for $\DS \VEC{v} \in \RR^k$.
...
$\VEC{w} = \phi(\VEC{u}) \in W \cap \partial H_k$, then it follows from the
...
$\DS g:V \to W \subset \RR^k$ are two differentiable
...
$f:U\to V$ is a diffeomorphism.  Using $W=U$ and $\DS g=f^{-1}$ in
the previous relation, we get that
$\DS (f^{-1})_\ast \circ f_\ast =  (f^{-1} \circ f)_\ast = \Id_\ast$
and $\DS f_\ast \circ (f^{-1})_\ast
=  (f \circ f^{-1})_\ast = \Id_\ast$.
Thus $\DS (f_\ast)^{-1} = (f^{-1})_\ast$.

\begin{prop} \label{manifCTS}
Let $\DS S\subset \RR^n$ be a $k$-dimensional manifold of
class $\DS C^1$ and $\VEC{u} \in S$.  Suppose that
$(W_i,U_i,\phi_i)$ for $i=1,2$ are local charts about $\VEC{u} \in S$.  If
$\DS \VEC{w}_i = \phi_i^{-1}(\VEC{u})$ for $i=1,2$, then
\begin{equation} \label{manifTang}
(\phi_1)_\ast(\TS_{\VEC{w}_1} W_1) = (\phi_2)_\ast(\TS_{\VEC{w}_2} W_2) \ .
\end{equation}
Moreover
$\DS (\phi_2^{-1} \circ \phi_1)_\ast: \TS_{\VEC{w}_1} W_1 \to
...
$\DS \diff (\phi_2^{-1} \circ \phi_1)(\VEC{w}_1)$
is a linear isomorphism of $\DS \RR^k$ onto itself.  Thus
$\DS \diff (\phi_2^{-1} \circ \phi_1)(\VEC{w}_1)$ has rank
$k$.
Hence $\diff \phi_1(\VEC{w})$ has rank $k$ and it is one-to-one because
its domaine is $\DS \RR^k$.  Thus
$\DS \TS_{\VEC{x}} S = (\phi_1)_\ast \left(\TS_{\VEC{w}_1} W_1\right)$
is a linear subspace of $\DS \TS_{\VEC{x}} \RR^n$ of dimension $k$.

\begin{defn}
Suppose that $S_i$ is a $k_i$-dimensional manifold of class
$\DS C^q$ for $i=1,2$.
A function $f:S_1 \to S_2$ is of class $\DS C^q$ if
$\DS \phi_2^{-1} \circ f \circ \phi_1:
\phi_1^{-1}(f^{-1}(U_2)) \to W_2$ is of class $\DS C^q$ for
...
$f^{-1}:S_2 \to S_1$ are functions of class $\DS C^1$.
\end{defn}

The reader should prove that to determine if a map $f:S_1 \to S_2$
between two manifolds $S_1$ and $S_2$ is of class $\DS C^q$,
it suffices to prove that $\DS \phi_2^{-1} \circ f 
\circ \phi_1: \phi_1^{-1}(f^{-1}(U_2)) \to W_2$ is of class
$\DS C^q$ for local charts $(W_1,U_1,\phi_1)$ of an atlas
...
manifolds of class $\DS C^1$, then $f(\Int S_1) \subset \Int S_2$ and 
$f(\partial S_1) \subset \partial S_2$.  Moreover,
$\DS f\big|_{\Int S_1}:\Int S_1 \to \Int S_2$ and 
$\DS f\big|_{\partial S_1}:\partial S_1 \to \partial S_2$ are
diffeomorphisms.
\end{theorem}

Suppose that $S_i$ is a $k_1$-dimensional manifold of class
$\DS C^1$ for $i=1,2$, and that
$f:S_1 \to S_2$ is a map of class $\DS C^1$.  Since
$\DS S_1^\circ = \emptyset$ when
$k <n$, we cannot define the derivative of $f$ from an extension of
$f$ to $\DS \RR^n$ as we did for the derivative of a
...
$\DS f_\ast: \TS_{\VEC{u}_1} S_1 \to \TS_{\VEC{u}_2} S_2$
...
$\DS (\phi_2^{-1} \circ f \circ \phi_1)_\ast
\left(\TS_{\VEC{w}_1} W_1\right) \subset \TS_{\VEC{w}_2} W_2$
where $\DS \VEC{w}_1 = \phi_1^{-1}(\VEC{u}_1)$
and $\DS \VEC{w}_2 = \phi_2^{-1}(\VEC{u}_2)$.
If $\DS (\VEC{u}_1,\VEC{x}) \in \TS_{\VEC{u}_1}S_1$, then
$(\VEC{u}_1,\VEC{x}) = (\phi_1)_\ast(\VEC{w}_1,\VEC{y})$ for some
unique $\DS (\VEC{w}_1,\VEC{y}) \in \TS_{\VEC{w}_1}W_1$.
We set $\DS f_\ast(\VEC{u_1},\VEC{x}) = (\phi_2)_\ast \big(
...
and a substance of dimension $k_i$ in $\DS \RR^n$.

\begin{defn}
Suppose that $S$ is a $k$-dimensional manifold of class $\DS C^1$.
...
If $S$ is a $k$-dimensional manifold, then we can give a structure of
$2k$-dimensional manifold to $\TS S$.  Suppose that $(W,U,\phi)$ is a local
chart of $S$.  Let
$\DS \tilde{W} = W \times \RR^k$ and
$\DS \tilde{U} = \TS\, U = \bigcup_{\VEC{u} \in U} \TS_{\VEC{u}} S$, and
...
Note that $\DS \tilde{W} = W \times \RR^k$ can be seen as an
open subset of $H_{2k}$ because
$\DS H_k \times \RR^k \cong  H_{2k}$ by permuting coordinates.
...
$\DS \RR^n$ of dimension $k$.  
Recall that if $(W,U,\phi)$ is a local chart with $\VEC{u} \in U$
and $\VEC{w} \in W$ such that $\VEC{u} = \phi(\VEC{w})$, then
$\DS \diff \phi(\VEC{w})(\RR^k) \subset \RR^n$ is a subspace
of dimension $k$ of $\DS \RR^n$.  Therefore, we may associate an
...
\]
to $\TS_{\VEC{u}} S$ where $\DS \{ \VEC{x}_{\VEC{u},i} \}_{i=1}^k$ is
a basis of the subspace $\DS \diff \phi(\VEC{w})(\RR^k)$.

\begin{defn}
Let $S$ be a $k$-dimensional manifold of class $\DS C^1$.
...
without boundary is not orientation preserving, then we can easily produce
a local chart which will be orientation preserving.  Choose a linear map
$\DS T:\RR^k \to \RR^k$ with
$\det(T) = -1$ (e.g.\ a permutation of two coordinates), then
$\DS (T^{-1}(W), V, \phi \circ T)$ is a local chart which is
...
\end{align*}
This implies that
$\DS \diff (\phi_2^{-1} \circ \phi_1)(\VEC{w}_1)$ preserves the
orientation on $\DS \RR^k$.  Hence
$\DS \det( \diff (\phi_2^{-1} \circ \phi_1)(\VEC{w}_1) ) > 0$.

\begin{defn}
A $k$-dimensional manifold $S$ of class $\DS C^1$ is
...
\]
for all $\DS \{ (\VEC{u}, \VEC{x}_i \}_{1\leq i \leq k}
\subset \TS_{\VEC{u}} S$ such that
$\DS [(\VEC{u},\VEC{x}_1), (\VEC{u},\VEC{x}_2), \ldots,
...
$\DS \VEC{q}\in \RR^k \setminus H_k$ and
...
$\DS \partial S \cong \RR^{k-1}$ is $\DS (-1)^k$ times the
standard orientation on $\DS \RR^{k-1}$.  We have that
...
For $(n-1)$-dimensional manifold $S$ in $\DS \RR^n$, we may
...
$\VEC{u} \in S$ is the unit vector $\DS \VEC{n}_{\VEC{u}} =
...
(\VEC{u},\VEC{e}_n)]$,
the standard orientation on $\DS \RR^n$.
\end{defn}
...
$\rho:]-a,a[\to S$ of class $\DS C^1$ such that
...
$\DS \VEC{w} = \phi^{-1}(\VEC{u})$.  The function
\begin{align*}
...
because $\phi\circ \rho$ is of class $\DS C^1$ (composition
of functions of class $\DS C^1$) and
...
that $\DS \tilde{h}: \RR^n  \to \RR^k$ is of class
$\DS C^1$ and
$\DS \tilde{h}\big|_{S \cap U} = \phi^{-1}: U \to W$.
...
Suppose that $S$ is a $k$-dimensional manifold of class
$\DS C^1$ and that $\VEC{u} \in S$.
A {\bfseries tangent vector}\index{Tangent Vector} to $S$ at
$\VEC{u}$ is a map $\DS q: C^\infty(S) \to \RR$ with the
...
\end{equation}
for all real valued functions $\DS g \in C^\infty(S)$.
...
Suppose that $\DS V \subset \RR^n$ is an open set such that 
...
\begin{enumerate}
\item $0 \leq \psi(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in \RR^n$, 
\item $\psi(\VEC{x}) = 1$ for all $\VEC{x} \in B$ and
\item $\psi(\VEC{x}) = 0$ for all $\DS \VEC{x} \in \RR^n \setminus B$.
\end{enumerate}
We can get the function $\psi$ from Theorem~\ref{cov1}, Partition
of Unity, with the open cover
$\DS \{ V, \RR^n \setminus \overline{B}\}$.  If
\[
g(\VEC{u}) = \begin{cases}
\DS \pi_j(\phi^{-1}(\VEC{u})) \psi(\VEC{u})
& \quad \text{if} \ \VEC{u} \in U \\
0 & \DS \quad \text{if}\ \VEC{u} \in \RR^n \setminus U
\end{cases}
\]
for $1 \leq j \leq k$ fixed, then $\DS g \in C^\infty(S)$.
The projection $\DS \pi_j:\RR^k \to \RR$ defined by
$\pi_j(\VEC{w}) = w_j$ is used to obtain the
$\DS j^{th}$ component of $\DS \phi^{-1}$.
Since $\DS
(g\circ \phi)(\VEC{w}) = \pi_j(\phi^{-1}(\phi(\VEC{w}))) \psi(\phi(\VEC{w}))
= w_j$ for all
$\DS \VEC{w} \in \phi^{-1}(S\cap B) \subset W$, an open neighbourhood
of $\DS \phi^{-1}(\VEC{u})$, we get
...
The existence of the vector $\DS \VEC{y} \in \RR^k$ is
independent of the local chart used.  Suppose that
$(W_m,U_m,\phi_m)$ for $m=1,2$ are two local
charts of $S$ and $\VEC{u} \in U_1 \cap U_2$.  Moreover, suppose that
there exists $\DS \VEC{y}_1 \in \RR^k$ such that 
\[
q(g) = \sum_{i=1}^k y_{1,i}
\pdfdx{(g\circ \phi_1)}{w_i}\Big|_{\VEC{w}=\phi_1^{-1}(\VEC{u})}
\]
for all $\DS g \in C^\infty(S)$ is the local representation 
...
\pdfdx{(g\circ \phi_2)}{w_j}\Big|_{\VEC{w}=\phi_2^{-1}(\VEC{u})} 
\end{align*}
for all $\DS g \in C^\infty(S)$ if and only if
...
$\DS j^{th}$ component of $\DS \phi_2^{-1} \circ \phi_1$.
This relation (\ref{TSv2Eq2}) is exactly
the relation provided by $\DS \phi_2^{-1} \circ \phi_1$ 
in Proposition~\ref{manifCTS}.

\subI{$\breve{\TS}_{\VEC{u}} S$ is a Vector Space over $\RR$}
If $\DS q_m \in \breve{\TS}_{\VEC{u}} S$ for $m=1,2$
and
\[
...
Suppose that $\DS g_1,g_2 \in C^\infty(S)$ and that
$q \in \breve{\TS}_{\VEC{u}} S$ is given by (\ref{TSv2Eq1}).
Since $\DS g_1 g_2 \in C^\infty(S)$, we may compute
\begin{align*}
...
$\DS g_1,g_2 \in C^\infty(S)$, (2) $q(\lambda g) = \lambda q(g)$ 
for all $\DS g \in C^\infty(S)$ and $\lambda \in \RR$, and
(3) $q(g_1g_2) = q(g_1)\, g_2(\VEC{u}) + g_1(\VEC{u})\, q(g_2)$ for all
$\DS f,g \in C^\infty(S)$ and $\VEC{u} \in S$.
...
$\DS \VEC{y} \in \RR^k$ given by (\ref{TSv2Eq1}) 
...
$\DS \VEC{y} = \VEC{e}_j \in \RR^k$ for $1 \leq j \leq k$ are
denoted $\DS \pdydx{}{w_j}\Big|_{\VEC{w} = \phi^{-1}(\VEC{u})}$.
It follows
that $\DS \left\{ \pdydx{}{w_j}\Big|_{\VEC{w} = \phi^{-1}(\VEC{u})}
...
manifold $\DS S \subset \RR^n$ of class $\DS C^1$ 
is a map
$\DS F : S \to \TS\, S = \bigcup_{\VEC{u}\in S} \TS_{\VEC{u}} S$
defined by $F(\VEC{u}) = (\VEC{u}, f(\VEC{u})) \in \TS_{\VEC{u}} S$
for all $\VEC{u} \in S$ where $\DS f:S \to \RR^n$.
\end{defn}

Let $(W,U,\phi)$ be a local chart of $S$.  There exists a unique
vector field
$\DS \tilde{F}:W \to \bigcup_{\VEC{w} \in W} \TS_{\VEC{w}} W$
...
$\DS (\diff \phi(\VEC{w})) \big(\tilde{f}(\VEC{w})\big)
...
We say that $F$ is of class $\DS C^j$ on
$S$ if $\tilde{F}$ is of class $\DS C^j$ on $W$ for all
...
$\sigma:I \to S$; namely,
$\DS \tilde{\sigma} = \phi^{-1} \circ \sigma$
...
\begin{rmk}
Suppose that $\DS F : S \to \TS S$ is    \label{rmkVFwithOper}
a vector field on a $k$-dimensional manifold
$\DS S \subset \RR^n$ of class $\DS C^1$, and that
...
The vector field 
$F$ is of class $\DS C^j$ on $S$ if, for each local chart
$(W,U,\phi)$, the function $\tilde{f}_i :W \to \RR$ are of class
$\DS C^j$.

For all $\DS g \in C^\infty(S)$, we have
\[
...
$\DS \phi^\ast (\omega) : W \to \bigcup_{\VEC{w}\in W}
\Omega^p\left( \TS_{\VEC{w}} W \right)$ and
$\DS \phi^\ast(\omega)(\VEC{w})
\in \Omega^p\left( \TS_{\VEC{w}} W \right)$
for all $\VEC{w} \in W$ as expected.

\begin{rmk}
If $S$ is a $k$-dimensional manifold of class $\DS C^1$,
then the set $\DS \TS^\ast S
...
$\DS C^j$ on a
$k$-dimensional manifold $S$ of class $\DS C^j$
if, for every local chart $(W,U,\phi)$ of $S$, the
differential $p$-form $\DS \phi^\ast(\omega)$ is of class
$\DS C^j$ on $W$.
\end{defn}

More explicitly, let $\omega$ be a differential $p$-form on a
$k$-dimensional manifold $S$ of class $\DS C^j$ and
...
are of class $\DS C^j$, then $\omega$ is a differential $p$-form 
of class $\DS C^j$ on $S$.

Suppose that $S_1$ and $S_2$ are two manifolds of class $\DS C^1$ 
and that $f :S_1 \to S_2$ is a function of class $\DS C^1$.
...
subsets of $\DS \RR^n$ in Section~\ref{stokesDefDiffF}
...
class $\DS C^1$ and that $f:S_1 \to S_2$ is a function of
class $\DS C^1$.  If $\omega_i$ is a differential $p_i$-form
...
If $\omega$ is a differential $p$-form is of class $\DS C^j$
on a $k$-dimensional manifold $S$ of class $\DS C^j$,
how are we going to define $\df{\omega}$?  We cannot simply derive the
$\omega$ as we did for differential $p$-forms on $\DS \RR^n$ because
$\omega$ is only defined on $S$ which is not an open subset of
$\DS \RR^n$ when $k<n$.  The following 
theorem will answer this question.

\begin{theorem}\label{manifdOOd}
Let $\omega$ be a $p$-form of class $\DS C^1$ on a
$k$-dimensional manifold $S$ of class $\DS C^1$.
There exists a unique differential $(p+1)$-form $\mu$ on $S$ such that
$\DS \phi^\ast(\mu) = \df{(\phi^\ast(\omega))}$ on $W$ for
...
where $\DS (\VEC{u},\VEC{x}_i) = \phi_\ast(\VEC{w},\VEC{y}_i)$ for
...
of $S$.  Since
$\DS \phi_2^{-1}\circ \phi_1: \phi_1(U_1\cap U_2) \to
\phi_2(U_1\cap U_2)$
is a diffeomorphism, the map
$\DS (\phi_2^{-1} \circ \phi_1)_\ast : \TS_{\VEC{w}} W_1 \to
...
$\DS \VEC{w}_2 = \phi_2^{-1}(\phi_1(\VEC{w}_1))$.

If $\DS (\phi_1)_\ast(\VEC{w}_1,\VEC{y}_j) = (\VEC{u},\VEC{x}_j)$
...
$1\leq i \leq p+1$.
Thus $\DS \phi^\ast(\mu) = \df{(\phi^\ast(\omega))}$ on $W$.
\end{proof}
...
class $\DS C^1$ and that $f:S_1 \to S_2$ is a function of
class $\DS C^1$.
If $\omega$ be a $p$-form of class $\DS C^1$ on $S_2$.
then $\DS f^\ast (\df{\omega}) = \df{(f^\ast(\omega))}$.
\end{prop}
...
$\DS \df{(\phi_1^\ast(f^\ast(\omega)))}
= \phi_1^\ast(\df{(f^\ast(\omega))})$ on $W_1$.  Moreover, we have
\begin{align*}
...
\]
where $\DS \omega_{i_1,i_2,\ldots,i_p}:S\to \RR$ as a
differential $q$-form on a $k$-dimensional manifold $S$ of class
$\DS S^1$.  This in fact represents the local representation of the
...
$1 \leq i \leq k$ represent the local coordinates in $\DS \RR^k$
...
we may write a differential $p$-form $\omega$ on $\DS \RR^n$ as
\begin{equation} \label{kfromonSeq2}
\omega = \sum_{1\leq i_1< i_2< \ldots< i_p\leq n} \omega_{i_1,i_2,\ldots,i_p}
\df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots \wedge \df{x_{i_p}} \ ,
\end{equation}
where $\DS \omega_{i_1,i_2,\ldots,i_p}:\RR^n\to \RR$ for all
indices.  If $S$ is a $k$-dimensional manifold of class $\DS C^1$
in $\DS \RR^n$, then it is also frequent in the literature to use
...
= \pi_i(\VEC{x}) = x_i$ for all
$\DS (\VEC{u},\VEC{x}) \in \TS_{\VEC{u}}\RR^n$,
$\DS \VEC{u} \in \RR^n$ and $1\leq i \leq n$.
Since $\TS_{\VEC{u}} S$ is a linear subspace of
$\DS \TS_{\VEC{u}} \RR^n$ for all $\VEC{u} \in S$, we 
have that $\DS \df{x_i}\big|_S$ is a differential $1$-form
...
$\DS \{ \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots \wedge
\df{x_{i_p}} : 1 \leq i_1 < i_2 < \ldots < i_p \leq n \}$ is a set of
generators for $\DS \Omega^p\left(\TS_{\VEC{u}} S\right)$ but 
not a basis.
...
\begin{rmk}
Suppose that $\DS F : S \to \TS S$ is  \label{rmkdyVFwithOper}
a vector field on a $k$-dimensional manifold $\DS S \subset \RR^n$
of class $\DS C^1$.  Moreover, suppose that
$g:S \to \RR$ is a function of class $\DS C^\infty$.
...
$\VEC{u} = \phi(\VEC{w})$ for $\VEC{w} \in W$, then we get
\begin{align*}
...
$\DS \tilde{F}(\VEC{w}) = \sum_{i=1}^k \tilde{f}_i(\VEC{w})
...
$\DS V \subset \RR^k$.  The
{\bfseries integral of $\omega$}\index{Integral of a Differential Form}
on $V$ is defined by
$\DS \int_V \omega = \int_V f(\VEC{x}) \dx{\VEC{x}}$.
\end{defn}

\begin{defn}
Let $S$ be a $k$-dimensional oriented manifold of class $\DS C^1$
...
$\DS g^\ast(\omega)$ is a differential $k$-form with compact
support in $W$ and
\[
...
and $\DS g^{-1}:V \to W$ is continuous, we have that
$\DS \supp g^\ast(\omega) = g^{-1}(\supp \omega)$ is a
compact set.

If $\omega = f \df{x_1}\wedge\df{x_2}\wedge\ldots\wedge\df{x_k}$, then
we have
\begin{align*}
\int_{W} g^\ast(\omega)
...
$\DS \omega$ combined with Theorem~\ref{stokesCofV}
...
$\DS \int_S \omega = \int_{W_1} \phi_1^\ast(\omega) 
= \int_{W_2} \phi_2^\ast(\omega)$.  To be more precise,
let $\DS \tilde{W}_i = \phi_i^{-1}(U_1 \cap U_2)$ for $1=1,2$ and
$\DS g = \phi_2^{-1}\circ \phi_1$.  Then $g$ is an
orientation preserving diffeomorphism between $\tilde{W}_1$ and
$\tilde{W}_2$.  Hence
\[
...
a partition of unity $\DS \{\psi_j\}_{j\in \NNp}$ subordinate 
to the open cover $\DS \{V_\alpha\}_{\alpha\in A}$ of $S$,
where $V_\alpha$ is an open set in $\DS \RR^n$ such that
...
$\DS \{\psi_j\}_{j \in \NNp}$ be a partition of
...
$\DS \omega_j = \psi_j\, \omega = 0$ for all
$\DS j \in \NNp$ but a finite number of
$\DS j \in \NNp$ because $\omega$ has a compact support.
...
Let $\DS \{\psi_j\}_{j\in \NNp}$ be a partition of unity subordinate 
to $\A$ and $\DS \{\tilde{\psi}_j\}_{j\in \NNp}$ be a partition of
...
$\DS \sum_{j_2\in \NNp} \tilde{\psi}_{j_2}(\VEC{x}) = 1$ for
all $\VEC{x}$.
...
$\DS \sum_{j_1\in \NNp} \psi_{j_1}(\VEC{x}) = 1$ for all $\VEC{x}$.
\end{proof}
...
Let $S$ and $\tilde{S}$ be two $k$-dimensional manifolds of class
$\DS C^1$.  Suppose that $\omega$ is a differential $k$-form
on $\tilde{S}$ with compact support and that $g :S \to \tilde{S}$ is
an orientation preserving diffeomorphism.  Then
$\DS \int_{\tilde{S}} \omega = \int_S g^\ast(\omega)$.
\end{prop}

\begin{proof}
Let $K = \supp\, \omega$.
Then $\DS \supp g^\ast(\omega) = g^{-1}(K)$.
This is a compact set because $\DS g^{-1}$ is continuous and
...
be used for both $\omega$ and $\DS g^\ast(\omega)$.
...
$\DS \left\{ \sigma_m \right\}_{1 \leq m \leq M}$
of singular $k$-cubes of class $\DS C^1$ such that:
\begin{enumerate}
\item $\sigma_m(I_k)$ is a closed cube in $W$ with sides parallel to
the axes in $H_k$, 
\item $\DS \sigma_{m_1}(I_k) \cap
...
\begin{enumerate}
\item $\DS \supp \upsilon \subset
\left( \bigcup_{m=1}^M \sigma_m(I_k) \right)^\circ$ if
$\supp \upsilon \cap \partial H_k = \emptyset$ or
\item $\DS \supp \upsilon \subset \bigcup_{m=1}^M \sigma_m(I_k)$ and
$\DS \supp \upsilon \cap
...
$W$ of $H_k$ with the induced topology from $\DS \RR^k$, there
...
The collection $\DS \{ B_{\VEC{w}} \}_{\VEC{w} \in \supp \upsilon}$ is an
open cover of the compact set $\supp \upsilon$.  Therefore, there exists a
finite subcover $\DS \{ B_{\VEC{w}_j} \}_{1 \leq j \leq J}$ of
$\supp \upsilon$.  By subdividing the cubes 
$\DS \left\{\,\overline{B_{\VEC{w}_j}}\, \right\}_{1 \leq j \leq J}$,
we may get a finite collections of closed cubes
$\DS \{ B_m\}_{1 \leq m \leq M}$ such that
$\DS \supp \upsilon \subset \bigcup_{m=1}^m B_m \subset
\overline{V} \subset W$ and
$\DS B_{m_1} \cap B_{m_2}$ for $m_1 \neq m_2$ is either the
...
singular $k$-cubes of class $\DS C^1$.
\end{proof}
...
$\DS C^1$ with boundary.  Suppose that $\mu_{\VEC{u}}$ is
...
If $\DS \left\{ \sigma_m \right\}_{1 \leq m \leq M}$ is a finite
collection of singular $k$-cubes satisfying (3(b))
of Lemma~\ref{stokesLem1} with
$\DS \upsilon = \phi^\ast(\omega)$, then
$\DS \int_{\partial S} \omega = \int_{\partial \sigma}
\phi^\ast(\omega)$.
\end{lemma}

\begin{proof}
Since $\DS \supp \phi^\ast (\omega)$ is a compact subset of $W$,
we may effectively apply Lemma~\ref{stokesLem1} to get the finite
collection $\DS \left\{ \sigma_m \right\}_{1 \leq m \leq M}$.

We have that
$\DS \sigma = \sum_{m=1}^M \sigma_m$ is a $k$-chain in $W$
whose image contains the support of
$\DS \phi^\ast(\omega)$ and such that the
image of the $(k-1)$-chain
$\DS \sum_{\sigma_m(I_k) \cap \partial H_k\neq \emptyset}
(\sigma_m)_{k,0}$ contains the support of the differential $(k-1)$-form
$\DS \phi^\ast \left(\omega\big|_{U\cap \partial S}\right)
= \phi^\ast(\omega)\big|_{W\cap \partial H_k}$ on
$W \cap \partial H_k$.

Since $\phi$ is orientation preserving, we have
\begin{align*}
\mu_{\VEC{u}} &=
...
the standard orientation on $H_k$ by a factor of $\DS (-1)^k$.
The two orientations match when $k$ is even and are of opposite signs
when $k$ is odd.  See also Remark~\ref{rmkHkOrient}.  Thus, we have
\[
...
Since $\DS \partial \sigma =
\sum_{m=1}^M \left( \sum_{i=1}^k (-1)^i \big( (\sigma_m)_{i,0} 
- (\sigma_m)_{i,1}\big) \right)$, we get
\begin{align*}
...
$\DS \int_{(\sigma_m)_{i,j}} \phi^\ast(\omega) = 0$ if
$\DS \supp \phi^\ast(\omega) \cap
(\sigma_m)_{i,j}(I_{k-1}^\circ) = \emptyset$ with
$j=0,1$ and $1\leq i \leq k$,
and $\DS \int_{(\sigma_{m_1})_{i,1}}
\phi^\ast(\omega) = -\DS \int_{(\sigma_{m_2})_{i,0}} \phi^\ast(\omega)$
...
class $\DS C^1$, and $\omega$ is a differential $(k-1)$-form
on $S$.  Then,
...
\begin{proof}
Let $\DS \A = \{(W_\alpha, U_\alpha, \phi_\alpha)\}_{\alpha\in A}$
be an atlas of orientation preserving local charts for $S$.
Since $S$ is compact, we can find a finite open cover
$\DS \left\{ U_{\alpha_i} \right\}_{1 \leq i \leq I}$ of $S$.
Therefore\\
$\DS \A_I = \{(W_{\alpha_i}, U_{\alpha_i}, \phi_{\alpha_i})
\}_{1\leq i \leq I}$
is a finite atlas of orientation preserving local charts for $S$.  Let
$\DS \left\{\psi_i\right\}_{1 \leq i \leq I}$ be a
...
(topological neighbourhood of $U_{\alpha_i}$), then there
...
$\DS \upsilon = \phi_i^\ast(\omega_i)$.
Consider the $k$-chain $\DS \sigma = \sum_{m=1}^M \sigma_m$ 
...
\]
because
$\DS \phi_i^\ast(\omega_i)\big|_{\partial \sigma} = 0$.  Since
$\DS \omega_i\big|_{\partial S} = 0$, we also get
$\DS \int_{\partial S} \omega_i = 0$.  Thus
$\DS \int_S \df{\omega_i} = \int_{\partial S} \omega_i$.

\subQ{ii} If $(\supp \omega_i) \cap \partial U_{\alpha_i} \neq \emptyset$,
then wee may use Lemma~\ref{stokesLem2} with $U$ and $W$ replaced
by $U_{\alpha_i}$ and $W_{\alpha_i}$ respectively, and
$\DS \upsilon = \phi_i^\ast(\omega_i)$, to conclude that
$\DS
...
\subQ{iii}
Since $\DS \sum_{i=1}^I \psi_i = 1$,  we get that
$\DS 0 = \df{\left(\sum_{i=1}^I \psi_i\right)} =
...
$\DS \left\{ \psi_i\right\}_{1 \leq i \leq I}$
is a partition of unity subordinate to the open cover
$\DS \left\{ V_{\alpha_i} \right\}_{1 \leq i \leq I}$ of $S$, the second
...
$\DS \left\{ \psi_i \right\}_{1 \leq i \leq I}$
is a partition of unity subordinate to the open cover
$\DS \left\{ V_{\alpha_i} \right\}_{1 \leq i \leq I}$ of
$\partial S$.
\end{proof}
...
$\DS C^1$ and $\omega$ is a differential
...
$\DS \left\{\psi_j\right\}_{j \in \NNp}$ be a
...
Let $S$ be a oriented $k$-dimensional manifold of class $\DS C^1$
...
$\DS \phi^\ast \omega
= f \df{w_1}\wedge \df{w_2} \wedge \ldots \wedge \df{w_k}$
is the local representation of $\omega$ on $W$, then
$\DS \eta(g) = \int_W g(\phi(\VEC{w})) \, f(\VEC{w})
...
$\DS \eta(g) = \int_S g \dx{\eta_{\omega}}$ for all
$g \in C_0(S)$.
\end{enumerate}
\end{theorem}
...
\stage{1} Let $\DS \A = \left\{
(W_\alpha,U_\alpha,\phi_\alpha)\right\}_{\alpha\in A}$
be an atlas of orientation preserving local charts for $S$ and
$\DS \{\psi_j\}_{j\in \NNp}$ be a partition of unity subordinate 
to the atlas $\A$.  Suppose that $\alpha_j \in A$ is an index such that
$\supp \psi_j \subset V_{\alpha_j}$ for $\DS j \in \NNp$.
We also assume that
$\DS \phi_\alpha^\ast \omega = f_\alpha \df{w_1}\wedge
...
there exists a finite subset $\DS J \subset \NNp$ such that
$\DS g = \sum_{j\in J} \psi_j g$
...
support, then we may then define $\DS \int_S g \omega$ for
all (Lebesgue) integrable function $g$ on $S$ as
$\DS \int_S g \dx{\eta_{\omega}}$.
...
makes sense to set $\DS \int_S \omega = \int_S \dx{\eta_{\omega}}$.
...
$\DS \int_S \omega = \int_S \dx{\eta_{\omega}}$ is justified
...
Using (\ref{functCcSEq1omg}), we get
\begin{align*}
|\eta(g_1) - \eta(g_2)| &=
...
\item $\DS \{\psi_j\}_{j\in \NNp}$ is a partition of unity
  subordinate to the atlas $\A$,
\item $\alpha_j \in A$ is an index such that
$\supp \psi_j \subset V_{\alpha_j}$ for $\DS j \in \NNp$, and
\item $\DS \phi_\alpha^\ast \omega
= f_\alpha \df{w_1}\wedge \df{w_2} \wedge
...
Then
\begin{enumerate}
\item $\DS \left|\int_S g \omega \right| \leq \int_S |g|
\dx{\eta_{|\omega|}}$ for all $g \in C_c(S)$,
\item 
$\DS \left|\int_S \omega \right| \leq \int_S \dx{\eta_{|\omega|}}$
...
\]
for some finite subset $\DS J \subset \NNp$, and
...
$\DS g_m \in C_c(S)$ such that
$0 \leq g_m(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in S$,
...
$\DS V \supset K$ in $\DS \RR^n$, we can use a partition of
unity for instance to proof that there exists a function $g$ of class
$\DS C^\infty$ with compact support such that
...
number of $\DS j \in \NNp$.  Let $\DS J \subset \NNp$
be the finite set of indices such that $\psi_j\big|_K \neq 0$.
As usual, we assume that $\alpha_j \in A$ is an index such that
$\supp \psi_j \subset U_{\alpha_j}$ for $\DS j \in \NNp$.  Then
$\DS \supp \omega \subset \bigcup_{j\in J} U_{\alpha_j}$.
Hence
\[
...
For $m>0$, choose $\DS g_m \in C_c(S)$ such that
$0 \leq g_m(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in S$,
$g_m(\VEC{x}) = 1$ for all $\DS \VEC{x} \in \{ \VEC{x} \in S :
\dist{\VEC{x}}{S \setminus V} \geq 1/m \}$
and $g_m(\VEC{x}) = 0$ for all $\DS \VEC{x} \in S \setminus V$.
...
set $\DS R \subset \RR^n$ is a collection of open sets
$\DS \{V_j\}_{j\in \NNp}$ of $\DS \RR^n$ such that
$R \subset V_j$ for all $\DS j \in \NNp$ and, if
$\DS V \subset \RR^n$ is an open set containing $R$,
then there exists $\DS J\in \NNp$ such that
$V_j \subset V$ for $j\geq J$.
\end{defn}

\begin{defn} \label{defnNegligible}
Let $S$ be a oriented $k$-dimensional manifold of class $\DS C^1$
without boundary.  A closed subset $\DS R \subset \RR^n$ is
{\bfseries negligible}\index{Negligible Set} in $S$ if 
there exist
\begin{enumerate}
\item an open set $\DS G\subset \RR^n$ such that $R \subset G$,
\item a fundamental sequence $\DS \{G_j\}_{j\in \NNp}$
of $R$ in $G$ with $\overline{G_j} \subset G$ for all $j$, and
\item a sequence of functions $\DS \{ g_j\}_{j\in \NNp}$ 
such that
{
\renewcommand{\labelenumii}{\alph{enumii}.}
\begin{enumerate}
\item $g_j : G \to \RR$ is of class $\DS C^1$ for all
$\DS j \in \NNp$,
\item $0 \leq g_j(\VEC{x}) \leq 1$ for all $\VEC{x} \in G$ and
$\DS j \in \NNp$,
\item $g_j(\VEC{x}) = 1$ for all $\VEC{x} \in G\setminus G_j$ and
$\DS j \in \NNp$,
\item there exists for each $\DS j\in \NNp$ an open set
$\tilde{G}_j \subset G_j$ 
such that $R \subset \tilde{G}_j$ and $g_j(\VEC{x}) = 0$ for all
$\VEC{x} \in \tilde{G}_j$, and,
\item for any $(k-1)$-form $\omega$ of class $\DS C^1$
...
$\DS \lim_{j\to \infty} \eta_{|\df{g_j}\wedge \omega|}(G \cap S) = 0$.
\end{enumerate}
}
\end{enumerate}
\end{defn}
...
$\DS C^1$ without boundary.  Let
$\DS O \subset \RR^n$ be an open set such that
$\overline{S} \subset O$, and $\omega$ be a differential
$(k-1)$-form of class $\DS C^1$ on $O$ 
...
(\ref{GenStokesThEq0}) is $\DS \int_{\partial S} \omega$.
...
$\DS \int_S \omega = \int_{\Int S} \omega$.  To justify this
...
$\DS M_1 = \{ \VEC{x} \in \RR^3 : x_1^2 + x_2^2 = x_3^2 \ \text{with}
\ 0 < x_3 \leq 1 \}$ and $\DS M_2 =
...
$\DS \partial S = \bigcup_{m=1}^M \Int M_m$ for some
manifolds $M_m$, then we write $\DS \int_{S_r} \omega
= \sum_{m=1}^M \int_{M_m} \omega$.
\end{rmk}

\begin{egg}
We consider our cone $C$     \label{coneEgg2}
...
$\DS \int_S \df{\omega} = \int_{S_r} \omega$.

Despite the fact that $C$ is not a manifold, we often write
$\DS \int_C \df{\omega}$ instead of
$\DS \int_S \df{\omega}$.  We accept this informal terminology
because $S_s$ is a set of measure zero in $\DS \RR^3$.
Moreover, we write
$\DS \int_{S_r} \omega = \int_{M_1} \omega + \int_{M_2} \omega$
despite the fact that $S_r = \Int M_1 \cup \Int M_2$ as we
explained in the previous remark.  We also informally write
$\DS \int_{\partial C}$ instead of 
$\DS \int_{S_r}$ though $\partial C$ is the topological
...
$\DS \{G_j\}_{j\in \NNp}$ of $S_s$ in $G$ 
and sequence of functions $\DS \{ g_j\}_{j\in \NNp}$
satisfying Definition~\ref{defnNegligible}.  Moreover, there exists a
collection of open sets $\DS \{\tilde{G}_j\}_{j\in \NNp}$ such that
$S_s \cap \supp\, \omega \subset \tilde{G}_j \subset G_j$ and
$g_j = 0$ on $\tilde{G}_j$ for all $\DS j \in \NNp$.

Let $\omega_j = g_j \omega$ for all $\DS j\in \NNp$ \footnote{Since
...
is a compact set for all $\DS j\in \NNp$ because
...
We now show that
$\DS \bigcap_{j\in \NNp} G_j \cap S_r = \emptyset$.
If $\VEC{u} \in S_r$, then there exists a local chart
$(W,U,\phi)$ such that $\VEC{u} \in U$.  Recall that
$U = V \cap \overline{S}$ where $\DS V \in \RR^n$ is an open
...
$\DS \RR^n$ containing $S_s$.  By definition of fundamental
sequence for $S_s$, there exists $\DS J\in \NNp$ such that
$G_j \subset O \setminus \overline{B_\delta(\VEC{u})}$ for
$j \geq J$.  Thus $\VEC{u} \not\in G_j \cap S_r$ for $j \geq J$.

Since $\DS \bigcap_{j\in \NNp} G_j \cap S_r = \emptyset$,
we have from measure theory \footnote{Use Theorem~1.19 in \cite{Rrca}
with $\DS A_n = \bigcap_{1 \leq j \leq n} G_j \cap S_r$
...
\stage{ii} The proof that $\DS \int_S g_j \df{\omega}$
converge to $\DS \int_S \df{\omega}$ as $j \to \infty$ is
...
We now show that
$\DS \bigcap_{j\in \NNp} G_j \cap S = \emptyset$.
...
(\ref{GenStokesThEq6}), then we get (\ref{GenStokesThEq0}).
\end{proof}
...
Let $S$ be a oriented $k$-dimensional manifold of class $\DS C^1$.  If
$R_1$ and $R_2$ are negligible in $S$, then $R_1 \cup R_2$ is
negligible in $S$.
\end{prop}

\begin{proof}[Proof (Sketch)]
Suppose that $\DS \{G_{i,j}\}_{j \in \NNp}$ and
$\DS \{g_{i,j}\}_{j \in \NNp}$ are given by the
definition of negligible sets applied to $R_i$ in $S$ for
$i = 1,2$.   It suffices to show that
$\DS \{G_{1,j} \cup G_{2,j}\}_{j \in \NNp}$ and
$\DS \{g_{1,j}\, g_{2,j}\}_{j \in \NNp}$ satisfy the
...
Thus the measure $\DS \eta_{|\df{(g_{1,j}\,g_{2,j})} \wedge \omega|}$
associated to $\df{(g_{1,j}\,g_{2,j})} \wedge \omega$ is the sum of the
measures $\DS \eta_{|g_1 \df{g_{2,j}} \wedge \omega|}$
and $\DS \eta_{|g_2 \df{g_{1,j}} \wedge \omega|}$
associated to $g_{1,j}\df{g_{2,j}} \wedge \omega$ and
$g_{2,j}\df{g_{1,j}} \wedge \omega$ respectively.
\end{proof}

\begin{prop} \label{propCrit2}
Let $S$ be a oriented $n$-dimensional manifold of class $\DS C^1$.
...
We split $I_q$ into $\DS p^q$ cubes of the form
\[
...
$\DS \tilde{I}_{i_1,i_2,\ldots,i_q} \subset \RR^n$ of
volume $\DS \left(\frac{C_0 \sqrt{q}}{p}\right)^n$ and
$\DS R = \sigma(I_q) \subset
\bigcup_{\substack{1 \leq i_j \leq p\\1\leq j \leq q}}
\tilde{I}_{i_1,i_2,\ldots,i_q}$.

\stage{b} Let $\DS
G_p = \{ \VEC{x} \in \RR^n : \dist{\VEC{x}}{R} < 2/p \}$.  We
construct a function $\DS g_p:\RR^n \to \RR$ of
class $\DS C^\infty$ \footnote{This is more than needed but
it comes for free.} such that:
{\renewcommand{\labelenumi}{\roman{enumi}.}
\begin{enumerate}
\item $0 \leq g_p(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in \RR^n$,
\item $g_p(\VEC{x}) = 1$ for all
$\DS \VEC{x} \in \RR^n \setminus G_p$,
...
$\DS \left|\pdydx{g_p}{x_s}(\VEC{x})\right| \leq C_1 p$
for all $\DS \VEC{x} \in \RR^n$ and $1\leq s \leq n$.
\end{enumerate}
}

Choose a function $\DS \psi:\RR^n \to \RR$ of class
$\DS C^\infty$ such that
$0 \leq \psi(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in \RR^n$,
...
Therefore, the functions $g_p$ are of class $\DS C^\infty$ and
$0 \leq g_p(\VEC{x}) \leq 1$ for all $\DS \VEC{x} \in \RR^n$
because $0 \leq \psi(\VEC{x}) \leq 1$ for all
$\DS \VEC{x}\in \RR^n$.  Thus (i) is satisfied.

Given $\VEC{x}$ such that $\DS \dist{\VEC{x}}{R} < (4p)^{-1}$, there
exists at least one $\VEC{z} \in R_p$ such that
$\DS \| \VEC{x} - (2p)^{-1} \VEC{z} \| \leq (2p)^{-1}$.
For this value of $\VEC{z}$, we have that
$\DS \| p\left(\VEC{x} - (2p)^{-1} \VEC{z}\right) \| \leq 1/2 $.
It follows from the definition of $\psi$ that
$\DS \psi\left(p\left(\VEC{x} - (2p)^{-1}
\VEC{z}\right)\right) =0$ and so $g_p(\VEC{x}) = 0$.
Therefore, we may take $\DS \tilde{G}_p = \{ \VEC{x} \in \RR^n :  
\dist{\VEC{x}}{R} < (4p)^{-1}\}$.  Hence (iii) is satisfied.

Given $\VEC{x}$ such that $\DS \dist{\VEC{x}}{R} \geq 2/p$ and
$\VEC{z} \in R_p$, we have that
$\DS \|\VEC{x} - (2p)^{-1}\VEC{z}\|
\geq \|\VEC{x} - \VEC{y}\| - \|\VEC{y} - (2p)^{-1}\VEC{z}\|$ for all
$\VEC{y} \in R$ implies that $\DS \|\VEC{x} - (2p)^{-1}\VEC{z}\|
\geq 2/p - 1/p = 1/p$ because there is at least one $\VEC{y} \in R$
with $\DS \|\VEC{y} - (2p)^{-1}\VEC{z}\| \leq 1/p$ since
$\VEC{z} \in R_p$.
Hence $\DS \| p\left(\VEC{x} - (2p)^{-1} \VEC{z}\right) \| \geq 1$
for all $\VEC{z} \in R_p$.
It follows from the definition of $\psi$ that
$\DS \psi\left(p\left(\VEC{x} - (2p)^{-1} \VEC{z}\right)\right) =1$
for all $\VEC{z} \in R_p$ and so $g_p(\VEC{x}) = 1$.
Therefore, we have that $g_p(\VEC{x}) = 1$ for
$\DS \VEC{x} \in \RR^n \setminus G_p$.  Hence (ii) is
satisfied.
...
\bigg) \ .
\end{equation}
Let $\DS M = \max_{1\leq i \leq n}\, \sup_{\VEC{x} \in \RR^n}
\left|\pdydx{\psi}{x_i}(\VEC{x})\right|$.  We have that $M < \infty$
because $\DS \pdydx{\psi}{x_i}(\VEC{x}) = 0$ for
$\|\VEC{x}\| \leq 1/2$ or $\|\VEC{x}\|\geq 1$.  We get
\[
...
\]
for all $\DS \VEC{x} \in \RR^n$,
$\DS \VEC{z} \in \ZZ^n$ and $1 \leq i \leq n$.  Hence
\begin{align*}
...
\leq p M
\end{align*}
for all $\DS \VEC{x} \in \RR^n$ and all $\VEC{z} \in R_p$
because $0 \leq \psi(\VEC{y}) \leq 1$ for all $\DS \VEC{y} \in \RR^n$.

Moreover, the number of terms in the sum in
(\ref{propCrit2Eq1}) does not grow with $p$.  There are at most
$\DS 9^n$ terms independently of the value of $p$.  To understand why
it is so, we first note that
$\DS \psi\left(p\left(\VEC{y} - (2p)^{-1}\VEC{z}\right)\right) = 1$
for all $\DS \VEC{y} \in \{ \VEC{y} \in \RR^n :
\|\VEC{y} - (2p)^{-1} \VEC{z}\| > 1/p\}$.  Thus, the partial derivatives
of $\DS \psi\left(p\left(\VEC{y} - (2p)^{-1}\VEC{z}\right)\right)$
are null in the set $\DS \VEC{y} \in \{ \VEC{y} \in \RR^n :
...
$\DS \|\VEC{x} - (2p)^{-1} \VEC{z}\| \leq 1/p$.
Suppose that $\DS \VEC{z}_j \in \ZZ^n$ satisfies
$\DS \|\VEC{x} - (2p)^{-1} \VEC{z}_j\| \leq 1/p$ for
$j = 1,2$.
Then $\DS \|(2p^{-1}\VEC{z}_1 - (2p)^{-1}\VEC{z}_2 \| \leq 
...
$\VEC{z}_j$ are in $\DS \ZZ^n$, there are at most $9$
...
$\DS \left|\pdydx{g_p}{x_i}(\VEC{x})\right| \leq C_1 p$ for
all $\VEC{x} \in \RR^n$ where $\DS C_1 = 9^n M$.

\stage{c} We show that $G_p$ can be covered by closed cubes whose total
volume is at most $\DS \frac{(4 + C_0 \sqrt{q})^n}{p^2}$.

Given $\VEC{x} \in G_p$,
choose $\DS \VEC{y} \in \tilde{I}_{i_1,i_2,\ldots,i_q}$
...
$\DS \frac{4}{p} + \frac{C_0\sqrt{q}}{p}$.  The volume of
this cube is $\DS \left(\frac{4 + C_0 \sqrt{q}}{p}\right)^n$.  Hence
$G_p$ is covered by closed cubes whose total volume is at most
$\DS p^q \left(\frac{4+C_0 \sqrt{q}}{p}\right)^n =
...
complete the proof that $\DS \{G_j\}_{j > 0}$ and
$\DS \{g_j\}_{j > 0}$ are associated to the
...
We have that $\DS \left|\pdydx{g_p}{x_s}(\VEC{x})\right| \leq C_1 p$
...
$\DS \supp \pdydx{g_p}{x_s} \subset \overline{G_p}
...
\begin{defn}
Let $W$ be an open subset of $\DS \RR^{k-1}$ and $f:W \to \RR$ be a
...
if $\DS f^{-1}(S_{f,W}) \subset \RR^{k-1}$ is a set of
measure zero and, for each compact set $K \subset W$, there exists a
...
\item an open subset $U$ of $S$,
\item an open subset
$\DS W = W_1 \times W_2 \subset \RR^{k-1} \times \RR$
...
$(k-1)$-form of class $\DS C^1$ defined in an open set of
$\DS \RR^n$ containing $\overline{S}$.  Then
$\DS \int_S \df{\omega} = \int_{\partial S} \omega$.
\end{theorem}
...
Since $S$ is closed in $\DS \RR^n$, we have that 
...
$\DS \tau_{\VEC{u}} \in \T^2\left( \TS_{\VEC{u}} S\right)$
\end{defn}

Consider
$\DS \tau : S \to \bigcup_{\VEC{u}\in S} \T^2(\TS_{\VEC{u}} S)$
where $\tau(\VEC{u})$ is an inner product on
$\DS \TS_{\VEC{u}} S$ for all $\VEC{u} \in S$.
If $(W,U,\phi)$ is a local chart of $S$, then
$\DS (\phi^\ast(\tau))(\VEC{w})$ is an inner product on
...
\end{align*}
for all $\DS (\VEC{w},\VEC{y}_1), (\VEC{w},\VEC{y}_2)
\in \TS_{\VEC{w}} W \cong \RR^k$ and $\VEC{w} \in W$.
It follows from Theorem~\ref{stokesBT} that we can express $\tau$ locally as
$\DS \phi^\ast(\tau) = \sum_{1 \leq i_1 \leq i_2\leq k} \tau_{i_1,i_2}
\df{w_{i_1}} \otimes \df{w_{i_2}}$ 
where $\DS \tau_{i_1,i_2}:W \to \RR$ for all indices.
We say that $\tau$ is of class $\DS C^j$ on $U$
if each $\DS \tau_{i_1,i_2}$ is of class $\DS C^j$
on $W$.  If this is true for all local chart $(W,U,\phi)$, then 
we say that $\tau$ is of class $\DS C^j$ on $S$.

\begin{defn} \label{defnCmInnerP}
Let $S$ be a $k$-dimensional manifold of class $\DS C^j$, and
$\DS \tau : S \to \bigcup_{\VEC{u}\in S} \T^2(\TS_{\VEC{u}} S)$
be a map such that $\tau(\VEC{u})$ is an inner product on
$\DS \TS_{\VEC{u}} S$ for all $\VEC{u} \in S$.
If $\tau$ is of class $\DS C^j$ on $S$, then we say that the
inner product on $\TS_{\VEC{u}} S$ given by
$\ps{}{}_{\VEC{u}} =  \tau(\VEC{u})$ is of class $\DS C^j$ on $S$.
\end{defn}

\begin{prop}  \label{propEIPonS}
Let $S$ be a $k$-dimensional manifold of class $\DS C^j$.
Then there exists a map
$\DS \tau : S \to \bigcup_{\VEC{u}\in S} \T^2(\TS_{\VEC{u}} S)$
of class $\DS C^j$ such that $\tau(\VEC{u})$ is an inner product on
$\DS \TS_{\VEC{u}} S$ for all $\VEC{u} \in S$.
\end{prop}

\begin{proof}
Suppose that
$\DS \A = \left\{ (W_\alpha,U_\alpha,
\phi_\alpha)\right\}_{\alpha\in A}$ is an atlas of
orientation preserving local charts for $S$ and that
$\DS \{\psi_j\}_{j\in \NNp}$ is a partition of unity
subordinate to the atlas $\A$.  Given $\DS j \in \NNp$, we
assume that $\alpha_j \in A$ is an index such that
$\supp \psi_j \subset U_{\alpha_j}$.

For each local chart $(W_\alpha,U_\alpha,\phi_\alpha)$, we define 
$\DS \tau_{U_\alpha} : U_\alpha \to \bigcup_{\VEC{u}\in U_\alpha}
...
product on $\DS \RR^k$.  Note that $\tau_{U_\alpha}$ is of class
$\DS C^j$ because it is defined from $\phi_\alpha$.  In
fact, $\DS \phi_\alpha^\ast(\tau_{U_\alpha})
= \sum_{i=1}^k \df{w_i} \otimes \df{w_i}$.

We have that $\DS \tau = \sum_{j\in \NN} \psi_j \tau_{U_{\alpha_j}}$
...
class $\DS C^j$.  In particular, the sum is finite at
...
$\DS C^1$, we have seen in Section~\ref{sectOrientRn} that
...
the manifold $S$ is defined as $\DS \int_S \dx{V}$.
\end{defn}
...
manifold of class $\DS C^1$ in $\DS \RR^n$, and
that the orientation on $S$ is given by
$\DS \mu_{\VEC{u}} = [ (\VEC{u}, \VEC{t}(\VEC{u})) ]$ where 
...
$\DS \phi^\ast(\nu)$ is continuous on $W$.  Note that
$\phi'(w) \neq \VEC{0}$ for all $w \in W$ since $\phi'(w)$ is a rank
one by definition of local charts.
...
Let $C$ be a smooth curve in $\DS \RR^3$ (i.e.\ an
oriented $1$-dimensional manifold of class $\DS C^1$) and let
...
\end{align*}
where $\ps{}{}$ is the standard inner product on $\DS \RR^3$.
...
$\DS \VEC{q} \in \RR^3$.
With $\VEC{q} = \VEC{e}_i$ for $1\leq i \leq 3$, we get
\[
...
\begin{rmk}
If we use the local chart introduced in Example~\ref{eggVolElem1D},
then we can also prove (\ref{manifCXYZeq2}).  Recall that
$\DS \VEC{t}(\VEC{u}) = \|\phi'(w)\|^{-1} \phi'(w)$
for $\VEC{u} \in U$ and $w\in W$ such that $\phi(w) = \VEC{u}$.

We have from Proposition~\ref{propAstoR} (1) that
$\DS \phi^\ast(\df{x_i})(w) = \phi_i'(w) \df{w}$.
...
manifold of class $\DS C^1$ in $\DS \RR^3$.  The
...
= [(\VEC{u},\VEC{e}_1),(\VEC{u},\VEC{e}_2),(\VEC{u},\VEC{e}_3)]$.
By definition of the cross product, we also have
\[
\nu(\VEC{u})\big((\VEC{u},\VEC{x}_1),(\VEC{u},\VEC{x}_2)\big)
...
Given $\DS (\VEC{w},\VEC{y}_i) \in \TS_{\VEC{w}} W \cong \RR^2$
...
Let $S$ be a $2$-dimensional oriented manifold in $\DS \RR^3$ and let
...
and all $\DS \VEC{q} \in \RR^3$.  With $\VEC{q} = \VEC{e}_i$, we get
\begin{align*}
n_i(\VEC{u})
...
Stokes' theorem that $\DS 0 = \int_{\partial S} \eta
= \int_S \df{\eta} = \int_S \nu$.   But $\int_S \nu$ is a
...
$\DS S^{n-1} \equiv \{ \VEC{x} \in \RR^n : \|\VEC{x}\|=1\}$.

Suppose that $\mu_{\VEC{u}}$ is the orientation on
$\DS \TS_{\VEC{u}} S^{n-1}$ for $\DS \VEC{u} \in S^{n-1}$
and that
$\DS \big(\VEC{u},\VEC{n}(\VEC{u})\big) \in \TS_{\VEC{u}} \RR^n$
is the outward unit normal to $S$ at $\VEC{u} \in S$ as in
Definition~\ref{manifNormal}.
We note that $\VEC{n}(\VEC{u}) = \VEC{u}$ for all
$\DS \VEC{u} \in S^{n-1}$.
If $\DS \left\{ (\VEC{u},\VEC{v}_{\VEC{u},i})
...
$\DS [(\VEC{u},\VEC{n}(\VEC{u})),(\VEC{u},\VEC{v}_{\VEC{u},1}), \ldots,
...
\end{pmatrix}
\]
for all $\DS (\VEC{u},\VEC{v}_1) \in \TS_{\VEC{u}} S^{n-1}$
and $\DS \VEC{u} \in S^{n-1}$.
It is easy to prove that $\nu$ is a differential $(n-1)$-form on
$\DS S^{n-1}$ of class $\DS C^\infty$ since the
standard local atlas for $\DS S^1$ is of class
$\DS C^\infty$.  Moreover, if 
$\DS \left\{ (\VEC{u},\VEC{v}_{\VEC{u},i})
\right\}_{1 \leq i \leq n-1} \subset \TS_{\VEC{u}} S^{n-1}$
is an orthonormal basis on $\DS \TS_{\VEC{u}} S^{n-1}$
...
$\DS V= \RR^2 \setminus \{\VEC{0}\}$ that is not exact.

We now show that we can always find a closed differential $n-1$-form on
$\DS \RR^n \setminus \{ \VEC{0}\}$ for $n\geq 2$ that is not
exact.  For this purpose, we will use the volume element $\nu$ on
$\DS S^{n-1}$.   We have that $\nu$ is closed because
it is a differential $(n-1)$-form on a $(n-1)$-dimensional manifold.

Consider the retraction
$\DS \rho : \RR^n \setminus \{\VEC{0}\} \to S^{n-1}$ defined by
$\DS \rho(\VEC{x}) = \|\VEC{x}\|^{-1} \VEC{x}$, and
the inclusion $\DS \iota : S^{n-1} \to \RR^n \setminus \{\VEC{0}\}$
defined by $\iota(\VEC{u}) = \VEC{u}$.
We have that $\DS \rho \circ \iota = \Id: S^{n-1} \to S^{n-1}$.

The differential $(n-1)$-form $\DS \rho^\ast(\nu)$ on
$\DS \RR^n \setminus \{\VEC{0}\}$ is closed because
$\DS \df{(\rho^\ast(\nu))} = \rho^\ast(\df{\nu}) = 0$
since $\df{\nu} = 0$.
However, $\DS \rho^\ast(\nu)$ is not exact.  This is proved by
contradiction.  Suppose that $\DS \rho^\ast(\nu)$ is exact and
$\DS \rho^\ast(\nu) = \df{\eta}$ for some differential
$(n-2)$-form on $\DS \RR^n \setminus \{\VEC{0}\}$.
Then $\DS \nu = (\rho \circ \iota)^\ast \nu
= \iota^\ast( \rho^\ast(\nu)) = \iota^\ast(\df{\eta})
= \df{(\iota^\ast(\eta))}$.  However, this is a contradiction because
we know from Example~\ref{eggHkDim} that $\nu$ cannot be exact on
$\DS S^{n-1}$ since $\DS \partial S^{n-1} = \emptyset$.
\end{egg}

Let $\nu$ be the volume element on $\DS S^{n-1}$, and let
$\DS \iota:S^{n-1} \to \RR^n \setminus \{\VEC{0}\}$
and $\DS \rho: \RR^n\setminus \{\VEC{0}\} \to S^{n-1}$ be
respectively the inclusion and retraction defined in Example~\ref{HR0dim}.
Consider the following differential $(n-1)$-form on $\DS \RR^n$.
...
\end{pmatrix}
\]
for all $\DS \VEC{x} \in \RR^n$ and
$\DS (\VEC{x},\VEC{y}_j) \in \TS_{\VEC{x}} \RR^n$
with $1 \leq j \leq n-1$.  Thus $\DS \nu = \iota^\ast(\tilde{\nu})$.
However, $\DS \rho^\ast(\nu)$ is not equal to $\tilde{\nu}$.  Instead,
we have the following result.

\begin{prop} \label{propRanEntn}
Let $\nu$ be the volume element on $\DS S^{n-1}$,
$\tilde{\nu}$ be the differential $(n-1)$-form given in
(\ref{oemagRn}), and
$\DS \rho: \RR^n\setminus \{\VEC{0}\} \to S^{n-1}$ be
the retraction defined by
$\DS \rho(\VEC{x}) = \|\VEC{x}\|^{-1}\VEC{x}$ for
$\DS \VEC{x} \in \RR^n\setminus \{\VEC{0}\}$.
Then $\DS
\rho^\ast(\nu)(\VEC{u}) = \|\VEC{u}\|^{-n} \tilde{\nu}(\VEC{u})$
in $\DS \Omega^{n-1}\left(\TS_{\VEC{u}} \RR^n\right)$
for all $\DS \VEC{u} \in \RR^n\setminus \{\VEC{0}\}$.
\end{prop}

\begin{proof}
An orthonormal basis of $\DS \TS_{\VEC{u}} \RR^n$ is given
by the outward unit normal
$\DS \VEC{n}_{\VEC{u}}=(\VEC{u},\VEC{n}(\VEC{u}))
= (\VEC{u}, \|\VEC{u}\|^{-1}\VEC{u})$ to
$\partial B_{\|\VEC{u}\|}(\VEC{0})$ at $\VEC{u} \in
\partial B_{\|\VEC{u}\|}(\VEC{0})$ and an orthonormal basis
$\DS \{ (\VEC{u},\VEC{v}_i)\}_{1\leq i \leq n-1}$
of $\TS_{\VEC{u}} (\partial B_{\|\VEC{u}\|}(\VEC{0}))$.

Because of the multi-linearity of $\DS \rho^\ast(\nu)$, it
...
$\DS (\VEC{u}, \|\VEC{u}\|^{-1}\VEC{u})$ or one of
$\DS (\VEC{u},\VEC{v}_i)$ for $1 \leq i \leq n-1$.

We first note that
$\DS \rho_\ast(\VEC{u}, \|\VEC{u}\|^{-1}\VEC{u})
= (\|\VEC{u}\|^{-1}\VEC{u}, \VEC{0})$
because
$\DS \diff \rho(\VEC{u}) \left(\|\VEC{u}\|^{-1}\VEC{u}\right)$ is the
directional derivative of $\rho$ at $\VEC{u}$ in the radial direction
$\VEC{n}(\VEC{u})$.
But $\DS \rho(\VEC{u}) = \|\VEC{u}\|^{-1}\VEC{u}$ is constant
...
$\DS (\VEC{u}, \|\VEC{u}\|^{-1}\VEC{u})$.  As for the right
hand side of (\ref{RiOEq1}), if 
$\DS (\VEC{u},\VEC{y}_j) = (\VEC{u}, \|\VEC{u}\|^{-1}\VEC{u})$
...
when each $(\VEC{u},\VEC{y}_j)$ is one of $\DS (\VEC{u},\VEC{v}_i)$
...
Therefore $\DS (\diff \rho(\VEC{u})) \VEC{v}_j
= \|\VEC{u}\|^{-1} \VEC{v}_j$ for $1\leq j \leq n-1$.
Hence, for any permutation $\sigma \in S_{n-1}$, we have
\begin{align*}
...
\]
where $\DS F(\VEC{u}) = \int_0^1 r^{n-1}f(r \VEC{u}) \dx{r}$
for $\DS \VEC{u} \in S^{m-1}$.
\end{prop}

\begin{proof}
To prove the proposition, we use the projections
$\DS \pi_S: S^{n-1} \times \RR \to S^{n-1}$ and
$\DS \pi_I:S^{n-1} \times \RR \to \RR$.  We consider the
differential $n$-form on $\DS S^{n-1} \times \RR$ defined by
...
\stage{i}
Let $\DS g:S^{n-1}\times [0,1] \to \RR$ be the function
defined by $\DS g(\VEC{u},r) = r^{n-1} f(r\VEC{u})$ for
$\DS \VEC{u} \in S^{n-1}$ and $r \in [0,1]$.

Since $\DS S^{n-1}$ is a compact manifold, we may choose a
finite atlas $\A = \{ (W_i,U_i,\phi_i) \}_{1\leq i \leq I}$ of
$\DS S^{n-1}$ and a finite partition of unity
...
is an atlas of $\DS S^{n-1}\times ]0,1[$.
We assume that
...
\stage{ii}
Let $\DS h:B_1(\VEC{0})\setminus \{\VEC{0}\}
\to S^{n-1} \times ]0,1]$ be the map defined by
$\DS h(\VEC{x}) = \big(\rho(\VEC{x}), N(\VEC{x})\big)$
for all $\DS \VEC{x} \in B_1(\VEC{0})\setminus \{\VEC{0}\}$,
where $\DS \rho: \RR^n \setminus \{\VEC{0}\} \to S^{n-1}$ is
the retraction previously defined and
$\DS N: \RR^n \setminus \{\VEC{0}\} \to ]0,\infty[$ is defined
by $N(\VEC{x}) = \|\VEC{x}\|$ for
$\DS \VEC{x} \in \RR^n \setminus \{\VEC{0}\}$.  Using the
...
\end{align*}
for all $\DS \VEC{x} \in \RR^n \setminus \{\VEC{0}\}$.  It
follows that
\begin{align*}
...
\ldots \wedge \df{x}_n
\end{align*}
for all $\DS \VEC{x} \in \RR^n \setminus \{\VEC{0}\}$.
