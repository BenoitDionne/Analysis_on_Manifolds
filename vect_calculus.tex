...
vector field $F:V \to \RR^n$ along a curve $C$ is defined by
...
$\sigma:[a,b] \to \RR^n$ is a parametric representation of $C$, both
of them consistent with the selected orientation on $C$.  The integral
is independent of the parametric representation of $C$ used as long as
this parametric representation is consistent with the orientation on
$C$.  We write
...
Let $V$ be an open subset of $\RR^n$.  Suppose that $C \subset V$ is
a curve with a parametric representation $\sigma: [a,b] \to V$
consistent with its orientation.
...
replaced by $\tilde{\sigma}:[0,1]\to \RR^n$ defined by
...
$\tilde{\sigma}_{1,0}(0) = \sigma(a)$, we have that
\[
...
Let $V$ be an open subset of $\RR^n$
...
path-connected subsets.  In $\RR^n$, connected and path-connected
open subsets are the same.  It is not true in all general topological
spaces and it is not even true for close subsets of $\RR^n$.  For the
present context, it suffices to know that $W \subset \RR^n$ is
...
If $F = \graD f$ in $V$, it follows from the fundamental theorem for line
integral that
...
define $L$ in (\ref{ALNALa}).  If $L < \infty$, we say that the
...
\end{split}
\end{equation}
where we have used Schwarz inequality in $\RR^n$ for the second inequality.
...
$[a,b]$.  From the previous lemma, we have that
\[
...
$\sigma':[a,b]\to \RR^n$ is continuous on the compact set
...
$\breve{\sigma}(t) = \sigma(s^{-1}(t))$ for $0 \leq t \leq L$, then
...
If $S$ is a surface in $\RR^3$, there is a close link between the
selected orientation on $S$ and the outward unit normal to $S$
...
$\rho(w_1,w_2)= w_1\VEC{e}_1 + w_2\VEC{e}_2 + g(w_1,w_2)\VEC{e}_3$ for
$(w_1,w_2)$ in $W$.  In particular, we have that
\[
\VEC{n}(\rho(\VEC{w})) =
...
triangles $R_i$ are very small, the area of $\rho(R)$ should be
...
beginning of this section, the tradition is to define the
...
\begin{proof}
This is a special case of Stokes' theorem, Theorem~\ref{TheStokesTh},
with $\omega = F_1 \df{x_1} + F_2 \df{x_2} + F_3 \df{x_3}$.  We get that
...
+ F_3 \df{x_1}\wedge\df{x_2}$.  We have that
\[
  \int_{\partial S} \omega = \int_S \df{\omega} \ ,
\]
where
...
consistent with the orientation on $S_1$.  If it has not been the case, we
could have changed the parametric representation by inverting the
...
Suppose that $g:]a,b[ \to \RR^n$ is a differentiable vector valued
function such that $\|g(w)\| = c$, a constant, for all $w \in ]a,b[$.
Then $g'(w)$ is orthogonal to $g(w)$ for all $w \in ]a,b[$.
\end{lemma}

\begin{proof}
If we derive both sides of $g(w) \cdot g(w) = c^2$ with respect to
...
$\kappa(\VEC{u})=0$ for all $\VEC{u} \in C$, we have that $C$ is a
straight line.
...
\begin{rmk}
Why did we introduce this extra factor $\|\sigma'(w)\|^{-1}$ in
...
neighbourhood $I \subset \RR$ of $0$ and an open subset $U$ of $\RR^3$
...
\end{align*}
If $\epsilon$ is really small, the image of the box $P_0$ along the flow is
\[
\phi(t, P_0) \approx
...
Since $\VEC{w} = (0,0,\omega)$, we have that
\[
...
In general, an object that spin like a spinning top with an angular
...
$\curL F = 0$, then the object does not rotate.
...
$g(t) = (t,t,t)$ for $0 \leq t \leq 1$.  We have that
\[
...
$\displaystyle g(t) = (t,t^2,t^3)$ for $0 \leq t \leq 1$.  We have that
\begin{align*}
\int_C F \cdot \dx{\VEC{s}}
&= \int_C F \cdot g'(t) \dx{t}
...
as required by the orientation on the curve $C$.  We have that
\begin{align*}
\int_C F \cdot \dx{\VEC{s}}
&= \int_C F \cdot g'(t) \dx{t}
...
$\displaystyle \sigma(t) = (t,t^2)$ for $0 \leq t \leq 1$.  We have that
\begin{align*}
...
$\displaystyle x^2+y^2\leq a$.  We have that
\begin{align*}
...
$t^3 -t = t(t^2-1)$.  We have that $x=t^2-1$ decreases from $0$ to
$-1$ with $y =t^3-t>0$ when $t$ goes from $-1$ to $0$, and $x=t^2-1$
increases from $-1$ to $0$ with $y =t^3-t<0$ when $t$ goes from $0$ to
$1$.

We select the orientation on $C$ such that $D$ is to the left when
travelling along $C$.  Using Green's Theorem, we get that
\[
...
\subQ{c} According to the divergence theorem, we have that
\begin{align*}
\iint_S F \cdot \dx{\VEC{S}} &= \iiint_R \diV F \, \dx{V}
...
\subQ{d} According to the divergence theorem, we have that
\[
\iint_S F \cdot \dx{\VEC{S}} = \iiint_R \diV F \, \dx{V}
...
\subQ{e} According to the divergence theorem, we have that
\begin{align*}
\iint_S F \cdot \dx{\VEC{S}} &= \iiint_R \diV F \, \dx{V}
= \iiint_R 2z \dx{V}
...
\stage{ii} According to the divergence theorem, we have that
\[
\iint_S F \cdot \dx{\VEC{S}} = \iiint_R \diV F \, \dx{V}
...
\]
for $0 \leq r \leq 2$ and $0 \leq \theta \leq 2\pi$.  We have that
\[
...
\]
for $0 \leq r \leq 1$ and $0 \leq \theta \leq 2\pi$.  We have that
\[
\pdydx{g}{r} \times \pdydx{g}{\theta}
= \det \begin{pmatrix}
\VEC{e}_1 & \VEC{e}_2 & \VEC{e}_3 \\
...
and $0 \leq \theta \leq 2\pi$.  We have that
\[
\pdydx{g}{r} \times \pdydx{g}{\theta}
= \det \begin{pmatrix}
\VEC{e}_1 & \VEC{e}_2 & \VEC{e}_3 \\
...
\subQ{a} Using Green's Theorem, we get that
\[
...
enclosed by $C$ and the circle $C_\epsilon$, we get from Green's
Theorem that
\[
...
\end{align}
where $f:\RR^2\to \RR$ and $g:\RR^2\to \RR$ are two twice continuously
differentiable functions.\\
Note that $\graD^2 f = \diV(\graD f)$ is the {\bfseries
...
above.  However, for $(x,y) \in C(r_1)$, we have that
\[
...
\begin{sol}
\subQ{a} From the divergence theorem, we have that
\[
...
of class $C^2$ in any open set containing $B_r(\VEC{0})$ as required.
...
the resulting equation from the previous equation, we get
(\ref{GreenFormNo2}).
...
continuous and $\VEC{a}_r \to \VEC{0}$ as $r \to 0^+$, we get that
$\displaystyle
...
To answer the question, we use Stokes' theorem to get that
\[
\int_{\partial S} f \graD g \cdot \dx{\VEC{s}}
...
\stage{i} Using Stokes' theorem, we have that
\[
\int_{C_r} F \cdot \dx{\VEC{s}} - \int_{C_1} F \cdot \dx{\VEC{s}}
= \epsilon \int_{\partial S_r} F \cdot \dx{\VEC{s}}  
...
by $C$ and the circle $C_\epsilon$, we get from Stokes' theorem that
\[
\int_C F \cdot \dx{\VEC{s}} - \int_{C_\epsilon} F \cdot \dx{\VEC{s}}
= \int_{\partial S} F \cdot \dx{\VEC{s}}  
= \iint_S \curL F \cdot \dx{\VEC{S}}
...
expression for $f$ in the second equation in (\ref{finalB}), we get that
...
last equation in (\ref{finalB}), we get that
...
in the last equation in (\ref{consQ4}), we get that
...
in the last equation in (\ref{consQ3}), we get that
...
\begin{question}
For each of the vector fields $F$ defined on $\RR^3$ below, determine
if it is conservative and if there exists a vector field $G$ such that
$F = \curL G$.  If it is possible, find a vector field $G$.
...
$\phi(x,z) = 0$ for all $(x,z)$, we get that
...
this expression for $f$ in the second equation in (\ref{finalA}), we
get that
$\displaystyle e^x \cos(y) + \pdydx{\phi}{y} = e^x \cos(y)$.  Thus
$\displaystyle \pdydx{\phi}{y} = 0$ and we conclude that $\phi$ is only a
function of $z$.  If we substitute
$\displaystyle f(x,y,z) = e^x \sin(y) + \phi(z)$ in the
last equation in (\ref{finalA}), we get that
