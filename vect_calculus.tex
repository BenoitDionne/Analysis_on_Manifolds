...
Let $\Omega$ be an open subset of $\DS \RR^3$ and
$\DS F:\Omega \rightarrow \RR^3$ be a differentiable vector
...
\]
If $\DS F:\Omega \rightarrow \RR^3$ is a twice continuously
...
Let $f: \Omega \to \RR$ and $\DS F:\Omega \to \RR^3$ be sufficiently
...
$\DS \sigma:I_1 = [0,1] \to \RR^n$.  If $\sigma(0) = \VEC{x}_0$ and
...
$\DS \sigma = \sum_{j=1}^n \sigma_i\,\VEC{e}_i$
with $\DS \sigma_j \in C^1([a,b])$ for $1 \leq j\leq n$.
...
$\DS \sigma:[a,b]\to \RR^n$ and assume that
...
The parametric representation $\DS \sigma:[a,b] \to \RR^n$
...
parametric representation $\DS \sigma:[a,b] \to \RR^n$ of
...
{$\DS \sigma:[a,b]\rightarrow \RR^n$ is a parametric
...
Euclidean norm in $\DS \RR^n$.  Using the change of variable
...
Let $\DS F:V \rightarrow \RR^n$ be a continuous
{\bfseries vector fields}\index{Vector Field}; namely, a continuous
vector valued function from $V$ to $\DS \RR^n$.
The {\bfseries line integral}\index{Vector Field!Line Integral} of a
vector field $\DS F:V \to \RR^n$ along a curve $C$ is defined by
...
$\DS \sigma:[a,b] \to \RR^n$ is a parametric representation
...
replaced by $\DS \tilde{\sigma}:[0,1]\to \RR^n$ defined by
$\tilde{\sigma}(w) = \sigma(a + w(b-a))$ for $0 \leq w \leq 1$.  We
get that
$\DS \int_{\tilde{\sigma}} \df{f} = \int_{\partial
\tilde{\sigma}} f$.

Since
$\DS \df{f} = \sum_{i=1}^n \pdydx{f}{x_i} \df{x_i}$, we have
\begin{align*}
...
$\DS \sigma:[a,b]\rightarrow \RR^n$ is
...
Let $V$ be an open subset of $\DS \RR^n$
and $\DS F:V \rightarrow \RR^n$ be a continuous vector field.
The integral $\DS \int_C F \cdot \VEC{t} \dx{s}$ is
independent of the curve $C \subset V$ joining two points
$\VEC{a}$ and $\VEC{b}$ in $V$ if and only if
$\DS \oint_C F \cdot \VEC{t} \dx{s} = 0$ for all
...
$\DS \sigma:[a,b]\rightarrow V$ consistent with its
orientation. 
...
\end{align*}
Hence $\DS \int_{C_1} F \cdot \VEC{t} \dx{s}
...
$\VEC{a}$, then $\DS \int_{C_1} F\cdot \VEC{t} \dx{s}
= -\int_{C_2} F\cdot \VEC{t} \dx{s}$.
...
$\DS \int_{C_3} F\cdot \VEC{t} \dx{s} = \int_{C_1} F\cdot
\VEC{t} \dx{s} + \int_{C_2} F\cdot \VEC{t} \dx{s}$.
\end{rmk}

\begin{defn}
Let $\DS F:V \rightarrow \RR^n$ be a continuous vector field.  If
$\DS \int_C F \cdot \VEC{t} \dx{s}$ is independent
...
Let $V$ be an open subset $\DS \RR^n$ and 
$\DS F:V \rightarrow \RR^n$ be a continuous
...
path-connected subsets.  In $\DS \RR^n$, connected and path-connected
open subsets are the same.  It is not true in all general topological
spaces and it is not even true for close subsets of
$\DS \RR^n$.  For the
present context, it suffices to know that $\DS W \subset \RR^n$ is
...
We prove that $\DS \pdydx{f}{x_i}(\VEC{x}) = F_i(\VEC{x})$
...
Suppose that $C$ is a curve in $\DS \RR^n$ with the parametric
representation $\DS \sigma:[a,b] \to \RR^n$.  Recall that we
...
Let $C$ be a curve in $\DS \RR^n$ with the parametric
representation $\DS \sigma:[a,b] \to \RR^n$.
...
Suppose that $C$ is a curve in $\DS \RR^n$ with the parametric
representation $\DS \sigma:[a,b] \to \RR^n$, then the length
...
\begin{lemma}
If $\DS f:[a,b]\to \RR^n$ is continuous, then
\[
...
\int_a^b f(t) \dx{t} =
\begin{pmatrix}
\DS \int_a^b f_1(t) \dx{t} & \DS \int_a^b f_2(t) \dx{t}
& \ldots & \DS \int_a^b f_n(t) \dx{t}
\end{pmatrix}^\top \ .
\]
Given any unit vector $\DS \VEC{v} \in \RR^n$, we get
...
\end{split}
\end{equation}
where we have used Schwarz inequality in $\DS \RR^n$ for the
second inequality.
We may assume that $\DS \int_a^b f(t) \dx{t} \neq \VEC{0}$
...
Therefore $\DS \|\sigma'(t)\| - \|\sigma'(t_i)\|
...
$\DS \int_{t_{i-1}}^{t_i} \|\sigma'(t_i)\| \dx{t} 
...
$\DS s^{-1}:[0,L] \to [a,b]$ exists and is a continuously
differentiable function.  If we set
$\DS \breve{\sigma}(t) = \sigma(s^{-1}(t))$ for
$0 \leq t \leq L$, then
...
continuously differentiable function $\DS \rho:W \to \RR^n$ where
$W$ is an open subset of $\DS \RR^2$ and $\diff \rho(\VEC{w})$
is of rank $2$ for all $\VEC{w} \in W$.  Thus, a surface is a $2$-dimensional
manifold without boundary of class $\DS C^1$ of
$\DS \RR^n$ which can be represented with only
one local chart.
...
$\DS \rho: W \to \RR^n$ be one-to-one except on a set of measure zero.
...
\VEC{e}_1 & \VEC{e}_2 & \VEC{e}_3 \\
\DS \pdydx{\rho_1}{w_1}(\VEC{w}) & \DS
\pdydx{\rho_2}{w_1}(\VEC{w}) &
\DS \pdydx{\rho_3}{w_1}(\VEC{w}) \\[0.7em]
\DS \pdydx{\rho_1}{w_2}(\VEC{w}) & \DS
\pdydx{\rho_2}{w_2}(\VEC{w}) &
\DS \pdydx{\rho_3}{w_2}(\VEC{w})
\end{pmatrix}
\]
...
If $S$ is a surface in $\DS \RR^3$, then there is a close
...
$\DS \mu_{\rho(\VEC{w})} = \left[ \left(\rho(\VEC{w}),
...
Suppose that $\DS F:\Omega \rightarrow \RR^3$ is a vector field.
...
namely, the integral
$\DS \int_S F \cdot \VEC{n} \dx{S}$ in (\ref{classicSurfInt})
is nothing else than the integral $\DS \int_S \omega$ defined
...
$\DS (w_1,w_2) \in W \subset \RR^2$, then
\begin{align*}
&\iint_S f \dx{S} \\
...
Suppose that $\DS \rho: R \to \RR^3$ is a parametric
representation of a surface $S$ where $\DS R \subset \RR^2$
is an open rectangle.  Suppose that we cut $R$ into small triangles
$R_i$.  For each triangle $R_i$. let $A_i$ be the area of the
triangles in $\DS \RR^3$ having vertices at
...
$\DS \rho(R_i) \subset \RR^3$ by $A_i$.   If the
...
Let $\DS S \subset \RR^2$ be a compact $2$-dimensional
manifold of class $\DS C^1$ with boundary.  Suppose that
$\DS F_i:\RR^2 \to \RR$ is of class $\DS C^1$ for
$1\leq i \leq 2$.  Then
...
Since $S$ is a $2$-dimensional manifold in $\DS \RR^2$, the
integral on the right hand side of (\ref{SGTEq1}) is simply
$\DS \int_S \left(\pdydx{F_2}{x_1} - \pdydx{F_1}{x_2}\right) \dx{x_1}
\dx{x_2}$.  Note that $\DS (S^\circ,S^\circ,\Id)$ is a local chart
...
Let $\DS S \subset \RR^3$ be a compact and oriented
$2$-dimension manifold of class $\DS C^1$ with boundary.
Suppose that $\DS F:\RR^3\to \RR^3$ is a vector field of class
$\DS C^1$.  Then
...
$\DS n_1 \dx{A} = \df{x_2}\wedge\df{x_3}$,
$\DS n_2 \dx{A} = \df{x_3}\wedge\df{x_1}$ and
$\DS n_3 \dx{A} = \df{x_1}\wedge\df{x_2}$ on $S$.  Thus
\begin{align}
...
for compact manifolds of class $\DS C^1$, we could restated
...
manifold without boundary of class $\DS C^1$.  In that
...
$\DS C^\infty$ with compact support and such that
...
$\DS S^1$, we can show using one of the convergence theorems
for integrals (i.e. Theorem~\ref{thBCT} or Lebesgue dominated
convergence theorem) that
$\DS \int_{]0,2\pi[} \sigma^\ast(\psi_1 \omega) \to
\int_{]0,2\pi[} \sigma^\ast(\omega)$ and
$\DS \int_{]-\epsilon,\epsilon[} \sigma^\ast(\psi_2 \omega)
\to 0$ as $\epsilon \to 0$.
...
\end{align*}
However, since $\omega$ is continuous of the compact set
$\DS S^2$, we can show as we mentioned for the integral over
$\DS S^1$ that
$\DS \int_{\{(\theta,\phi);0<\theta<2\pi,0<\phi<\pi\}}
\rho^\ast(\psi_1 \omega)
\to
...
\]
This form of $\DS \pdydx{\rho}{w_1} \times \pdydx{\rho}{w_2}$
is not surprising because the outward unit normal to $S$ at
$\rho(\VEC{w}) \in S$ is given by $\DS
\VEC{n}(\rho(\VEC{w})) =
...
Let $\DS F:V \rightarrow \RR^3$ be a continuously
differentiable vector field on a convex open subset $V$ of
$\DS \RR^3$.  If $F$ satisfies
...
subset of $\DS \RR^3$ is star-shaped.  Since $\df{\omega} = 0$
...
$\DS \oint_{\tilde{C}} F \cdot \dx{\VEC{s}}$ is independent of the
...
Let $\DS S \subset \RR^3$ be a compact $3$-dimension
manifold of class $\DS C^1$ with a boundary.  Suppose that
$\DS F:\RR^3\to \RR^3$ is a
vector field of class $\DS C^1$.  Then
...
$\DS n_1 \dx{A} = \df{x_2}\wedge\df{x_3}$,
$\DS n_2 \dx{A} = \df{x_3}\wedge\df{x_1}$ and
$\DS n_3 \dx{A} = \df{x_1}\wedge\df{x_2}$ on $\partial S$
...
half-sphere $\DS x_1^2+x_2^3+x_3^2<4$ with $x_3> 0$.
...
\]
Note that
$\DS \rho_1\left(\frac{\pi}{4},0\right) = \begin{pmatrix}
...
We have that
$\DS \left\|\pdydx{\rho_1}{w_1}(\VEC{w}) \times 
...
Again, $\DS \pdydx{\rho_2}{w_1}(\VEC{w}) \times
...
differentiable vector field $\DS G:V \to \RR^3$.
\end{prop}
...
$\DS \RR^3$ is star-shaped.  Since $\df{\omega} = 0$ (i.e.
...
and $\VEC{w}$ are two vectors in $\DS \RR^n$,
then $\DS \VEC{v}\cdot \VEC{w} = \VEC{v}^\top \VEC{w}
= \VEC{w}^\top \VEC{v}$.

\subsection{Tangent Vector, Curvature and Torsion}

We start with the basic concept of tangent line to a curve.
Let $C$ is a curve in $\DS \RR^3$ with a continuously
differentiable parametric representation
$\DS \sigma:]a,b[ \rightarrow \RR^3$.
...
\begin{egg}
Consider the curve $C$ in $\DS \RR^3$      \label{DG_df1}
...
As before, let $\DS C \subset \RR^3$ be a curve and
$\DS \sigma:]a,b[\rightarrow \RR^3$ be a
...
Suppose that $\DS g:]a,b[ \to \RR^n$ is a differentiable vector valued
function such that $\|g(w)\| = c$, a constant, for all $w \in ]a,b[$.
Then $g'(w)$ is orthogonal to $g(w)$ for all $w \in ]a,b[$.
\end{lemma}

\begin{proof}
If we derive both sides of $\DS g(w) \cdot g(w) = c^2$ with respect to
...
$\DS \dfdx{\big(\VEC{t}(\sigma(w))\big)}{w}$ and
...
\begin{rmk}
Why did we introduce this extra factor $\DS \|\sigma'(w)\|^{-1}$ in
...
Suppose that $\DS \sigma : [a,b] \to \RR^3$ is a curve of
...
$\DS \breve{\sigma} = \sigma \circ s^{-1}:[0,L] \to \RR^3$
...
$\DS \VEC{t}(\VEC{u}) = \breve{\sigma}'(t)$ because
$\DS \|\breve{\sigma}'(t)\| = 1$ for all $t \in ]0,L[$.
...
$\DS \|\sigma'(w)\|^{-1}$ in (\ref{curvatureEq1}).
...
Let $\DS \VEC{N}(\VEC{u}) = \|\VEC{k}(\VEC{u})\|^{-1} \VEC{k}(\VEC{u})
= (\kappa(\VEC{u}))^{-1}\VEC{k}(\VEC{u})$ and
$\DS \VEC{B}(\VEC{u}) = \VEC{t}(\VEC{u}) \times \VEC{N}(\VEC{u})$
for $\VEC{\VEC{u}} \in C$.
...
$\DS \dfdx{\VEC{B}(\sigma(w))}{w}$ is orthogonal to
...
$\DS \left(\dfdx{\VEC{t}(\sigma(w))}{w} \right)$ and
$\VEC{N}(\sigma(w))$ are both parallel to $\VEC{k}(\sigma(w))$.
Hence
$\DS \dfdx{\VEC{B}(\sigma(w))}{w} \cdot \VEC{t}(\sigma(w)) = 0$
for all $w\in ]a,b[$ because
$\DS \VEC{t}(\sigma(w)) \times \dfdx{\VEC{N}(\sigma(w))}{w}$
is orthogonal to $\VEC{t}(\sigma(w))$.
Since
$\DS \dfdx{\VEC{B}(\sigma(w))}{w}$ is orthogonal to both
...
$\DS \begin{pmatrix} \sigma'(w) & \sigma''(w) & \sigma'''(w)
\end{pmatrix}$.
...
Suppose that $\DS \rho: I_2 \rightarrow \RR^3$ is a
parametric representation of class $\DS C^1$ consistent with
the orientation on the surface $S$.   This
parametric representation is such that
$\DS \pdydx{\rho}{w_1} \times \pdydx{\rho}{w_2}$ points
...
$\DS P_1 = \{w_{1,i}: 0 \leq i \leq N_1\}$ and
$\DS P_2 = \{w_{2,i} : 0 \leq i \leq N_2\}$ of the $w_1$
...
\DS \pdydx{\rho_1}{w_1}(w_{1,i},w_{2,j}) \Delta w_1 &
\DS \pdydx{\rho_2}{w_1}(w_{1,i},w_{2,j}) \Delta w_1 &
\DS \pdydx{\rho_3}{w_1}(w_{1,i},w_{2,j}) \Delta w_1 \\[1em]
\DS \pdydx{\rho_1}{w_2}(w_{1,i},w_{2,j}) \Delta w_2 &
\DS \pdydx{\rho_2}{w_2}(w_{1,i},w_{2,j}) \Delta w_2 &
\DS \pdydx{\rho_3}{w_2}(w_{1,i},w_{2,j}) \Delta w_2
\end{pmatrix}
\end{align*}
is the volume of the box defined by the vectors
$F(\rho(w_{1,i},w_{2,j}))$,
$\DS \pdydx{\rho}{w_1}(w_{1,i},w_{2,j}) \Delta w_1$ and
$\DS \pdydx{\rho}{w_2}(w_{1,i},w_{2,j}) \Delta w_2$ that we have
...
For each $\DS \VEC{x} \in U$, we may view
...
$\DS \RR^3$ to $\DS \RR^3$, we find that the image
of a point
...
Since $\DS \pdydx{\phi}{t}(t,\VEC{x}) = F(\phi(t,\VEC{x}))$, we have
\[
...
yields
$\DS  \VEC{v}_i(0,\VEC{x}) = \epsilon\, \VEC{e}_i$
for $1 \leq i \leq 3$,
...
$\DS V(t,\VEC{x}) = \VEC{v}_1(t,\VEC{x}) \cdot
...
$\DS \epsilon^3$, we get (\ref{diff_geom_DD3}).
...
We consider
$\DS \VEC{w}(t) = \diff_{\VEC{x}}\phi(t,\VEC{x}) \VEC{w}_0$
and $\DS \VEC{v}(t) = \diff_{\VEC{x}}\phi(t,\VEC{x}) \VEC{v}_0$.
Proceeding as we did to obtain (\ref{diff_geom_DD2}), we get
$\DS \dydx{\VEC{w}}{t}(0) = \diff F(\VEC{x}) \VEC{w}_0$
and
$\DS \dydx{\VEC{v}}{t}(0) = \diff F(\VEC{x}) \VEC{v}_0$.
...
\end{equation}
We may write
$\DS \diff F(\VEC{x}) = S(\VEC{x}) + W(\VEC{x})$, where
$\DS S(\VEC{x}) = \left( \diff F(\VEC{x}) +
\diff F(\VEC{x})^\top \right)/2$ and
$\DS W(\VEC{x}) = \left( \diff F(\VEC{x})
...
&= \frac{1}{2}
\begin{pmatrix}
0 & \DS \pdydx{F_1}{x_2}(\VEC{x}) - \pdydx{F_2}{x_1}(\VEC{x}) &
\DS \pdydx{F_1}{x_3}(\VEC{x}) - \pdydx{F_3}{x_1}(\VEC{x}) \\[1em]
\DS \pdydx{F_2}{x_1}(\VEC{x}) - \pdydx{F_1}{x_2}(\VEC{x}) & 0 &
\DS \pdydx{F_2}{x_3}(\VEC{x}) - \pdydx{F_3}{x_2}(\VEC{x}) \\[1em]
\DS \pdydx{F_3}{x_1}(\VEC{x}) - \pdydx{F_1}{x_3}(\VEC{x}) &
\DS \pdydx{F_3}{x_2}(\VEC{x}) - \pdydx{F_2}{x_3}(\VEC{x}) & 0
\end{pmatrix} \\
&= \frac{1}{2}
\begin{pmatrix}
0 & \DS -\left(\curL F(\VEC{x})\right)_3 &
\DS \left(\curL F(\VEC{x})\right)_2 \\
\DS \left(\curL F(\VEC{x})\right)_3 & 0 &
\DS -\left(\curL F(\VEC{x})\right)_1 \\
\DS -\left(\curL F(\VEC{x})\right)_2 & 
\DS \left(\curL F(\VEC{x})\right)_1 & 0
\end{pmatrix} \ ,
\end{align*}
where $(\curL F(\VEC{x}))_i$ is the $\DS i^{th}$ coordinates of
$\curL F(\VEC{x})$.
...
$\DS \curL \VEC{v}  = 2\, \omega\, \VEC{e}_3$.
...
$\DS F:\RR^3 \to \RR^3$ and a point
$\DS \VEC{y} \in \RR^3$.  Let $\VEC{n}$ be
...
\]
Since $\DS \lim_{r\to 0}\VEC{u}_r = \VEC{y}$, we get
\[
...
$\DS \int_{\partial S_r} F \cdot \VEC{t} \dx{s}$ expresses the
displacement along $\partial S_r$.  Thus, for very small $r$,
$\DS \frac{1}{\pi r^2} \int_{\partial S_r} F \cdot \VEC{t} \dx{s}$
...
value with
$\DS \VEC{n} = \|\curL F(\VEC{y})\|^{-1} \curL F(\VEC{y})$.
...
$\DS E(k) = \int_0^{\pi/2} \sqrt{1- k^2\sin^2(t)}\dx{t}$ with
$0<k<1$ are called elliptic integrals.

\subQ{a} Write the arc length of the ellipse
$\DS \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1$ in terms of an
elliptic integral.\\
\subQ{b} Write the arc length of the curve $C$ given by intersection
of the sphere $\DS x^2 + y^2 + z^2 =4$ and the cylinder
$\DS x^2 + y^2 -2xy = 0$ for $z\geq 0$.
\end{question}
...
\]
This suggests that $\DS g:[0,\pi]\to \RR^3$
defined by
\[
...
$\DS g:[0,2\pi]\to \RR^3$ defined by
$\DS g(t) = (2\cos(t),2\sin(t), t^2)$.
Compute $\DS \int_C\sqrt{z} \dx{s}$.
\end{question}

\begin{sol}
Since $g'(t) = (-2\sin(t), 2\cos(t) , 2t )$, we have that
$\DS \|g'(t)\| = 2\sqrt{1 + t^2}$.  Thus
\[
...
$\DS \int_C F \cdot \dx{\VEC{s}}$.

\subQ{a} $C$ is the straight line from the origin to $(1,1,1)$ and
$\DS F(x,y,z) = (yz , x^2, xz)$.\\
\subQ{b} $C$ is the curve $\DS \{(x,x^2,x^3) : 0 \leq x \leq 1\}$ from
$(0,0,0)$ to $(1,1,1)$ and $\DS F(x,y,z) = (yz , x^2, xz)$.\\
\subQ{c} $C$ is the circle $\DS x^2+y^2 =1$ oriented clockwise and
$F(x,y,z) = (x-y,x+y)$.
\end{question}
...
$\DS g(t) = (t,t^2,t^3)$ for $0 \leq t \leq 1$.  We have
\begin{align*}
\int_C F \cdot \dx{\VEC{s}}
&= \int_C F \cdot g'(t) \dx{t}
...
compute $\DS \int_C \omega$.

\subQ{a} $C$ is the curve $\DS \{(x,x^2) : 0 \leq x \leq 1\}$
from $(0,0)$ to $(1,1)$ and
$\DS \omega = xe^{-y} \df{x} + \sin(\pi x)\df{y}$.\\
...
$\DS \omega = y^2 \df{x} - 2 x\df{y}$.
\end{question}

\begin{sol}
\subQ{a} The curve $C$ is the singular $1$-cube given by
$\DS \sigma(t) = (t,t^2)$ for $0 \leq t \leq 1$.  We have
\begin{align*}
...
$0 \leq t \leq 1$.  Since
$\DS \int_{\sigma_1} \omega = \int_{[0,1]} \sigma_1^\ast(\omega)
= \int_0^1 0 \dx{t} = 0$,
$\DS \int_{\sigma_2} \omega = \int_{[0,1]} \sigma_2^\ast(\omega)
= \int_0^1 (-2) \dx{t} = -2$ and
$\DS \int_{\sigma_3} \omega = \int_{[0,1]} \sigma_3^\ast(\omega)
...
\begin{question}
Let $\DS F:\RR^n \to \RR^n$ be a continuous vector fields
and $\DS C \subset \RR^n$ be a curve.  Prove that
$\DS  \left| \int_C F \cdot \dx{\VEC{s}} \right|
\leq \int_C \|F\|\dx{s}$.
\end{question}

\begin{sol}
Let $\DS \sigma:[0,1]\to \RR^n$ be a parametric
...
$\DS g: [a,b] \to \RR^n$ of $C$ such that $g$ is one-to-one except
possibly at a finite number of points.
Since $C$ is the union of two curves $C_1$ and $C_2$, there exists
$c$ between $a$ and $b$ such that $\DS g: [a,c] \to \RR^n$
is a parametric representation of $C_1$ and
$\DS g: [c,b] \to \RR^n$ is a parametric representation of $C_2$.
...
We have that
$\DS L_P(g) \leq L_Q(g) = L_{Q_1}(g) + L_{Q_2}(g) \leq L_1 + L_2$.
...
$\DS L_1 - L_{P_1}(g) < \epsilon/2$ and
$\DS L_2 - L_{P_2}(g) < \epsilon/2$.
...
\begin{question}
Compute the surface integral $\DS \iint_S z\dx{S}$ over the
...
$\DS \left\| \pdydx{g}{\theta} \times \pdydx{g}{\phi} \right\| =
a^2 \sin(\phi)$.  Thus
\begin{align*}
...
$\DS g(x,y) = (x, y, \sqrt{a^2-x^2-y^2})$ for
$\DS x^2+y^2\leq a$.  We have
\begin{align*}
...
\end{align*}
On the disk $\DS x^2 +y^2 \leq a$, we have that
$-a\leq x \leq a$ and
$\DS -\sqrt{a^2-x^2} \leq y \leq \sqrt{a^2-x^2}$.
Hence
\begin{align*}
...
\begin{question}
Compute the surface integral $\DS \iint_S( x^2 + y^2)\dx{S}$
over the portion $S$ of the sphere $\DS x^2+y^2+z^2=25$
...
\begin{question}
Compute $\DS \int_S (x^2 + y^2 - 2z^2) \dx{S}$ where $S$ is
...
$\DS \int_S x^2 \dx{S} = \int_S y^2 \dx{S} = \int_S z^2 \dx{S}$.
Thus $\DS \int_S (x^2 + y^2 - 2z^2) \dx{S} = 0$.
\end{sol}

\begin{question}
Compute the integral $\DS \iint_S xyz \dx{S}$ where $S$ is
...
$\DS \phi(x,y) = (x, y, 2-2x-y)$ for $0\leq x \leq 1$ and
$1-x\leq y \leq 2-2x$.
...
$\DS g(r,\theta) = \left( r\cos(\theta),  r\sin(\theta) ,
r^2 \cos(\theta)\sin(\theta) \right)$
for $0 \leq r \leq a$ and $0\leq \theta \leq 2\pi$.
Since
$\DS \pdydx{g}{r} = \left( \cos(\theta) , \sin(\theta) ,
2r \cos(\theta)\sin(\theta) \right)$
and\\
$\DS \pdydx{g}{\theta} = \left( -r \sin(\theta) , r\cos(\theta) ,
r^2 (\cos^2(\theta)-\sin^2(\theta)) \right)$,
we get that\\
$\DS \left\| \pdydx{g}{r} \times \pdydx{g}{\theta} \right\|
...
\begin{question}
Find the area of the surface $\DS z=x^2+y^2$ inside the cylinder
$\DS x^2+y^2 = a^2$.
\end{question}
...
Let $S$ be the surface defined in the statement of the question.  A
parametric representation of this surface is given by
$\DS g(r,\theta) = \left( r\cos(\theta), r\sin(\theta),  r^2
\right)$ for $0 \leq r \leq a$ and $0\leq \theta \leq 2\pi$.
Since
$\DS \pdydx{g}{r} = \left( \cos(\theta) , \sin(\theta) ,
2r \right)$ and
$\DS \pdydx{g}{\theta} = \left( -r \sin(\theta) ,
...
$\DS z=x^2+y^2$ and above by
the sphere $\DS x^2+y^2+z^2=4z$ for $z\geq 0$.  Find the
area of the surface of $S$.
\end{question}

\begin{sol}
The value of $z$ at the intersection of the paraboloid with the sphere is
given by $\DS z + z^2 = 4z$; namely,
$\DS z^2-3z=z(z-3)=0$.  For $z=0$, the sphere
...
rotation of the circle $\DS (x-b)^2+z^2 = a^2$ about the $z$ axis.
\end{question}

\begin{sol}
\pdfbox{vect_calculus/question31}
A parametric representation of the circle $\DS (x-b)^2+z^2 = a^2$
is given by\\
$\DS h(\phi) =
...
$\DS z=x^2+y^2$ which is
inside the region bounded by the cylinders $\DS x^2+y^2=2$
and $\DS x^2+y^2=6$.
\end{question}

\begin{question}
Find the area of the portion of the sphere of radius $R$ centred at
the origin which is inside the cylinder $\DS x^2+y^2 -Ry\leq 0$ and
satisfies $x \geq 0$.
\end{question}
...
$\DS \iint_S \VEC{F} \cdot \dx{\VEC{S}}$ where
$F(x,y,z) = (x, y, z)$ and $S$ is the cylinder defined by
$\DS x^2+y^2=1$ and $0\leq z \leq 1$.  The orientation on $S$ is
...
$\DS \phi(\theta,z) = \big( \cos(\theta), \sin(\theta), z\big)$
...
$\DS \VEC{n} = \pdydx{\phi}{\theta} \times \pdydx{\phi}{z}$
...
Evaluate directly the surface integral
$\DS \iint_S \VEC{F} \cdot \dx{\VEC{S}}$ where
$\DS F(x,y,z) = \big( x^2y, -3 xy^2, 4y^3\big)$ and $S$ is the
portion of the surface $\DS z=x^2+y^2+9$ with
...
required and\\
$\DS \dx{S} = \left\|\pdydx{\phi}{x} \times \pdydx{\phi}{y}\right\|
...
$R$.\\
\subQ{b} $R$ is the surface of the cylinder $\DS y^2+z^2 = 1$ with
...
\begin{question}
Compute $\DS \int_S F \cdot \dx{\VEC{S}}$    \label{surfQuestionA}
...
\subQ{b} $\DS F(x,y,z) = (x^2,z,-y)$ and $S$ is the unit
...
$S$ is give by a normal pointing in the direction of $z$ positive.\\
\subQ{d} $\DS F(x,y,z) = (0,0,z^2)$ and $S$ is the boundary
of the region enclosed by the cylinder $\DS x^2+y^2=1$ and
...
enclosed by the surface $\DS z=x^2+y^2$ and the sphere
$\DS x^2+y^2+z^2=20$.
...
Since
$\DS \pdydx{g}{u} = (1,0,v)$ and
$\DS \pdydx{g}{v} = (0,1,u)$, we get that
$\DS \pdydx{g}{u} \times \pdydx{g}{v} = (-v,-u,1)$.
...
\subQ{b} We have that
$\DS \iint_S F \cdot \dx{\VEC{S}} = \iint_S F \cdot \VEC{n} \dx{S}$
...
= \iint_S x^3 \dx{S} \ .
\]
Since $\DS x \mapsto x^3$ is an odd function and $S$ is
symmetric with respect to the $y,z$ plane, the last integral is
null.  Hence $\DS \iint_S F \cdot \dx{\VEC{S}} = 0$.
...
Since
$\DS \pdydx{g}{u} = (1,0,-1)$ and
$\DS \pdydx{g}{v} = (0,1,-1)$, we get
$\DS \pdydx{g}{u} \times \pdydx{g}{v} = (1,1,1)$.
...
\end{align*}
Since
$\DS \iint_{S_1} \dx{S} = \iint_{S_3} \dx{S} =
  \iint_{x^2+y^2\leq 1} \dx{S} = \pi$,
the area of a disk of radius $1$, we get that
$\DS \iint_S F \cdot \dx{\VEC{S}} = (b^2-a^2)\pi$.

\subQ{e} We first find the intersection of the two surfaces.
On the intersection, $z$ must satisfy
$\DS z^2+z-20 = (z+5)(z-4)=0$.  Since
$\DS z = x^2+y^2$ is non negative, we get $z=4$.  Thus the
intersection is the circle $\DS x^2+y^2=4$ with $z=4$.
...
\]
where
$\DS S_1 = \{ (x,y,\sqrt{20-x^2-y^2}) : x^2 + y^2 \leq 4 \}$ is the
top of $S$ and
$\DS S_2 = \{ (x,y,x^2+y^2) : x^2 + y^2 \leq 1\}$ is the bottom
of $S$.
...
and
$\DS \pdydx{g_2}{r} = \left( \cos(\theta), \sin(\theta), 2r \right)$,
we get that
$\DS \pdydx{g_2}{\theta} \times \pdydx{g_2}{r}
...
representation
$\DS \sigma(t) = (t^2-1,t^3-t)$ for $-1 \leq t \leq 1$.
We assume that the parametric representation is consistent with the
orientation on $C$.
\end{question}

\begin{sol}
\stage{i} Since $\DS \sigma'(t) = (2t, 3t^2-1)$. the line
integral is
\begin{align*}
...
$\DS t^3 -t = t(t^2-1)$.  We have that $\DS x=t^2-1$
decreases from $0$ to $-1$ with $\DS y =t^3-t>0$ when $t$
goes from $-1$ to $0$, and $\DS x=t^2-1$ increases from $-1$
to $0$ with $\DS y =t^3-t<0$ when $t$ goes from $0$ to 
$1$.
...
boundary of the disk $\DS D = \{ (x,y) : x^2 + y^2 \leq 1
\}$, we have
\[
...
\begin{question}
Let $D$ be the annulus $\DS \{ (x,y) : 1 \leq x^2 + y^2 \leq 4\}$.
Compute $\DS \int_{\partial D} x y^2 \dx{y} - x^2 y \dx{x}$
where $\partial D$ has the positive orientation with respect to $D$.
\end{question}

\begin{sol}
We have $\DS \int_{\partial D} F \cdot \dx{\VEC{s}}$ where
$\DS F(x,y) = (-x^2y, x y^2)$.  Thus
\[
...
\begin{question}
Find the simple closed curve $C$ of class $\DS C^1$
that maximize the line integral
$\DS I_C = \int_C F \cdot \dx{\VEC{s}}$ where
$\DS F(x,y) = (y^3, 3x-x^3)$.
\end{question}

\begin{sol}
Suppose that $C = \partial D$ is the boundary of a bounded region $D$
with a boundary of class $\DS C^1$.  Moreover, suppose that the
...
$\DS D=\{ (x,y) : x^2 + y^2 \leq 1\}$, the largest region on which
$\DS 1-x^2-y^2$ is non-negative.  Thus $C$ is the unit
...
is given by $\DS -\int_{\partial S} y \dx{x}$ where
...
\begin{sol}
We show that
$\DS \int_a^b f(x) \dx{x} = -\int_{\partial S} y \dx{x}$.
We have that
$\DS \int_{\partial S} y \dx{x} = \sum_{i=1}^4 \int_{C_i} y \dx{x}$
where
...
$\DS \int_{C_i} y \dx{x} = 0$ for $i=1$, $2$ and $4$ because
...
$\DS \int_{\partial S} y \dx{x} = - \int_a^b f(x) \dx{x}$
as required.
\end{sol}
...
\begin{sol}
Since $\DS \dydx{x}{u} = a (1 -\cos(u)) >0$ for
...
\]
However $\DS \int_{C_1} x \dx{y} = 0$ and
\begin{align*}
\int_{C_2} x \dx{y}
...
compute the integral $\DS \iint_S F \cdot \dx{\VEC{S}}$
...
$\DS \frac{x^2}{a^2} + \frac{y^2}{b^2} + \frac{z^2}{c^2} = 1$.\\
\subQ{e} $\DS F(x,y,z) = (x^2, -2xy,z^2)$ and $S$ is the
boundary of the cylinder
$R = \{ (x,y,z) : (x,y)\in D \text{ and } a \leq z \leq b\}$,
where $\DS D\subset \RR^2$ is a surface with a boundary of
class $\DS C^1$ and an area that is equal to $A$.
\end{question}
...
= \iiint_R 2x \dx{V}
\]
where $\DS R = \{ (x,y,z) : x^2 + y^2 + z^2 \leq 1 \}$ is the
...
$\DS u^2 + v^2 + w^2 \leq 1$.  Since
$\DS \left|\frac{\partial(x,y,z)}{\partial(u,v,w)}\right| = abc$,
we get
\[
...
$\DS V = 4\pi r^3/3$ to compute the volume of a ball
of radius $r$.  Thus
\[
  \iint_S F \cdot \dx{\VEC{S}} = 
...
$\DS F(x,y,z) = (x^2+y^2+z^2) (x, y, z)$ and $S$ be the
...
origin.  Compute $\DS \iint_S F \cdot \dx{\VEC{S}}$ with and
...
$\DS \pdydx{g}{\phi} \times \pdydx{g}{\theta}$ points outside the
...
$\DS F(x,y,z) = \big(x^3 +y\sin(z), y^3+z\sin(x), 2z\big)$
and the solid
$\DS R = \{ (x,y,z) : 1 \leq x^2 + y^2 + z^2 \leq 9
\ \text{and}\ z\geq 0\}$.
Compute $\DS \iint_{\partial R} F\cdot \dx{\VEC{S}}$
...
$\DS \iint_{\partial R} F\cdot \dx{\VEC{S}}
= \iiint_{R} \diV F \dx{x}\dx{y}\dx{z}$
where
$\DS \diV F = \pdydx{F_1}{x} + \pdydx{F_2}{y} + \pdydx{F_3}{z}
= 3x^2 + 3 y^2 + 2$.
...
Since $\DS |\det \diff g(\theta,\phi,r)| = r^2 \sin(\phi)$,
we get
\begin{align*}
...
\begin{question}
Consider the vector field
$\DS F(x,y,z) = \big( x^2y, xy^2, 2xyz\big)$.  Let $R$
be the tetrahedron with vertices at $(0,0,0)$, $(2,0,0)$, $(0,1,0)$ and
$(0,0,2)$.  Compute $\DS \iint_{\partial R} F\cdot \dx{\VEC{S}}$
where the orientation on $\partial R$ is given by the unit normal
pointing outside $R$.
\end{question}

\begin{question}
Consider the vector Field $\DS F:\RR^3\to \RR^3$ defined by
$\DS F(x,y,z) = \big(xz^2, z, x^2z \big)$.  Compute the integral
$\DS \iint_D F\cdot \dx{\VEC{S}}$ where $D$ is the surface of the
solid $\DS R = \{(x,y,z) : 1 \leq x^2+z^2 \leq 4 \ \text{and}
\ -1 \leq y \leq 1\}$ and
...
\begin{sol}
From the divergence theorem, we have that
$\DS \iint_D F\cdot \dx{\VEC{S}} = \int_R \diV F$
where $\DS \diV F = z^2 + x^2$.  To compute the
...
\begin{question}
Let $R$ be a bounded region in $\DS \RR^3$ with a piecewise
continuously differentiable boundary $\partial R$.  Show that the
volume of $R$ is given by
$\DS \frac{1}{3} \iint_{\partial R} F \cdot \dx{\VEC{S}}$
...
\]
where $\DS \iiint_R \dx{V}$ is the volume of $R$.
\end{sol}

\begin{question}
Use Stokes' theorem to evaluate $\DS \int_C F \cdot \dx{\VEC{s}}$
...
$\DS x^2+y^2=4$.  The orientation on $C$ is counterclockwise
...
According to Stokes' theorem, we have that
$\DS \int_C F\cdot \dx{\VEC{s}} = \iint_S \curL F \cdot \dx{\VEC{S}}$
where
\[
\curL F = \det \begin{pmatrix}
\VEC{e}_1 & \VEC{e}_2 & \VEC{e}_3 \\
...
= \det \begin{pmatrix}
\VEC{e}_1 & \VEC{e}_2 & \VEC{e}_3 \\
\DS (a/\sqrt{2}) \cos(\theta) &
\DS (a/2)\sin(\theta) &
\DS -(a/2)\sin(\theta) \\
\DS -(a r/\sqrt{2}) \sin(\theta) &
\DS (a r/2)\cos(\theta) &
\DS -(a r/2)\cos(\theta)
...
$\DS \left( 0,\frac{a^2r}{2\sqrt{2}},\frac{a^2r}{2\sqrt{2}}\right)$
...
\begin{question}
Compute $\DS \iint_S \curL F \cdot \dx{\VEC{S}}$ where
$\DS F(x,y,z) = (y , x-2x^3z,xy^3)$ and $S$ is the upper half of the
...
$\DS \iint_S \curL F \cdot \dx{\VEC{S}}
...
$\DS \int_{\partial S} F \cdot \dx{\VEC{s}}
= \iint_S \curL F \cdot \dx{\VEC{S}}$
where $D$ is the disk
$\DS D = \{ (x,y,0) : x^2 + y^2 \leq a^2 \}$ and the
...
\begin{question}
Let $\DS F(x,y,z) = (-x^2y, x^3, x^2 + yz)$ and $S$ be the
lower half of the ellipsoid
$\DS \frac{x^2}{4} + \frac{y^2}{9} + \frac{z^2}{25} = 1$;
...
compute $\DS \iint_S \curL F \cdot \dx{\VEC{S}}$.  We could
...
\noindent{\bfseries Note}: Instead of computing the line integral
$\DS \int_{\partial S} F \cdot \dx{\VEC{s}}$, we may use
Stokes' theorem again to replace this integral by a surface integral;
namely, $\DS \int_{\partial S} F \cdot \dx{\VEC{s}}
= \iint_D \curL F \cdot \dx{\VEC{S}}$
where $D$ is the region
$\DS D = \left\{ (x,y,0) : \frac{x^2}{4} + \frac{y^2}{9}
...
\begin{question}
Let $\DS F(x,y,z) = \left(\frac{-y}{x^2+y^2},
\frac{x}{x^2+y^2},0 \right)$ for $(x,y) \neq (0,0)$.

\subQ{a} Show that $\curL F = \VEC{0}$.\\
\subQ{b} Show that $\DS \int_C F \cdot \dx{\VEC{s}} = 2 \pi$
where $C$ is the circle
$\DS C = \{ (x,y,b) : x^2 +y^2 = a^2\}$ oriented
...
$\DS S \subset \RR^3$ such that $C = \partial S$ and $F$ is
...
\begin{question}
Consider the vector field
$\DS F(x,y,z) = \big( yz, xyz, \cos(xy)e^{xz}\big)$.
Let $R$ be the portion of the paraboloid
$\DS z= 11 - x^2 - y^2$ above the plan
$z=2$.  Compute
$\DS \int_{\partial R} F \cdot \dx{\VEC{s}}$ where the
...
According to Stokes' theorem, we have that
$\DS \int_{\partial R} F \cdot \dx{\VEC{s}} =
...
$\DS F:\RR^3\to \RR^3$ is smooth, we
may use any surface $V$ whose boundary is $\partial R$.  We take $V$
to be the disk $\DS x^2+y^2 \leq 9$ with $z=2$.
...
\begin{question}
Consider the vector field
$\DS F(x,y,z) = \big(xy^3z, \sin(xyz) , xyz\big)$.
Let $R$ be the portion of the cone $\DS y^2 = x^2+z^2$
...
that $\DS \int_{\partial R} F \cdot \dx{\VEC{s}} = 0$.
...
Hence, $\DS \int_R \curL F \cdot \dx{\VEC{S}} =
...
\begin{question}
Let $\DS F:\RR^2\setminus \{\VEC{0}\} \to \RR^2$ be a continuously
differentiable vector fields such that
$\DS \pdydx{F_2}{x} - \pdydx{F_1}{y} = 0$ on 
$\RR^2\setminus \{\VEC{0}\}$.

\subQ{a} Prove that $\DS \int_{C_r} F \cdot \dx{\VEC{s}} = \alpha$, a
...
$\DS \int_C F \cdot \dx{\VEC{s}} = \alpha$
...
\subQ{c} Let
$\DS F_0(x,y) = \left( \frac{-y}{x^2+y^2}, \frac{x}{x^2+y^2}\right)$.
Prove that $\DS G = F - \frac{\alpha}{2\pi} F_0$ is conservative on
$\DS \RR^2\setminus \{\VEC{0}\}$ and thus is the gradient of
a function. Such a result is fundamental in the studies of Green's
Functions.
\end{question}

\begin{sol}
Let $\DS C_r = \{ (x,y) : x^2 + y^2 = r^2 \}$ and
\[
S_r = \begin{cases}
...
Thus $\DS \int_{C_r} F \cdot \dx{\VEC{s}}
= \int_{C_1} F \cdot \dx{\VEC{s}}$ for all $r$.  If we let
$\alpha = \DS \int_{C_1} F \cdot \dx{\VEC{s}}$,
then $\DS \int_{C_r} F \cdot \dx{\VEC{s}} = \alpha$ for all
$r>0$.
...
orientation on $\partial S$.  Thus $\DS
...
$\DS \int_C F_0\cdot \dx{\VEC{s}} = 2\pi$ for any closed curve
...
\end{align}
where $\DS f:\RR^2\to \RR$ and
$\DS g:\RR^2\to \RR$ are two twice continuously
differentiable functions.\\
Note that $\DS \graD^2 f = \diV(\graD f)$ is the {\bfseries
Laplacian}\index{Laplacien} of $f$ and is more often denoted $\Delta f$.\\
\subQ{c} Let $\DS g(x,y) = \ln(x^2+y^2)$.  Verify that
$\DS \graD g = \frac{2}{x^2+y^2}\, (x,y)$ and
$\DS \graD^2 g(x,y) = 0$ for $(x,y)\neq (0,0)$.\\
\subQ{d} Suppose that $\DS f:\RR^2\to \RR$ is twice continuously
differentiable and $\DS \graD^2 f = 0$.  Use $g$ defined in (c)
...
third component in $\DS \RR^3$, we may substitute
...
$\DS F=f \graD g = \left( f\pdydx{g}{x} , f \pdydx{g}{y} \right)$ in
(\ref{equC}) to get
\begin{align*}
&\int_{\partial S} f \graD g \cdot \VEC{n} \dx{s} =
...
= \frac{2}{r_1} \VEC{n}
\]
and $\DS g(x,y) = \ln(x^2+y^2) = \ln(r_1^2)$.  Thus
$\DS \int_{C_{r_1}} f \graD g \cdot \VEC{n} \dx{s}
...
$\DS \int_{C_{r_2}} f \graD g \cdot \VEC{n} \dx{s}
= \frac{2}{r_2} \int_{C_{r_2}} f \dx{s}$ and
$\DS \int_{C_{r_2}} g \graD f \cdot \VEC{n} \dx{s} = 0$.
...
\subQ{e}
From (d), we have that
$\DS \frac{1}{2\pi r} \int_{C_r} f \dx{s} = M$
for all $r$, where $M$ is a constant.  From the Mean Value Theorem for
Integrals, we have for each $r$ that
$\DS \frac{1}{2\pi r} \int_{C(r)} f \dx{s} = f(\VEC{a}_r)$
for some $\VEC{a}_r \in C_r$.  So $f(\VEC{a}_r) = M$ for all $\VEC{a}_r$.
Since $\DS \lim_{r\to 0} \VEC{a}_r = \VEC{0}$ and
$f$ is continuous, we get that
$\DS M = \lim_{r\to 0^+} f(\VEC{a}_r) = f(\VEC{0})$.
\end{sol}

\begin{question}
Let $R$ be a bounded region in $\DS \RR^3$ with a piecewise smooth
boundary $\partial R$.  Suppose that $\DS f,g:\RR^3\to \RR$ are two
...
$\DS \diV F = \pdydx{f}{x_1} g + f \pdydx{g}{x_1}$, we get
...
$\DS \iiint_R \pdydx{f}{x_1} g \dx{V}$
on both sides of the previous equality.
\end{sol}

\begin{question}
Suppose that $\DS f:\RR^3\to \RR$           \label{divCons}
is a function of class $\DS C^2$.
Let $R$ be a bounded region in $\DS \RR^3$ with a piecewise smooth
...
\]
\subQ{b} If $\DS \graD^2 f = 0$, show that
\[
...
= \iiint_R \| \graD f \|^2 \dx{V} \ .
\end{align*}
because $\DS \graD^2 f = 0$ on $R$ by assumption.
\end{sol}

\begin{question}
Let              \label{introGreenFunct}
$\DS g(\VEC{x}) = \frac{1}{\|\VEC{x}\|}$ for
$\DS \VEC{x} \in \RR^3 \setminus \{\VEC{0}\}$.

\subQ{a} Compute $\graD g$ and show that
$\DS \graD^2 g(\VEC{x}) = 0$ for all $\VEC{x} \neq \VEC{0}$.\\
\subQ{b} Show that
$\DS \iint_{\partial B_r(\VEC{0})} \pdydx{g}{\VEC{n}} \dx{S}
= - 4 \pi$
...
\subQ{d} Show that
$\DS \iint_{\partial R} \pdydx{g}{\VEC{n}} \dx{S} = - 4 \pi$
for any bounded region $R$ with a piecewise smooth boundary such
that $\DS \VEC{0} \in R^\circ$.  The
...
$\DS \VEC{n}(\VEC{x}) = \|\VEC{x}\|^{-1} \VEC{x}$ for
...
= 4\pi r^2 \ .
\]
Thus $\DS  \iint_{\partial B_r(\VEC{0})} \pdydx{g}{\VEC{n}} \dx{S}
= - 4\pi$.
...
extended to $\DS \RR^3$) and thus it is certainly not
of class $\DS C^2$ in any open set containing $B_r(\VEC{0})$
as required.
...
$\DS \iint_{\partial R_\epsilon} \pdydx{g}{\VEC{n}} \dx{S}
...
$\DS \iint_{\partial B_\epsilon(\VEC{0})} \pdydx{g}{\VEC{n}} \dx{S}$
...
\label{GreenFormNo2}
\end{align}
where $\DS f, g : \RR^3 \to \RR$ are two functions of class
$\DS C^2$, the vector $\VEC{n}(\VEC{x})$ is the unit normal
...
\subQ{b} Suppose that $\DS f: \RR^3 \to \RR$ is a function
of class $\DS C^2$ and $\DS \graD^2 f = 0$, prove that
\[
...
for all $\DS \VEC{a} \in \RR^3$ and $r>0$.
\end{question}
...
$\DS g(\VEC{x}) = \frac{1}{\|\VEC{x}\|}$ for $\VEC{x} \in R$,
we get
\[
...
= 0
\]
because $\DS \graD^2 f= 0$ by hypothesis and $\DS
...
\]
Similarly, we get
$\DS \iint_{\partial B_{r_2}(\VEC{0})} f \graD g \cdot \dx{\VEC{S}}
= - \frac{1}{r_2^2} \iint_{\partial B_{r_2}(\VEC{0})} f \dx{S}$
and
$\DS \iint_{\partial B_{r_2}(\VEC{0})} g \graD f \cdot \dx{\VEC{S}}
= 0$.  Thus
\[
\iint_{\partial B_{r_2}(\VEC{0})}
\left( f \graD g - g \graD f\right) \cdot \dx{\VEC{S}}
...
$\DS \frac{1}{4\pi r^2} \iint_{\partial B_r(\VEC{0})} f \dx{S} = C$,
...
$\DS \frac{1}{4\pi r^2} \iint_{\partial B_r(\VEC{0})} f
\dx{S} = f(\VEC{a}_r)$ for some $\VEC{a}_r \in \partial B_r(\VEC{0})$.
Note that $\DS \iint_{\partial B_r(\VEC{0})} f \dx{S} = 4\pi r^2$
is the area of the sphere of radius $r$.  Since $f$ is 
continuous and $\VEC{a}_r \to \VEC{0}$ as $\DS r \to 0^+$, we get that
$\DS
...
\begin{question}
Suppose that $\DS S \subset \RR^3$ is a surface with a piecewise
...
If $\DS f:\RR^3 \to \RR$ is a function of class
$\DS C^1$ and $\DS g : \RR^3 \to \RR$ is function
of class $\DS C^2$, prove that
...
$\DS \curL( f \graD g) = \graD f \cdot \graD g$.
...
= \graD f \times \graD g
\end{align*}
because $\DS \pdydxnm{g}{z}{y}{2}{}{} = \pdydxnm{g}{y}{z}{2}{}{}$,
$\DS \pdydxnm{g}{z}{x}{2}{}{} = \pdydxnm{g}{x}{z}{2}{}{}$
and $\DS \pdydxnm{g}{y}{x}{2}{}{} = \pdydxnm{g}{x}{y}{2}{}{}$
for functions of class $\DS C^2$ like $g$.
...
$\df{(\df{g})} = 0$. 
Moreover, we have
$\DS \graD f \times \graD g \longleftrightarrow \df{f}
\wedge \df{g}$.  Thus
...
\begin{question}
Suppose that $\DS F:\RR^3 \to \RR^3$ is a continuously differentiable
vector field such that $\curL F \cdot \VEC{e}_2 = 0$ everywhere in the
$x,z$ plane.  Show that $\DS \int_C F \cdot \dx{\VEC{s}}$
...
\begin{sol}
Let $\DS C_r = \{ (x,0,z) : x^2 + z^2 = r^2 \}$ and
\[
S_r = \begin{cases}
...
$\DS \int_{C_r} F \cdot \dx{\VEC{s}} = \int_{C_1} F \cdot
\dx{\VEC{s}}$ for all $r$.  If
$\DS K = \int_{C_1} F \cdot \dx{\VEC{s}}$,
then $\DS \int_{C_r} F \cdot \dx{\VEC{s}} = K$ for all $r>0$.
...
find a function $f$.\\
\subQ{a} $\DS F(x,y,z) = \big( yz^2, xz^2, 2xyz \big)$.\\
\subQ{b} $\DS F(x,y,z) = \big(2xy + x^2, x^2 - y^2, 0)$.\\
\subQ{c} $\DS F(x,y,z) = \big(3y^2 + 5x^4, x^5 + 6xy, 0 \big)$.\\
\subQ{d} $\DS F(x,y,z) = \big(yz -y \sin(xy),
xz -x \sin(xy)  + z\cos(yz), xy + y\cos(yz)\big)$.\\
\subQ{e} $\DS F(x,y,z) = \big(y-z, x-z, x-y\big)$.
\end{question}
...
\end{pmatrix} = \VEC{0}
\]
for all $\DS (x,y,z)\in \RR^3$ and $F$ is a continuously
differentiable vector field on the convex set $\DS \RR^3$,
...
function $\DS f:\RR^3\to \RR$ such that $\graD f = F$.  The
...
$\DS f(x,y,z) = xyz^2 + \phi(y,z)$.  If we substitute this
expression for $f$ in the second equation in (\ref{finalB}), then we get that
$\DS xz^2 + \pdydx{\phi}{y} = xz^2$.  Thus
$\DS \pdydx{\phi}{y} = 0$ and we conclude that $\phi$ is only a
function of $z$.  If we substitute
$\DS f(x,y,z) = xyz^2 + \phi(z)$ in the
last equation in (\ref{finalB}), then we get that
$\DS 2xyz + \dydx{\phi}{z} = 2xyz$.  Thus
$\DS \dydx{\phi}{z} = 0$ and $\phi(z) = C$, a
constant.  We find that $\DS f(x,y,z) = xyz^2 + C$.
...
$\DS f(x,y,z) = x^2y + \frac{x^3}{3} + \phi(y,z)$.  If we
substitute this expression for $f$ in the second equation in
(\ref{consQ4}), then we get that
$\DS x^2 + \pdydx{\phi}{y} = x^2 - y^2$.  Thus
$\DS \pdydx{\phi}{y} = -y^2$ and we conclude that
$\DS \phi(y,z) = - \frac{y^3}{3} + \phi(z)$.  If we
substitute
$\DS f(x,y,z) = x^2y + \frac{x^3}{3} - \frac{y^3}{3} + \phi(z)$
in the last equation in (\ref{consQ4}), then we get that
$\DS \dydx{\phi}{z} = 0$.  Thus $\phi(z) = C$, a
constant.  We find that $\DS
...
$\DS f(x,y,z) = xyz + \cos(xy) + \phi(y,z)$.  If we
substitute this expression for $f$ in the second equation in
(\ref{consQ3}), then we get that
$\DS xz -x \sin(xy) + \pdydx{\phi}{y} = xz - x\sin(xy) + z
\cos(yz)$.   Thus $\DS \pdydx{\phi}{y} = z\cos(yz)$ and we
conclude that $\DS \phi(y,z) = \sin(yz) + \phi(z)$.  If we
substitute
$\DS f(x,y,z) = xyz + \cos(xy) + \sin(yz) + \phi(z)$
in the last equation in (\ref{consQ3}), then we get that
$\DS xy + y\cos(yz) + \dydx{\phi}{z} = xy + y \cos(yz)$.
Thus $\DS \dydx{\phi}{z} = 0$ and $\phi(z) = C$, a
constant.  We find that $\DS
...
\begin{question}
For each of the vector fields $F$ defined on $\DS \RR^3$
...
\subQ{b} $\DS F(x,y,z) = \big(0,2yz+z, z\big)$.\\
\subQ{c} $\DS F(x,y,z) = \big(x^3+yz, y - 3x^2y, 4y^2-z) \big)$.\\
\subQ{d} $\DS F(x,y,z) = \big(xy+z , xz, -yz+x\big)$.\\
\subQ{e} $\DS F(x,y,z) = \big(x e^{-x^2z^2} - 6 x, 5y+2z,
z-z e^{-x^2z^2}\big)$.
\end{question}
...
\]
for all $\DS (x,y,z) \in \RR^3$ and
$\DS F:\RR^3 \to \RR^3$ is continuously
differentiable on the convex set $\DS \RR^3$, it follows
from Proposition~\ref{DGclosedexact} that there exists
$\DS G:\RR^3\to \RR^3$ such that $\curL G = F$.
As explained in Remark~\ref{CEtrick}, we may seek
$\DS G:\RR^3\to \RR^3$ such that
\begin{align*}
...
field $\DS G:\RR^3\to \RR^3$ because we would then have that
$\diV F = \diV(\curL F) = 0$ on $\DS \RR^3$.
...
\end{align*}
If we choose $\DS \phi(x,y) = 4xy^2$ and $\psi(x,y) = 0$
for all $(x,y)$, then we get that
$\DS G(x,y,z) = \Big(yz - 3x^2yz), -x^3z - \frac{yz^2}{2} + 4xy^2,
0 \Big)$.
...
\]
If we choose $\DS \phi(x,y) = \frac{x^2}{2}$ and
$\psi(x,y) = 0$ for all $(x,y)$, then we get that
$\DS G(x,y,z) = \Big( \frac{xz^2}{2} ,-xyz - \frac{z^2}{2}
+ \frac{x^2}{2} , 0 \Big)$.
...
\end{align*}
If we choose $\DS \psi(x,z) = z^2$ and
$\phi(x,z) = 0$ for all $(x,z)$, then we get that
$\DS G(x,y,z) = \Big( -yz + yz e^{-x^2z^2} +z^2 , 0,
xy e^{-x^2z^2} - 6 xy \Big)$.
\end{sol}

\begin{question}
Consider the vector field $\DS F:\RR^3 \to \RR^3$ defined by
$\DS F(x,y,z) = \big( e^x\sin(y) , e^x\cos(y) , z^2 \big)$.
Evaluate the
integral $\DS \int_C F\cdot \dx{\VEC{s}}$ where $C$ is the curve
with the parametric representation
$\DS \sigma(t) = \big( t^{1/3}, t^5, e^{t^{1/2}} \big)$ for
...
we get from Proposition~\ref{DGconserveB} that $F$ is conservative and
we get from Proposition~\ref{DGconserveA} that there exists a
function $\DS f:\RR^3\to \RR$ such that $\graD f = F$.
Hence
\begin{align*}
...
$\DS f(x,y,z) = e^x \sin(y) + \phi(y,z)$.  If we substitute
this expression for $f$ in the second equation in (\ref{finalA}), then
we get that
$\DS e^x \cos(y) + \pdydx{\phi}{y} = e^x \cos(y)$.  Thus
$\DS \pdydx{\phi}{y} = 0$ and we conclude that $\phi$ is only a
function of $z$.  If we substitute
$\DS f(x,y,z) = e^x \sin(y) + \phi(z)$ in the
last equation in (\ref{finalA}), then we get that
$\DS \dydx{\phi}{z} = z^2$.  Thus
$\DS \phi(z) = \frac{z^3}{3}$.  We may ignore the constant
...
$\DS f(x,y,z) = e^x \sin(y) + \frac{z^3}{3}$.
Hence
$\DS \int_C F\cdot \dx{\VEC{s}} = f(1.1.e) - f(0,0,1) =
e\sin(1) +(e^3-1)/3$.
\end{sol}

\begin{question}
Let $C$ be the path in $\DS \RR^3$ defined by the segments
...
$\DS \int_{C} 2xyz \dx{x} + x^2z \dx{y} + x^2y \dx{z}$.
\end{question}

\begin{question}
Compute the integral
$\DS \int_C (2y-3z)\dx{x} + (2x+z)\dx{y} +(y-3x)\dx{z}$
where $C$ is the curve with the parametric representation
$\DS \sigma(t) = (t^{1/3},  \sqrt{t} e^{-t}, e^{\cos(t\pi)})$
for $0\leq t \leq 1$.
...
\begin{question}
Let $\DS F(x,y,z) = - \frac{GMm}{(x^2+y^2+z^2)^{3/2}} \, ( x, y, z)$.
