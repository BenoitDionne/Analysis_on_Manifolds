...
\stage{2}
Since $\pi_{S_2} \circ \tilde{f} = f \circ \pi_{S_1}$, we get that
\[
...
$\breve{\phi} = \tilde{\phi}_\ast$.  In particular, we have that
\begin{align}
...
\check{M}_{\eta(\VEC{u},\tilde{E}(\VEC{u}))}$, we get that
\begin{align*}
...
Since $\displaystyle \eta^\ast(\rho) = \rho$, we get that
\[
\rho(\eta(\VEC{u},\VEC{x}))
...
for $a \leq t \leq c$ \footnote{We could prove directly that
$\check{\rho}(\check{\sigma}(t))\big(\check{\sigma}_\ast(t,1)\big) = 0$.
...
\]
for $i =1,2$.  Using Proposition~\ref{propEtaOmegai}, we get that
\begin{align}
...
\end{equation}
on $\TS_{(\VEC{u},\VEC{x})} S_c$.  Similarly, we have that
...
smooth differential $2$-form on $U$, we get that
\[
...
\]
for $i =1,2$.
If we compute the differential of (\ref{O1cP1psP2Eq2}), we get that
\begin{align}
...
deduce (\ref{FirstStructEq4}), we get that
...
Example~\ref{eggConnPAp2}, we have that
\[
...
(\ref{O1cP1psP2Eq2}) and (\ref{O1cP1psP2Eq3}).  We have that
\[
...
For the right hand side of (\ref{LieBEq1}), we have that
\begin{align*}
&F_1(\VEC{u})(\omega(F_2)) - F_2(\VEC{u})(\omega(F_1))
- \omega(\VEC{u})([F_1,F_2](\VEC{u})) \\
...
equations in Definition~\ref{defnFCartanEs}, we get that
...
equation in Definition~\ref{defnSecondCSE}, we get that
...
$\displaystyle g \in C^\infty(S)$, we have that
\begin{align*}
...
for all $(r,\theta) \in \check{W}$. we get that
\begin{align*}
P_\ast(V_1(t,\eta))
...
Lemma~\ref{lemmaMRs2}, we have that
\[
...
Theorem~\ref{thmRconnect}, we have that
\[
...
However, using Corollary~\ref{cordyOmF1F2}, we have that
\begin{align}
&\df{\big(Q^\ast(\omega_2)\big)}(t,\eta)\big(V_1(t,\eta),V_2(t,\eta)\big)
\nonumber \\
...
Using Corollary~\ref{cordyOmF1F2}, we have that
\begin{align}
&\df{\big(Q^\ast(\rho)\big)}(t,\eta)\big(V_1(t,\eta),V_2(t,\eta)\big)
\nonumber \\
...
$\VEC{u}$, and
\item $h(r) = \big(Q^\ast(\omega_2)(V_2) \circ \check{\phi}\big)(r,\theta)
...
$K = G \circ \tilde{K} \circ G^{-1}:U \to U$.
...
$g = \sigma_\eta^{-1} \circ \beta_2$.  The function is piecewise
$\displaystyle C^\infty$.
We get that
\begin{align*}
L_{\beta_2} &= \int_a^c \big\|(\beta_2)_\ast(t,1) \big\|_{\beta_2(t)} \dx{t}
...
$\sigma_\eta$ because it is parameterized by arc length.  We get that that
...
The map $g$ is defined by $g = \tilde{G}_2 \circ G_1^{-1}:U_1 \to U_2$.
...
Theorem~\ref{thmConn1form} for $i =1,2$.  we have that
\begin{align*}
...
and $\df{\omega_2^{[2]}} = -\rho^{[2]}  \wedge \omega_1^{[2]}$, we get that
\[
...
= G_\ast \circ (g_f)_\ast \circ G^{-1}_\ast$, we get that
\begin{align*}
...
Likewise,
$\displaystyle \diff (\phi^{-1} \circ f \circ \phi)(\VEC{w})
= \lim_{i\to \infty} \diff (\phi^{-1} \circ f \circ \phi)(\VEC{w_i})
= \lim_{i\to \infty} \Id_{\RR^k} = \Id_{\RR^k}$.}
...
$\partial D$ of $D = \{\VEC{x} \in \RR^2 : \|\VEC{x}\| < r \}$ for some $r>0$.
...
$f_{w,\theta}(0) = - e^{i\theta} w \in D$, we get that
...
$\CC$ and $\RR^2$ and first note that
\[
...
We can now confirm that the ``straight lines'' that we defined at the
...
parameterized) through the origin.  We now use that fact that
...
should satisfy $e^{i\theta}p_2 = q_2$ to get $f_{w,\theta}(p_2) = q_2$ because
...
(\ref{LFTEq3}), then we get that
\[
r^2(1 - e^{i\nu}) p_1 + r^2(1+e^{i\nu}) w = -2 r^2 e^{i(\theta-\nu)}q_1
\]
we can always solve for $p_1$ or $w$ because at least one of
$1 - e^{i\nu}$ or $1 + e^{i\nu}$ is non-null.  We can substitute $p_1$
or $w$ into (\ref{LFTEq2}) to get a system of linear equations (the
real and imaginary parts) for $w$ or $p_1$ respectively.  Once we have
$w$ and $p_1$ in terms of $\theta$, we can use the equation
$q_2 = e^{i\theta}p_2 = \pm e^{i\theta} |q_2| p_1$ to determine
$\theta$ and thus $p_2$.
