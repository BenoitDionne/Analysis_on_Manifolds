...
multilinear map $\DS \tau:H^k\to \RR$.  The set of all
$k$-tensors on $V$ is denoted $\DS \T^k(H)$.
\end{defn}

The space $\DS \T^k(H)$ is a vector space over $\RR$ where
...
\]
for all $\DS \tau_1, \tau_2 \in \T^k(H)$ and all $a, b \in \RR$.
It is also important to note that $\DS \T^1(H) = H^\ast$,
the dual space of $H$.

\begin{defn}
The {\bfseries product of two tensors}\index{Tensor!Product}
$\DS \tau_1 \in \T^{k_1}(H)$ and
$\DS \tau_2 \in \T^{k_2}(H)$ is the tensor
$\DS \tau_1\otimes \tau_2 \in \T^{k_1+k_2}(H)$ defined by
\[
...
Let $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ be a basis of $H$
and let $\DS \B^\ast = \{ \phi_i \}_{i=1}^n$ be
...
\stage{i} We prove that any $\DS \tau \in \T^k(H)$ can be expressed
...
$\DS \VEC{w}_j = \sum_{m_j=1}^n a_{m_j,j} \VEC{v}_{m_j}$ for some
...
$2$-tensor $\DS \tau \in \T^2(H)$ such that $\tau$ is
...
Suppose that $\DS \tau\in \T^2(H)$ is an inner product on a
vector space $H$ of dimension $n$ over the real.  There exists a basis
$\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ of $H$ such that
...
Let $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ be a basis of $H$, it
is possible to choose an inner product on $H$ such that $\B$ is an
orthonormal basis.  Let $\DS \E = \{ \VEC{e}_i \}_{i=1}^n$
be the canonical basis of $\DS \RR^n$ and
$\ps{}{}$ be the standard inner product on $\DS \RR^n$
defined by
$\DS \ps{\VEC{x}_1}{\VEC{x}_2} = \sum_{j=1}^n x_{j,1}x_{j,2}$
for $\DS \VEC{x}_i = \sum_{j=1}^n x_{j,i}\VEC{e}_j$ and
$1 \leq i \leq 2$.
Moreover, let $\DS f:H \to \RR^n$ be the linear
isomorphism defined by $f(\VEC{v}_i) = \VEC{e}_i$ for $1 \leq i \leq n$.
We define the inner product $\DS \tau \in \T^2(H)$ by
\begin{align*}
...
\end{align*}
for $\DS \VEC{w}_i = \sum_{j=1}^n w_{j,i}\VEC{v}_j$ and
...
\begin{defn}
A $k$-tensor $\DS \tau \in \T^k(H)$ is called
...
$k$-tensor on $H$ is denoted $\DS \Omega^k(H)$.
\end{defn}

It is easy to verify that $\DS \Omega^k(H)$ is a linear
subspace of $\DS \T^k(H)$.

\begin{prop} \label{propAltDefn}
Given $\DS \tau \in \T^k(H)$, let
\[
...
Then $\DS \alt:\T^k(H) \to \Omega^k(H)$.
\end{prop}

\begin{proof}
Suppose that $\DS \tau \in \T^k(H)$ and $\tilde{\sigma}$ is a
fixed transposition on $J_k$.  We have that
$\DS \tilde{\sigma}^{-1} = \tilde{\sigma}$ and
\begin{align*}
...
\begin{enumerate}
\item If $\DS \tau \in \Omega^k(H)$, then $\alt(\tau)=\tau$.
\item If $\DS \tau \in \T^k(H)$, then $\alt(\alt(\tau))=\alt(\tau)$.
\item If $\DS \tau_i \in \Omega^{k_i}(H)$ for $1 \leq i \leq 2$
and $\alt(\tau_1) = 0$, then $\alt(\tau_1 \otimes \tau_2) =
\alt(\tau_2 \otimes \tau_1) = 0$.
\item If $\DS \tau_i \in \Omega^{k_i}(H)$ for $1 \leq i \leq 3$,
...
Hence, for $\DS \tau \in \Omega^k(H)$, we have
\begin{align*}
&\alt(\tau)(\VEC{v}_1, \VEC{v}_2, \ldots ,\VEC{v}_k)
= \frac{1}{k!} \sum_{\sigma\in S_k} \sgn(\sigma)
\tau(\VEC{v}_{\sigma(1)}, \VEC{v}_{\sigma(2)}, \ldots ,\VEC{v}_{\sigma(k)}) \\
...
We have that $\DS S_{k_1+k_2} = \bigcup_{1\leq j \le J}
...
The proof that $\DS \alt(\tau_2 \otimes \tau_1) = 0$ is
...
$\tau_1 \wedge \tau_2$ of
$\DS \tau_1 \in \Omega^{k_1}(H)$ and
$\DS \tau_2\in \Omega^{k_2}(H)$ is the tensor
$\DS \tau_1\wedge \tau_2 \in \Omega^{k_1+k_2}(H)$ defined by
\[
...
$\DS f^\ast: \T^k(H_2)\to \T^k(H_1)$ by
\[
(f^\ast(\tau))(\VEC{v}_1,\VEC{v}_2, \ldots,\VEC{v}_k) = 
\tau(f(\VEC{v}_1),f(\VEC{v}_2), \ldots,f(\VEC{v}_k))
\]
for all $\DS \tau \in \T^k(H_2)$ and $\VEC{v}_i \in H_1$ for
$1 \leq i \leq k$.  We say that
$\DS f^\ast(\tau)$ is the {\bfseries pull-back}\index{Pull-Back}
of $\tau$ by $f$.
...
\begin{enumerate}
\item If $\DS \tau_1,\tau_2 \in \Omega^{k_1}$
and $\DS \tau_3 \in \Omega^{k_2}(H)$, then
$(\tau_1 + \tau_2)\wedge \tau_3 = \tau_1 \wedge \tau_3 + \tau_2
\wedge \tau_3$.
\item If $\DS \tau_1\in \Omega^{k_1}(H)$ and
$\DS \tau_2,\tau_3 \in \Omega^{k_2}(H)$, then
$\tau_1\wedge(\tau_2+ \tau_3) = \tau_1 \wedge \tau_2 + \tau_1
\wedge \tau_3$.
\item If $\DS \tau_i \in \Omega^{k_i}(H)$
for $1 \leq i \leq 2$ and $a \in \RR$, then
$(a \tau_1) \wedge \tau_2 = \tau_1 \wedge (a\tau_2) = 
a (\tau_1 \wedge \tau_2)$.
\item If $\DS \tau_i \in \Omega^{k_i}(H)$
for $1 \leq i \leq 2$, then
$\DS \tau_1 \wedge \tau_2 = (-1)^{k_1 k_2} \tau_2 \wedge \tau_1$.
\item If $\DS \tau_i \in \Omega^{k_i}(H)$
for $1 \leq i \leq 2$ and $f:H_1\to H_2$ is a linear map from the
vector space $H_1$ to the vector space $H_2$, then 
$\DS f^\ast(\tau_1 \wedge \tau_2) = f^\ast\tau_1 \wedge f^\ast \tau_2$.
\item If $\DS \tau_i\in \Omega^{k_i}(H)$ for $1\leq i \leq 3$, then
\[
...
$\DS \sgn(\tilde{\sigma}_{k_1}) = (-1)^{k_1 k_2}
...
$\DS \sgn(\sigma) = (-1)^{k_1 k_2}\sgn(\tilde{\sigma})$.
Hence
\begin{align*}
...
$(\tau_1\wedge \tau_2) \wedge \tau_3 = \tau_1 \wedge ( \tau_2 \wedge \tau_3)$
for all $\DS \tau_i \in \Omega^{k_i}(H)$ with $1\leq i \leq 3$.
Inductively, we may define
$\tau_1\wedge \tau_2 \wedge \ldots \wedge \tau_j$ for all
$\DS \tau_i\in \Omega^{k_i}(H)$ with $1\leq i \leq j$.

\begin{cor} \label{corMwedgesR}
$\DS \tau_1\wedge \tau_2 \wedge \ldots \wedge \tau_j
...
Given $\DS \tau_i \in \Omega^1(H)$ for $1\leq i \leq k$, we have
...
$\DS \sigma \mapsto \sigma\circ\rho^{-1}$ for
...
$k$ distinct $\phi_i$ from $\DS \BB^\ast$.  However, for
each set of indices $1 \leq i_1 < i_2 < \ldots < i_k\leq n$, all $k!$
combinations
$\DS \{ \phi_{i_{\sigma(j)}} \}_{j=1}^k$ for $\sigma \in S_k$
...
Let $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ be a basis of the
vector space $H$ over $\RR$.  If $\DS \tau \in \Omega^n(H)$ and
$\DS \VEC{w}_i = \sum_{i=1}^n a_{j,i} \VEC{v}_j$
...
$\DS \rho:\RR^n \times \ldots \times \RR^n \to \RR$ defined by
\[
...
\begin{cor}
Suppose that $\DS \tau \in \Omega^k(H)$ and
...
$\DS M_\sigma = \begin{pmatrix} 0 & 0 & 0 & 1 \\ 1 & 0 & 0 & 0 \\
...
$\DS M_\sigma = \begin{pmatrix} 0 & 0 & 1 & 0 \\ 0 & 1 & 0 & 0 \\
...
Since $\DS \VEC{w}_{\sigma(i)} = \sum_{i=1}^n a_{j,\sigma(i)} \VEC{v}_j$
...
If $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ is a basis
...
$\DS \E = \{ \VEC{e}_i \}_{i=1}^n$ is the canonical basis of
$\DS \RR^n$.
\end{defn}

If $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$
and $\DS \C = \{ \VEC{w}_i \}_{i=1}^n$ are two orthonormal bases
with respect to an inner product $\DS \tau\in \T^2(H)$, then
$A$ in Theorem~\ref{stokesTBE} is an orthogonal matrix, namely
$\DS A^\top A = \Id_n$.  This follows from
\begin{align*}
...
\end{align*}
Hence, if $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$
and $\DS \C = \{ \VEC{w}_i \}_{i=1}^n$ are two orthonormal bases
with respect to the inner product $\DS \tau \in \T^2(H)$,
...
$\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ on $H$ with respect to
the inner product $\DS \tau \in \T^2(H)$.
Since $\DS \Omega^n(H)$ is of dimension one, there exists a unique
...
orthonormal basis $\DS \C = \{ \VEC{w}_i \}_{i=1}^n$ belonging to
the orientation $[\VEC{v}_1, \VEC{v}_2, \ldots, \VEC{v}_n]$.

\begin{defn}
Let $\DS \B = \{ \VEC{v}_i \}_{i=1}^n$ be an orthonormal
basis on $S$ with respect to the inner product
$\DS \tau \in \T^2(H)$.  The unique
$\DS \nu \in \Omega^n(H)$
such that $\DS \nu(\VEC{v}_1,\VEC{v}_2, \ldots, \VEC{v}_n) = 1$
is called the {\bfseries volume element}\index{Volume Element} on $H$
determined by the inner product $\DS \tau \in \T^2(H)$ and
the orientation $[\VEC{v}_1, \VEC{v}_2, \ldots, \VEC{v}_n]$.
\end{defn}
...
$\DS \RR^n$ determined by the standard inner product
$\tau \equiv \ps{}{}$ on $\DS \RR^n$
and the orientation $[\VEC{e}_1, \VEC{e}_2, \ldots, \VEC{e}_n]$,
where $\DS \E = \{ \VEC{e}_i \}_{i=1}^n$ is the canonical basis of
$\DS \RR^n$.

To conclude this section, we extend the classical cross product in
$\DS \RR^3$ to $\DS \RR^n$ for $n>3$.
Given $\DS \VEC{w}_i \in \RR^n$ for $1\leq i \leq n-1$, let
$\DS g \in \Omega^1(\RR^n)$ be the linear functional defined by
\[
g(\VEC{v}) = \det
...
\]
for $\DS \VEC{v} \in \RR^n$.
From the theory of representation of linear functionals on
$\DS \RR^n$,
there exists a unique $\DS \VEC{z}\in \RR^n$ such that 
...
\end{pmatrix}$ for all $\DS \VEC{v} \in \RR^n$.
\end{defn}
...
\begin{defn}
Let $\DS V \subset \RR^n$ be an open set.  Given $\VEC{v} \in V$, the
...
\]
for all $\DS \VEC{x}_i \in \RR^n$, where
$\DS \tau\in \T^k(\RR^n)$.  We say that
$\tau_{\VEC{v}}$ is generated by $\tau$.
\end{defn}

The space $\DS \T^k(\TS_{\VEC{v}}V)$ is a vector
space over $\RR$.  If
$\DS \tau_{\VEC{v}}^{[1]}, \tau_{\VEC{v}}^{[2]} \in \T^k(\TS_{\VEC{v}}V)$
are generated by $\DS \tau^{[1]},\tau^{[2]} \in \T^k(\RR^n)$
respectively, and $a, b \in \RR$, then
$\DS a \tau_{\VEC{v}}^{[1]}+ b \tau_{\VEC{v}}^{[2]}
\in \T(\TS_{\VEC{v}}V)$
is generated by $\DS a \tau^{[1]} + b \tau^{[2]} \in \T(\RR^n)$.
This follows from
\begin{align*}
...
\end{align*}
for all $\DS \VEC{x}_i \in \RR^n$ and $1 \leq i \leq k$.
...
Let $\DS V \subset \RR^n$ be an open set and $\VEC{v} \in V$.
The {\bfseries product of two tensors}\index{Tensor!Product}
$\DS \tau_{\VEC{v}}^{[1]} \in \T^{k_1}(\TS_{\VEC{v}}V)$ and
$\DS \tau_{\VEC{v}}^{[2]} \in \T^{k_2}(\TS_{\VEC{v}}V)$ is the tensor
$\DS \tau_{\VEC{v}}^{[1]}\otimes \tau_{\VEC{v}}^{[2]} \in
...
\end{align*}
for all $\DS \VEC{x}_i \in \RR^n$.
\end{defn}

If $\DS \tau_{\VEC{v}}^{[i]} \in \T^{k_i}(\TS_{\VEC{v}}V)$
is generated by $\DS \tau^{[i]} \in \T^{k_1}(\RR^n)$
for $1\leq i \leq 2$, then
$\DS \tau_{\VEC{v}}^{[1]}\otimes \tau_{\VEC{v}}^{[2]} \in
\T^{k_1+k_2}(\TS_{\VEC{v}}V)$
is generated by
$\DS \tau^{[1]}\otimes \tau^{[2]} \in \T^{k_1+k_2}(\RR^n)$.
This follows from
\begin{align*}
&(\tau_{\VEC{v}}^{[1]}\otimes \tau_{\VEC{v}}^{[2]})\big( (\VEC{v},\VEC{x}_1),
(\VEC{v},\VEC{x}_2), \ldots, (\VEC{v}, \VEC{x}_{k+1+k_2})\big) \\
...
$\DS \tau_{\VEC{v}}^{[1]} \in \Omega^{k_1}(\TS_{\VEC{v}}V)$ and
$\DS \tau_{\VEC{v}}^{[2]} \in \Omega^{k_2}(\TS_{\VEC{v}}V)$
is the alternating tensor
$\DS \tau_{\VEC{v}}^{[1]}\wedge \tau_{\VEC{v}}^{[2]} \in
...
Suppose that
$\DS \tau_{\VEC{v}}^{[i]} \in \Omega^{k_i}(\TS_{\VEC{v}}V)$ is
generated by $\DS \tau^{[i]} \in \Omega^{k_i}(\RR^n)$ for
$1\leq i \leq 2$.
Then $\DS \tau_{\VEC{v}}^{[1]} \wedge \tau_{\VEC{v}}^{[2]} \in
\Omega^{k_1+k_2}(\TS_{\VEC{v}}V)$ is generated by
$\DS \tau^{[1]} \wedge \tau^{[2]} \in \Omega^{k_1+k_2}(\RR^n)$.
This follows from
\begin{align*}
& (\tau_{\VEC{v}}^{[1]} \wedge \tau_{\VEC{v}}^{[2]})\big( (\VEC{v},\VEC{x}_1),
...
Suppose that $\DS V\subset \RR^n$ and
$\DS W \subset \RR^m$ are two open sets.
If $f:V \to W$ is a function of class $\DS C^1$ and
...
$\DS f^\ast: \T^k(\TS_{f(\VEC{v})} W) \to \T^k(\TS_{\VEC{v}} V)$
defined by
\begin{align*}
...
\end{align*}
for all $\DS \VEC{x}_i \in \RR^n$.
We say that $\DS f^\ast(\tau_{f(\VEC{v})})$ is the
...
$\VEC{v} \in V$, then
\begin{enumerate}
\item $\DS f^\ast(\tau_{f(\VEC{v})}^{[1]} + \tau_{f(\VEC{v})}^{[2]})
= f^\ast(\tau_{f(\VEC{v})}^{[1]}) + f^\ast(\tau_{f(\VEC{v})}^{[2]})$ for all
$\DS \tau_{f(\VEC{v})}^{[i]} \in \T^k(\TS_{f(\VEC{v})}W)$.
\item $\DS f^\ast( a\, \tau_{f(\VEC{v})}) =
a f^\ast(\tau_{f(\VEC{v})})$
for all $\DS \tau_{f(\VEC{v})} \in \T^k(\TS_{f(\VEC{v})}W)$
and $a \in \RR$.
\item $\DS f^\ast( \tau_{f(\VEC{v})}^{[1]} \otimes
\tau_{f(\VEC{v})}^{[2]})
= f^\ast(\tau_{f(\VEC{v})}^{[1]}) \otimes f^\ast(\tau_{f(\VEC{v})}^{[2]})$ for
all $\DS \tau_{f(\VEC{v})}^{[i]} \in \T^{k_i}(\TS_{f(\VEC{v})} W)$.
\item $\DS f^\ast( \tau_{f(\VEC{v})}^{[1]}
\wedge \tau_{f(\VEC{v})}^{[2]})
= f^\ast(\tau_{f(\VEC{v})}^{[1]}) \wedge f^\ast(\tau_{f(\VEC{v})}^{[2]})$ for
all $\DS \tau_{f(\VEC{v})}^{[i]} \in \Omega^{k_i}(\TS_{f(\VEC{v})} W)$.
\end{enumerate}
\end{prop}
...
$\DS C^k$ if each function $f_i$ is of class
$\DS C^k$.
...
$\DS F,G:V \to \bigcup_{\VEC{v} \in V} \TS_{\VEC{v}} V$ are two
vector fields, then $\ps{F}{G}$ is defined by
$\ps{F}{G}(\VEC{v})
= \ps{F(\VEC{v})}{G(\VEC{v})}_{\VEC{v}} = \tau(f(\VEC{v}),g(\VEC{v}))$
for all $\VEC{v} \in V$, where $\DS \tau \in \T^2(\RR^n)$ is
an inner product.
...
\begin{defn}
Let $V$ be an open subset of $\DS \RR^n$.  A
...
\tilde{\phi}_{i_k} \ ,
\]
where $\DS \omega_{i_1,i_2,\ldots,i_k}:V\to \RR$ for all indices.
We say that the differential $k$-form is of class $\DS C^m$
if each $\DS \omega_{i_1,i_2,\ldots,i_k}$ is of class
$\DS C^m$.
...
$\DS \diff f (\VEC{v}) \in (\RR^n)^\ast$.
As we have done with $\phi_i$ above, we may define the following
differential $1$-form on $V$.

\begin{defn} \label{defnd01}
Given $f:V\to \RR$ is of class $\DS C^1$, the differential $1$-form
$\DS \df{f} :V \to \bigcup_{\VEC{v}\in V}
\Omega^1\left( \TS_{\VEC{v}} V \right)$ is defined by
$\df{f}(\VEC{v}) \, \big((\VEC{v},\VEC{x})\big)
= \diff f (\VEC{v}) \VEC{x}$
for each $\VEC{v}\in V$ and all $\DS \VEC{x} \in \RR^n$.
\end{defn}

If $\DS \pi_i:\RR^n \to \RR$ is the projection defined by
$\pi_i(\VEC{x}) = x_i$ for all $\DS \VEC{x} \in \RR^n$, then
the differential $1$-form $\df{\pi_i}$ is defined by
$\DS \df{\pi_i}(\VEC{v}) \big( (\VEC{v},\VEC{x}) \big) =
\diff \pi_i (\VEC{v}) \VEC{x} = \pi_i(\VEC{x}) = x_i$
for each $\VEC{v} \in V$ and all $\DS \VEC{x} \in \RR^n$.
Thus $\DS \df{\pi_i}(\VEC{v}) \in \Omega^1(\TS_\VEC{v} V)$ 
is generated by $\DS \pi_i \in \Omega^1(\RR^n)$ for all
$\VEC{v} \in V$.

For each $\VEC{v} \in V$, the dual basis of the canonical basis
$\DS \E = \left\{ (\VEC{v}, \VEC{e}_i) \right\}_{i=1}^n \subset
\TS_{\VEC{v}} V$ is
$\DS \left\{ \df{\pi_i}(\VEC{v}) \right\}_{i=1}^n \subset
(\TS_{\VEC{v}}V)^\ast$.
...
(\VEC{v},\VEC{w}_2), \ldots , (\VEC{v},\VEC{w}_k) \big)
\end{align*}
for all $\VEC{v}\in V$ and $\DS \VEC{w}_i \in \RR^n$,
where we have used Proposition~\ref{propTssT} to obtain the second
equality.

\begin{theorem}  \label{thmdefn01}
Suppose that $\DS V\subset \RR^n$ is an open subset and
$f:V \to \RR$ is of class $\DS C^1$, then
\[
...
Suppose that $\DS V\subset \RR^n$ and
$\DS W \subset \RR^m$ are two open sets.
If $f:V \to W$ is a function of class $\DS C^1$,
then we can extend $\DS f^\ast$ to a map from
differential $k$-forms on $W$ to differential $k$-forms on $V$ by
$\DS f^\ast(\omega)(\VEC{v}) = f^\ast(\omega(f(\VEC{v})))$ for
every differential $k$-forms $\omega$ on $W$ and $\VEC{v} \in V$.
We say that $\DS f^\ast(\omega)$ is the
...
Suppose that $\DS V\subset \RR^n$ and
$\DS W \subset \RR^m$ are two open sets.
If $f:V \to W$ is a function of class $\DS C^1$, then
\begin{enumerate}
\item $\DS f^\ast(\df{x_i}) = \sum_{j=1}^n \pdydx{f_i}{x_j} \df{x_j}$
for $1 \leq i \leq m$.
\item $\DS f^\ast(\omega_1 + \omega_2) = f^\ast(\omega_1) +
f^\ast(\omega_2)$ for all differential $k$-forms $\omega_i$ on $W$.
\item $\DS f^\ast( g\, \omega) = ( g\circ f)f^\ast(\omega)$
for all differential $k$-form $\omega$ on $W$ and functions
$g:W\to \RR$ of class $\DS C^1$.
\item $\DS f^\ast( \omega_1 \wedge \omega_2) = f^\ast(\omega_1)
...
Suppose that $V$ and $W$ are two open subsets of $\DS \RR^n$.
...
$\DS q_{i,j} = \dydx{f_i}{x_j}(\VEC{v})$.
Moreover, let
$\DS \E_{\VEC{v}} = \left\{ (\VEC{v}, \VEC{e}_i) \right\}_{i=1}^n$
be the canonical basis of $\TS_{\VEC{v}}V$.
Recall that $\DS \df{x_i}(\VEC{v}) = \df{\pi_i}(\VEC{v}) \in
\Omega^1(\TS_{\VEC{v}}V)$ for $1\leq i \leq n$
is generated by the projection $\DS \pi_i:\RR^n \to \RR$ defined by
$\pi_i(\VEC{x}) = x_i$ for all $\DS \VEC{x} \in \RR^n$.

Given $\DS \VEC{w}_k \in \RR^n$ for $1\leq k \leq n$, we may write
$\DS \VEC{w}_k = \sum_{j=1}^n a_{j,k} \VEC{e}_j$
for $1 \leq k \leq n$ and some $a_{j,k} \in \RR$ where
$\DS \left\{ \VEC{e}_i \right\}_{i=1}^n$ is the canonical
basis on $\DS \RR^n$.  Let $A$ be the matrix with entries $a_{j,k}$.
...
\df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots \wedge \df{x_{i_k}}
\]
is a differential $k$-form of class $\DS C^1$ on an open set
$\DS V \subset \RR^n$.  We define a
...
Suppose that $\DS V\subset \RR^n$ and
$\DS W \subset \RR^m$ are two open subsets.
\begin{enumerate}
\item $\DS \df{(\omega + \mu )} = \df{\omega} + \df{\mu}$
for all differential $k$-forms $\omega$ and $\mu$ of class
$\DS C^1$ on $V$.
\item $\DS \df{(\omega \wedge \mu)} =
\df{\omega}\wedge \mu + (-1)^p \omega \wedge \df{\mu}$
for all differential $p$-forms $\omega$ and differential $q$-form
$\mu$ of class $\DS C^1$ on $V$.
\item $\df[2]{\omega} \equiv \df{(\df{\omega})} = 0$
for all differential $k$-forms $\omega$ of class
$\DS C^2$ on $V$.
\item $\DS f^\ast (\df{\omega}) = \df{f^\ast(\omega)}$ for all
differential $k$-forms $\omega$ of class $\DS C^1$ on $W$ and all
functions $f:V\to W$ of class $\DS C^1$.
\end{enumerate}
\end{theorem}
...
$\DS \omega = \sum_{1\leq i_1 <i_2 < \ldots < i_p \leq n}
\omega_{i_1,i_2,\ldots,i_p} \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots
\wedge \df{x_{i_p}}$ and \\
$\DS \mu = \sum_{1\leq j_1 <j_2 < \ldots < j_q \leq n}
...
\end{align*}
where the factor of $\DS (-1)^p$ comes from
$\DS \df{x_m} \wedge \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots
\wedge \df{x_{i_p}} \wedge \df{x_{j_1}} \wedge \df{x_{j_2}} \wedge \ldots
\wedge \df{x_{j_q}} = (-1)^p
...
\end{align*}
because
$\DS \pdydxnm{\omega_{i_1,i_2,\ldots,i_k}}{x_j}{x_m}{2}{}{}
= \pdydxnm{\omega_{i_1,i_2,\ldots,i_k}}{x_m}{x_j}{2}{}{}$ for all $0
\leq j, m \leq n$ since $\DS \omega_{i_1,i_2,\ldots,i_k} \in C^2(V)$
implies that
\[
\pdydxnm{\omega_{i_1,i_2,\ldots,i_k}}{x_j}{x_m}{2}{}{}
\df{x_m} \wedge \df{x_j}
...
$\DS \df{ (f^\ast \df{x_j})} = \df{ (f^\ast \df{\pi_j})}
...
of class $\DS C^j$ for $j$ large enough to meet the
...
\]
defined on $\DS V= \RR^2 \setminus \{ \VEC{0}\}$ (see
warning below).
...
$\DS W = \RR^2\setminus \{(x_1,0) : x_1 \geq 0 \}$ and
...
$\DS \pdydx{\mu}{x_1} = \pdydx{\theta}{x_1} =
\frac{-x_2}{x_1^2+x_2^2}$ and
$\DS \pdydx{\mu}{x_2} = \pdydx{\theta}{x_2} =
...
$\DS f_1: \RR^2 \setminus \{\VEC{0}\} \to \RR$ is the function defined
by $\DS f_1(\VEC{x}) = -x_2/(x_1^2 + x_2^2)$ and
$\DS f_2: \RR^2 \setminus \{\VEC{0}\} \to \RR$ is the function defined
by $\DS f_2(\VEC{x}) = x_1/(x_1^2 + x_2^2)$.  To introduce
...
Suppose that $\DS \omega = \sum_{i=1}^n \omega_i \df{x_i}$ is a
closed differential $1$-form on $\DS \RR^n$.  Let
\begin{equation}  \label{introPLEq0}
\mu(\VEC{x}) = \sum_{j=1}^n \int_0^1 \omega_j(t \VEC{x}) x_j \dx{t}
\end{equation}
for $1\leq j \leq n$ and $\DS \VEC{x} \in \RR^n$.
Then
...
\wedge \df{x_i} = 0$.
Therefore, with $\DS (\VEC{x},\VEC{y}) \in \TS_{\VEC{x}} \RR^n$ 
where $y_i = 0$ for $i\neq p,q$ and $y_i = 1$ for $i=p,q$, we get from
$\df{\omega}(\VEC{x})(\VEC{x},\VEC{y}) = 0$ that
$\DS \pdydx{\omega_p}{x_q}(\VEC{x}) 
...
If $\DS V\subset \RR^n$ is an open set which is star-shaped
...
\df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots \wedge \df{x_{i_k}} \ .
\]
Let $\DS g_{i_1,i_2,\ldots,i_k,j}: V \to \RR$ for
...
\end{align}
where we have used
$\DS \df{x_{i_j}} \wedge \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots
...
It suffices to use the map $\DS H:V \times [0,1] \to V$
...
in $\DS \RR^n$ is a continuous function
$\DS \sigma :I_m \to \RR^n$, where $I_m$ is the unit cube
$\DS I_m = \{(x_1,x_2,\ldots,x_m): 0 \leq x_j\leq 1
\text{ for } 1\leq j \leq m \}\subset \RR^m$.  For $m=0$, we define
$\DS \RR^m$ and $I_m$ as $\{0\}$.
\end{defn}
...
$\DS \RR^n$.  A
singular $1$-cube is a map $\DS \sigma: [0,1] \to \RR^n$;
namely, a {\bfseries curve}\index{Curve} in $\DS \RR^n$.

\begin{defn}
The {\bfseries standard $\mathbf{m}$-cube}\index{Standard $m$-Cube}
is the function $\DS \stc_m : I_m \to \RR^m$ defined by
$\stc_m(\VEC{x}) = \VEC{x}$ for all $\VEC{x} \in I_m$.
\end{defn}

\begin{defn}
An {\bfseries $\mathbf{m}$-chain}\index{$m$-Chain} in $\DS \RR^n$
is an expression of the form
$\DS \sum_{j=1}^J a_j \sigma_j$, where $a_j \in \ZZ$ and
$\sigma_j$ is a singular $m$-cube in $\DS \RR^n$ for
...
$\DS \stc_{1,1,1}:\{0\} \to \RR^n$ is defined by
$\stc_{1,1,1}(0) = \stc_1(1)$
and $\DS \stc_{1,1,0}:\{0\} \to \RR^n$ is defined by
...
{We have drawn a singular $2$-cube in $\DS \RR^3$ with its boundary
...
$\DS (-1)^{j+i+s_1+s_2}$ and the coefficient of
$(\sigma_{i,s_1})_{j-1,s_2} (\VEC{y})$ is
$\DS (-1)^{j-1+i+s_1+s_2}$, they have opposite signs and thus
...
$\omega$ on an open set $\DS V\subset \RR^n$ and all
singular $k$-cubes $\sigma: I_k \to V$ are assumed to be of class
$\DS C^j$ for $j$ as sufficiently large.  Recall that
$\sigma:I_k \to V$ of class $\DS C^j$ means that there
exist an open set $U \supset I_k$ and a function
$\tilde{\sigma}:U \to \RR^n$ of class $\DS C^j$ such that
...
$\DS \int_{\sigma} \omega = \omega(\sigma(0))$.
\end{defn}
...
$\DS \int_{\sigma} \omega = \sum_{j=1}^J a_j \int_{\sigma_j} \omega
...
\omega = \frac{-x_2}{x_1^2+x_2^2} \df{x_1} + \frac{x_1}{x_1^2+x_2^2} \df{x_2}
\]
defined on $\DS V= \RR^2 \setminus \{ \VEC{0}\}$ and introduced
...
to evaluate $\DS \stc_{k,j,s}^\ast \df{x_i}$ for
...
\end{cases}
\end{align*}
for all $\VEC{w} \in I_{k-1}$ and $\DS \VEC{y} \in \RR^{k-1}$.
Therefore,
\[
...
$\DS V \subset \RR^n$ and
that $\sigma$ is an $k$-chain in $V$ with $\DS k \in \NNp$.  Then
\[
...
$\omega$ a differential $(k-1)$-form on $\DS \RR^k$.  We have that
...
\begin{cases}
0 & \quad \text{if} \ i \neq j \\
\DS \int_{I_{k-1}} f(y_1,\ldots, y_{i-1}, s, y_i, \ldots, y_{k-1})
...
\]
for all differential $(k-1)$-forms $\omega$ on $\DS \RR^k$.

\subQ{ii}
Suppose that $\sigma$ is a general singular $k$-cube in an open set
$\DS V \subset \RR^n$ and that $\omega$ is a differential
$(k-1)$-form on $V$.  Since
\[
...
\end{align*}
It follows from (i) applied to $\DS \sigma^\ast(\omega)$ and
...
\subQ{iii}
Finally, if $\sigma = \DS \sum_{j=1}^J a_j \sigma_j$ is a $k$-chain
in an open set $\DS V \subset \RR^n$ and $\omega$ is a
