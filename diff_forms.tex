...
\stage{i} We prove that any $\tau \in \T^k(H)$ can be expressed
...
(\ref{stokesBTEq1}), we get $a_{m_1,m_2,\ldots,m_k}=0$
...
\begin{proof}
Suppose that $\tau \in \T^k(H)$ and $\tilde{\sigma}$ is a
fixed transposition on $J_k$.  We have that
$\tilde{\sigma}^{-1} = \tilde{\sigma}$ and
\begin{align*}
...
Hence, for $\tau \in \Omega^k(H)$, we have that
\begin{align*}
&\alt(\tau)(\VEC{v}_1, \VEC{v}_2, \ldots ,\VEC{v}_k)
= \frac{1}{k!} \sum_{\sigma\in S_k} \sgn(\sigma)
\tau(\VEC{v}_{\sigma(1)}, \VEC{v}_{\sigma(2)}, \ldots ,\VEC{v}_{\sigma(k)}) \\
...
\begin{enumerate}
\item If $\tau_1,\tau_2 \in \Omega^{k_1}$
and $\tau_3 \in \Omega^{k_2}(H)$, then
$(\tau_1 + \tau_2)\wedge \tau_3 = \tau_1 \wedge \tau_3 + \tau_2
\wedge \tau_3$.
\item If $\tau_1\in \Omega^{k_1}(H)$ and
$\tau_2,\tau_3 \in \Omega^{k_2}(H)$, then
...
$\tau_1 \wedge \tau_2 = (-1)^{k_1 k_2} \tau_2 \wedge \tau_1$.
\item If $\displaystyle \tau_i \in \Omega^{k_i}(H)$
for $1 \leq i \leq 2$ and $f:H_1\to H_2$ is a linear map from the
vector space $H_1$ to the vector space $H_2$, then 
$f^\ast(\tau_1 \wedge \tau_2) = f^\ast\tau_1 \wedge f^\ast \tau_2$.
\item If $\tau_i\in \Omega^{k_i}(H)$ for $1\leq i \leq 3$, then
\[
...
\stage{4} Given $\sigma \in S_{k_1+k_2}$, we have that
\begin{align*}
...
$k_1 -j +k_2 +1 < i \leq k_1+k_2$, we have that
\begin{align*}
...
\end{align*}
Finally, for $i = k_1-j+1$, we have that
\begin{align*}
i &\xrightarrow{\sigma_{k_1-j+1,k_1-j+2}}
...
We therefore have that $\sgn(\sigma) = (-1)^{k_1 k_2}\sgn(\tilde{\sigma})$.
Hence
\begin{align*}
...
Given $\tau_i \in \Omega^1(H)$ for $1\leq i \leq k$, we have that
...
$\sigma \mapsto \sigma\circ\rho^{-1}$ for $\sigma \in S_k$ is one-to-one
...
\begin{proof}
\stage{i} We first show that any $\tau \in \Omega^k(H)$ can be written
as a linear combination of elements in $\NNN$.
...
$1\leq j_1 < j_2 < \ldots < j_k \leq n$, we get that
\begin{align}
(\phi_{i_1} \wedge \phi_{i_2} \wedge \ldots \wedge \phi_{i_k})
(\VEC{v}_{j_1}, \VEC{v}_{j_2}, \ldots, \VEC{v}_{j_k})
= \begin{cases}
...
(\ref{thOkbasisEq1}), we get $a_{m_1,m_2,\ldots,m_k} =0$.
...
$k$ distinct $\phi_i$ from $\BB^\ast$.  However, for each set of indices
$1 \leq i_1 < i_2 < \ldots < i_k\leq n$, all $k!$ combinations
...
$\rho:\RR^n \times \ldots \times \RR^n \to \RR$ defined by
\[
...
\begin{cor}
Suppose that $\tau \in \Omega^k(H)$ and $\VEC{w}_i \in H$ for $1\leq i \leq n$.
Then
\begin{enumerate}
...
belonging to the chosen group for the orientation on $H$, we denote this
...
Given $\VEC{w}_i \in \RR^n$ for $1\leq i \leq n-1$, let
...
From the theory of representation of linear functionals on $\RR^n$,
...
\end{pmatrix}$ for all $\VEC{v} \in \RR^n$.
\end{defn}
...
\]
for all $a_i \in \RR$ and $\VEC{x}_i \in \RR^n$.
For each $\VEC{v} \in V$, we have that $\TS_{\VEC{v}}V$ is linearly
isomorphic to $\RR^n$.  The canonical basis
...
\end{align*}
for all $\VEC{x}_i \in \RR^n$ and $1 \leq i \leq k$.
...
generated by $\tau^{[i]} \in \Omega^{k_i}(\RR^n)$ for $1\leq i \leq 2$.
...
\stage{2} For $\VEC{v} \in V$ fixed, we have that
\begin{align*}
&f^\ast(a\, \tau_{f(\VEC{v})})
\big( (\VEC{v},\VEC{x}_1) , (\VEC{v},\VEC{x}_2) , \ldots,
(\VEC{v},\VEC{x}_k)\big)
...
\stage{4} For $\VEC{v} \in V$ fixed, we have that
\begin{align*}
...
= \phi_i(\VEC{x})$ for each $\VEC{v} \in V$ and all $\VEC{x} \in \RR^n$.
...
we can extend $\displaystyle f^\ast$ to a map from
...
\begin{proof}
\stage{1} For every $\VEC{v} \in V$, we have that
\begin{align*}
...
\item $f^\ast (\df{\omega}) = \df{f^\ast(\omega)}$ for all
...
\wedge \df{x_{j_q}} \ .
\end{align*}
From the definition of the operator $\df{}$, we get that
\begin{align*}
...
\wedge \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots \wedge \df{x_{i_p}} \right)
\\
...
\end{align*}
where the factor of $(-1)^p$ comes from
$\df{x_m} \wedge \df{x_{i_1}} \wedge \df{x_{i_2}} \wedge \ldots
\wedge \df{x_{i_p}} \wedge \df{x_{j_1}} \wedge \df{x_{j_2}} \wedge \ldots
\wedge \df{x_{j_q}} = (-1)^p
...
$k$-forms $\mu_j$.  From (1), (2) and (3) above, we get that
\begin{align*}
...
$\df{ (f^\ast \df{x_j})} = \df{ (f^\ast \df{\pi_j})}
...
= \df{f^\ast \mu_j} \wedge f^\ast \df{x_j} \ .
\]
Finally, using this relation and our hypothesis of induction, we get that
\begin{align*}
f^\ast \df{\omega}
&= \sum_{j=1}^n\left(f^\ast \df{\mu_j} \wedge f^\ast \df{x_j}\right)
...
$f_1: \RR^2 \setminus \{\VEC{0}\} \to \RR$ is the function defined
by $f_1(\VEC{x}) = -x_2/(x_1^2 + x_2^2)$ and
$f_2: \RR^2 \setminus \{\VEC{0}\} \to \RR$ is the function defined
by $f_2(\VEC{x}) = x_1/(x_1^2 + x_2^2)$.  To introduce the variable
$\VEC{x}$ and be rigorously correct, we should have written instead
\[
...
domain $V$, the integral formula in (\ref{introPLEq0}) (with
...
If it is not, we consider $\tilde{V} = V - \{\VEC{y}\}
...
$\stc_{1,1,1}:\{0\} \to \RR^n$ is defined by $\stc_{1,1,1}(0) = \stc_1(1)$
and $\stc_{1,1,0}:\{0\} \to \RR^n$ is defined by
...
{We have drawn a singular $2$-cube in $\RR^3$ with its boundary
...
\end{align}
for all $\VEC{y} \in I_{m-2}$.  If $i = j-1$, the section
...
However, if we compute directly the integral $\int_\sigma \omega$, we get
that
\[
...
\end{align}
If we combine (\ref{stokesFTC2}) and (\ref{stokesFTC3}), then we get that
\[
