...
smooth manifolds; namely, manifolds of class $\DS C^\infty$.
Similarly, all the differential forms will be of class
$\DS C^\infty$.  These restrictions may often be reduced to
$\DS C^j$ with $j$ larger than the dimension of the manifold.
...
$\DS C^\infty$ \footnotemark and a point $\tilde{\VEC{u}}
...
$\DS C^\infty$ on a closed interval, we assume that
there exists an open set $V \supset [0,1]$ such that $H$ can be
extended to a $\DS C^\infty$ function from
$S \times V$ to $S$.}

It is easy to show that $\DS \RR^n$ is contractible to the origin.
It suffices to use the map $\DS H:\RR^n \times [0,1] \to \RR^n$
...
$\DS \{(W_\alpha,U_\alpha,\phi_\alpha)\}_{\alpha\in A}$ is
an atlas for $S$, then
$\DS \left\{\big(W_\alpha \times J, U_\alpha \times J,
...
$\DS \df{x} (\VEC{u},x) \big( (\VEC{u},x),(\VEC{v},y)\big) = y$
for all $(\VEC{u},x) \in S \times J$ and
$\DS \big( (\VEC{u},x),(\VEC{v},y)\big) \in
...
Let $\DS \left\{ \psi_j(\VEC{u}) \right\}_{1\leq j \leq k}
\subset \left(\TS_{\VEC{u}} S\right)^\ast$ be the dual basis
associated to the basis \\
$\DS \left\{ \big( \VEC{u}, (\diff \phi)(\phi^{-1}(\VEC{u})) \VEC{e}_j
...
\end{align*}
We have that
$\DS \left\{ \tilde{\psi}_j(\VEC{u},x) \right\}_{1\leq j \leq k+1}
\subset \left(\TS_{(\VEC{u},x)} (S\times J) \right)^\ast$ is the dual basis
associated to the basis\\
$\DS \left\{ \big( (\VEC{u},x), \big(
...
$\DS \iota_x^\ast(\omega)$ is the differential $q$-form on $S$
defined by
\begin{align*}
...
$\DS \big(W \times J, U \times J, (\phi,\Id)\big)$
of $S\times J$ where $\DS \big(W, U, \phi\big)$ is a local
chart of $S$.  We have that $\iota_x:S \to S \times J$ for $x \in J$
and its local representation $\tilde{\iota}_x:W \to W \times J$
satisfy the relation
$\DS (\phi,\Id) \circ \tilde{\iota}_x = \iota_x \circ \phi$;
...
Since $\DS \diff \tilde{\iota}_x(\VEC{w}) (\VEC{y}) =
(\VEC{y},0)$ and
\[
...
\end{align*}
for all $\VEC{w} \in W$ and $\DS \VEC{y} \in \RR^k$, 
This translates into
$\DS (\iota_x)_\ast (\VEC{u},\VEC{v}) =
...
then $\DS \iota_1^\ast(\omega) - \iota_0^\ast(\omega)
...
$\DS \df{\tilde{w}_j}(\VEC{w},x)
\big((\VEC{w},x),(\VEC{v},y)\big) = \df{w_j}(\VEC{w})\big(\VEC{w},\VEC{v}\big)
= v_j$ and
$\DS \df{x}(\VEC{w},x) \big((\VEC{w},x),(\VEC{v},y)\big) = y$
for $1\leq j \leq k$ and $\DS \big((\VEC{w},x),(\VEC{v},y)\big) \in
...
$\DS \tilde{\iota}_x(\VEC{w}) = (\VEC{w},x)$ for all $\VEC{w} \in W$
...
$\DS \tilde{\iota}_1^\ast(\df{x} \wedge \tilde{\omega}_2) =
...
= \df{(F(H^\ast(\omega)))} \ .
\]
We may take $\DS \eta = F(H^\ast(\omega))$.
\end{proof}
...
$q$-form on $\DS \RR^k$, and that $\eta$ is the differential
$(q-1)$-form on $\DS \RR^k$ defined by
\[
...
\]
for $\DS \VEC{x} \in \RR^k$.  Then $\df{\eta} = \omega$.
\end{prop}
...
Let $\DS H: \RR^k \times \RR \to \RR^k$ be the map
defined by $H(\VEC{x},r) = r \VEC{x}$ for
$\DS (\VEC{x},r) \in \RR^k \times \RR$.
If we apply Proposition~\ref{propi1i0dffd} to
$\DS H^\ast(\omega)$, then we get
\begin{equation} \label{detaEomEq1}
i_1^\ast(H^\ast(\omega)) - i_0^\ast(H^\ast(\omega)) =
\df{(F(H^\ast(\omega)))} + F(\df{(H^\ast(\omega))}) \ .
\end{equation}

Let $\DS Z:\RR^k \to \RR^k$ be the function defined by
$Z(\VEC{x}) = \VEC{0}$ for all $\DS \VEC{x} \in \RR^k$.  We
have
...
\end{align*}
for all $\DS \big( (\VEC{x},r), (\VEC{v}, z)\big)
\in \TS_{(\VEC{x},r)} (\RR^k \times \RR)$ and
$\DS (\VEC{x},r) \in \RR^k \times \RR$ with
...
\df{\tilde{x}_{i_q}} \right)
\end{align*}
on $\DS \RR^k \times \RR$.  It follows from
(\ref{defFSItoS}) that
...
on $\DS \RR^k$.
Moreover
$\DS \df{(H^\ast(\omega))} = H^\ast(\df{\omega}) = H^\ast(0)
= 0$ because $\omega$ is a differential $k$-form on $\DS \RR^k$.
Thus
...
\begin{defn}
Let $S$ be a $k$-dimensional smooth manifold.
The {\bfseries $\DS \mathbf{q^{th}}$ (de Rham) Cohomology Module
...
denoted $\DS H_c^q(S)$, is the set of all equivalent
...
\begin{egg}
We have that the dimension of $\DS H^k_c(\RR^k)$ is greater
than $0$.

To prove this statement, let
$\DS \omega = f \df{x_1} \wedge
\df{x_2} \wedge \ldots \wedge \df{x_k}$ where
$\DS f:\RR^k \to [0,\infty[$ is
a continuous function with compact support such that $f(\VEC{x}) > 0$
for some $\DS \VEC{x} \in \RR^k$.  Then
$\DS \int_{\RR^k} \omega = \int_{\RR^k} f(\VEC{x}) \dx{\VEC{x}} > 0$.
...
Stokes' theorem that
$\DS \int_{\RR^k} \omega = \int_{B_r(\VEC{0})} \omega 
...
smooth manifold, then the dimension of $\DS H_c^k(S)$ is
greater than $0$.
...
$\DS \phi^\ast(\omega) = f \df{w_1} \wedge
...
$\DS \int_S \omega = \int_W f(\VEC{w}) \dx{\VEC{w}} > 0$.
Suppose that there is a differential $(k-1)$-form $\eta$ on $S$ with
...
It follows from Stokes' theorem that $\DS \int_S \omega 
...
Then $\DS H_c^q(S)$ is a vector space.
\end{prop}
...
\begin{rmk}
Let $S$ be a $k$-dimensional smooth manifold.
The null element $[0]$ in $\DS H_c^q(S)$ is the collection of
...
We want to compute the $\DS q^{th}$ cohomology modules with
...
manifold without boundary.  Then $\DS H_c^k(S) \cong \RR$ if
and only if the map $\DS M : H_c^k(S) \to \RR$ defined by
...
theorem that
$\DS \int_S \omega_1 - \int_S \omega_2 = \int_S (\omega_1 - \omega_2)
= \int_S \df{\eta} = \int_{\partial S} \eta = 0$.  So
$\DS \int_S \omega_1 = \int_S \omega_2 =0 $ and thus
$M([\omega_1]) = M([\omega_2])$.

Suppose that $M$ is an isomorphism between $\DS H_c^k(S)$
...
Conversely, suppose that $\DS H_c^k(S) \cong \RR$ and that
...
It follows that $M$ is null on $\DS H_c^k(S)$ which is not
...
Let $\DS C = \bigcup_{U_c \in \BB} U_c$.  We have that $C$ is
...
$\DS 0 = \int_{\RR} \omega = \int_{\RR} f'(x) \dx{x}
...
\stage{ii} We prove that $\DS H_c^{k-1}(S) \cong \RR$
for all connected and oriented $(k-1)$-dimensional smooth manifolds without
boundary implies that $\DS H_c^k(\RR^k) \cong \RR$.  Again, we
do so by proving that $\DS M:H_c^k(\RR) \to \RR$
defined in Proposition~\ref{propHkSMequiv} is an isomorphism.

Suppose that $\omega$ is a differential $k$-form on $\DS \RR^k$
with compact support and that $\DS \int_{\RR^k} \omega = 0$.
Since $\supp \omega$ is compact, there exists $b>0$ such that
$\supp \omega \subset B_b(\VEC{0})$.

Let $\DS h:\RR^k \to \RR^k$ be the diffeomorphism defined by
$h(\VEC{x}) = b\, \VEC{x}$ for $\DS \VEC{x} \in \RR^k$.  We have that
$\DS h^\ast(\omega)$ is a differential $k$-form on
$\DS \RR^k$ with
$\DS \supp\, h^\ast(\omega) \subset B_1(\VEC{0})$.
If we prove that there exists a differential $(k-1)$-form $\eta$ on
$\DS \RR^k$ with compact support such that
$\DS h^\ast(\omega) = \df{\eta}$, then
$\DS \omega = (h^{-1})^\ast(\df{\eta})
= \df{\left( (h^{-1})^\ast(\eta)\right)}$ where
$\DS (h^{-1})^\ast(\eta)$ is a differential $(k-1)$-form $\eta$
on $\DS \RR^k$ with compact support.  Thus
$[\omega] = [0]$ in $\DS H_c^k(\RR^k)$.
...
Let $\eta$ be the differential $(k-1)$-form on $\DS \RR^k$ defined by
...
\dx{t}\right) \, \rho^\ast(\nu)(\VEC{x})
\end{align*}
for $\DS \VEC{x} \in \RR^k \setminus \{\VEC{0}\}$ where
$\DS \rho:\RR^k \setminus \{\VEC{0}\} \to \SS^{k-1}$ is the
retraction defined by $\DS \rho(\VEC{x}) = \|\VEC{x}\|^{-1} \VEC{x}$ 
and $\nu$ is the volume element on $\DS S^{k-1}$.
We have used the fact that $g(\VEC{x}) = 0$ for $\|\VEC{x}\| > 1$ to
modify the upper limit of integration.

Let $\DS G:\RR^k \to \RR$ be the function defined by
$\DS G(\VEC{x}) = \int_0^1 t^{k-1} g(t \VEC{x}) \dx{t}$
for $\DS \VEC{x} \in \RR^k$.  We have that
$\DS \eta(\VEC{x}) = G(\rho(\VEC{x})) \, \rho^\ast(\nu)(\VEC{x})
= \rho^\ast(G \, \nu)(\VEC{x})$ for all
$\DS \VEC{x} \in \RR^k \setminus \{\VEC{0}\}$.  Moreover, we get from
...
Using the induction hypothesis with $\DS S^{k-1}$, we have
that $\DS M: H_c^{k-1}(S^{k-1}) \to \RR$ defined by
$\DS M([\omega]) = \int_{S^{k-1}} \omega$ is an isomorphism.
So $[G\, \nu] = [0]$ in $\DS H_c^{k-1}(S^{k-1})$; namely,
there exists a differential $(k-2)$-form $\xi$ on
$\DS S^{k-1}$ with compact support such that $G \, \nu = \df{\xi}$
on $\DS S^{k-1}$.  It follows that
$\DS \eta = \rho^\ast(G\, \nu) = \rho^\ast(\df{\xi}) =
\df{\left(\rho^\ast(\xi)\right)}$ on
$\DS \RR^k \setminus \{\VEC{0}\}$.

Choose $0 < \epsilon < \delta <1$ and let
$\DS h:\RR^k \to \RR$ be a function of class
$\DS C^\infty$ such that $h(\VEC{x}) = 1$ for
$\|\VEC{x}\| \geq \delta$ and $h(\VEC{x}) = 0$ for $\|\VEC{x}\| < \epsilon$.
Then $\DS h \rho^\ast(\xi)$ is a differential $(k-2)$-form on
$\DS \RR^k\setminus \{\VEC{0}\}$.
Since $\DS h \rho^\ast(\xi)$ is null in a neighbourhood of the
origin, we may assume that $\DS h \rho^\ast(\xi)$ is defined on
$\DS \RR^k$ by setting $\DS h \rho^\ast(\xi)(\VEC{0}) = 0$.

We have that $\DS \eta - \df{(h \rho^\ast(\xi))}$ 
is a differential $(k-2)$-form on $\DS \RR^k$ such that\\
$\DS \df{\left(\eta - \df{(h \rho^\ast(\xi))}\right)}
= \df{\eta} = \omega$ and
\begin{align*}
...
So $\DS \eta - \df{(h \rho^\ast(\xi))}$ has a compact support
in $B_1(\VEC{0})$.

This shows that $[\omega] = [0]$ in $\DS H_c^k(\RR^k)$.

\stage{iii} We prove that $\DS H_c^k(\RR^k) \cong \RR$
implies $\DS H_c^k(S) \cong \RR$
for all connected and oriented $k$-dimensional smooth manifolds without
boundary.

Let $\DS \A = \left\{ (W_\alpha,U_\alpha,
...
assume that the $W_i$ are open ball in $\DS \RR^k$ centred
at the origin
...
$\DS W_{\VEC{x}} = B_r(\phi^{-1}(\VEC{x})) \subset W$.  Let
...
$\DS \{(W_{\VEC{x}},U_{\VEC{x}},\phi_{\VEC{x}})\}_{\VEC{x} \in S}$
is an atlas on $S$ which orientation preserving local charts such
that the $W_{\VEC{x}}$ are open ball in $\DS \RR^k$.}.
...
$\DS H_c^k(S)$; namely, $\omega$ is not exact.  Given
...
$\DS \NNp$ such that
$\DS \tilde{\omega} = \sum_{j\in J} \psi_j\, \tilde{\omega}$.
...
If $[\psi_j\, \tilde{\omega}] = [0]$ in $\DS H_c^k(S)$, then
...
$\DS \{U_{\tau_m} \}_{0 \leq m \leq M}$ with $\tau_m \in A$
...
$\DS [\phi_{\tau_m}^\ast(\omega_{m+1})] \neq [0]$ in
$\DS H_c^k(\RR^k)$ \footnote{Since 
$\DS \supp \phi_{\tau_m}^\ast(\omega_{m+1}) \subset W_{\tau_m}$,
we may assume that $\DS \phi_{\tau_m}^\ast(\omega_{m+1})$ is
defined on $\DS \RR^k$ by setting it to $0$ outside $W_{\tau_m}$.}
Since we assume that
$\DS H_c^k(\RR^k) \cong \RR$, there exists $a_{m+1} \in \RR$
and a differential $(k-1)$-form $\tilde{\xi}_m$ on
$\DS \RR^k$ with compact
support such that $\DS \phi_{\tau_m}^\ast(\omega_{m+1}) - a_{m+1}
\phi_{\tau_m}^\ast(\omega_m) = \df{\tilde{\xi}_m}$.  Proceeding as in the
prove of (ii), since $\DS \supp \phi_{i_m}^\ast(\omega_{m+1})$
...
$\DS \phi_{\tau_m}^\ast(\xi_m) = \tilde{\xi}_m$ on $U_{\tau_m}$.
Hence, $\DS \df{\tilde{\xi}_m} =
...
$\DS \omega_{M+1} =  \psi_j \tilde{\omega}$
...
(i) and (iii) implies that $\DS H_c^1(S) \cong \RR$
for all connected and oriented $1$-dimensional smooth manifolds without
boundary.  This result combined with (ii) implies that
$\DS H_c^2(\RR^2) \cong \RR$.  Then, this result combined with (iii)
implies that $\DS H_c^2(S) \cong \RR$
...
$\DS H_c^1(S^1) \cong \RR$ could be proved directly.

We first note that all differential $1$-form on $\DS S^1$
are closed because there are no non-null differential $2$-form on
$\DS S^1$.  They also all have compact support because
$\DS S^1$ is a compact set.  Moreover, every exact
differential $1$-form $\omega$ on $\DS S^1$ is of the form
$\omega  = \df{f}$ for some $\DS f:S^1 \to \RR$ of class
$\DS C^\infty$.
...
$\DS H_c^1(S^1)$ and $\RR$ given by
$[a\, \omega] \mapsto a$ for all $a \in \RR$.
\end{egg}

The next step should normally be to compute $\DS H_c^q(S)$
...
$\DS H_c^q(S) = 0$ when
$\DS H_c^q(S) = \{[0]\}$.  We will adopt this
...
{\bfseries $\DS \mathbf{q^{th}}$ (de Rham) Cohomology Module}
\index{Cohomology Module}\index{(de Rham) Cohomology
Module|see{Cohomology Module}}
of $S$, denoted $\DS H^q(S)$, is the set of all equivalent
...
\begin{rmk}
$\DS H^q(S)$ is also called the
{\bfseries $\DS \mathbf{q^{th}}$ cohomology
...
\begin{prop}
Suppose that $S$ is a $k$-dimensional smooth manifold.
Then $\DS H^q(S)$ is a vector space.
\end{prop}

\begin{rmk}
Let $S$ be a $k$-dimensional smooth manifold.
The null element $[0]$ in $\DS H^q(S)$ is the collection of
...
$\DS H^k(S)$ is larger than $0$.  To prove this claim, we
...
Theorem~\ref{theoHckCR} that $\DS H^k(S) = H^k_c(S) \cong \RR$.
\end{egg}

\begin{egg}
We have from Example~\ref{HR0dim} that
$\DS H^{n-1}\left(\RR^n \setminus \{ \VEC{0}\}\right)$
...
$\DS H^0(S)$ is equal to the number of (connected)
components of $S$.
\end{prop}
...
$\df{f}=0$ implies that
$\DS 0 = \phi^\ast(\df{f}) = \df{(\phi^\ast f)}
= \df{(f\circ \phi)}
= \sum_{j=1}^k \pdfdx{(f \circ \phi)}{w_j} \df{w_j} = 0$ on $W$ and so
$\DS \pdfdx{(f \circ \phi)}{w_j} = 0$ on $W$ for $1\leq j \leq k$.
...
\end{cases}
\]
Then $\DS \{ [\Chi_i] \}_{i\in I}$ is a basis of
$\DS H^0(S)$.

It is important to note that each equivalent class $[f]$ of
$\DS H^0(S)$ consists in one element only, the function $f$.
...
$\DS H^q(S) = 0$ for $0 <q \leq k$.
In particular, we get the following result.

\begin{cor} \label{corHqg0e0}
$\DS H^q(\RR^k) = 0$ for $0 <q \leq k$.
\end{cor}

Now that we have computed the $\DS q^{th}$ cohomology module
of $\DS \RR^k$ for $0 \leq q \leq k$, we would like to do
the same for other smooth manifolds than $\DS \RR^k$.  In
fact, we can easily
obtain the $\DS q^{th}$ cohomology module of the smooth manifolds
that are diffeomorphic to $\DS \RR^k$.

Suppose that $S_1$ and $S_2$ are two $k$-dimensional smooth manifolds.  Let
$f:S_1 \to S_2$ be a function of class $\DS C^\infty$.  We
have that $\DS f^\ast$ maps differential $q$-forms on $S_2$ to 
...
$\DS f^\ast(\omega_2) - f^\ast(\omega_1) = f^\ast(\omega_2- \omega_1)
= f^\ast (\df{\eta})  = \df{(f^\ast(\eta))}$.  Since
$\DS f^\ast(\eta)$ is
a differential $(q-1)$-form on $S_1$, we have that
$\DS f^\ast(\omega_2) \sim f^\ast(\omega_1)$ and
$\DS [f^\ast(\omega_2)] = [f^\ast(\omega_1)]$.
...
$\DS C^\infty$.  Then
$\DS f^\sharp : H^q(S_2) \to H^q(S_1)$ is a linear map.
\item  Suppose that $S_i$ for $1\leq i \leq 3$ are three
$k$-dimensional smooth manifolds and that $f_i:S_i \to S_{i+1}$ for
$1\leq i \leq 2$ are two function of class $\DS C^\infty$.  Then
$\DS (f_2 \circ f_1)^\sharp = f_1^\sharp \circ f_2^\sharp$.
\end{enumerate}
\end{prop}

We leave the proof of this proposition to the reader.

If $f:S_1 \to S_2$ is a diffeomorphism of class $\DS C^\infty$
between two $k$-dimensional smooth manifolds $S_1$ and $S_2$, then
$\DS f^\sharp: H^q(S_2) \to H^q(S_1)$ is a linear isomorphism.
Therefore $\DS H^q(S_2) \cong H^q(S_1)$.

Our next goal is to compute the $\DS q^{th}$ cohomology modules
of $\DS \RR^k \setminus \{\VEC{0}\}$ for $0 \leq q \leq k$.
This will also give us the $\DS q^{th}$ cohomology module of
$\DS S^{k-1}$ for $0 \leq q \leq k-1$ as we will show shortly.
...
$\DS C^\infty$.  Then $\DS f^\sharp = g^\sharp$
as maps from $\DS H^q(S_2)$ to $\DS H^q(S_1)$.
\end{prop}
...
\end{align*}
Thus $\DS g^\ast(\omega) \sim f^\ast(\omega)$.  it follows that
$\DS f^\sharp([\omega]) = [f^\ast(\omega)] = [g^\ast(\omega)]
=g^\sharp([\omega])$.
\end{proof}

\begin{prop} \label{propHqSkm1cHqRk}
$\DS H^q(S^{k-1}) \cong H^q(\RR^k \setminus \{\VEC{0}\})$
for $0 \leq q \leq k$.
\end{prop}

\begin{proof}
The inclusion $\DS \iota:S^{k-1} \to \RR^k\setminus \{\VEC{0}\}$
defined by $\iota(\VEC{u}) = \VEC{u}$ and the retraction
$\DS \rho:\RR^k\setminus \{\VEC{0}\}$ defined by
$\DS \rho(\VEC{u}) = \|\VEC{u}\|^{-1} \VEC{u}$ satisfy
$\DS \rho \circ \iota = \Id : S^{k-1} \to S^{k-1}$.
However,
$\DS \iota \circ \rho : \RR^k \setminus \{\VEC{0}\} \to
\RR^k \setminus \{\VEC{0}\}$ is not the identity but it is homotopic to
the identity $\DS \Id: \RR^k \setminus \{\VEC{0}\} \to
\RR^k \setminus \{\VEC{0}\}$.  An homotopy
$\DS H: (\RR^k \setminus \{\VEC{0}\})\times [0,1] \to
\RR^k \setminus \{\VEC{0}\}$ between $\iota \circ \rho$
and $\Id$ is given by
$H(\VEC{u},t) = t \VEC{u} + (1-t) (\iota\circ \rho)(\VEC{u})$ for all
$\DS (\VEC{u},t) \in (\RR^k \setminus \{\VEC{0}\})\times [0,1]$.
It follows from Proposition~\ref{propfsEgs} that
$\DS (\iota \circ \rho)^\sharp = \Id^\sharp : H^q(\RR^k \setminus
\{\VEC{0}\}) \to H^q(\RR^k \setminus \{\VEC{0}\})$ for $0 \leq q \leq k$.

Hence,
$\DS \iota^\sharp \circ \rho^\sharp = (\rho \circ \iota)^\sharp = 
\Id^\sharp : H^q(S^{k-1}) \to H^q(S^{k-1})$ and
$\DS \rho^\sharp \circ \iota^\sharp =
(\iota \circ \rho)^\sharp = \Id^\sharp : H^q(\RR^k \setminus
\{\VEC{0}\}) \to H^q(\RR^k \setminus \{\VEC{0}\})$ for $0 \leq q \leq k$.
Thus
$\DS \rho^\sharp : H^q(S^{k-1}) \to H^q(\RR^k \setminus \{\VEC{0}\})$
is the inverse of
$\DS \iota^\sharp : H^q(\RR^k \setminus \{\VEC{0}\}) \to
H^q(S^{k-1})$.  Therefore $\DS \rho^\sharp$ provides an
isomorphism between $\DS H^q(S^{k-1})$ and
$\DS H^q(\RR^k \setminus \{\VEC{0}\})$.
\end{proof}
...
\begin{cor}
$\DS H^0(S^{k-1}) \cong H^0(\RR^k \setminus \{\VEC{0}\})
\cong \RR$.
\end{cor}
...
submanifolds of the metric space $\DS \RR^n$ and if we use
...
subsets of some Euclidean space $\DS \RR^n$ and their
topology is induced from the topology on $\DS \RR^n$.
...
some Euclidean space $\DS \RR^n$.  Therefore, it admits a
countable basis.
...
$\DS W \subset H_k$ which is locally compact.
...
$\DS \{ V_x\}_{x\in S}$ is an open cover of $S$.  Since $S$
is a Lindelöf space, there exists a countable open subcover
$\DS \{ V_{x_i}\}_{i \in \NN}$ of $S$.
Hence $\DS S = \bigcup_{i\in \NN} \overline{V}_{x_i}$
...
Suppose that $\DS \{ B_\tau \}_{\tau \in T}$ is an open cover of
a manifold $S$.  There exists an atlas
$\DS \JJ = \{ (W_j,U_j,\phi_j) \}_{j \in \NN}$ on $S$ 
...
$\DS S = \bigcup_{i\in \NN} K_i$ where the $K_i$ are compact
sets.
...
Since $\DS S = \bigcup_{i\in \NN} K_i \subset \bigcup_{i\in \NN} V_i
\subset S$, we get that $\DS S = \bigcup_{i \in \NN} V_i$.
If we set $V_{-1} = \emptyset$, then we have that 
$\DS S = \bigcup_{i\in \NN} (\overline{V}_i \setminus V_{i-1})$
where each $\overline{V}_i \setminus V_{i-1}$ is a compact set because it
is a closed set of the compact set $\overline{V}_i$.

Given $i \in \NN$ and
$\DS \VEC{u} \in \overline{V}_i \setminus V_{i-1}$,
...
$\DS W_{\VEC{u}} = \phi^{-1}(U_{\VEC{u}})$ and
$\phi_{\VEC{u}} = \phi$, then we get a local chart
$(W_{\VEC{u}},U_{\VEC{u}},\phi_{\VEC{u}})$ with
$\DS \VEC{u} \in U_{\VEC{u}} \subset (V_{i+1} \setminus
\overline{V}_{i-2})$ and $\DS U_{\VEC{u}} \subset B_\tau$ for
some $\tau \in T$.
Repeating this for all $\VEC{x} \in S$, we get an atlas
$\DS \{(W_{\VEC{u}},U_{\VEC{u}},\phi_{\VEC{u}}) \}_{\VEC{u} \in S}$
on $S$ such that $\DS
U_{\VEC{u}} \subset (V_{i+1} \setminus \overline{V}_{i-2})$
for $\VEC{u} \in \overline{V}_i \setminus V_{i-1}$ and
$\DS U_{\VEC{u}} \subset B_\tau$ for some $\tau \in T$.

Since $\DS \{U_{\VEC{u}}\}_{\VEC{u} \in \overline{V}_0}$ is
an open cover of the compact set $\overline{V}_0$, there is a finite
subcover $\DS \{U_{\VEC{u}_j}\}_{0 \leq j \leq J_0}$
for some $J_0 \in \NN$.  This yields a finite number of local charts
$\DS \{ (W_j,U_j,\phi_j) \}_{0 \leq j \leq J_0}$
such that $\DS \{U_j\}_{0\leq j \leq J_0}$ is an open cover
of $\overline{V}_0$ and $U_j \subset V_1$ for $1 \leq j \leq J_0$.

Let $I_{-1} = 0$.  Suppose that we have a finite number of local charts\\
$\DS \{ (W_j,U_j,\phi_j) \}_{J_{m-1}+1 \leq j \leq J_m}$
for some $J_{i-1} < J_i$ and $i\geq 0$ such that
$\DS \{ U_j \}_{J_{i-1}+1 \leq j \leq J_i}$
is an open cover of $\DS \overline{V}_i \setminus V_{i-1}$ and
$U_j \subset (V_{i+1} \setminus \overline{V}_{i-2})$ for
$J_{i-1}+1 \leq j \leq J_i$.

Since $\DS \{U_{\VEC{u}}\}_{\VEC{u} \in \overline{V}_{i+1}\setminus V_i}$
is an open cover of the compact set
$\DS \overline{V}_{i+1}\setminus V_i$, there is a finite
subcover $\DS \{U_{\VEC{u}_j}\}_{J_i+1 \leq j \leq J_{i+1}}$
for some $J_{i+1} \in \NN$.  This yields a finite number of local charts
$\DS \{ (W_j,U_j,\phi_j) \}_{J_i+1 \leq j \leq J_{i+1}}$
such that
$\DS \{ U_j \}_{J_i+1 \leq j \leq J_{i+1}}$
is an open cover of $\DS \overline{V}_{i+1} \setminus V_i$ and
...
We have that $\DS \VEC{u} \in \overline{V}_i \setminus V_{i-1}$ for
some $i \in \NN$.  If we choose an open neighbourhood
$\DS V_{\VEC{u}} \subset V_{i+1} \setminus \overline{V}_{i-2}$
...
$U_\alpha$.
Since $\DS \{ V_{\VEC{x}} \}_{\VEC{x} \in K}$ is an open
cover of the compact set $K$, there exists a finite subcover
$\DS \{ V_{\VEC{x}_i} \}_{1 \leq i \leq I}$ of $K$.

Let $\DS V = \bigcup_{1 \leq i \leq I}V_{\VEC{x}_i}$.
...
Since $\DS \{U_i\}_{i \in \NN}$ is a locally finite open cover of
...
$\DS K = \bigcup_{0 \leq i \leq I} K_i$ for all $I$ shows
...
$\DS (\iota \circ \rho)^\sharp = \Id^\sharp : H^q(S\times \RR)
\to H^q(S\times \RR)$.
...
for $0 \leq q \leq k$.
Thus
$\DS \rho^\sharp : H^q(S) \to H^q(S \times \RR)$
is the inverse of
$\DS \iota^\sharp : H^q(S\times \RR) \to H^q(S)$.
Therefore $\DS \rho^\sharp$ provides an
isomorphism between $\DS H^q(S)$ and
$\DS H^q(S \times \RR)$.
\end{proof}

We are now ready to complete the computations of
$\DS H^q(\RR^k \setminus \{\VEC{0}\})$ for the missing cases
of $0 < q < k-1$.

\begin{prop}
$\DS H^q(\RR^k \setminus \{\VEC{0}\}) \cong H^q(S^{k-1}) = 0$
for $0 < q < k-1$.
\end{prop}

\begin{proof}
We prove by induction on $k >2$ that
$\DS H^q(\RR^k\setminus \{\VEC{0}\}) = 0$ for $0 < q < k-1$.

\stage{i}
We first prove the hypothesis of induction for $k=3$.
Suppose that $\omega$ is a differential $1$-form on
$\DS \RR^3 \setminus \{\VEC{0}\} = R_+ \cup R_-$ where
$\DS R_+ = \RR^3 \setminus \{ (0,0,x_3) : x_3 \geq 0 \}$ and
$\DS R_- = \RR^3 \setminus \{ (0,0,x_3) : x_3 \leq 0 \}$.
...
$\DS \RR^3 \setminus \{\VEC{0}\} = R_+ \cup R_-$ because
$\eta_+(\VEC{u}) - c = \eta_-(\VEC{u})$ for $\VEC{u} \in R_-\cap R_+$.
We have that $\df{\eta} = \omega$ on
$\DS \RR^3 \setminus \{\VEC{0}\}$.  Thus $[\omega] = [0]$ in
$\DS H^1(\RR^3 \setminus \{\VEC{0}\})$.
This proves that  $\DS H^q(\RR^3 \setminus \{\VEC{0}\}) = 0$
for $0 < q < 2$.

\stage{ii} We assume that the hypothesis of induction is true for
$k=j-1$ with $j>3$; namely,
$\DS H^q(\RR^{j-1}\setminus \{\VEC{0}\}) = 0$ for $0 < q < j-2$.
...
Suppose that $\omega$ is a differential $1$-form on
$\DS \RR^j \setminus \{\VEC{0}\} = R_+ \cup R_-$ where
$\DS R_+ = \RR^j \setminus I_+$ and
$\DS R_- = \RR^j \setminus I_-$.
...
\]
This is a well defined continuous function on
$\DS \RR^j \setminus \{\VEC{0}\} = R_+ \cup R_-$ because
$\eta_+(\VEC{u}) - c = \eta_-(\VEC{u})$ for $\VEC{u} \in R_-\cap R_+$.
We have that $\df{\eta} = \omega$ on
$\DS \RR^j \setminus \{\VEC{0}\}$.  Thus $[\omega] = [0]$ in
$\DS H^1(\RR^j \setminus \{\VEC{0}\})$.
This proves that  $\DS H^1(\RR^j \setminus \{\VEC{0}\}) = 0$.

Suppose that $\omega$ is a differential $q$-form on
$\DS \RR^j \setminus \{\VEC{0}\} = R_+ \cup R_-$ with $1<q<j-1$.
...
open cover $\{ R_+,R_-\}$ of $\DS \RR^j\setminus \{\VEC{0}\}$,
and let $\DS \psi_+ = \sum_{\supp \psi_j \subset R_+} \psi_j$ and
$\DS \psi_- = \sum_{\supp \psi_j \subset R_-} \psi_j$.  Note
that the two sums are finite on compact sets according to
Proposition~\ref{cov4}.  We have that
$\psi_+$ and $\psi_-$ are two functions of class $\DS C^\infty$
...
$\DS H^q(\RR^j \setminus \{\VEC{0}\}$.  This proves 
that $\DS H^q(\RR^j \setminus \{\VEC{0}\}) = 0$.
\end{proof}

We are now in a position to prove Theorem~\ref{thrmHqRkE0lk}

\begin{proof}[Proof (of Theorem~\ref{thrmHqRkE0lk})]
\stage{i} We first compute $\DS H^0_c(\RR^k)$.  
Let $\DS f:\RR^k \to \RR$ be smooth function.  We can proceed
as in the proof of Proposition~\ref{H0kdimS} to show that $f$ is
constant on $\DS \RR^k$.  Namely, we have that
$\DS \df{f} = \sum_{i=1}^k \pdydx{f}{x_i} \df{x_i} = 0$ on
$\DS \RR^k$ implies $\DS \pdydx{f}{x_i} =0$ for
$1\leq i \leq k$ on $\DS \RR^k$.  Thus $f$ is constant.  But
since $f$ has a compact support, we must have that $f = 0$ on
$\DS \RR^k$.  Thus $\DS H_c^0(\RR^k) = 0$.

\stage{ii} We compute $\DS H^1_c(\RR^k)$.  
Suppose that $\omega$ is a differential $1$-form with compact support
on $\DS \RR^k$.  We have from Proposition~\ref{propDeEo} or
Poincaré Lemma (Theorem~\ref{closedexact} or
Theorem~\ref{closedexactCntrct}) that
there exists a differential $0$-form $f$ on $\DS \RR^k$ such that
$\omega = \df{f}$.

Since $\omega$ has a compact support, there exists $r>0$ such that
$\supp \omega \subset B_r(\VEC{0})$.  It follows from $\df{f} = \omega = 0$ on 
$\DS \RR^k \setminus B_r(\VEC{0})$ that
$f$ is constant on $\DS \RR^k \setminus B_r(\VEC{0})$.
Suppose that $f(\VEC{x}) = c$ for
$\DS \VEC{x} \in \RR^k \setminus B_r(\VEC{0})$.
Then $f -c$ is a differential $0$-form on $\DS \RR^k$ with
...
get that $\DS H^1_c(\RR^k) = 0$.

\stage{iii} We now assume that $1 < q <k$.  Suppose that $\omega$ is a
differential $q$-form with compact support on $\DS \RR^k$.
...
$\DS \RR^k$ such that $\omega = \df{\eta}$.

Since $\omega$ has a compact support, there exists $r>0$ such that
$\supp \omega \subset B_r(\VEC{0})$.  Since
$\DS \RR^k \setminus \overline{B_r(\VEC{0})}$ is diffeomorphic to
$\DS \RR^k \setminus \{\VEC{0}\}$ \footnote{A diffeomorphism
is given by $\DS g:\RR^k \setminus \overline{B_r(\VEC{0})} \to
\RR^k \setminus \{\VEC{0}\}$ defined by
$\DS g(\VEC{x}) = \VEC{x} - r\|\VEC{x}\|^{-1} \VEC{x}$.},
we get from the previous proposition that
$\DS H^{q-1}\big(\RR^k \setminus \overline{B_r(\VEC{0})}\,\big) \cong
H^{q-1}(\RR^k \setminus \{\VEC{0}\}) \cong 0$ (see comments after
Proposition~\ref{propFsharpP}).  Thus, there exists a differential
$(q-2)$-form $\mu$ on $\DS \RR^k \setminus \overline{B_r(\VEC{0})}$
such that $\eta = \df{\mu}$

Choose a function $\DS \psi:\RR^k \to \RR$ of class
$\DS C^\infty$
such that $\psi(\VEC{x}) = 0$ for $\VEC{x} \in B_{5r/4}(\VEC{0})$ and
$\psi(\VEC{x}) = 1$ for
$\DS \VEC{x} \in \RR^k \setminus B_{2r}(\VEC{0})$.
...
$\DS \RR^k \setminus B_{2r}(\VEC{0})$.  Thus 
$\eta - \df{\tilde{\mu}}$ is a differential $(q-1)$-form with
compact support on $\DS \RR^k$.  Moreover,
...
We will use Mayer-Vietoris argument to compute the $\DS q^{th}$ 
cohomology module of the sphere $\DS S^k$ from scratch.

Let $\DS S_n^{k-1} = S^{k-1}\setminus \{-\VEC{e}_k\}$ and
$\DS S_s^{k-1} = S^{k-1}\setminus \{\VEC{e}_k\}$ be,
respectively, the sphere in $\DS \RR^k$ minus the
``south pole'' and the sphere in $\DS \RR^k$ minus the
``north pole''.

The following information about $\DS S_n^{k-1}$ and
$\DS S_s^{k-1}$ will be useful later.

\begin{lemma} \label{lemSnsContr}
$\DS S_n^{k-1}$ and $\DS S_s^{k-1}$ are smoothly
contractible to a point.
\end{lemma}

\begin{proof}
We prove that $\DS S_n^{k-1}$ is smoothly
contractible to $\VEC{e}_k$ and leave to the reader the task of
proving that $\DS S_s^{k-1}$ is smoothly
contractible to $-\VEC{e}_k$.

Let $\DS p:S_n^{k-1} \to \RR^{k-1}$ be the stereographic
projection defined in the following figure.
\pdfbox{cohomology/stereoP}
We have represented $\DS \RR^{k-1}$ as the classical tangent
plane $\VEC{e}_k + \{ (x_1,x_2, \ldots, x_{k-1},0) : x_i \in \RR \
\text{and}\ 1 \leq i \leq k-1 \}$ to the sphere $\DS
S^{k-1}$ at $\VEC{e}_k$.  In particular, the origin in $\DS
\RR^{k-1}$ is associated to $\VEC{e}_k$ in the tangent plane.

Let $\DS H:S^{k-1}_n \times [0,1] \to S^{k-1}_n$ be the
map defined by $\DS H(\VEC{u},t) = p^{-1}(t\,p(\VEC{u}))$ for
$\DS (\VEC{u},t) \in S^{k-1} \times [0,1]$.  $H$ is a smooth
function such that $H(\VEC{u},0) = \VEC{e}_k$ and
$H(\VEC{u},1) = \VEC{u}$ for all $\DS \VEC{u} \in S^{k-1}_n$.
\end{proof}

\begin{prop}  \label{propHqSeHqSns}
$\DS H^q(S^{k-1}) = H^q(S^{k-1}_n \cup S^{k-1}_s)
= H^{q-1}(S^{k-1}_n \cap S^{k-1}_s)$ for $q>1$.  
\end{prop}

\begin{proof}
The goal is to build a linear isomorphism between
$\DS H^q(S^{k-1}_n \cup S^{k-1}_s)$ and
$\DS H^{q-1}(S^{k-1}_n \cap S^{k-1}_s)$.

\stage{i} We first define a function that maps closed differential
$q$-form on $\DS S^{k-1}_n \cup S^{k-1}_s$ to closed 
differential $(q-1)$-form on $\DS S^{k-1}_n \cap S^{k-1}_s$.

Let $\omega$ be a closed differential $q$-form on
$\DS S^{k-1}_n \cup S^{k-1}_s$.  Since
$\DS S^{k-1}_n$ and $\DS S^{k-1}_s$ are smoothly
...
$\DS S^{k-1}_n$ such that $\omega = \df{\eta_n}$ and a
differential $(q-1)$-form $\eta_s$ on $\DS S^{k-1}_s$ such
that $\omega = \df{\eta_s}$.

Let $\eta = \eta_n - \eta_s$.  we have that $\eta$ is a closed
differential $(q-1)$-form on $\DS S^{k-1}_n \cap S^{k-1}_s$
because $\df{\eta} = \df{\eta_n} - \df{\eta_s} = 0$ on
$\DS S^{k-1}_n \cap S^{k-1}_s$.

We now prove that the map $\DS F: H^q(S^{k-1}_n \cup S^{k-1}_s)
...
differential $(q-1)$-forms on $\DS S^{k-1}_n$ and
$\DS S^{k-1}_s$ respectively such that
$\omega = \df{\tilde{\eta}_n}$ on $\DS S^{k-1}_n$ and
$\omega = \df{\tilde{\eta}_s}$ on $\DS S^{k-1}_s$,
then $\df{(\eta_n - \tilde{\eta}_n)} = \df{\eta_n} - \df{\tilde{\eta}_n} = 0$
on $\DS S^{k-1}_n$.  It follows from Poincaré lemma that
there exists a differential $(q-2)$-form $\mu_n$ on $\DS S^{k-1}_n$
such that $\eta_n - \tilde{\eta}_n = \df{\mu_n}$.  Similarly,
there exists a differential $(q-2)$-form $\mu_s$ on $\DS S^{k-1}_s$
such that $\eta_s - \tilde{\eta}_s = \df{\mu_s}$.  Thus
$(\eta_n - \eta_s) -(\tilde{\eta}_n - \tilde{\eta}_s) 
= \df{(\mu_n - \mu_s)}$ on $\DS S^{k-1}_n \cap S^{k-1}_s$.
...
$(q-1)$-form $\mu$ on $\DS S^{k-1}$ such that
$\DS \tilde{\omega} = \omega + \df{\mu}$.
Hence, the differential $(q-1)$-form $\eta_n+\mu$ on
$\DS S^{k-1}_n$ is such that $\tilde{\omega} = \df{(\eta_n+\mu)}$ and
the differential $(q-1)$-form $\eta_s+\mu$ on
$\DS S^{k-1}_s$ is such that $\tilde{\omega} = \df{(\eta_s+\mu)}$.
...
differential $(q-1)$-form on $\DS S^{k-1}_n \cap S^{k-1}_s$
to closed differential $q$-form on $\DS S^{k-1}_n \cup S^{k-1}_s$.

Let $\{ \psi_n, \psi_s\}$ be a partition of unity subordinate to the
open cover $\DS \{ S^{k-1}_n, S^{k-1}_s\}$ of
$\DS S^{k-1}$ such that
$\DS \supp \psi_n \subset S^{k-1}_n$ and
$\DS \supp \psi_s \subset S^{k-1}_s$.
Given a closed differential $(q-1)$-form $\eta$ on
$\DS S^{k-1}_n \cap S^{k-1}_s$, we define two closed
differential $(q-1)$-form $\eta_n$ and $\eta_s$ on $\DS S^{k-1}_n$
and $\DS S^{k-1}_s$ respectively as it follows.
\[
\eta_n(\VEC{u}) = \begin{cases}
\psi_s(\VEC{u}) \eta(\VEC{u}) &
\DS \quad \text{if} \ \VEC{u} \in S^{k-1}_n \setminus
\{\VEC{e}_k \} \\
...
\DS \quad \text{if} \ \VEC{u} \in S^{k-1}_s \setminus
\{-\VEC{e}_k \} \\
0 &\quad \text{if} \ \VEC{u} = -\VEC{e}_k
\end{cases}
\]
...
its compact support is in $\DS S^{k-1}_s$ and
$\psi_n$ is null in a neighbourhood of $-\VEC{e}_k$ since
its compact support is in $\DS S^{k-1}_n$.
We define a differential $q$-form $\omega$ on $\DS S^{k-1}$
as it follows.
\[
\omega(\VEC{u}) = \begin{cases}
\df{\eta_n}(\VEC{u}) & \DS \quad \text{if} \ \VEC{u} \in S^{k-1}_n \\
...
on $\DS S^{k-1}_n \cap S^{k-1}_s$.  It is obviously closed
because $\df{\,\omega} = \df[2]{\eta_n} = 0$ on $\DS S^{k-1}_n$
and $\df{\,\omega} = -\df[2]{\eta_s} = 0$ on $\DS S^{k-1}_s$.

We prove that the map $\DS G: H^{q-1}(S^{k-1}_n \cap S^{k-1}_s)
\to H^q(S^{k-1}_n \cup S^{k-1}_s)$ defined by $G([\eta]) = [\omega]$
is well defined.

Suppose that $\tilde{\eta}$ is another closed differential $(q-1)$-form on
$\DS S^{k-1}_n \cap S^{k-1}_s$ such that $\eta \sim \tilde{\eta}$.
We then have that $\tilde{\eta} = \eta + \df{\tilde{\mu}}$ for some
differential $(q-2)$-form $\tilde{\mu}$ on
$\DS S^{k-1}_n \cap S^{k-1}_s$.  We follow the procedure
...
$\DS S^{k-1}_n \cap S^{k-1}_s$, we may define a
differential $(q-2)$-form $\mu$ on $\DS S^{k-1}$ by
\[
\mu(\VEC{u}) = \begin{cases}
\tilde{\mu}_n(\VEC{u}) & \DS \quad \text{if} \ \VEC{u} \in
...
$\DS S^{k-1}_s$.  Thus $\tilde{\omega} \sim \omega$ on
$\DS S^{k-1}$.

\stage{iii}  We have that $\DS F = G^{-1}$.  It is follows
directly from the definitions of $F$ and $G$ that
$\DS F\circ G = \Id: H^{q-1}(S^{k-1}_n \cap S^{k-1}_s) \to 
H^{q-1}(S^{k-1}_n \cap S^{k-1}_s)$.  To prove that 
$\DS G\circ F = \Id: H^q(S^{k-1}_n \cup S^{k-1}_s) \to 
H^q(S^{k-1}_n \cup S^{k-1}_s)$, we consider a differential $q$-form
$\omega$ on $\DS H^q(S^{k-1}_n \cup S^{k-1}_s)$.

According to the definition of $F$, starting with $\omega$, we get a
differential $(q-1)$-form $\eta_n$ on $\DS S^{k-1}_n$ such
that $\omega = \df{\eta_n}$ and a differential $(q-1)$-form $\eta_s$
on $\DS S^{k-1}_s$ such that $\omega = \df{\eta_s}$, and set
...
$\tilde{\eta}_s$ on $\DS S^{k-1}_n$ and $\DS S^{k-1}_s$ by
\[
\tilde{\eta}_n(\VEC{u}) = \begin{cases}
\psi_s(\VEC{u}) \eta(\VEC{u}) &
\DS \quad \text{if} \ \VEC{u} \in S^{k-1}_n \setminus
...
\end{cases}
\]
We then define a differential $q$-form $\omega$ on $\DS S^{k-1}$ by
\[
...
$\DS S^{k-1}_n \cap S^{k-1}_s$, we may define the
differential $(q-1)$-form $\mu$ on $\DS S^{k-1}$ by
\[
\mu(\VEC{u}) = \begin{cases}
...
= \omega - \df{(\eta_s + \psi_n\eta)}$ on $\DS S^{k-1}_n$ and
$\tilde{\omega} = -\df{(\psi_n \eta)} = -\df{(\eta - \psi_s\eta)}
= -\df{(\eta_n - \eta_s - \psi_s\eta)}
= \omega - \df{(\eta_n - \psi_s\eta)}$ on $\DS S^{k-1}_s$.
...
$\DS S^{k-1}_n \cap S^{k-1}_s$ is diffeomorphic to
$\DS \RR^{k-1} \setminus \{\VEC{0}\}$.  Using spherical coordinates 
defined by $\VEC{x} \mapsto (\|\VEC{x}\|^{-1}\VEC{x},\|\VEC{x}\|)$
for $\DS \VEC{x} \in \RR^{k-1} \setminus \{\VEC{0}\}$, we
have that $\DS \RR^{k-1} \setminus \{\VEC{0}\}$ is
diffeomorphic to $\DS S^{k-2} \times ]0,\infty[$.  Therefore,
$\DS S^{k-1}_n \cap S^{k-1}_s$ is diffeomorphic to
$\DS S^{k-2} \times ]0,\infty[$.  It then follows from
Lemma~\ref{lemScStimesR} (slightly modified) that
$\DS H^{q-1}(S^{k-1}_n \cap S^{k-1}_s) \cong
...
$\DS H^1(S^1) \cong \RR$ and
$\DS H^1(S^k) = 0$ for $k>1$.
\end{prop}

\begin{proof}
\stage{i} We get from Example~\ref{CnotEpart2} that the dimension of
$\DS H^1(S^1)$ is non null.  Since $\DS S^1$ is a
compact manifold without boundary, we may use
Proposition~\ref{propHkSMequiv} to prove that 
$\DS H^1(S^1) \cong \RR$.

Suppose that $\omega$ is a differential $1$-form on $\DS S^1$
such that $\DS \int_{S^1} \omega = 0$.

Consider the path $\DS \sigma : \RR \to S^1$ defined by
$\sigma(s) = (\cos(2\pi s),\sin(2\pi s))$ for $s \in \RR$.  Let
$\DS g(t) = = \int_0^t \sigma^\ast(\omega)$ for $t \in \RR$.
We have by
hypothesis that $\DS g(0) = g(1) = \int_0^1 \sigma^\ast(\omega)
\int_{\sigma(]0,1[)} \omega = \int_{S^1} \omega = 0$.
Thus $\DS g = f \circ \sigma = \sigma^\ast(f)$
for some functions $\DS f:S^1 \to \RR$.
If we derive both sides of
$\DS \sigma^\ast(f)(t) = g(t) = \int_0^t \sigma^\ast(\omega)$
with respect to $t$, then we get
$\DS \sigma^\ast(\df{f}) = \df{(\sigma^\ast(f))}
= \sigma^\ast(\omega)$.
...
Choose $\DS \VEC{u}_0 \in S^k$.  Since $\DS S^k$
is path connected, we can select for each
$\DS \VEC{u} \in S^k$ a path $\DS \sigma:[0,1] \to S^k$
...
$\DS \tilde{\sigma}:[0,1] \to S^k$
is another path between $\VEC{u}_0$ and $\VEC{u}$.  Then
$\sigma - \tilde{\sigma}$ is a closed path in $\DS S^k$
which is the boundary of a region $\DS R \subset S^k$.
...
$\DS \int_{\sigma} \omega = \int_{\tilde{\sigma}} \omega$.
...
so $\DS S^k$) matches the orientation provided by
$\sigma - \tilde{\sigma}$.
...
To compute the derivative of $f$ at $\DS \VEC{u}_1 \in S^k$,
...
$\DS \phi^\ast \omega = \sum_{j=1}^k \tilde{\omega}_j \df{w_j}$.
If we show that
$\DS \phi^\ast(\df{f}) = \df{(\phi^\ast(f))} = \phi^\ast(\omega)$
...
$\DS \df{(\phi^\ast(f))}(\VEC{w}_1) = \phi^\ast(\omega)(\VEC{w}_1)$,
we need to prove that
$\DS \pdfdx{\big(\phi^\ast(f)\big)}{w_j}(\VEC{w}_1) =
...
$\DS H^q(S^{k-1}) = 0$ for $1 < q+1 < k$ because
$\DS H^q(S^{k-1}) \cong H^{q-1}(S^{k-2}) \cong H^{q-2}(S^{k-3}) 
\cong \ldots \cong H^1(S^{k-q}) = 0$ for $k-q > 1$
and $\DS H^{k-1}(S^{k-1}) \cong \RR$ for $k >1$ because
$\DS H^{k-1}(S^{k-1}) \cong H^{k-2}(S^{k-2}) \cong \ldots
\cong H^1(S^1) \cong \RR$ for $k>1$.  For the $\DS 0^{th}$
cohomology module on $\DS S^{k-1}$, we have that
$\DS H^0(S^{k-1}) \cong \RR$ for $k>1$ according to
...
that $f:S_1 \to S_2$ is a function of class $\DS C^\infty$.
Is there a relation between $\DS \int_{S_2} \omega$ and
$\DS \int_{S_1} f^\ast(\omega)$ for all
...
because $\DS H^k(S_2) \cong \RR$ according to
...
$\DS C^\infty$ and {\bfseries proper}\index{Proper Function}; namely,
$\DS f^{-1}(K)$ is compact if $K$ is compact.  Obviously, we
have in that context to consider $\DS H_c^k(S_i) \cong \RR$
for $1\leq i \leq 2$ instead of $\DS H^k(S_i) \cong \RR$
for $1\leq i \leq 2$.
...
proper function of class $\DS C^\infty$.  Suppose that $\VEC{v}$ is a
regular value of $f$ and set, for each
$\DS \VEC{u} \in f^{-1}(\{\VEC{v}\})$,
...
$\DS f^{-1}(\{\VEC{v}\})$ is the union of isolated points
...
$\DS \phi_2^{-1}(\VEC{u}_2)$ is a regular value of
$\DS \phi_2^{-1} \circ f \circ \phi_1$ for all local charts
$(W_1,U_1,\phi_1)$ of $S_1$ and local charts $(W_2,U_2,\phi_2)$ of
$S_2$ with $\DS \phi_2^{-1}(\VEC{u}_2) \in W_2$.
...
Since $f$ is proper, $\DS f^{-1}(\{\VEC{v}\})$ is also a compact
set.  Therefore, $\DS f^{-1}(\{\VEC{v}\})$ is the finite
union of isolate points.

Suppose that $\DS f^{-1}(\{\VEC{v}\}) = \{ \VEC{u}_1,
...
$\DS f^{-1}(U) = \bigcup_{1\leq i \leq q} U_i$
...
$K_1 \subset \tilde{U}$.
Since $f$ is a proper function, $\DS f^{-1}(K_1)$ is a
compact subset of $S_1$.  Thus $\DS K_2 = f^{-1}(K_1) \setminus
...
representation $\DS \phi^\ast(\omega) = g \df{w_1} \wedge
\df{w_2} \wedge \ldots \wedge \df{x_k}$ on $U$ and null on
$S_2 \setminus U$.  Since
$\DS \supp f^\ast(\omega) \subset f^{-1}(\supp \omega)
...
\]
because
$\DS \int_{U_i} f^\ast(\omega) = \int_U \omega$ if $f$ is
orientation preserving and
$\DS \int_{U_i} f^\ast(\omega) = -\int_U \omega$ if $f$ does
...
homotopic functions of class $\DS C^\infty$, then $\deg(f) = \deg(g)$.
\end{cor}
...
There does not exist a nowhere null vector field on $\DS S^{k-1}$ 
if $k$ is odd.
\end{prop}

\begin{proof}
If $\DS g_0:S^{k-1} \to S^{k-1}$ is the restriction to
$\DS S^{k-1}$ of the identity map
$\DS \Id:\RR^k \to \RR^k$,
then $\deg(g_0) = 1$ because $\det \Id = 1$.
  
If $\DS g_1:S^{k-1} \to S^{k-1}$ is the restriction to
$\DS S^{k-1}$ of the map $\DS -\Id:\RR^k \to \RR^k$,
so $g_1$ is the antipodal map defined by $g(\VEC{u}) =-\VEC{u}$
for all $\DS \VEC{u} \in S^{k-1}$,
...
$\DS F : S^{k-1} \to \bigcup_{\VEC{u}\in S} \TS_{\VEC{u}} S^{k-1}$.
Let $\DS F(\VEC{u}) = (\VEC{u},f(\VEC{u})) \in
\TS_{\VEC{u}} S^{k-1}$ for all $\DS \VEC{u} \in S^{k-1}$.
We have that $\DS f:S^{k-1} \to \RR^k$ is a smooth function
such that $f(\VEC{u}) \neq \VEC{0}$ for all
$\DS \VEC{u} \in S^{k-1}$ because $F$ is nowhere null.

Let $\DS H:S^{k-1} \times [0,1] \to S^{k-1}$ be the
function defined by
\[
H(\VEC{u},t) = \cos(\pi t) \VEC{u} + \sin(\pi t)
\, \|f(\VEC{u}\|^{-1} f(\VEC{u})
\]
for all $\DS (\VEC{u},t) \in S^{k-1}$.
The function $H$ is an homotopy between $g_0$ and $g_1$.  This is a
contradiction.
\pdfbox{cohomology/nnVF2}
Note that $\| H(\VEC{u},t)\| = 1$ for all
$\DS (\VEC{u},t) \in S^{k-1}$ because
$f(\VEC{u})$ is perpendicular to $\VEC{u}$ by definition of
$\DS \TS_{\VEC{u}} S^{k-1}$.  Hence, if we set
$\DS \VEC{y} = \|f(\VEC{u})\|^{-1} f(\VEC{u})$ for
$\DS \VEC{u} \in S^{k-1}$, then we get
\begin{align*}
...
that goes through $\DS \|f(\VEC{u})\|^{-1}f(\VEC{u})$.  
\end{proof}

If $k$ is even, then it is always possible to define a nowhere null
vector field of $\DS S^{k-1}$.  For instance,
$\DS F:S^{k-1} \to \bigcup_{\VEC{u}\in S} \TS_{\VEC{u}} S^{k-1}$
defined by $F(\VEC{u}) = (\VEC{u}, f(\VEC{u})$ for
$\DS \VEC{u} \in S^{k-1}$ where
$f(\VEC{u}) = (-u_2,u_1,-u_4,u_3, \ldots, -u_k,u_{k-1})$ is such a
vector field.  It is represented in the figure below for $\DS S^1$.
