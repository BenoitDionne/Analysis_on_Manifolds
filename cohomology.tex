...
$\displaystyle H^\ast(\omega)$ we get
...
(\ref{detaEomEq4}) and (\ref{detaEomEq5}) into (\ref{detaEomEq1}), we
get $\df{\eta} = \omega$.
\end{proof}
...
$\omega$ in the class $[0]$ means that that
...
(\ref{theoHckCREq1}) and use Proposition~\ref{propRanEntn}, we get
\begin{align*}
\eta(\VEC{x}) &= \sum_{j=1}^k (-1)^{j-1}
...
\]
where $\eta$ a differential $(k-1)$-form on $S$ with compact support
...
= \phi_{\tau_m}^\ast(\df{\xi_m})$ and we get that
...
$[0]$ means that that $\omega = \omega - 0 = \df{\eta}$ for some
...
$\phi_{\VEC{u}} = \phi$, we get a local chart
...
Thus, $\omega \sim 0$ and we obtain that $[\omega] = [0]$ in $\displaystyle H^1_c(\RR^k)$.
...
Hence, $\omega \sim 0$ and we get that $[\omega] = [0]$ in $\displaystyle H^q_c(\RR^k)$.
...
\stage{iii}  We have that $F = G^{-1}$.  It is follows directly from
the definitions of $F$ and $G$ that
...
\]
Since $\sigma_2([0,1]) \subset U$, we get that
...
$\sigma_2 = \phi \circ \sigma_h$.  We get that
\begin{align*}
...
regular value of $f$ and set, for each $\VEC{u} \in f^{-1}(\{\VEC{v}\})$,
...
$\displaystyle \VEC{u} \in S^{k-1}$, we get that
\begin{align*}
