...
$\beta$ is a path from $x_1 \in X$ to $x_2 \in X$, then the
{\bfseries product}\index{Product of Paths} of $\alpha$ and $\beta$, denoted
...
\]
for $0 \leq t \leq 1$.
Also, $\DS \alpha^{-1}$ is the path from $x_1$ to $x_0$ defined by
$\DS \alpha^{-1}(t) = \alpha(1-t)$ for $0 \leq t \leq 1$.
\end{defn}
...
for the range $X$ of $H$, then we may simply write $\alpha \dotsim \beta$. 
\end{defn}
...
$\DS \alpha_1^{-1},\alpha_2^{-1}$ are two paths from
$x_1\in X$ to $x_0 \in X$ such that
$\DS \alpha_1^{-1} \dotsim \alpha_2^{-1}$.
...
$\DS [\alpha]^{-1} = [\alpha^{-1}]$ for all loops $\alpha$
...
$\DS \mathbf{1^{st}}$ homotopy
group}\index{$\DS 1^{st}$ Homotopy Group|see{Fundamental Group}} of
the pair $(X,x_0)$.
\end{defn}
...
\begin{proof}
It follows from $\DS f^{-1} \circ f = \Id_X$ that
$\DS (f^{-1})_\ast \circ f_\ast = (f^{-1} \circ f)_\ast = (\Id_X)_\ast
= \Id_{\pi_1(X,x_0)}$ and
from $\DS f \circ f^{-1} = \Id_Y$ that
$\DS f_\ast \circ (f^{-1})_\ast = (f \circ f^{-1})_\ast =
(\Id_Y)_\ast = \Id_{\pi_1(Y,f(x_0))}$.  Thus $f_\ast$ has an inverse given by
$\DS (f_\ast)^{-1} = (f^{-1})_\ast$.
\end{proof}
...
$\DS \beta_\sharp([\sigma]) = [\beta^{-1} \sigma \beta]$ for all
$\sigma \in \pi_1(X,x_0)$.
...
We have that $\DS \beta^{-1} \sigma_0 \beta$ and
$\DS \beta^{-1}\sigma_1 \beta$ are two loops in $X$ at $x_1$.
...
\end{cases}
\]
Then $\DS \tilde{H}(0,s) = \beta^{-1}(0) = x_1$ and
...
for $0\leq t \leq 1$.
Thus
$\DS \beta^{-1} \sigma_0 \beta \dotsim \beta^{-1}\sigma_1 \beta$.
...
easy to verify that $\DS \beta_\sharp^{-1} = (\beta^{-1})_\sharp$.
\end{proof}

If $X$ is a path-connected topological space, then it follows from the
...
The set $X$ is a curve in $\DS \RR^2$ with the induced
topology from $\DS \RR^2$.  There is no open neighbourhood
...
\begin{enumerate}
\item $\DS p^{-1}(U) = \bigcup_{\tau \in T} V_\tau$ with
...
of $\DS S^1$.  In the lower illustration, if we assume that
$\DS S^1$ is represented by a circle of radius $1$ centred
at the origin, then $p$ is given by $\DS p(t) = e^{2\pi t i}$ for
$t \in \RR$.}{FundGr2}
...
$\DS W_1 \subset \alpha^{-1}(V_\tau)$ of $\breve{y}$.
Similarly, since $\beta$ is continuous, there exists an open neighbourhood
$\DS W_2 \subset \beta^{-1}(V_\tau)$ of $\breve{y}$.
...
theorem.  For instance, if $x \in X$ and $x = p(q)$, then there exists a
one-to-one map from the quotient group\\
$\DS \pi_1(X,x) / p_\ast(\pi_1(Q,q))$
onto $\DS p^{-1}(\{x\})$.  To prove this result, we first
construct a map $M$ from $\DS \pi_1(X,x)$ onto
$\DS p^{-1}(\{x\})$ as it follows.  Given
...
In particular, if $H = \{[e_x]\}$ for $x \in X$, then we get a universal
...
$\DS [\beta]\, p_\ast(p_1(Q,q_1))\, [\beta]^{-1}= p_\ast(p_1(Q,q_2))$.
...
$q \in Q$; namely,\\
$\DS [\beta]\, p_\ast(\pi_1(Q,q))\, [\beta]^{-1} =
...
to $\DS \pi_1(X,p(q)) / p_\ast (\pi_1(Q,q))$.
...
$\DS \pi_1(S^1,1) \cong \ZZ$.  In fact, since
$\DS S^1$ is path-connected, we have that
$\DS \pi_1(S^1) \cong \ZZ$.
\end{prop}
...
universal and regular covering of $\DS S^1$.
...
$\DS \pi_1(S^1) \cong {\cal G}(Q,p) \cong \ZZ$.
\end{proof}
...
$\DS R([\sigma_X],[\sigma_Y]) = [(\sigma_X,\sigma_Y)]$ 
...
$\DS R\circ (P_\ast,Q_\ast) = \Id_{\pi_1(X\times Y, (x_0,y_0))}$ and \\
$\DS (P_\ast,Q_\ast) \circ R = \Id_{\pi_1(X, x_0) \times \pi_1(Y,y_0)}$.
\end{proof}

\begin{egg}
The torus $\DS \torus{2} \subset \RR^3$         \label{eggFGtorus}
is isomorphic to $\DS S^1 \times S^1$.
This can be seen from the parametric representation of the torus.
Hence, it follows from the previous proposition that
$\DS \pi_1(\torus{2}) \cong \pi_1(S^1) \times \pi_1(S^1) \cong
...
$\DS \RR^2$.  We first need a new definition and a lemma.
...
Let $\DS D = \{ \VEC{x} \in \RR^2 : \|\VEC{x}\| \leq 1 \}$.
There does not exist any retraction of $D$ onto
$\DS S^1 = \{\VEC{x} \in \RR^2 : \|\VEC{x}\|=1 \}$.
\end{lemma}

\begin{proof}
Suppose that there is a retraction $\DS g:D \to S^1$.
Let $\DS \iota:S^1 \to D$ be the inclusion map.  
...
$\DS \iota_\ast(\pi_1(S^1,\VEC{e}_1)) \subset \pi_1(D,\VEC{e}_1) =
\{[e_{\VEC{e}_1}]\}$ implies that
$\DS (g_\ast \circ \iota_\ast)(\pi_1(S^1,\VEC{e}_1))
...
be $\DS \pi_1(S^1,\VEC{e}_1) \cong \ZZ \not\cong
\{[e_{\VEC{e}_1}]\}$.
\end{proof}
...
$f(\VEC{y}) = \VEC{y}$.
Let $\DS g:D \to S^1$ be the function defined by
$\DS g(\VEC{x})$ is the intersection with the circle
$\DS S^1$ of the line through $\VEC{x}$ originating from
...
Moreover $g(\VEC{x}) = \VEC{x}$ for all $\DS \VEC{x} \in S^1$.
Thus, $g$ is a retraction of $D$ onto $\DS S^1$.
This is a contradiction of Lemma~\ref{lenNoRetract}.
\end{proof}

The proof of the Brouwer fixed point theorem is standard and can be
generalized to $\DS \RR^{n+1}$ with $n>1$ using a group
structure on $\DS S^n$ other than the fundamental group of
$\DS S^n$ because
$\DS \pi_1(S^n,\VEC{e}_1) = \{[ e_{\VEC{e}_1}]\}$ for $n>1$.
